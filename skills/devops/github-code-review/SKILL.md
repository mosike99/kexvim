...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _kexvim_env="${KEXVIM_HOME:-$PWD}/.env"; [ -f "$_kexvim_env" ] && grep -q "^GITHUB_TOKEN=" "$_kexvim_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_kexvim_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${KEXVIM_HOME:-$PWD}/skills/devops/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
```bash
source "${KEXVIM_HOME:-$PWD}/skills/devops/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by kexvim. Code looks clean — good test coverage, no security concerns."
