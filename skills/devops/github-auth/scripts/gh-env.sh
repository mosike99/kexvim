#!/usr/bin/env bash
# GitHub environment detection helper for kexvim skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _kexvim_env="${KEXVIM_HOME:-$PWD}/.env"; [ -f "$_kexvim_env" ] && grep -q "^GITHUB_TOKEN=" "$_kexvim_env" 2>/dev/null; then
