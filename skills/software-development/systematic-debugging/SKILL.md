...
description: "Debug in 4 phases: root cause before fixing."
version: 1.2.0
...
## Silent Process / Worker Death Checklist

When a daemon/worker "just dies" or a long task "silently interrupts" with **no logs and no error**,
the root cause is usually that the process died and nobody noticed. Absence of logs ≠ absence of activity.
Run this checklist in order, chaining the evidence:

1. **Is the process even alive?** `pgrep -f <pattern>`; `cat /proc/<pid>/status` → `Threads` count, `VmRSS`.
   Dead process = task ended when the process died.
2. **Are logs actually being captured?** Check spawn `stdio` (is it `ignore`/`/dev/null`?),
   whether worker/child stdout/stderr have `data` listeners, and log-file mtime vs process activity.
   Daemonized processes often write logs nowhere — that's why "no logs" is a red herring, not evidence.
3. **Do crash handlers exist?** `grep -rn "uncaughtException\|unhandledRejection" src/`.
   Node worker_threads: unhandledRejection defaults to **throw = worker death**; without a handler,
   one stray rejection silently kills the thread.
4. **Is there restart/watchdog coverage?** Worker `exit` listeners? Does the watchdog monitor worker
   liveness, or only a stop-file / SIGTERM? No exit listener + no liveness check = nobody restarts it.
5. **Memory pressure?** `VmRSS` vs `free -m`; long-running context growth; unconsumed stdout pipes
   accumulate chunks in memory. Small machines (3-4G) with co-resident heavy processes (LSPs, DBs) OOM easily.
6. **Socket leaks?** `ss -tnp` — `CLOSE-WAIT` entries with `Recv-Q > 0` = peer closed but local never
   closed (leak; can cause intermittent disconnects).
7. **Find the hot thread:** per-thread CPU from `/proc/<pid>/task/*/stat` (`utime`=field 14, `stime`=field 15);
   high values identify which worker is actually busy.

**Classic root-cause chain:** crash handlers absent + no exit listener + logs lost = worker died
silently and nobody restarted it. Report the evidence chain, not just the symptom.

## kexvim Integration

### Investigation Tools

Use these kexvim tools during Phase 1:
