...
- **会话树**：对话以树形分支组织——任意消息可分叉出新分支，随时切回历史分支继续对话（原分支内容不丢），分叉自动生成分支摘要，Web 面板树形导航
...
- **`data/config.yaml`** — LLM provider、Agent 行为、会话重置策略、平台适配器（QQ）、语言（`language: en` 切换英文界面）
...
| `KEXVIM_LANGUAGE` | `zh-CN` | 界面语言（`zh-CN` / `en`） |
