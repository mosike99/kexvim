...
`;return Xe.writeFileSync(r,s,"utf-8"),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3}).code!==0?!1:b.runSyncResult("loginctl enable-linger "+pr.userInfo().username,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=Ur.join(pr.homedir(),".config","systemd","user",`${e}.service`);return Xe.existsSync(t)&&(b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:5e3}),Xe.unlinkSync(t),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3})),!0}catch{return!1}}isServiceInstalled(e){let t=Ur.join(pr.homedir(),".config","systemd","user",`${e}.service`);return Xe.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`systemctl --user is-active ${e}`,{timeoutMs:3e3});return t.code!==0?!1:t.stdout.trim()==="active"}startService(e){return b.runSyncResult(`systemctl --user start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`systemctl --user enable ${e}`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`systemctl --user disable ${e}`,{timeoutMs:5e3}).code===0}};import*as Qe from"fs";import*as qn from"os";import*as Ct from"path";var Gn=class{platform="win32";findKexvimPids(){let e=b.runSyncResult(`powershell -Command "Get-Process node | Where-Object { $_.CommandLine -match 'Main\\.ts|dev\\.mjs|kexvim\\.js' } | Select-Object -ExpandProperty Id"`,{timeoutMs:5e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
...
npx tsx src/Main.ts
`;return Qe.writeFileSync(Ct.join(r,`${e.name}.bat`),s,"utf-8"),!0}catch{return!1}}uninstallService(e){if(!(b.runSyncResult(`nssm remove ${e} confirm`,{timeoutMs:1e4}).code===0)){let r=Ct.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup"),s=Ct.join(r,`${e}.bat`);if(Qe.existsSync(s))try{Qe.unlinkSync(s)}catch{}}return!0}isServiceInstalled(e){if(b.runSyncResult(`nssm status ${e}`,{timeoutMs:3e3}).code===0)return!0;let t=Ct.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");return Qe.existsSync(Ct.join(t,`${e}.bat`))}isServiceRunning(e){let t=b.runSyncResult(`powershell -Command "Get-Service ${e} | Select-Object -ExpandProperty Status"`,{timeoutMs:5e3});return t.code!==0?!1:t.stdout.trim().toLowerCase()==="running"}startService(e){return b.runSyncResult(`net start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`net stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`sc config ${e} start=auto`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`sc config ${e} start=disabled`,{timeoutMs:5e3}).code===0}};import*as Ze from"fs";import*as mr from"os";import*as Rt from"path";var Kn=class{platform="darwin";findKexvimPids(){let e=b.runSyncResult('pgrep -f "tsx.*Main\\.ts"',{timeoutMs:3e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){try{return process.kill(e,t?"SIGKILL":"SIGTERM"),!0}catch{return!1}}installService(e){try{let t=Rt.join(mr.homedir(),"Library","LaunchAgents");Ze.mkdirSync(t,{recursive:!0});let r=Rt.join(t,`${e.name}.plist`),n=`<?xml version="1.0" encoding="UTF-8"?>
...
`)}return l.spawnFreshDaemon(e,process.argv[1])}};import*as Be from"node:worker_threads";var gr=class l{static workersLaunched=!1;static workerInstances=new Map;static async startWatchdog(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D .stop_watchdog\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let t=Pt.join(e,"data",".stop_watchdog");setInterval(()=>{if(G.existsSync(t)){try{G.unlinkSync(t)}catch{}process.exit(0)}},3e3),process.on("SIGTERM",()=>{Fe.spawnFreshDaemon(e),process.exit(0)}),await new Promise(()=>{})}static async startGuardian(){let e=I.load();S.init(e.language||"zh-CN");let t=new Tt(xt.load());console.log(`[guardian] ready \u2014 ${t.llmClient.model}`);let r=Be.parentPort;if(!r){await new Promise(()=>{});return}r.on("message",async s=>{let n=s;if(!(!n||n.type!=="repair"))try{let i=await t.process(String(n.text??""));r.postMessage({type:"repair-result",reqId:n.reqId,text:i})}catch(i){r.postMessage({type:"repair-result",reqId:n.reqId,text:`[guardian error] ${i.message}`})}})}static terminateWorkers(){for(let e of l.workerInstances.values())e.terminate();l.workerInstances.clear()}static launchConsoleWorkers(){for(let t of["watchdog","agent"]){let r=new Be.Worker(process.argv[1],{workerData:{role:t},stdout:!0,stderr:!0});r.stdout.on("data",s=>process.stdout.write(`[${t[0]}] ${s}`)),r.stderr.on("data",s=>process.stderr.write(`[${t[0]}] ${s}`)),r.on("exit",s=>console.log(`[${t}] exited (${s})`)),l.workerInstances.set(t,r)}let e=()=>{l.terminateWorkers()};process.on("SIGTERM",e),process.on("SIGINT",e),process.on("exit",e)}static launchDaemon(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u542F\u52A8 daemon\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");if(ke.recordBoot(e))try{G.appendFileSync(l.resolveLogPath(),`[kexvim] \u26A0\uFE0F restart_loop guard TRIPPED: ${ke.bootCount(e)} boots in ${ke.WINDOW_SECONDS}s. \u81EA\u52A8\u62C9\u8D77\u6682\u505C\uFF0C\u8BF7\u4EBA\u5DE5\u68C0\u67E5\uFF08kexvim status / kexvim clear-loop\uFF09
