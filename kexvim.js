...
`)}catch{return""}}static discoverContextFiles(e,t){let r=[],s=t||Ge;for(let n of Mo){if(n.includes("*"))continue;let i=`${e}/${n}`;try{s.existsSync(i)&&r.push({name:n,fullPath:i})}catch{}}return r}static loadProjectContext(e,t=2e4){for(let r of Mo){if(r==="SOUL.md"||r.includes("*"))continue;let s=Vt.join(e,r);try{if(!Ge.existsSync(s))continue;let n=Ge.readFileSync(s,"utf-8").trim();if(!n)continue;return n=ht.stripFrontmatter(n).trim(),n.length>t&&(n=`${n.slice(0,t)}
...
`):""}async execute(e,t,r){let s=String(e.pattern||"").trim(),n=String(e.path||".").trim(),i=String(e.file_glob||"").trim(),o=Number(e.limit)||20;if(!s)return"Error: pattern is required";try{let a=[],c=[n],d=new Set;try{if(kt.statSync(n).isFile())return this.searchFile(n,s,i,a,o)}catch{}for(;c.length>0&&a.length<o;){let u=c.shift();if(d.has(u))continue;d.add(u);let p;try{p=kt.readdirSync(u,{withFileTypes:!0})}catch{continue}for(let m of p){if(m.name.startsWith(".")&&m.name!=="."||m.name==="node_modules")continue;let g=Wo.join(u,m.name);if(m.isDirectory()){c.push(g);continue}if(!(i&&!m.name.endsWith(i.replace("*",""))))try{let f=kt.readFileSync(g,"utf-8");if(f.includes(s)){let h=f.split(`
...
`,cr=class{db;constructor(e){this.db=new Yd(e),this.db.exec("PRAGMA busy_timeout = 15000"),this.db.exec(Vd)}async upsert(e){let t=this.db.prepare(`
...
`;return Xe.writeFileSync(r,s,"utf-8"),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3}).code!==0?!1:b.runSyncResult("loginctl enable-linger "+pr.userInfo().username,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=Ur.join(pr.homedir(),".config","systemd","user",`${e}.service`);return Xe.existsSync(t)&&(b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:5e3}),Xe.unlinkSync(t),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3})),!0}catch{return!1}}isServiceInstalled(e){let t=Ur.join(pr.homedir(),".config","systemd","user",`${e}.service`);return Xe.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`systemctl --user is-active ${e}`,{timeoutMs:3e3});return t.code!==0?!1:t.stdout.trim()==="active"}startService(e){return b.runSyncResult(`systemctl --user start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`systemctl --user enable ${e}`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`systemctl --user disable ${e}`,{timeoutMs:5e3}).code===0}};import*as Qe from"fs";import*as qn from"os";import*as Et from"path";var Gn=class{platform="win32";findKexvimPids(){let e=b.runSyncResult(`powershell -Command "Get-Process node | Where-Object { $_.CommandLine -match 'Main\\.ts|dev\\.mjs|kexvim\\.js' } | Select-Object -ExpandProperty Id"`,{timeoutMs:5e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){let r=t?"/F":"";return b.runSyncResult(`taskkill ${r} /PID ${e}`,{timeoutMs:5e3}).code===0}installService(e){if(b.runSyncResult("nssm --version",{timeoutMs:3e3}).code===0&&b.runSyncResult(`nssm install ${e.name} "${e.execPath}" "${e.workDir}"`,{timeoutMs:1e4}).code===0)return!0;try{let r=Et.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");Qe.mkdirSync(r,{recursive:!0});let s=`@echo off
cd /d "${e.workDir}"
set KEXVIM_DIR=${e.workDir}
npx tsx src/Main.ts
`;return Qe.writeFileSync(Et.join(r,`${e.name}.bat`),s,"utf-8"),!0}catch{return!1}}uninstallService(e){if(!(b.runSyncResult(`nssm remove ${e} confirm`,{timeoutMs:1e4}).code===0)){let r=Et.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup"),s=Et.join(r,`${e}.bat`);if(Qe.existsSync(s))try{Qe.unlinkSync(s)}catch{}}return!0}isServiceInstalled(e){if(b.runSyncResult(`nssm status ${e}`,{timeoutMs:3e3}).code===0)return!0;let t=Et.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");return Qe.existsSync(Et.join(t,`${e}.bat`))}isServiceRunning(e){let t=b.runSyncResult(`powershell -Command "Get-Service ${e} | Select-Object -ExpandProperty Status"`,{timeoutMs:5e3});return t.code!==0?!1:t.stdout.trim().toLowerCase()==="running"}startService(e){return b.runSyncResult(`net start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`net stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`sc config ${e} start=auto`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`sc config ${e} start=disabled`,{timeoutMs:5e3}).code===0}};import*as Ze from"fs";import*as mr from"os";import*as Ct from"path";var Kn=class{platform="darwin";findKexvimPids(){let e=b.runSyncResult('pgrep -f "tsx.*Main\\.ts"',{timeoutMs:3e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){try{return process.kill(e,t?"SIGKILL":"SIGTERM"),!0}catch{return!1}}installService(e){try{let t=Ct.join(mr.homedir(),"Library","LaunchAgents");Ze.mkdirSync(t,{recursive:!0});let r=Ct.join(t,`${e.name}.plist`),n=`<?xml version="1.0" encoding="UTF-8"?>
...
</plist>`;return Ze.writeFileSync(r,n,"utf-8"),b.runSyncResult(`launchctl load ${r}`,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=Ct.join(mr.homedir(),"Library","LaunchAgents",`${e}.plist`);return Ze.existsSync(t)&&(b.runSyncResult(`launchctl unload ${t}`,{timeoutMs:5e3}),Ze.unlinkSync(t)),!0}catch{return!1}}isServiceInstalled(e){let t=Ct.join(mr.homedir(),"Library","LaunchAgents",`${e}.plist`);return Ze.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`launchctl list local.${e}`,{timeoutMs:3e3});if(t.code!==0)return!1;let r=t.stdout.trim();return r.includes(`local.${e}`)&&!r.startsWith("-")}startService(e){let t=Ct.join(mr.homedir(),"Library","LaunchAgents",`${e}.plist`);return b.runSyncResult(`launchctl load ${t}`,{timeoutMs:5e3}).code===0}stopService(e){let t=Ct.join(mr.homedir(),"Library","LaunchAgents",`${e}.plist`);return b.runSyncResult(`launchctl unload ${t}`,{timeoutMs:5e3}).code===0}enableService(e){return this.startService(e)}disableService(e){return this.stopService(e)}};function tu(){switch(typeof process<"u"?process.platform:"linux"){case"win32":return new Gn;case"darwin":return new Kn;default:return new Wn}}var Ki=null;function Rt(){return Ki||(Ki=tu()),Ki}var we=class l{static DEV_WEB_PORT=8787;static RELEASE_WEB_PORT=8788;static isDevEntry(){let e=process.argv[1]??"";if(/dev\.mjs$/i.test(e))return!0;try{if(/kexvim(?:\.mjs)?$/i.test(Ee.basename(e)))return De.existsSync(Ee.join(Ee.dirname(e),"dist","dev.mjs"))}catch{}return!1}static webPort(){let e=Number(process.env.KEXVIM_WEB_PORT);return Number.isFinite(e)&&e>0?e:l.isDevEntry()?l.DEV_WEB_PORT:l.RELEASE_WEB_PORT}static findPid(){let e=l.webPort();try{if(process.platform==="win32"){let t=b.runSyncResult(`netstat -ano | findstr :${e} | findstr LISTENING`,{timeoutMs:5e3});for(let r of(t.stdout||"").split(/\r?\n/)){let s=r.trim().match(/LISTENING\s+(\d+)\s*$/);if(s)return Number(s[1])}}else{let r=(b.runSyncResult(`ss -ltnp 'sport = :${e}'`,{timeoutMs:5e3}).stdout||"").match(/pid=(\d+)/);if(r)return Number(r[1])}}catch{}return null}static webEntry(e){let t=Ee.join(e,"dist","dev.mjs");if(De.existsSync(t))return t;let r=Ee.join(e,"kexvim.js");return De.existsSync(r)?r:Ee.join(e,"src","Main.ts")}static restart(e){let t=l.findPid();if(t!==null){let r=Rt();try{process.platform==="win32"?b.runSyncResult(`taskkill /PID ${t} /F /T`,{timeoutMs:5e3}):r.killProcess(t,!0)}catch{}}return l.spawnWeb(e)}static ensureRunning(e){return l.findPid()!==null?!1:l.spawnWeb(e)}static spawnWeb(e){let t=l.webEntry(e),r=Ee.join(e,"data","log");try{De.mkdirSync(r,{recursive:!0})}catch{}let s=null;try{s=De.openSync(Ee.join(r,"web.log"),"a")}catch{}let n=b.spawn(process.execPath,[t,"web"],{cwd:e,stdio:s!==null?["ignore",s,s]:"ignore",detached:!0});return s!==null&&De.closeSync(s),n.unref(),!0}};var Fe=class l{static resolveEntry(e){let t=At.join(e,"dist","dev.mjs");if(Mt.existsSync(t))return t;let r=At.join(e,"kexvim.js");return Mt.existsSync(r)?r:At.join(e,"src","Main.ts")}static buildEnv(e){let t={};for(let[n,i]of Object.entries(process.env))i!==void 0&&(t[n]=i);let r=At.join(e,"data",".env");if(!Mt.existsSync(r))return t;let s=Mt.readFileSync(r,"utf-8");for(let n of s.split(`
`)){let i=n.trim();if(!i||i.startsWith("#"))continue;let o=i.indexOf("=");if(o===-1)continue;let a=i.slice(0,o).trim(),c=i.slice(o+1).trim();(c.startsWith('"')&&c.endsWith('"')||c.startsWith("'")&&c.endsWith("'"))&&(c=c.slice(1,-1)),a&&!(a in t)&&(t[a]=c)}return t}static spawnFreshDaemon(e,t){let r=t;if(!r){let n=process.argv[1];r=n&&Mt.existsSync(n)?At.resolve(n):l.resolveEntry(e)}let s=b.spawn(process.execPath,[r,"--daemon"],{cwd:e,stdio:"ignore",env:l.buildEnv(e),detached:!0});return s.unref(),s.pid??null}static selfRestart(e,t){try{we.restart(e)}catch(r){t?.(`[kexvim] web \u91CD\u542F\u5931\u8D25: ${r instanceof Error?r.message:String(r)}
`)}return l.spawnFreshDaemon(e,process.argv[1])}};import*as Be from"node:worker_threads";var gr=class l{static workersLaunched=!1;static workerInstances=new Map;static async startWatchdog(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D .stop_watchdog\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let t=It.join(e,"data",".stop_watchdog");setInterval(()=>{if(q.existsSync(t)){try{q.unlinkSync(t)}catch{}process.exit(0)}},3e3),process.on("SIGTERM",()=>{Fe.spawnFreshDaemon(e),process.exit(0)}),await new Promise(()=>{})}static async startGuardian(){let e=I.load();S.init(e.language||"zh-CN");let t=new xt(wt.load());console.log(`[guardian] ready \u2014 ${t.llmClient.model}`);let r=Be.parentPort;if(!r){await new Promise(()=>{});return}r.on("message",async s=>{let n=s;if(!(!n||n.type!=="repair"))try{let i=await t.process(String(n.text??""));r.postMessage({type:"repair-result",reqId:n.reqId,text:i})}catch(i){r.postMessage({type:"repair-result",reqId:n.reqId,text:`[guardian error] ${i.message}`})}})}static terminateWorkers(){for(let e of l.workerInstances.values())e.terminate();l.workerInstances.clear()}static launchConsoleWorkers(){for(let t of["watchdog","agent"]){let r=new Be.Worker(process.argv[1],{workerData:{role:t},stdout:!0,stderr:!0});r.stdout.on("data",s=>process.stdout.write(`[${t[0]}] ${s}`)),r.stderr.on("data",s=>process.stderr.write(`[${t[0]}] ${s}`)),r.on("exit",s=>console.log(`[${t}] exited (${s})`)),l.workerInstances.set(t,r)}let e=()=>{l.terminateWorkers()};process.on("SIGTERM",e),process.on("SIGINT",e),process.on("exit",e)}static launchDaemon(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u542F\u52A8 daemon\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");if(ke.recordBoot(e))try{q.appendFileSync(l.resolveLogPath(),`[kexvim] \u26A0\uFE0F restart_loop guard TRIPPED: ${ke.bootCount(e)} boots in ${ke.WINDOW_SECONDS}s. \u81EA\u52A8\u62C9\u8D77\u6682\u505C\uFF0C\u8BF7\u4EBA\u5DE5\u68C0\u67E5\uFF08kexvim status / kexvim clear-loop\uFF09
`)}catch{}try{q.writeFileSync(It.join(e,"data","kexvim.pid"),String(process.pid))}catch{}let r=It.join(e,"data","daemon.heartbeat"),s=()=>{try{q.writeFileSync(r,String(process.pid))}catch{}};s(),setInterval(s,3e4),process.on("exit",()=>{try{q.rmSync(It.join(e,"data","kexvim.pid"),{force:!0})}catch{}});let n=()=>{l._shuttingDown=!0,l.terminateWorkers(),process.exit(0)};process.on("SIGTERM",n),process.on("SIGINT",n);let i=()=>{if(process.platform!=="linux")return!1;try{let a=q.readFileSync("/proc/self/environ","utf-8");return a.includes("INVOCATION_ID")||a.includes("SYSTEMD_EXEC_PID")}catch{return!1}};process.platform!=="win32"&&process.on("SIGHUP",()=>{try{q.appendFileSync(l.resolveLogPath(),`[kexvim] SIGHUP received \u2014 ${i()?"systemd-managed, exiting for systemd restart":"standalone, spawning fresh daemon"}
`)}catch{}i()||Fe.selfRestart(e,a=>{try{q.appendFileSync(l.resolveLogPath(),a)}catch{}}),n()});let o=l.resolveLogPath();for(let a of["watchdog","agent"]){let c=new Be.Worker(process.argv[1],{workerData:{role:a},stdout:!0,stderr:!0});l.workerInstances.set(a,c);let d=m=>{try{q.appendFileSync(o,m)}catch{}};c.stdout.on("data",d),c.stderr.on("data",d);let u=()=>{let m=new Be.Worker(process.argv[1],{workerData:{role:a},stdout:!0,stderr:!0});l.workerInstances.set(a,m),m.stdout.on("data",d),m.stderr.on("data",d),m.on("error",g=>{try{q.appendFileSync(o,`[kexvim] worker '${a}' error: ${g.message}
`)}catch{}}),m.on("exit",p)},p=m=>{if(l.workerInstances.delete(a),m===0||m===null||l._shuttingDown){try{q.appendFileSync(o,`[kexvim] worker '${a}' exited (code=${m}) at ${new Date().toISOString()}
`)}catch{}return}let f=l.shouldStopRestarting(a);try{q.appendFileSync(o,`[kexvim] worker '${a}' exited (code=${m}) ${f?"\u2014 CRASH, restart halted (loop guard)":"\u2014 CRASH, restarting"} at ${new Date().toISOString()}
`)}catch{}f||setTimeout(u,2e3)};c.on("error",m=>{try{q.appendFileSync(o,`[kexvim] worker '${a}' error: ${m.message}
`)}catch{}}),c.on("exit",p)}}static resolveLogPath(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D kexvim.log\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let t=It.join(e,"data","log");try{q.mkdirSync(t,{recursive:!0})}catch{}return It.join(t,"kexvim.log")}static _shuttingDown=!1;static _crashTimes=new Map;static CRASH_WINDOW_MS=6e4;static MAX_CRASHES=3;static shouldStopRestarting(e){let t=Date.now(),r=(l._crashTimes.get(e)??[]).filter(s=>t-s<l.CRASH_WINDOW_MS);return r.push(t),l._crashTimes.set(e,r),r.length>l.MAX_CRASHES}static async handleWorkerDispatch(){if(!Be.isMainThread){let t=Be.workerData?.role||"agent";return t==="watchdog"?(await l.startWatchdog(),!0):t==="guardian"?(await l.startGuardian(),!0):!1}return process.argv.includes("--daemon")?(l.launchDaemon(),!0):(I.load().platform?.enabled&&!process.argv.slice(2).includes("web")&&(l.launchConsoleWorkers(),l.workersLaunched=!0),!1)}};import*as me from"node:fs";import*as Xr from"node:path";import*as ll from"node:worker_threads";var ru=new Set(["stop","new","newchat","reset","\u65B0\u4F1A\u8BDD","\u5207\u6362\u4F1A\u8BDD","\u5207\u4F1A\u8BDD"]);function Da(l,e){let t=process.env[l];if(t===void 0||t.trim()==="")return e;let r=parseFloat(t);return Number.isFinite(r)?r:e}function su(l,e){let t=process.env[l];return t===void 0||t.trim()===""?e:["1","true","yes","on"].includes(t.trim().toLowerCase())}function nu(l,e){let t=process.env[l]?.trim().toLowerCase();return t==="steer"||t==="queue"||t==="interrupt"?t:e}var fr=class l{adapters=new Map;config;messageHandler=null;activeSessions=new Map;pendingMessages=new Map;debounceTimers=new Map;debounceStates=new Map;_lastBusyNotifyTime=new Map;_sessionControllers=new Map;static _BUSY_COOLDOWN=30;static splitNatural(e,t=1500){if(!e||e.length<=t)return[e];let r=[],s="",n=()=>{s.trim()&&(r.push(s.trim()),s="")},i=e.split(/\n+/).filter(o=>o.length>0);for(let o of i)if(o.length>t){n();let a=o.match(/[^。！？；.!?;]+[。！？；.!?;]?/g)||[o],c="";for(let d of a){if(d.length>t){c&&(r.push(c.trim()),c="");for(let u=0;u<d.length;u+=t)r.push(d.slice(u,u+t));continue}c&&(c+d).length>t?(r.push(c.trim()),c=d):c+=d}c.trim()&&r.push(c.trim())}else s&&(s+`
...
`),j.chmodSync(t,493)));let r=e?null:[ee.join(Ft.homedir(),".bashrc"),ee.join(Ft.homedir(),".zshrc"),ee.join(Ft.homedir(),".profile")].find(s=>j.existsSync(s));if(r)j.readFileSync(r,"utf-8").includes(l)?console.log("[~] PATH \u5DF2\u914D\u7F6E"):(j.appendFileSync(r,`
export PATH="$PATH:${l}"
...
`),r.forEach((n,i)=>{let o=rt.displayTitle(n.summary,n.firstUserMsg),a=n.id===s?"  \u25C0 \u5F53\u524D":"";console.log(`${String(i+1).padStart(2)}. ${n.id.slice(0,8)} - ${o}  [${n.source}] ${String(n.msgCount).padStart(4)}\u6761${a}`)}),console.log(`
