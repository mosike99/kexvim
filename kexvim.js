...
`)});var Wt=class{get executionGuidance(){return"none"}get staleTimeoutFloorSeconds(){}get reasoningMaxOutputTokens(){}get modelName(){}};var Re=class{static FLOORS=[{slug:"deepseek-r1",floor:600},{slug:"deepseek-v4-flash",floor:600},{slug:"o1",floor:600},{slug:"o1-mini",floor:600},{slug:"o1-pro",floor:600},{slug:"o1-preview",floor:600},{slug:"o3",floor:600},{slug:"o3-pro",floor:600},{slug:"o3-mini",floor:300},{slug:"o4-mini",floor:300}].sort((e,t)=>t.slug.length-e.slug.length);static getStaleTimeoutFloor(e){if(typeof e!="string"||!e.trim())return;let t=e.trim().toLowerCase();if(t.includes("/")&&(t=t.split("/").pop()??t),!!t)return this.matchAny(t)}static isReasoningModel(e){return this.getStaleTimeoutFloor(e)!==void 0}static matchAny(e){for(let t of this.FLOORS)if(this.matches(e,t.slug))return t.floor}static matches(e,t){if(!e.startsWith(t))return!1;let r=e.slice(t.length);return r===""||r.startsWith("-")||r.startsWith(".")||r.startsWith("_")}};var Me=class extends Error{constructor(t,r,s,n=!1){super(t);this.statusCode=r;this.provider=s;this.retryable=n;this.name="ProviderError"}statusCode;provider;retryable},qt=class extends Me{constructor(t,r,s){super(t,429,s,!0);this.retryAfter=r;this.name="RateLimitError"}retryAfter};var zl=/^[\s]*<think>([\s\S]*?)<\/think>/m,Jl=/^[\s]*<reason>([\s\S]*?)<\/reason>/m,Yl=[/\b(sk-[a-zA-Z0-9]{20,})\b/g,/\b(Bearer\s+)([a-zA-Z0-9_\-\.]{20,})\b/gi,/(Authorization:\s*)(?:Bearer\s+)?[a-zA-Z0-9_\-\.]{20,}/gi,/(x-api-key:\s*)[a-zA-Z0-9_\-\.]{20,}/gi,/\b(ghp_[a-zA-Z0-9]{36,})\b/g,/\b(eyJ[a-zA-Z0-9_\-]{10,}\.[a-zA-Z0-9_\-]{10,}\.[a-zA-Z0-9_\-]{10,})\b/g],os=class{static stripThinkBlocks(e){return e?e.replace(/<think>[\s\S]*?<\/think>/g,"").replace(/<reason>[\s\S]*?<\/reason>/g,"").trim():""}static extractThinkContent(e){if(!e)return null;let t=e.match(zl);if(t)return t[1].trim();let r=e.match(Jl);return r?r[1].trim():null}static stripThinkTagsOnly(e){return e?e.replace(/<\/?(?:think|reason)>/g,"").trim():""}static stripThinkTagsFromReasoning(e){return e&&e.replace(/<think>[\s\S]*?<\/think>/g,"").trim()||null}static renderContent(e,t,r={}){let s={stripMode:"no_strip",contentRender:"content_only",stripThinkTagsFromReasoning:!0,...r},n=e??"",i=t??null,o=n;s.stripMode==="strip_think_content"?o=this.stripThinkBlocks(n):s.stripMode==="strip_think_tags"&&(o=this.stripThinkTagsOnly(n));let a=null;s.contentRender!=="content_only"&&(a=this.extractThinkContent(n));let c="";if(s.contentRender==="content_only")c=o;else if(s.contentRender==="content_and_think"){let u=[];a&&u.push(a),o&&u.push(o),c=u.join(`
...
`)}catch{return""}}static discoverContextFiles(e,t){let r=[],s=t||ze;for(let n of Do){if(n.includes("*"))continue;let i=`${e}/${n}`;try{s.existsSync(i)&&r.push({name:n,fullPath:i})}catch{}}return r}static loadProjectContext(e,t=2e4){for(let r of Do){if(r==="SOUL.md"||r.includes("*"))continue;let s=Qt.join(e,r);try{if(!ze.existsSync(s))continue;let n=ze.readFileSync(s,"utf-8").trim();if(!n)continue;return n=bt.stripFrontmatter(n).trim(),n.length>t&&(n=`${n.slice(0,t)}
...
Only proceed without loading a skill if genuinely none are relevant to the task.`}_shouldInjectEnforcement(e){return e!==!1}};var Ec=`Review the conversation above and update two things:
...
${e.content}`,"utf-8"),this.bumpUsage(e.name,"create"),!0}get(e){return this.list().find(t=>t.name===e)??null}update(e,t){let r=this.get(e);if(!r)return;let s={...r,...t,name:r.name,tags:t.tags??r.tags,relatedSkills:t.relatedSkills??r.relatedSkills,version:r.version,author:r.author},n=this.findSkillDir(e)??(s.category?P.join(this.skillsDir,"auto",s.category,s.name):P.join(this.skillsDir,"auto",s.name));F.mkdirSync(n,{recursive:!0});let i=(s.tags??[]).length>0?`    tags: [${s.tags.map(d=>`"${d}"`).join(", ")}]
...
`+l.sanitizeContext(e)+`
</memory-context>`}},Mr=class l{providers=[];toolToProvider=new Map;hasExternal=!1;bg=new Mi;addProvider(e){if(!(e.name==="builtin")){if(this.hasExternal){console.warn(S.t("memory.memmgr_rejected_provider",{name:e.name}));return}this.hasExternal=!0}this.providers.push(e);for(let r of e.getToolSchemas()){let s=l.normalizeToolSchema(r);if(!s)continue;let n=s.name;if(!(!n||typeof n!="string")){if(Ia.has(n)){console.warn(S.t("memory.memmgr_shadowed_tool",{providerName:e.name,toolName:n}));continue}this.toolToProvider.has(n)?console.warn(`Memory tool name conflict: '${n}' already registered by ${this.toolToProvider.get(n).name}`):this.toolToProvider.set(n,e)}}}get providersList(){return[...this.providers]}getProvider(e){return this.providers.find(t=>t.name===e)}buildSystemPrompt(){let e=[];for(let t of this.providers)try{let r=t.systemPromptBlock();r&&r.trim()&&e.push(r)}catch{}return e.join(`
...
`,ur=class{db;constructor(e){this.db=new iu(e),this.db.exec("PRAGMA busy_timeout = 15000"),this.db.exec(ou)}async upsert(e){let t=this.db.prepare(`
...
`;return Ze.writeFileSync(r,s,"utf-8"),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3}).code!==0?!1:b.runSyncResult("loginctl enable-linger "+gr.userInfo().username,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=Kr.join(gr.homedir(),".config","systemd","user",`${e}.service`);return Ze.existsSync(t)&&(b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:5e3}),Ze.unlinkSync(t),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3})),!0}catch{return!1}}isServiceInstalled(e){let t=Kr.join(gr.homedir(),".config","systemd","user",`${e}.service`);return Ze.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`systemctl --user is-active ${e}`,{timeoutMs:3e3});return t.code!==0?!1:t.stdout.trim()==="active"}startService(e){return b.runSyncResult(`systemctl --user start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`systemctl --user enable ${e}`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`systemctl --user disable ${e}`,{timeoutMs:5e3}).code===0}};import*as et from"fs";import*as Yn from"os";import*as Mt from"path";var Vn=class{platform="win32";findKexvimPids(){let e=b.runSyncResult(`powershell -Command "Get-Process node | Where-Object { $_.CommandLine -match 'dev\\.mjs|kexvim\\.js' } | Select-Object -ExpandProperty Id"`,{timeoutMs:5e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){let r=t?"/F":"";return b.runSyncResult(`taskkill ${r} /PID ${e}`,{timeoutMs:5e3}).code===0}installService(e){if(b.runSyncResult("nssm --version",{timeoutMs:3e3}).code===0&&b.runSyncResult(`nssm install ${e.name} "${e.execPath}" "${e.workDir}"`,{timeoutMs:1e4}).code===0)return!0;try{let r=Mt.join(process.env.APPDATA||Yn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");et.mkdirSync(r,{recursive:!0});let s=`@echo off
cd /d "${e.workDir}"
...
`;return et.writeFileSync(Mt.join(r,`${e.name}.bat`),s,"utf-8"),!0}catch{return!1}}uninstallService(e){if(!(b.runSyncResult(`nssm remove ${e} confirm`,{timeoutMs:1e4}).code===0)){let r=Mt.join(process.env.APPDATA||Yn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup"),s=Mt.join(r,`${e}.bat`);if(et.existsSync(s))try{et.unlinkSync(s)}catch{}}return!0}isServiceInstalled(e){if(b.runSyncResult(`nssm status ${e}`,{timeoutMs:3e3}).code===0)return!0;let t=Mt.join(process.env.APPDATA||Yn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");return et.existsSync(Mt.join(t,`${e}.bat`))}isServiceRunning(e){let t=b.runSyncResult(`powershell -Command "Get-Service ${e} | Select-Object -ExpandProperty Status"`,{timeoutMs:5e3});return t.code!==0?!1:t.stdout.trim().toLowerCase()==="running"}startService(e){return b.runSyncResult(`net start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`net stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`sc config ${e} start=auto`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`sc config ${e} start=disabled`,{timeoutMs:5e3}).code===0}};import*as tt from"fs";import*as fr from"os";import*as At from"path";var Xn=class{platform="darwin";findKexvimPids(){let e=b.runSyncResult('pgrep -af "dev\\.mjs|kexvim\\.js" | grep -v " web"',{timeoutMs:3e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){try{return process.kill(e,t?"SIGKILL":"SIGTERM"),!0}catch{return!1}}installService(e){try{let t=At.join(fr.homedir(),"Library","LaunchAgents");tt.mkdirSync(t,{recursive:!0});let r=At.join(t,`${e.name}.plist`),n=`<?xml version="1.0" encoding="UTF-8"?>
...
</plist>`;return tt.writeFileSync(r,n,"utf-8"),b.runSyncResult(`launchctl load ${r}`,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=At.join(fr.homedir(),"Library","LaunchAgents",`${e}.plist`);return tt.existsSync(t)&&(b.runSyncResult(`launchctl unload ${t}`,{timeoutMs:5e3}),tt.unlinkSync(t)),!0}catch{return!1}}isServiceInstalled(e){let t=At.join(fr.homedir(),"Library","LaunchAgents",`${e}.plist`);return tt.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`launchctl list local.${e}`,{timeoutMs:3e3});if(t.code!==0)return!1;let r=t.stdout.trim();return r.includes(`local.${e}`)&&!r.startsWith("-")}startService(e){let t=At.join(fr.homedir(),"Library","LaunchAgents",`${e}.plist`);return b.runSyncResult(`launchctl load ${t}`,{timeoutMs:5e3}).code===0}stopService(e){let t=At.join(fr.homedir(),"Library","LaunchAgents",`${e}.plist`);return b.runSyncResult(`launchctl unload ${t}`,{timeoutMs:5e3}).code===0}enableService(e){return this.startService(e)}disableService(e){return this.stopService(e)}};function uu(){switch(typeof process<"u"?process.platform:"linux"){case"win32":return new Vn;case"darwin":return new Xn;default:return new Jn}}var Qi=null;function It(){return Qi||(Qi=uu()),Qi}var Te=class l{static DEV_WEB_PORT=8787;static RELEASE_WEB_PORT=8788;static isDevEntry(){let e=process.argv[1]??"";if(/dev\.mjs$/i.test(e))return!0;try{if(/kexvim(?:\.mjs)?$/i.test(Fe.basename(e)))return Be.existsSync(Fe.join(Fe.dirname(e),"dist","dev.mjs"))}catch{}return!1}static webPort(){let e=Number(process.env.KEXVIM_WEB_PORT);return Number.isFinite(e)&&e>0?e:l.isDevEntry()?l.DEV_WEB_PORT:l.RELEASE_WEB_PORT}static findPid(){let e=l.webPort();try{if(process.platform==="win32"){let t=b.runSyncResult(`netstat -ano | findstr :${e} | findstr LISTENING`,{timeoutMs:5e3});for(let r of(t.stdout||"").split(/\r?\n/)){let s=r.trim().match(/LISTENING\s+(\d+)\s*$/);if(s)return Number(s[1])}}else{let r=(b.runSyncResult(`ss -ltnp 'sport = :${e}'`,{timeoutMs:5e3}).stdout||"").match(/pid=(\d+)/);if(r)return Number(r[1])}}catch{}return null}static webEntry(e){let t=Fe.join(e,"dist","dev.mjs");if(Be.existsSync(t))return t;let r=Fe.join(e,"kexvim.js");if(Be.existsSync(r))return r;throw new Error(`[kexvim] \u627E\u4E0D\u5230 web \u542F\u52A8\u5165\u53E3\uFF08\u65E0 dist/dev.mjs \u4E5F\u65E0 kexvim.js\uFF09\uFF1A${e}`)}static restart(e){let t=l.findPid();if(t!==null){let r=It();try{process.platform==="win32"?b.runSyncResult(`taskkill /PID ${t} /F /T`,{timeoutMs:5e3}):r.killProcess(t,!0)}catch{}}return l.spawnWeb(e)}static ensureRunning(e){return l.findPid()!==null?!1:l.spawnWeb(e)}static spawnWeb(e){let t=l.webEntry(e),r=Fe.join(e,"data","log");try{Be.mkdirSync(r,{recursive:!0})}catch{}let s=null;try{s=Be.openSync(Fe.join(r,"web.log"),"a")}catch{}let n=b.spawn(process.execPath,[t,"web"],{cwd:e,stdio:s!==null?["ignore",s,s]:"ignore",detached:!0,env:{...process.env,NODE_NO_WARNINGS:"1"}});return s!==null&&Be.closeSync(s),n.unref(),!0}};var je=class l{static resolveEntry(e){let t=hr.join(e,"dist","dev.mjs");if(Pt.existsSync(t))return t;let r=hr.join(e,"kexvim.js");if(Pt.existsSync(r))return r;throw new Error(`[kexvim] \u627E\u4E0D\u5230\u542F\u52A8\u5165\u53E3\uFF08\u65E0 dist/dev.mjs \u4E5F\u65E0 kexvim.js\uFF09\uFF1A${e}`)}static buildEnv(e){let t={};t.NODE_NO_WARNINGS="1";for(let[n,i]of Object.entries(process.env))i!==void 0&&(t[n]=i);let r=hr.join(e,"data",".env");if(!Pt.existsSync(r))return t;let s=Pt.readFileSync(r,"utf-8");for(let n of s.split(`
`)){let i=n.trim();if(!i||i.startsWith("#"))continue;let o=i.indexOf("=");if(o===-1)continue;let a=i.slice(0,o).trim(),c=i.slice(o+1).trim();(c.startsWith('"')&&c.endsWith('"')||c.startsWith("'")&&c.endsWith("'"))&&(c=c.slice(1,-1)),a&&!(a in t)&&(t[a]=c)}return t}static spawnFreshDaemon(e,t){let r=t;if(!r){let n=process.argv[1];r=n&&Pt.existsSync(n)?hr.resolve(n):l.resolveEntry(e)}let s=b.spawn(process.execPath,[r,"--daemon"],{cwd:e,stdio:"ignore",env:l.buildEnv(e),detached:!0});return s.unref(),s.pid??null}static selfRestart(e,t){try{Te.restart(e)}catch(r){t?.(`[kexvim] web \u91CD\u542F\u5931\u8D25: ${r instanceof Error?r.message:String(r)}
`)}return l.spawnFreshDaemon(e,process.argv[1])}};import*as Ue from"node:worker_threads";var yr=class l{static workersLaunched=!1;static workerInstances=new Map;static async startWatchdog(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D .stop_watchdog\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let t=Lt.join(e,"data",".stop_watchdog");setInterval(()=>{if(H.existsSync(t)){try{H.unlinkSync(t)}catch{}process.exit(0)}},3e3),process.on("SIGTERM",()=>{je.spawnFreshDaemon(e),process.exit(0)}),await new Promise(()=>{})}static async startGuardian(){let e=I.load();S.init(e.language||"zh-CN");let t=new Ct(Et.load());console.log(`[guardian] ready \u2014 ${t.llmClient.model}`);let r=Ue.parentPort;if(!r){await new Promise(()=>{});return}r.on("message",async s=>{let n=s;if(!(!n||n.type!=="repair"))try{let i=await t.process(String(n.text??""));r.postMessage({type:"repair-result",reqId:n.reqId,text:i})}catch(i){r.postMessage({type:"repair-result",reqId:n.reqId,text:`[guardian error] ${i.message}`})}})}static terminateWorkers(){for(let e of l.workerInstances.values())e.terminate();l.workerInstances.clear()}static launchConsoleWorkers(){for(let t of["watchdog","agent"]){let r=new Ue.Worker(process.argv[1],{workerData:{role:t},stdout:!0,stderr:!0});r.stdout.on("data",s=>process.stdout.write(`[${t[0]}] ${s}`)),r.stderr.on("data",s=>process.stderr.write(`[${t[0]}] ${s}`)),r.on("exit",s=>console.log(`[${t}] exited (${s})`)),l.workerInstances.set(t,r)}let e=()=>{l.terminateWorkers()};process.on("SIGTERM",e),process.on("SIGINT",e),process.on("exit",e)}static launchDaemon(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u542F\u52A8 daemon\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");if(Se.recordBoot(e))try{H.appendFileSync(l.resolveLogPath(),`[kexvim] \u26A0\uFE0F restart_loop guard TRIPPED: ${Se.bootCount(e)} boots in ${Se.WINDOW_SECONDS}s. \u81EA\u52A8\u62C9\u8D77\u6682\u505C\uFF0C\u8BF7\u4EBA\u5DE5\u68C0\u67E5\uFF08kexvim status / kexvim clear-loop\uFF09
`)}catch{}try{H.writeFileSync(Lt.join(e,"data","kexvim.pid"),String(process.pid))}catch{}let r=Lt.join(e,"data","daemon.heartbeat"),s=()=>{try{H.writeFileSync(r,String(process.pid))}catch{}};s(),setInterval(s,3e4),process.on("exit",a=>{try{H.appendFileSync(l.resolveLogPath(),`[kexvim] daemon EXIT code=${a} at ${new Date().toISOString()}
`)}catch{}try{H.rmSync(Lt.join(e,"data","kexvim.pid"),{force:!0})}catch{}});let n=()=>{try{H.appendFileSync(l.resolveLogPath(),`[kexvim] daemon shutdown (signal) at ${new Date().toISOString()}
`)}catch{}l._shuttingDown=!0,l.terminateWorkers(),process.exit(0)};process.on("SIGTERM",n),process.on("SIGINT",n);let i=()=>{if(process.platform!=="linux")return!1;try{let a=H.readFileSync("/proc/self/environ","utf-8");return a.includes("INVOCATION_ID")||a.includes("SYSTEMD_EXEC_PID")}catch{return!1}};process.platform!=="win32"&&process.on("SIGHUP",()=>{try{H.appendFileSync(l.resolveLogPath(),`[kexvim] SIGHUP received \u2014 ${i()?"systemd-managed, exiting for systemd restart":"standalone, spawning fresh daemon"}
`)}catch{}i()||je.selfRestart(e,a=>{try{H.appendFileSync(l.resolveLogPath(),a)}catch{}}),n()});let o=l.resolveLogPath();for(let a of["watchdog","agent"]){let c=new Ue.Worker(process.argv[1],{workerData:{role:a},stdout:!0,stderr:!0});l.workerInstances.set(a,c);let d=m=>{try{H.appendFileSync(o,m)}catch{}};c.stdout.on("data",d),c.stderr.on("data",d);let u=()=>{let m=new Ue.Worker(process.argv[1],{workerData:{role:a},stdout:!0,stderr:!0});l.workerInstances.set(a,m),m.stdout.on("data",d),m.stderr.on("data",d),m.on("error",g=>{try{H.appendFileSync(o,`[kexvim] worker '${a}' error: ${g.message}
`)}catch{}}),m.on("exit",p)},p=m=>{if(l.workerInstances.delete(a),m===0||m===null||l._shuttingDown){try{H.appendFileSync(o,`[kexvim] worker '${a}' exited (code=${m}) at ${new Date().toISOString()}
`)}catch{}return}let f=l.shouldStopRestarting(a);try{H.appendFileSync(o,`[kexvim] worker '${a}' exited (code=${m}) ${f?"\u2014 CRASH, restart halted (loop guard)":"\u2014 CRASH, restarting"} at ${new Date().toISOString()}
`)}catch{}f||setTimeout(u,2e3)};c.on("error",m=>{try{H.appendFileSync(o,`[kexvim] worker '${a}' error: ${m.message}
`)}catch{}}),c.on("exit",p)}}static resolveLogPath(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D kexvim.log\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let t=Lt.join(e,"data","log");try{H.mkdirSync(t,{recursive:!0})}catch{}return Lt.join(t,"kexvim.log")}static _shuttingDown=!1;static _crashTimes=new Map;static CRASH_WINDOW_MS=6e4;static MAX_CRASHES=3;static shouldStopRestarting(e){let t=Date.now(),r=(l._crashTimes.get(e)??[]).filter(s=>t-s<l.CRASH_WINDOW_MS);return r.push(t),l._crashTimes.set(e,r),r.length>l.MAX_CRASHES}static async handleWorkerDispatch(){if(!Ue.isMainThread){let t=Ue.workerData?.role||"agent";return t==="watchdog"?(await l.startWatchdog(),!0):t==="guardian"?(await l.startGuardian(),!0):!1}return process.argv.includes("--daemon")?(l.launchDaemon(),!0):(I.load().platform?.enabled&&!process.argv.slice(2).includes("web")&&(l.launchConsoleWorkers(),l.workersLaunched=!0),!1)}};import*as ge from"node:fs";import*as rs from"node:path";import*as hl from"node:worker_threads";var pu=new Set(["stop","new","newchat","reset","\u65B0\u4F1A\u8BDD","\u5207\u6362\u4F1A\u8BDD","\u5207\u4F1A\u8BDD"]);function Ga(l,e){let t=process.env[l];if(t===void 0||t.trim()==="")return e;let r=parseFloat(t);return Number.isFinite(r)?r:e}function mu(l,e){let t=process.env[l];return t===void 0||t.trim()===""?e:["1","true","yes","on"].includes(t.trim().toLowerCase())}function gu(l,e){let t=process.env[l]?.trim().toLowerCase();return t==="steer"||t==="queue"||t==="interrupt"?t:e}var vr=class l{adapters=new Map;config;messageHandler=null;activeSessions=new Map;pendingMessages=new Map;debounceTimers=new Map;debounceStates=new Map;_lastBusyNotifyTime=new Map;_sessionControllers=new Map;static _BUSY_COOLDOWN=30;static splitNatural(e,t=1500){if(!e||e.length<=t)return[e];let r=[],s="",n=()=>{s.trim()&&(r.push(s.trim()),s="")},i=e.split(/\n+/).filter(o=>o.length>0);for(let o of i)if(o.length>t){n();let a=o.match(/[^。！？；.!?;]+[。！？；.!?;]?/g)||[o],c="";for(let d of a){if(d.length>t){c&&(r.push(c.trim()),c="");for(let u=0;u<d.length;u+=t)r.push(d.slice(u,u+t));continue}c&&(c+d).length>t?(r.push(c.trim()),c=d):c+=d}c.trim()&&r.push(c.trim())}else s&&(s+`
`+o).length>t?(n(),s=o):s=s?s+`
`+o:o;return n(),r.filter(o=>o.length>0)}_busyStateProvider=null;_steerProvider=null;_redirectProvider=null;constructor(e={}){this.config={timeout:e.timeout??6e4,startupWait:e.startupWait??5e3,busyTextDebounceSeconds:e.busyTextDebounceSeconds??Ga("HERMES_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS",.35),busyTextHardCapSeconds:e.busyTextHardCapSeconds??Ga("HERMES_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS",1),busyAckEnabled:e.busyAckEnabled??mu("HERMES_GATEWAY_BUSY_ACK_ENABLED",!0),steerAckEnabled:e.steerAckEnabled??!0,busyAckDetail:e.busyAckDetail??!0,busyInputMode:e.busyInputMode??gu("HERMES_GATEWAY_BUSY_INPUT_MODE","interrupt"),busyTextMode:e.busyTextMode??(process.env.HERMES_GATEWAY_BUSY_TEXT_MODE?.trim().toLowerCase()==="queue"?"queue":"interrupt")}}getSignalForMessage(e){return this._sessionControllers.get(this.getSessionKey(e))?.signal}register(e){if(this.adapters.has(e.name))throw new Error(`Adapter '${e.name}' already registered`);this.adapters.set(e.name,e)}async unregister(e){let t=this.adapters.get(e);t&&(await t.stop(),this.adapters.delete(e))}setMessageHandler(e,t){this.messageHandler=e,this._busyStateProvider=t?.busyStateProvider??null,this._steerProvider=t?.steerProvider??null,this._redirectProvider=t?.redirectProvider??null,this.adapters.forEach(r=>{r.setMessageHandler(this.dispatch)})}getSessionKey(e){return e.groupId?`${e.groupId}:${e.userId}`:e.userId}toRuntimeSessionKey(e){return e.source?`${e.source.platform}:${e.source.chatId}`:e.groupId?`qq:group:${e.groupId}`:`qq:${e.userId.replace(/^user:/,"")}`}getSenderKey(e){return e.groupId?e.userId:""}releaseSessionGuard(e){this.activeSessions.has(e)&&this.activeSessions.delete(e)}isBypassCommand(e){if(!e.trim().startsWith("/"))return null;let t=e.trim().slice(1).split(" ")[0]?.toLowerCase();return t&&pu.has(t)?t:null}static _BUSY_QUEUE_MAX_PENDING=32;queueOrReplacePendingEvent(e,t){let r=this.pendingMessages.get(e);if(r||(r=[],this.pendingMessages.set(e,r)),(!!t.media_urls?.length||t.message_type==="photo")&&r.length>0){let n=r[0];n.text&&t.text?n.text=n.text+`
`+t.text:t.text&&(n.text=t.text);let i=n.media_urls,o=t.media_urls;o&&Array.isArray(o)&&(n.media_urls=[...i||[],...o])}else{if(r.length>=l._BUSY_QUEUE_MAX_PENDING){console.warn(`[gateway] Dropping busy follow-up for ${e} \u2014 pending queue at cap (${l._BUSY_QUEUE_MAX_PENDING})`);return}r.push({...t})}}dequeuePending(e){let t=this.pendingMessages.get(e);if(!t||t.length===0){this.pendingMessages.delete(e);return}let r=t.shift();return t.length===0&&this.pendingMessages.delete(e),r}isDebounceCandidate(e){return!!e.text?.trim()&&!e.text.trim().startsWith("/")}textDebounceDelay(e){let t=Date.now(),r=e.lastTs+this.config.busyTextDebounceSeconds*1e3,s=e.firstTs+this.config.busyTextHardCapSeconds*1e3;return Math.max(0,Math.min(r,s)-t)}canMergeDebounceEvents(e,t){return e.senderKey===t}queueOrDebounce(e,t){this.isDebounceCandidate(t)?this.queueTextDebounce(e,t):this.queueOrReplacePendingEvent(e,t)}dispatch=async e=>{if(!this.messageHandler)return;let t=this.getSessionKey(e);if(!this.activeSessions.has(t)){this.setSessionGuard(t),this.processMessage(t,e);return}let r=q.getProgress(this.toRuntimeSessionKey(e)),s=Date.now(),n=m=>{console.log(`[gateway] busy-dispatch ${t} \u2192 ${m} text="${(e.text||"").trim().slice(0,40)}"`)};if(this.isBypassCommand(e.text)){this._abortSessionController(t),this.queueOrDebounce(t,e),this._sendBusyAck(t,e,r,s,"interrupt"),n("bypass-interrupt");return}if(!r){this.queueOrDebounce(t,e),this._sendBusyAck(t,e,r,s,"queue"),n("queue-no-progress");return}if(this._busyStateProvider){let m=this._busyStateProvider(t);if(m.hasActiveSubagents){this.queueOrDebounce(t,e),this._sendBusyAck(t,e,r,s,"subagent"),n("queue-subagent");return}if(m.isCompressing){this.queueOrDebounce(t,e),this._sendBusyAck(t,e,r,s,"compression"),n("queue-compression");return}}let o=this.config.busyInputMode,a=this.config.busyTextMode;if(!!e.text?.trim()&&!e.text.trim().startsWith("/")&&!this.isBypassCommand(e.text)&&a==="queue"&&o!=="steer"){this.queueOrDebounce(t,e),this._sendBusyAck(t,e,r,s,"queue"),n("queue-text-mode");return}let d=!1;if(o==="steer"){let m=(e.text||"").trim();m?this._steerProvider&&this._steerProvider(t,m)?(d=!0,n("steer-injected")):(o="queue",n("steer-fail-queue")):o="queue"}let u=!1;o==="interrupt"&&(u=!!e.text?.trim()&&!e.media_urls?.length&&!e.media_types?.length&&!!this._redirectProvider&&this._redirectProvider(t,(e.text||"").trim()),u?n("redirect-ok"):(this._abortSessionController(t,e.text),n("abort-hard"))),!d&&!u&&(this.queueOrDebounce(t,e),n("queue-fifo"));let p=u?"redirect":o;n(`ack:${p}`),this._sendBusyAck(t,e,r,s,p)};_abortSessionController(e,t){let r=this._sessionControllers.get(e);r&&!r.signal.aborted&&(t&&t.trim()?r.abort(t.trim()):r.abort())}_sendBusyAck(e,t,r,s,n){if(!this.config.busyAckEnabled||n==="steer"&&!this.config.steerAckEnabled)return;let i=this._lastBusyNotifyTime.get(e)??0;if(s-i<l._BUSY_COOLDOWN*1e3)return;this._lastBusyNotifyTime.set(e,s);let o="";if(this.config.busyAckDetail&&r){let d=Math.max(1,Math.round((s-r.startTime)/6e4)),u=r.current??0,p=r.max??0,m=q.getCurrentTool()??"",g=[];d>0&&g.push(`\u5DF2\u8FD0\u884C ${d} \u5206\u949F`),p&&g.push(`\u8FED\u4EE3 ${u}/${p}`),m&&g.push(`\u6B63\u5728\u6267\u884C: ${m}`),g.length>0&&(o=` (${g.join(", ")})`)}let a;switch(n){case"interrupt":a="gateway.busy_interrupt";break;case"redirect":a="gateway.busy_redirect";break;case"subagent":a="gateway.busy_queue_subagent";break;case"compression":a="gateway.busy_queue_compression";break;case"queue":a="gateway.busy_queue";break;default:a="gateway.busy_steer";break}let c=S.t(a,{status:o});t.sendReply?.(c)}setSessionGuard(e){this.activeSessions.set(e,!0)}queueTextDebounce(e,t){let r=Date.now(),s=this.getSenderKey(t),n=this.debounceStates.get(e);if(n){if(!this.canMergeDebounceEvents(n,s)){this.flushTextDebounce(e);let i=this.pendingMessages.get(e),o=i&&i.length>0?i[0]:void 0;if(o&&this.canMergeDebounceEvents({senderKey:this.getSenderKey(o)},s)){o.text=o.text?o.text+`
...
===== \u5206\u652F\u5BF9\u8BDD\u5185\u5BB9\u7ED3\u675F =====`}static async compact(e,t,r){let s=e.getMessagesAsConversation(t,200);if(s.length<10)return null;let n=Math.min(40,Math.floor(s.length*.4)),i=s.length-n,o=s.slice(0,i);if(o.length===0)return null;let a=o.reduce((d,u)=>d+l.estimateTokens(u),0);if(a<500)return null;let c;return r?c=await r(o,void 0):c=`\u538B\u5B9E\u4E86 ${o.length} \u6761\u6D88\u606F\uFF0C\u8282\u7701\u7EA6 ${a} tokens\u3002`,{summary:c,firstKeptEntryId:i,tokensBefore:a,estimatedTokensAfter:a}}};var lp={qq:l=>new zr({appId:l.app_id,clientSecret:l.client_secret,apiBase:l.api_base,markdownSupport:l.markdown_support}),api_server:l=>new Jr({port:l.port,host:l.host,apiKey:l.api_key}),telegram:l=>new Yr({token:l.token,pollTimeout:l.poll_timeout,groupMentionOnly:l.group_mention_only}),discord:l=>new Vr({token:l.token,groupMentionOnly:l.group_mention_only}),weixin:l=>new Xr({token:l.token,accountId:l.account_id,groupMentionOnly:l.group_mention_only}),dingtalk:l=>new Qr({clientId:l.client_id,clientSecret:l.client_secret,groupMentionOnly:l.group_mention_only}),feishu:l=>new Zr({appId:l.app_id,appSecret:l.app_secret,groupMentionOnly:l.group_mention_only}),ws:l=>new ts({port:l.port,host:l.host,apiKey:l.api_key})};function gl(l,e){let t=[];for(let[r,s]of Object.entries(e)){let n=lp[r];if(!n){console.warn(S.t("gateway.unknown_adapter",{name:r}));continue}if(r==="qq"){let o=s;if(!o.app_id){console.warn("\u26A0\uFE0F QQ Bot app_id \u672A\u914D\u7F6E\uFF08config.yaml \u4E2D platform.adapters.qq.app_id\uFF0C\u6CE8\u518C\uFF1Ahttps://q.qq.com\uFF09\uFF0C\u8DF3\u8FC7 QQ \u9002\u914D\u5668");continue}if(!o.client_secret){console.warn("\u26A0\uFE0F QQ Bot client_secret \u672A\u914D\u7F6E\uFF08config.yaml \u4E2D platform.adapters.qq.client_secret\uFF0C\u6CE8\u518C\uFF1Ahttps://q.qq.com\uFF09\uFF0C\u8DF3\u8FC7 QQ \u9002\u914D\u5668");continue}}let i=n(s);l.register(i),t.push(i),console.error(S.t("gateway.adapter_registered",{name:r}))}return t}function ro(){let l=I.findProjectRoot();if(!l)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D .last_user\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");return rs.join(l,"data",".last_user")}var Qn=null,cp=0,Ft=new Map;function dp(){if(Qn)return Qn;let l=new hl.Worker(process.argv[1],{workerData:{role:"guardian"},stdout:!0,stderr:!0});return l.stdout.on("data",e=>{try{console.error(`[guardian] ${e}`)}catch{}}),l.stderr.on("data",e=>{try{console.error(`[guardian] ${e}`)}catch{}}),l.on("message",e=>{let t=e;if(!t||t.type!=="repair-result")return;let r=Ft.get(t.reqId);r&&(clearTimeout(r.timer),Ft.delete(t.reqId),r.resolve(String(t.text??"")))}),l.on("exit",e=>{Qn=null;for(let[,t]of Ft)clearTimeout(t.timer),t.resolve(`[guardian] \u7EBF\u7A0B\u5DF2\u9000\u51FA (code=${e})`);Ft.clear()}),l.on("error",e=>{try{console.error(`[guardian] worker error: ${e.message}`)}catch{}}),Qn=l,l}function up(l,e=18e4){let t=dp(),r=++cp;return new Promise(s=>{let n=setTimeout(()=>{Ft.delete(r),s(`[guardian] \u8D85\u65F6\u65E0\u54CD\u5E94\uFF08${Math.round(e/1e3)}s\uFF09\uFF0C\u7EBF\u7A0B\u53EF\u80FD\u4ECD\u5728\u5904\u7406`)},e);Ft.set(r,{resolve:s,timer:n});try{t.postMessage({type:"repair",reqId:r,text:l})}catch(i){clearTimeout(n),Ft.delete(r),s(`[guardian] \u53D1\u9001\u5931\u8D25: ${i.message}`)}})}function fl(l,e){let t=(()=>{try{return ge.readFileSync(e,"utf-8").trim()||null}catch{return null}})();if(!t)return;let r=l.find(c=>c.name!=="api_server");if(!r)return;let s=()=>r.sendText(t,"\u2705 Kexvim \u5DF2\u91CD\u65B0\u4E0A\u7EBF").catch(()=>{}),n=100,i=15e3,o=Date.now(),a=()=>{r.isConnected()||Date.now()-o>i?s():(n=Math.min(n*1.5,1e3),setTimeout(a,n))};a()}var Zn=class l{static createGatewayHandler(e){let{runtime:t,sessionStore:r,registry:s,signalProvider:n,sendTyping:i}=e;return async o=>{try{console.error(S.t("gateway.msg_in",{userId:o.userId,text:o.text.slice(0,100)}));let a=ro();if(!(o.source?o.source.chatType==="group":o.userId.startsWith("group:")))try{ge.writeFileSync(a,o.userId,"utf-8")}catch{}let d=o.text.trim(),u=d.toLowerCase();if(u.startsWith("repair/")||u.startsWith("\u4FEE\u590D/")){let C=(await up(d))?.trim();return C&&console.error(S.t("gateway.msg_out",{userId:o.userId,text:C.slice(0,100)})),C}let p="qq",m="c2c",g=o.userId,f,h=o.userId;if(o.source)p=o.source.platform,m=o.source.chatType,g=o.source.chatId,f=o.source.guildId??o.groupId,h=o.source.userId??o.userId;else{let A=o.userId.split(":");A.length>=2&&A[0]==="group"?(m="group",f=A[1],g=`group:${A[1]}`):A.length>=2&&A[0]==="user"&&(g=A[1]),h=A.length>=2?A[1]:o.userId}if(new Set(["/new","/newchat","/\u65B0\u4F1A\u8BDD","/\u5207\u6362\u4F1A\u8BDD","/\u5207\u4F1A\u8BDD","\u5207\u4F1A\u8BDD"]).has(d))try{let{sessionId:A,prevSessionId:C}=await t.startNewSession({source:p,chatType:m,chatId:g,userId:h}),L=C?`\u65E7\u4F1A\u8BDD\uFF08${C.slice(0,8)}\uFF09\u5DF2\u4FDD\u7559\uFF0C\u53EF\u968F\u65F6\u56DE\u6EAF\u3002`:"";return`\u2705 \u5DF2\u5207\u6362\u5230\u65B0\u4F1A\u8BDD\uFF08${A.slice(0,8)}\uFF09\u3002${L}`}catch(A){return`\u274C \u5207\u4F1A\u8BDD\u5931\u8D25: ${A.message}`}let y=/^\/resume\s+([A-Za-z0-9-]{8,})/;if(d==="/sessions"||d==="/\u4F1A\u8BDD\u5217\u8868"||y.test(d))try{let A=r.profile??"default";if(d==="/sessions"||d==="/\u4F1A\u8BDD\u5217\u8868"){let B=await r.listRecent(A,100);if(B.length===0)return"\u6682\u65E0\u5386\u53F2\u4F1A\u8BDD\u3002\u53D1\u9001 /new \u5F00\u59CB\u65B0\u4F1A\u8BDD\u3002";let re="";try{let fe=I.findProjectRoot(),ht=fe?rs.join(fe,"data",".current_session"):"";ht&&ge.existsSync(ht)&&(re=ge.readFileSync(ht,"utf-8").trim())}catch{}return`\u6700\u8FD1\u4F1A\u8BDD\uFF1A
${B.map((fe,ht)=>{let mo=fe.id===re?" \u25C0 \u5F53\u524D":"";return`${ht+1}. ${fe.id.slice(0,8)} - ${nt.displayTitle(fe.summary,fe.firstUserMsg)}  [${fe.source}]${mo}`}).join(`
`)}

\u7528 /resume <\u524D8\u4F4DID> \u6062\u590D\u3002`}let C=d.match(y),L=await t.resumeSession(C[1],{source:p,chatType:m,chatId:g,userId:h});return L?`\u2705 \u5DF2\u6062\u590D\u4F1A\u8BDD ${L.sessionId.slice(0,8)}\uFF08${L.messageCount} \u6761\u6D88\u606F\uFF09\uFF0C\u7EE7\u7EED\u5BF9\u8BDD\u3002`:`\u274C \u627E\u4E0D\u5230\u4F1A\u8BDD ${C[1]}\u3002\u7528 /sessions \u67E5\u770B\u53EF\u6062\u590D\u7684\u4F1A\u8BDD\u3002`}catch(A){return`\u274C \u4F1A\u8BDD\u64CD\u4F5C\u5931\u8D25: ${A.message}`}try{let A=I.findProjectRoot(),C=A?rs.join(A,"data","session-switch.json"):null;if(C&&ge.existsSync(C)){let L=JSON.parse(ge.readFileSync(C,"utf-8"));if(ge.rmSync(C,{force:!0}),L.sessionId){let B=await t.resumeSession(L.sessionId,{source:p,chatType:m,chatId:g,userId:h});return B?(console.error(`[CLI\u5207\u6362] \u5DF2\u6062\u590D\u4F1A\u8BDD ${B.sessionId.slice(0,8)}\uFF08${B.messageCount} \u6761\u6D88\u606F\uFF09`),`\u2705 \u5DF2\u5207\u6362\u5230\u5386\u53F2\u4F1A\u8BDD ${B.sessionId.slice(0,8)}\uFF08${B.messageCount} \u6761\u6D88\u606F\uFF09\uFF0C\u7EE7\u7EED\u5BF9\u8BDD\u3002`):`\u274C \u627E\u4E0D\u5230\u4F1A\u8BDD ${L.sessionId.slice(0,8)}\u3002`}}}catch(A){console.error(`[CLI\u5207\u6362] \u6807\u8BB0\u5904\u7406\u5931\u8D25: ${A.message}`);try{ge.rmSync(rs.join(I.findProjectRoot()??".","data","session-switch.json"),{force:!0})}catch{}}let v=r.recover({chatId:g,chatType:m,source:p,userId:h}).session?.id,w=o.sendReply?A=>{o.sendReply(A).catch(()=>{})}:void 0;try{i&&i(o.userId)}catch{}let R=n?.(o),_=Number(process.env.HERMES_AGENT_NOTIFY_INTERVAL||180),M=_>0?_*1e3:0,N=null,ye=()=>{N&&(clearInterval(N),N=null)};M>0&&w&&(N=setInterval(()=>{let A=t.getActivitySummary();if(!A)return;let C=Math.floor((Date.now()-A.startTime)/6e4);w(S.t("gateway.long_running",{elapsed:C,i:A.iteration,max:A.maxIterations,tool:A.currentTool?` \u2014 ${A.currentTool}`:""}))},M));let $;try{$=await t.chat(d,{source:p,chatType:m,chatId:g,userId:h,signal:R,statusCallback:w})}finally{ye()}let J=$.content?.trim();if(!J)return;if(console.error(S.t("gateway.msg_out",{userId:o.userId,text:J.slice(0,100)})),$.autoResetNotice)try{await o.sendReply?.($.autoResetNotice)}catch(A){console.warn(`[SessionReset] notice send failed: ${A?.message}`)}if(v)try{let A=r.buildContext(v);if(A.length>20&&gt.shouldCompact(A.reduce((C,L)=>C+(typeof L.content=="string"?L.content.length/4:0),0),128e3)){let C=async(L,B)=>{try{let re=B?gt.buildUpdateCompactionPrompt(L,B):gt.buildCompactionPrompt(L);return(await s.resolve(e.compactorProvider||"deepseek",e.compactorModel||"deepseek-chat").chat([{role:"system",content:gt.SUMMARIZATION_SYSTEM_PROMPT},{role:"user",content:re}],void 0,50,.3))?.choices?.[0]?.message?.content?.trim()||"\uFF08\u81EA\u52A8\u538B\u5B9E\u6458\u8981\uFF09"}catch{return"\uFF08\u81EA\u52A8\u538B\u5B9E\u6458\u8981\uFF09"}};gt.compact(r,v,C).catch(()=>{})}}catch{}return J}catch(a){let c=a.message,d=/401|403|unauthorized|forbidden|auth/i.test(c)&&!/429|rate/i.test(c),u=/429|rate.?limit|too many/i.test(c),p=/ENOTFOUND|ECONNREFUSED|ECONNRESET|ETIMEDOUT|fetch failed|socket/i.test(c),m=/5\d{2}|api.*error|server.*error|internal|timeout/i.test(c)&&!d&&!u&&!p,g;d?g="gateway.error_auth":u?g="gateway.error_rate_limit":p?g="gateway.error_network":m?g="gateway.error_api":g="gateway.error_unknown";let f=S.t(g,g==="gateway.error_api"||g==="gateway.error_unknown"?{msg:c}:{});return console.error(S.t("gateway.msg_error",{msg:c})),f}}}static installCronDeliverer(e,t){ne.instance.setDeliverer(async(r,s)=>{try{if(s==="all"){let n=t();if(!n)return;for(let i of e)if(i.name!=="api_server"){await i.sendText(n,r);break}}else{let n=s.indexOf(":");if(n<0)return;let i=s.slice(0,n),o=s.slice(n+1),a=e.find(c=>c.name===i);a&&await a.sendText(o,r)}}catch(n){console.error(`[CronScheduler] \u6295\u9012\u5931\u8D25 (${s}):`,n?.message)}})}static async startGatewayMode(e,t,r,s){let n=new vr,i=t.platform,o=ro(),a=null;try{ge.existsSync(o)&&(a=ge.readFileSync(o,"utf-8").trim()||null)}catch{}let c=i.adapters?gl(n,i.adapters):[],d=c.find(g=>g.name==="qq");be.notifyHandler=g=>{if(a){for(let f of c)if(f.name!=="api_server"){f.sendText(a,g).catch(()=>{});break}}try{r.appendSystemNotice(g)}catch{}},l.installCronDeliverer(c,()=>a);let u=c.find(g=>typeof g.sendTyping=="function"),p=l.createGatewayHandler({runtime:e,sessionStore:r,registry:s,compactorProvider:t.llm?.defaultProvider,compactorModel:t.llm?.defaultModel,signalProvider:g=>n.getSignalForMessage(g),sendTyping:u?g=>{u.sendTyping(g).catch(()=>{})}:void 0});n.setMessageHandler(p,{busyStateProvider:g=>e.getBusyState(g),steerProvider:(g,f)=>e.steer(f),redirectProvider:(g,f)=>e.redirect(f)}),console.error(S.t("gateway.starting"));let m=n.start();fl(c,o),await Promise.race([m,new Promise(g=>{let f=async()=>{console.error(S.t("gateway.shutting_down")),await n.stop(),g()};process.on("SIGINT",f),process.on("SIGTERM",f)})])}static async startGatewayWorker(e,t,r,s){let n=new vr,i=t.platform,o=ro(),a=i.adapters?gl(n,i.adapters):[];be.notifyHandler=p=>{let m=(()=>{try{return ge.readFileSync(o,"utf-8").trim()||null}catch{return null}})();if(m){for(let g of a)if(g.name!=="api_server"){g.sendText(m,p).catch(()=>{});break}}},l.installCronDeliverer(a,()=>{try{return ge.readFileSync(o,"utf-8").trim()||null}catch{return null}});let c=a.find(p=>typeof p.sendTyping=="function"),d=l.createGatewayHandler({runtime:e,sessionStore:r,registry:s,compactorProvider:t.llm?.defaultProvider,compactorModel:t.llm?.defaultModel,sendTyping:c?p=>{c.sendTyping(p).catch(()=>{})}:void 0});n.setMessageHandler(d,{busyStateProvider:p=>e.getBusyState(p),steerProvider:(p,m)=>e.steer(m),redirectProvider:(p,m)=>e.redirect(m)});let u=n.start();fl(a,o),await u}};import*as bl from"node:http";import*as U from"node:fs";import*as K from"node:path";import*as kl from"node:crypto";import{WebSocketServer as mp,WebSocket as vl}from"ws";var yl=`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
platform:
  ...
  adapters:
    qq:
      app_id: ""
      client_secret: ""
`;function wl(l){let e=process.platform==="win32",t=ee.join(l,e?"kexvim.cmd":"kexvim");j.existsSync(t)||(e?j.writeFileSync(t,`@echo off\r
...
`),j.chmodSync(t,493)));let r=e?null:[ee.join(Ut.homedir(),".bashrc"),ee.join(Ut.homedir(),".zshrc"),ee.join(Ut.homedir(),".profile")].find(s=>j.existsSync(s));if(r)j.readFileSync(r,"utf-8").includes(l)?console.log("[~] PATH \u5DF2\u914D\u7F6E"):(j.appendFileSync(r,`
export PATH="$PATH:${l}"
`),console.log(`[~] \u5DF2\u5C06 ${l} \u52A0\u5165 ${ee.basename(r)}\uFF0C\u91CD\u542F\u7EC8\u7AEF\u540E\u751F\u6548`));else if(e)try{let n=`$h = ${"'"+l.replace(/'/g,"''")+"'"}; $p = [Environment]::GetEnvironmentVariable('PATH','User'); if ($p -and $p.Contains($h)) { Write-Output 'exists' } elseif ($p) { [Environment]::SetEnvironmentVariable('PATH', $p + ';' + $h, 'User'); Write-Output 'added' } else { [Environment]::SetEnvironmentVariable('PATH', $h, 'User'); Write-Output 'added' }`,i=b.runFileSync("powershell",["-NoProfile","-Command",n],{maxBuffer:1024*1024});console.log(i.includes("exists")?"[~] PATH \u5DF2\u914D\u7F6E":"[~] \u5DF2\u5C06 Kexvim \u52A0\u5165 PATH\uFF0C\u65B0\u7EC8\u7AEF\u751F\u6548")}catch{console.log("[~] \u672A\u80FD\u81EA\u52A8\u8BBE\u7F6E PATH\uFF0C\u8BF7\u624B\u52A8\u5C06\u542F\u52A8\u5668\u76EE\u5F55\u52A0\u5165\u73AF\u5883\u53D8\u91CF PATH")}else console.log(`[~] \u624B\u52A8\u5C06\u4EE5\u4E0B\u884C\u52A0\u5165 shell \u914D\u7F6E\u6587\u4EF6 (~/.bashrc / ~/.zshrc):
    export PATH="$PATH:${l}"`)}async function Tl(l,e){let t=i=>{try{return j.accessSync(i),!0}catch{return!1}};if(t(ee.join(l,"data",".env"))){console.log("Kexvim \u5DF2\u521D\u59CB\u5316(data/.env \u5DF2\u5B58\u5728)\u3002"),wl(l);return}if(t(ee.join(l,"kexvim.js")))console.log("[~] kexvim.js \u5DF2\u5C31\u7EEA\uFF08\u5B89\u88C5\u811A\u672C\u5DF2\u63D0\u4F9B\uFF09");else if(j.mkdirSync(l,{recursive:!0}),xl("git")){console.log("[~] \u514B\u9686\u4ED3\u5E93...");try{b.runFileSync("git",["clone","--depth","1",e,l],{timeoutMs:12e4})}catch{console.error("[\u2717] \u514B\u9686\u5931\u8D25"),process.exit(1)}}else{console.log("[~] \u4E0B\u8F7D kexvim.js...");let i=ee.join(Ut.tmpdir(),"kexvim-init.js"),o=`${e}/raw/main/kexvim.js`;try{xl("wget")?b.runFileSync("wget",["-q","-O",i,o],{timeoutMs:6e4}):b.runFileSync("curl",["-fsSL","-o",i,o],{timeoutMs:6e4})}catch{console.error("[\u2717] \u4E0B\u8F7D\u5931\u8D25"),process.exit(1)}t(i)&&(j.copyFileSync(i,ee.join(l,"kexvim.js")),j.rmSync(i,{force:!0}))}let r=ee.join(l,"data",".env");if(j.mkdirSync(ee.join(l,"data"),{recursive:!0}),!t(r)){process.stdout.write("\u8BF7\u8F93\u5165 DeepSeek API Key: ");let i=await new Promise(o=>{process.stdin.setEncoding("utf8");let a=!1,c=()=>{process.stdin.removeListener("data",d),process.stdin.removeListener("end",u)},d=p=>{a||(a=!0,c(),o(String(p).trim()))},u=()=>{a||(a=!0,c(),o(""))};process.stdin.on("data",d),process.stdin.once("end",u)});i&&(j.writeFileSync(r,`DEEPSEEK_API_KEY=${i}
KEXVIM_HOME=${l}
`),console.log("[\u2713] API Key \u5DF2\u4FDD\u5B58"))}let s=ee.join(l,"data","config.yaml");t(s)||(j.writeFileSync(s,hp),console.log("[\u2713] config.yaml \u5DF2\u521B\u5EFA\uFF08\u8BF7\u6309\u9700\u8865\u5145\u914D\u7F6E\uFF09"));let n=ee.join(l,"skills");if(j.existsSync(n))console.log("[~] skills \u5DF2\u5B58\u5728\uFF0C\u8DF3\u8FC7");else{console.log("[~] \u4E0B\u8F7D\u516C\u5171\u6280\u80FD...");let i=j.mkdtempSync(ee.join(Ut.tmpdir(),"kexvim-skills-"));try{let o=b.isWindows?"nul":"/dev/null";b.runSync(`git clone --depth 1 ${e}.git "${i}" --single-branch 2>${o}`,{timeoutMs:6e4}),j.existsSync(ee.join(i,"skills"))?(j.cpSync(ee.join(i,"skills"),n,{recursive:!0}),console.log("[\u2713] \u6280\u80FD\u5DF2\u4E0B\u8F7D")):console.warn("[~] \u8FDC\u7A0B\u4ED3\u5E93\u4E2D\u6CA1\u6709 skills \u76EE\u5F55\uFF0C\u8DF3\u8FC7")}catch{console.warn("[~] \u6280\u80FD\u4E0B\u8F7D\u5931\u8D25\uFF08git \u672A\u5B89\u88C5\u6216\u65E0\u7F51\u7EDC\uFF09\uFF0C\u53EF\u624B\u52A8 clone \u540E\u590D\u5236 skills/ \u76EE\u5F55")}finally{j.rmSync(i,{recursive:!0,force:!0})}}wl(l),console.log("[\u2713] \u5B89\u88C5\u5B8C\u6210\u3002\u73B0\u5728\u53EF\u4EE5\u76F4\u63A5\u5728\u7EC8\u7AEF\u8F93\u5165: kexvim restart")}function xl(l){try{return b.runFileSync(process.platform==="win32"?"where":"which",[l]),!0}catch{return!1}}import*as Sr from"node:fs";import*as ss from"node:path";import*as El from"node:os";function yp(l){try{return b.runFileSync(process.platform==="win32"?"where":"which",[l]),!0}catch{return!1}}async function Cl(l,e){let t=r=>{try{return Sr.accessSync(r),!0}catch{return!1}};if(t(ss.join(l,"kexvim.js"))||(console.error("[\u2717] \u672A\u5B89\u88C5 Kexvim\uFF0C\u5148\u8FD0\u884C node kexvim.js init"),process.exit(1)),t(ss.join(l,".git"))){console.log("[~] git pull...");try{b.runFileSync("git",["pull","--ff-only"],{cwd:l,timeoutMs:12e4})}catch{console.error("[\u2717] \u62C9\u53D6\u5931\u8D25"),process.exit(1)}}else{console.log("[~] \u4E0B\u8F7D\u6700\u65B0 kexvim.js...");let r=ss.join(El.tmpdir(),"kexvim-update.js");try{let s=`${e}/raw/main/kexvim.js`;yp("wget")?b.runFileSync("wget",["-q","-O",r,s],{timeoutMs:6e4}):b.runFileSync("curl",["-fsSL","-o",r,s],{timeoutMs:6e4})}catch{console.error("[\u2717] \u4E0B\u8F7D\u5931\u8D25"),process.exit(1)}t(r)&&(Sr.copyFileSync(r,ss.join(l,"kexvim.js")),Sr.rmSync(r,{force:!0}))}console.log("[\u2713] \u66F4\u65B0\u5B8C\u6210\uFF0C\u6B63\u5728\u91CD\u542F...")}import*as W from"fs";import*as _e from"path";var Rl=It();function vp(l,e){if(!W.existsSync(_e.join(l,"src")))return!1;if(!W.existsSync(e))return!0;let t=W.statSync(e).mtimeMs,r=t,s=n=>{let i;try{i=W.readdirSync(n,{withFileTypes:!0})}catch{return}for(let o of i){let a=_e.join(n,o.name);if(o.isDirectory()){if(o.name==="node_modules"||o.name==="dist"||o.name==="data"||o.name===".git")continue;s(a)}else if(o.name.endsWith(".ts"))try{W.statSync(a).mtimeMs>r&&(r=W.statSync(a).mtimeMs)}catch{}}};return s(_e.join(l,"src")),s(_e.join(l,"packages")),r>t}function so(l){if(process.platform!=="win32")return[];let e=b.runSyncResult(`wmic process where "name='node.exe'" get ProcessId,CommandLine /format:list`,{timeoutMs:5e3});if(e.code!==0)return[];let t=e.stdout.replace(/\u0000/g,""),r=_e.resolve(l).toLowerCase(),s=o=>{let a=_e.resolve(o).toLowerCase();return a===r||a.startsWith(r+_e.sep)},n=[],i=null;for(let o of t.split(/\r?\n/)){let a=o.trim();if(a.startsWith("CommandLine="))i=a.slice(12).replace(/^"|"$/g,"");else if(a.startsWith("ProcessId=")){let c=Number(a.slice(10));if(i&&c>0&&c!==process.pid&&!/\sweb(\s|$)/.test(i)){let d=i.match(/(\S*(?:dev\.mjs|kexvim\.js))/);d&&s(d[1])&&n.push({pid:c,entry:d[1]})}i=null}}return n}function bp(l,e=64*1024){try{let t=W.statSync(l);if(!t.isFile()||t.size===0)return"";if(t.size<=e)return W.readFileSync(l,"utf-8");let r=W.openSync(l,"r");try{let s=Buffer.alloc(e),n=W.readSync(r,s,0,e,t.size-e);return s.toString("utf-8",0,n)}finally{W.closeSync(r)}}catch{return""}}function kp(l,e){return new Promise(t=>{let r=0,s=setInterval(()=>{r++;try{if(W.existsSync(l)){let n=bp(l);if(n.includes("Guardian agent \u5DF2\u5C31\u7EEA")||n.includes("kexvim \u5DF2\u5C31\u7EEA")){clearInterval(s),console.log(`[kexvim] \u2705 kexvim \u5DF2\u5C31\u7EEA\uFF08${r}s\uFF09`),t(!0);return}}}catch{}r>=e&&(clearInterval(s),t(!1))},1e3)})}async function Ml(l){let e=l,t=_e.join(e,"data","log");try{W.mkdirSync(t,{recursive:!0})}catch{}let r=_e.join(t,"kexvim.log"),s=_e.join(e,"dist","dev.mjs");if(vp(e,s)){console.log("[kexvim] \u68C0\u6D4B\u5230\u6E90\u7801\u53D8\u66F4\uFF0C\u81EA\u52A8\u6784\u5EFA...");let d=b.runSyncResult("npm run build:dev",{cwd:e,timeoutMs:12e4});d.code!==0&&(console.error(`[kexvim] \u274C \u6784\u5EFA\u5931\u8D25\uFF0C\u4E2D\u6B62\u91CD\u542F\uFF1A${d.stderr?.trim()||d.stdout?.trim()}`),process.exit(1)),console.log("[kexvim] \u2705 \u6784\u5EFA\u5B8C\u6210")}if(process.platform==="linux")try{let d=b.runSyncResult("systemctl --user is-active kexvim.service",{timeoutMs:5e3});if(d.code===0&&d.stdout.trim()==="active"){console.log("[kexvim] systemd \u6258\u7BA1\uFF08kexvim.service active\uFF09\uFF0C\u8D70 systemctl --user restart");let u=b.runSyncResult("systemctl --user restart kexvim.service",{timeoutMs:3e4});u.code!==0&&(console.error(`[kexvim] \u274C systemctl restart \u5931\u8D25: ${u.stderr?.trim()||u.stdout?.trim()}`),process.exit(1)),console.log("[kexvim] \u2705 systemctl restart \u5B8C\u6210");return}}catch{}let n=so(e),i=n.length>0?n.map(d=>d.pid):Rl.findKexvimPids(),o=n.length>0?n[0].entry:je.resolveEntry(e);if(console.log(`[kexvim] \u65E7\u8FDB\u7A0B: ${i.length} \u4E2A; \u5165\u53E3: ${o}`),W.writeFileSync(r,"","utf-8"),i.length>0){console.log("[kexvim] \u6E05\u7406\u65E7\u8FDB\u7A0B...");for(let d of i)process.platform==="win32"?b.runSyncResult(`taskkill /PID ${d} /F`,{timeoutMs:5e3}):Rl.killProcess(d,!0);console.log("[kexvim] \u2705 \u65E7\u8FDB\u7A0B\u5DF2\u6E05\u7406")}console.log("[kexvim] \u6B63\u5728\u542F\u52A8\u65B0 kexvim...");let a=je.spawnFreshDaemon(e,o);a===null&&(console.error("[kexvim] \u274C \u65E0\u6CD5\u542F\u52A8\u65B0\u8FDB\u7A0B"),process.exit(1)),console.log(`[kexvim] \u65B0 PID: ${a}\uFF0C\u7B49\u5F85\u5C31\u7EEA...`),await kp(r,30)||console.warn("[kexvim] \u26A0\uFE0F \u7B49\u5F85\u5C31\u7EEA\u8D85\u65F6\uFF0830s\uFF09\uFF0C\u8BF7\u68C0\u67E5 data/log/kexvim.log");try{Te.restart(e),console.log("[kexvim] \u2705 web \u8FDB\u7A0B\u5DF2\u5C31\u7EEA\uFF08\u5DF2\u786E\u4FDD\u62C9\u8D77\uFF09")}catch(d){console.error(`[kexvim] \u26A0\uFE0F web \u91CD\u542F\u5931\u8D25: ${d instanceof Error?d.message:String(d)}`)}console.log("[kexvim] \u65E5\u5FD7:",r),process.exit(0)}import*as ri from"fs";import*as no from"path";async function Al(l){try{let r=b.runSyncResult(`wmic process where "processid=${process.ppid}" get name,commandline /format:list`,{timeoutMs:3e3});ri.appendFileSync(no.join(l,"data","log","stop.log"),`[stop] pid=${process.pid} ppid=${process.ppid} argv=${process.argv.slice(2).join(" ")} at ${new Date().toISOString()}
[stop] parent: ${r.stdout.replace(/\u0000/g,"").trim()}
`)}catch{}let e=so(l);for(let r of e)r.pid!==process.pid&&(process.platform==="win32"?b.runSyncResult(`taskkill /PID ${r.pid} /F /T`,{timeoutMs:5e3}):It().killProcess(r.pid,!0),console.log(`[kexvim] \u5DF2\u505C\u6B62 PID ${r.pid} (${r.entry})`));let t=!1;try{let r=Te.findPid();r!==null&&(process.platform==="win32"?b.runSyncResult(`taskkill /PID ${r} /F /T`,{timeoutMs:5e3}):It().killProcess(r,!0),console.log(`[kexvim] \u5DF2\u505C\u6B62 web \u8FDB\u7A0B PID ${r}`),t=!0)}catch{}try{ri.rmSync(no.join(l,"data","kexvim.pid"),{force:!0})}catch{}e.length===0&&!t?console.log("[kexvim] \u6CA1\u6709\u8FD0\u884C\u4E2D\u7684 kexvim \u8FDB\u7A0B"):console.log("[kexvim] \u2705 kexvim \u5DF2\u505C\u6B62")}import*as z from"node:fs";import*as ns from"node:os";import*as te from"node:path";import{fileURLToPath as Sp}from"node:url";var ie=class l{static SERVICE_NAME="kexvim";static AUTOSTART_DIR="autostart";static KEEPALIVE_INTERVAL_MIN=1;static SYSTEMD_TEMPLATE=(e,t,r)=>`[Unit]
...
`;static platform=process.platform;static _findRoot(){let e=te.dirname(Sp(import.meta.url));for(let t=0;t<8;t++){if(z.existsSync(te.join(e,"package.json")))return e;let r=te.dirname(e);if(r===e)break;e=r}throw new Error("[kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF08\u65E0 package.json\uFF09\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002")}static _systemPath(e){if(l.platform==="win32")return"";let t=ns.homedir();return l.platform==="darwin"?te.join(t,"Library","LaunchAgents","com.kexvim.daemon.plist"):te.join(t,".config","systemd","user","kexvim.service")}static _dataPath(e){let t=te.join(e,"data",l.AUTOSTART_DIR);return z.mkdirSync(t,{recursive:!0}),l.platform==="win32"?te.join(t,"keepalive.cmd"):l.platform==="darwin"?te.join(t,"com.kexvim.daemon.plist"):te.join(t,"kexvim.service")}static _devEntry(e){let t=te.join(e,"dist","dev.mjs");if(z.existsSync(t))return t;let r=te.join(e,"kexvim.js");if(z.existsSync(r))return r;throw new Error("[kexvim] \u672A\u627E\u5230 daemon \u5165\u53E3\uFF08dist/dev.mjs \u6216 kexvim.js\uFF09\u3002")}static _template(e){let t=process.execPath,r=l._devEntry(e);return l.platform==="win32"?`@echo off\r
set NODE_NO_WARNINGS=1\r
cd /d "${e}"\r
${t} "${r}" keepalive\r
`:l.platform==="darwin"?l.LAUNCHD_TEMPLATE(t,e,r):l.SYSTEMD_TEMPLATE(t,e,r)}static writeTemplate(e){let t=e??l._findRoot(),r=l._dataPath(t);return z.writeFileSync(r,l._template(t)),r}static install(e){let t=e??l._findRoot(),r=l.writeTemplate(t),s=l._systemPath(t),n=[`[\u2713] \u6A21\u677F\u5DF2\u751F\u6210: ${r}`];if(l.platform==="win32")l._winInstall(t,n);else if(l.platform==="darwin"){z.mkdirSync(te.dirname(s),{recursive:!0}),z.copyFileSync(r,s),n.push(`[\u2713] \u5DF2\u5199\u5165: ${s}`);try{b.runFileSync("launchctl",["load","-w",s]),n.push("[\u2713] launchctl load \u5B8C\u6210")}catch{n.push("[\u2717] launchctl load \u5931\u8D25\uFF08\u9700\u5F53\u524D\u7528\u6237\u6709 GUI \u4F1A\u8BDD\uFF09")}}else{z.mkdirSync(te.dirname(s),{recursive:!0}),z.copyFileSync(r,s),n.push(`[\u2713] \u5DF2\u5199\u5165: ${s}`);try{b.runFileSync("systemctl",["--user","daemon-reload"]),b.runFileSync("systemctl",["--user","enable","kexvim.service"]),n.push("[\u2713] systemd user \u5355\u5143\u5DF2 enable")}catch(i){n.push(`[\u2717] systemctl \u6CE8\u518C\u5931\u8D25: ${i instanceof Error?i.message:String(i)}`)}try{b.runFileSync("loginctl",["enable-linger",ns.userInfo().username]),n.push("[\u2713] linger \u5DF2\u542F\u7528\uFF08\u5F00\u673A\u81EA\u542F\uFF09")}catch{n.push("[~] linger \u542F\u7528\u5931\u8D25\uFF08\u90E8\u5206\u7CFB\u7EDF\u9700 root\uFF09\uFF0C\u81EA\u542F\u53EF\u80FD\u53EA\u5728\u767B\u5F55\u540E\u751F\u6548")}}return n.join(`
...
Register-ScheduledTask -TaskName 'KexvimKeepAlive' -Action (New-ScheduledTaskAction -Execute '${r}' -Argument '"--no-warnings" "${s}" keepalive' -WorkingDirectory '${e}') -Trigger (New-ScheduledTaskTrigger -Once -At (Get-Date).AddMinutes(1) -RepetitionInterval (New-TimeSpan -Minutes ${l.KEEPALIVE_INTERVAL_MIN})) -Principal $p -Force | Out-Null`;try{b.runFileSync("powershell",["-NoProfile","-Command",n]),t.push("[\u2713] \u8BA1\u5212\u4EFB\u52A1 KexvimDaemon\uFF08\u5F00\u673A\u5EF6\u8FDF 1 \u5206\u949F\u62C9\u8D77\uFF0CS4U \u65E0\u7A97\u53E3\uFF09\u5DF2\u521B\u5EFA"),t.push(`[\u2713] \u8BA1\u5212\u4EFB\u52A1 KexvimKeepAlive\uFF08\u6BCF ${l.KEEPALIVE_INTERVAL_MIN} \u5206\u949F\u4FDD\u6D3B daemon+web\uFF0CS4U \u65E0\u7A97\u53E3\uFF09\u5DF2\u521B\u5EFA`)}catch(i){t.push(`[\u2717] \u521B\u5EFA\u8BA1\u5212\u4EFB\u52A1\u5931\u8D25: ${i instanceof Error?i.message:String(i)}`)}}static isProcessRunning(e){let t=e??l._findRoot(),r=te.join(t,"data","kexvim.pid");try{let s=Number.parseInt(z.readFileSync(r,"utf-8").trim(),10);return!Number.isFinite(s)||s<=0?!1:(process.kill(s,0),!0)}catch{return!1}}static resolveRoot(){return l._findRoot()}};function Il(){let l=ie.resolveRoot();console.log(ie.install(l));let e=ie.status(l);console.log(`[~] \u72B6\u6001: platform=${e.platform} installed=${e.installed} consistent=${e.consistent} active=${e.serviceActive}`),process.exit(0)}function Pl(){console.log(ie.uninstall(ie.resolveRoot())),process.exit(0)}function Ll(){let l=ie.resolveRoot(),e=ie.status(l),t=ie.isProcessRunning(l);console.log(`kexvim \u5E73\u53F0:        ${e.platform}`),console.log(`\u8FDB\u7A0B:               ${t?"\u8FD0\u884C\u4E2D":"\u672A\u8FD0\u884C"} (PID \u6587\u4EF6: data/kexvim.pid)`),console.log(`\u81EA\u542F\u914D\u7F6E:           ${e.installed?"\u5DF2\u5B89\u88C5":"\u672A\u5B89\u88C5"} @ ${e.configPath||"(Windows \u8BA1\u5212\u4EFB\u52A1)"}`),console.log(`\u914D\u7F6E\u4E00\u81F4\u6027(\u7EDF\u4E00):    ${e.consistent?"\u4E00\u81F4(\u6A21\u677F)":"\u4E0D\u4E00\u81F4(\u5C06\u88AB install \u8986\u76D6)"}`),console.log(`\u670D\u52A1\u72B6\u6001:           ${e.serviceActive?"active":"inactive"}`),console.log(`\u91CD\u542F\u5FAA\u73AF\u9632\u62A4:       ${Se.isTripped(l)?"\u26A0\uFE0F TRIPPED\uFF08\u7528 kexvim clear-loop \u89E3\u9664\uFF09":`\u6B63\u5E38 (${Se.bootCount(l)}/${Se.MAX_RESTARTS} in ${Se.WINDOW_SECONDS}s)`}`),process.exit(t?0:1)}function Nl(){Se.clear(ie.resolveRoot()),console.log("[\u2713] restart_loop \u72B6\u6001\u5DF2\u6E05\u9664"),process.exit(0)}import*as _r from"node:fs";import*as io from"node:path";var _p=9e4;function wp(l){let e=io.join(l,"data","daemon.heartbeat");try{let t=_r.statSync(e);if(Date.now()-t.mtimeMs>_p)return null;let r=Number.parseInt(_r.readFileSync(e,"utf-8").trim(),10);return Number.isFinite(r)&&r>0?r:null}catch{return null}}function Ol(){let l=ie.resolveRoot(),e=wp(l);if(e===null)je.spawnFreshDaemon(l)===null&&process.exit(1);else try{_r.writeFileSync(io.join(l,"data","kexvim.pid"),String(e))}catch{}try{Te.ensureRunning(l)}catch{}process.exit(0)}import*as is from"node:fs";import*as oo from"node:path";import{DatabaseSync as xp}from"node:sqlite";function $l(l){let e=oo.join(l,"kexvim.db");is.existsSync(e)||(console.error("[kexvim] \u274C \u627E\u4E0D\u5230\u4F1A\u8BDD\u5E93:",e),process.exit(1));let t=new xp(e,{readOnly:!0});try{let r=t.prepare(`
      ...
      WHERE s.is_test = 0
      ORDER BY s.updated_at DESC
    `).all();if(r.length===0){console.log("\u6682\u65E0\u5386\u53F2\u4F1A\u8BDD\u3002");return}let s="";try{let n=oo.join(l,".current_session");is.existsSync(n)&&(s=is.readFileSync(n,"utf-8").trim())}catch{}console.log(`\u5171 ${r.length} \u4E2A\u4F1A\u8BDD\uFF1A
`),r.forEach((n,i)=>{let o=nt.displayTitle(n.summary,n.firstUserMsg),a=n.id===s?"  \u25C0 \u5F53\u524D":"";console.log(`${String(i+1).padStart(2)}. ${n.id.slice(0,8)} - ${o}  [${n.source}] ${String(n.msgCount).padStart(4)}\u6761${a}`)}),console.log(`
\u7528\u6CD5: kexvim session <\u524D8\u4F4DID>  \u5207\u6362/\u6062\u590D\u8BE5\u4F1A\u8BDD`)}finally{t.close()}}import*as Dl from"node:fs";import*as ao from"node:path";import{DatabaseSync as Tp}from"node:sqlite";var Ep="session-switch.json";function Fl(l,e){let t=ao.join(l,"kexvim.db"),r=new Tp(t,{readOnly:!0}),s;try{s=r.prepare(`
      SELECT id, COALESCE(summary, '') AS summary,
             (SELECT COUNT(*) FROM messages m WHERE m.session_id = s.id) AS msgCount
      FROM sessions s
      WHERE s.is_test = 0 AND s.id LIKE ?
    `).get(`${e}%`)}finally{r.close()}s||(console.error(`[kexvim] \u274C \u627E\u4E0D\u5230\u4F1A\u8BDD "${e}"\u3002\u7528 kexvim sessions \u67E5\u770B\u5168\u90E8\u4F1A\u8BDD\u3002`),process.exit(1)),Dl.writeFileSync(ao.join(l,Ep),JSON.stringify({sessionId:s.id,requestedAt:Date.now()/1e3}),"utf-8");let n=(s.summary||"(\u65E0\u6807\u9898)").slice(0,30);console.log(`\u2705 \u5DF2\u6807\u8BB0\u5207\u6362\u5230\u4F1A\u8BDD ${s.id.slice(0,8)}\uFF08${s.msgCount} \u6761\u6D88\u606F\uFF0C${n}\uFF09`),console.log("   \u53D1\u9001\u4E0B\u4E00\u6761\u6D88\u606F\u540E daemon \u5C06\u6062\u590D\u8BE5\u4F1A\u8BDD\u7EE7\u7EED\u5BF9\u8BDD\u3002")}function lo(){console.log(`\u7528\u6CD5: kexvim <command>
...
\u5165\u53E3: kexvim \u547D\u4EE4\u7EDF\u4E00\u8F6C\u53D1\u4E3A node dev.js\uFF08\u5F00\u53D1\uFF09\u6216 node kexvim.js\uFF08\u5B89\u88C5\u7248\uFF09\uFF0C
      \u547D\u4EE4\u8FDB\u7A0B\u5B8C\u5168\u72EC\u7ACB\uFF0C\u4E0D\u4F9D\u8D56\u8FD0\u884C\u4E2D\u7684 kexvim \u5B9E\u4F8B\u3002`),process.exit(0)}function Bl(){console.log("kexvim (\u5F00\u53D1\u5165\u53E3 dist/dev.mjs / \u5B89\u88C5\u5165\u53E3 kexvim.js)"),process.exit(0)}import*as wr from"node:fs";import*as co from"node:path";import*as Ul from"node:readline";import{load as Cp,dump as Rp}from"js-yaml";function Mp(l){let e=wr.readFileSync(l,"utf-8"),t=Cp(e);if(!t||typeof t!="object"||Array.isArray(t))throw new Error(`config.yaml \u683C\u5F0F\u5F02\u5E38: ${l}`);return t}function jl(l,e){return new Promise(t=>{l.question(e,t)})}function Ap(l,e){let t=!!l?.enabled,r=l?.adapters??{};console.log(`\u914D\u7F6E\u6587\u4EF6: ${co.join(e,"config.yaml")}`),console.log(`platform.enabled: ${t}`);let s=Object.keys(r);if(s.length===0)console.log("\u5DF2\u914D\u7F6E\u5E73\u53F0: \u65E0");else{console.log("\u5DF2\u914D\u7F6E\u5E73\u53F0:");for(let n of s){let i=r[n]??{},o=Object.entries(i).map(([a,c])=>`${a}=${kr(n,a,c)}`).join(", ");console.log(`  ${n}: ${o||"(\u7A7A)"}`)}}console.log()}async function Hl(l){let e=co.join(l,"config.yaml");wr.existsSync(e)||(console.error("[\u2717] \u672A\u627E\u5230 config.yaml\uFF0C\u8BF7\u5148\u6267\u884C: kexvim init"),process.exit(1));let t=Mp(e),r=t.platform??{enabled:!0,adapters:{}};Ap(r,l);let s=Object.keys(Bt);console.log("\u652F\u6301\u7684\u5E73\u53F0:"),s.forEach((i,o)=>{let a=Bt[i];console.log(`  ${o+1}) ${a.name.padEnd(16)} ${a.desc}`)}),console.log("  0) \u9000\u51FA"),console.log();let n=Ul.createInterface({input:process.stdin,output:process.stdout});try{let i=(await jl(n,"\u8BF7\u9009\u62E9\u5E73\u53F0\u7F16\u53F7: ")).trim(),o=parseInt(i,10);if(!i||i==="0"||isNaN(o)||o<1||o>s.length){console.log("\u5DF2\u9000\u51FA");return}let a=s[o-1],c=Bt[a];console.log(),console.log(`\u914D\u7F6E ${c.name}\uFF08${a}\uFF09:`);let d={},u=r.adapters??{},p=u[a]??{};for(let m of c.fields){let g=p[m.key]!==void 0?` (\u5F53\u524D: ${String(kr(a,m.key,p[m.key]))})`:"",f=(await jl(n,`  ${m.label}${g}: `)).trim();if(f===""){if(!m.optional){console.error(`[\u2717] ${m.key} \u5FC5\u586B\uFF0C\u5DF2\u53D6\u6D88\uFF08\u672A\u4FEE\u6539\u914D\u7F6E\uFF09`);return}p[m.key]!==void 0&&(d[m.key]=p[m.key]);continue}f==="true"||f==="false"?d[m.key]=f==="true":/^\d+$/.test(f)?d[m.key]=Number(f):d[m.key]=f}u[a]=d,r.enabled=!0,r.adapters=u,t.platform=r,wr.writeFileSync(e,Rp(t,{indent:2,lineWidth:-1})),console.log(),console.log(`[\u2713] \u5DF2\u5199\u5165 platform.adapters.${a}:`);for(let[m,g]of Object.entries(d))console.log(`    ${m}: ${String(kr(a,m,g))}`);console.log("[\u2713] platform.enabled = true"),console.log(),console.log("\u91CD\u542F\u540E\u751F\u6548: kexvim restart")}finally{n.close()}}import*as oe from"node:fs";import*as ft from"node:path";var Ip=["kexvim.bat","kexvim.sh","package.json","README.md","LICENSE",".gitignore"];function Wl(l){return l>=1024*1024?`${(l/1024/1024).toFixed(1)} MB`:`${Math.round(l/1024)} KB`}async function ql(l){let e=ft.join(l,"release"),t=ft.join(l,"dist","kexvim.mjs"),r=ft.join(l,"skills");if(console.log("[release] \u6784\u5EFA dist/kexvim.mjs ..."),b.runSync("npm run build",{cwd:l,timeoutMs:18e4}),oe.mkdirSync(e,{recursive:!0}),console.log("[release] \u9759\u6001\u53D1\u5E03\u7269\u4FDD\u7559\u73B0\u6709\uFF08release/ \u5373\u516C\u5F00\u4ED3\u5DE5\u4F5C\u76EE\u5F55\uFF09"),!oe.existsSync(t))throw new Error(`[release] \u6784\u5EFA\u4EA7\u7269\u7F3A\u5931: ${t}`);oe.copyFileSync(t,ft.join(e,"kexvim.js"));let s=ft.join(e,"skills");oe.rmSync(s,{recursive:!0,force:!0}),oe.cpSync(r,s,{recursive:!0}),console.log(),console.log(`[\u2713] \u53D1\u5E03\u76EE\u5F55: ${e}`);let n=Ip.concat(["kexvim.js","skills"]);for(let o of n){let a=ft.join(e,o);if(oe.existsSync(a)){let c=oe.statSync(a).isDirectory()?oe.readdirSync(a).length:Wl(oe.statSync(a).size);console.log(`    ${o.padEnd(14)} ${typeof c=="number"?c+" \u4E2A\u6587\u4EF6":c}`)}}let i=oe.statSync(ft.join(e,"kexvim.js")).size;console.log(),console.log(`[release] kexvim.js ${Wl(i)}\uFF0C\u53D1\u5E03\u524D\u8BF7\u9A8C\u8BC1\u4EA7\u7269\u7279\u5F81\uFF08\u5982\u65B0\u6587\u6848\uFF09`)}var si=class l{static resolveUserDataDir(){let e=process.env.KEXVIM_USER_DATA_DIR;if(e)return e;let t=I.findProjectRoot()??I.findProjectRoot(Ht.dirname(process.argv[1]??""));if(!t)throw new Error("[kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\uFF0C\u6216\u8BBE\u7F6E KEXVIM_USER_DATA_DIR");return Ht.join(t,"data")}static async dispatchFromArgv(){let e=process.argv[2];if(!e)return!1;let t=e.toLowerCase(),r=t==="--help"||t==="-h"?"help":t==="--version"||t==="-v"?"version":t;return new Set(["init","update","restart","stop","install","uninstall","status","keepalive","clear-loop","sessions","session","platform","release","help","version"]).has(r)?(await l.handleCliCommand(r),!0):!1}static async handleCliCommand(e,t){let r=Ht.resolve(t??l.resolveUserDataDir(),".."),s=Ht.join(r,"data"),n="https://gitee.com/moscowzk/kexvim";switch(e){case"init":await Tl(r,n);return;case"update":await Cl(r,n);return;case"install":Il();return;case"uninstall":Pl();return;case"status":Ll();return;case"keepalive":Ol();return;case"clear-loop":Nl();return;case"restart":await Ml(r),process.exit(0);return;case"stop":await Al(r),process.exit(0);return;case"sessions":$l(s);return;case"platform":await Hl(s);return;case"release":await ql(r);return;case"session":{let i=process.argv[3];i||(console.error("\u7528\u6CD5: kexvim session <\u524D8\u4F4DID>\uFF08\u7528 kexvim sessions \u67E5\u770B\u5217\u8868\uFF09"),process.exit(1)),Fl(s,i);return}case"help":lo();return;case"version":Bl();return;default:lo();return}}};import*as we from"node:fs";import*as uo from"node:path";import*as Kl from"node:worker_threads";var po=()=>{let l=I.findProjectRoot();if(!l)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D kexvim.log\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let e=uo.join(l,"data","log");try{we.mkdirSync(e,{recursive:!0})}catch{}return uo.join(e,"kexvim.log")},Pp=100*1024*1024;try{let l=po();we.existsSync(l)&&we.statSync(l).size>Pp&&(we.rmSync(`${l}.1`,{force:!0}),we.renameSync(l,`${l}.1`))}catch{}process.on("uncaughtException",l=>{try{we.appendFileSync(po(),`[kexvim] uncaughtException at ${new Date().toISOString()}:
${l.stack??l.message}
`)}catch{}});process.on("unhandledRejection",l=>{try{let e=l instanceof Error?l.stack??l.message:String(l);we.appendFileSync(po(),`[kexvim] unhandledRejection at ${new Date().toISOString()}:
${e}
`)}catch{}try{console.error("[kexvim] unhandledRejection:",l)}catch{}});async function Gl(){try{await ti.main()}catch(l){console.error(l instanceof Error?l.message:String(l)),process.exit(1)}}Kl.isMainThread?si.dispatchFromArgv().then(l=>{l?process.exit(0):Gl()}):Gl();
