...
`)}catch{return""}}static discoverContextFiles(e,t){let r=[],s=t||Ke;for(let n of Mo){if(n.includes("*"))continue;let i=`${e}/${n}`;try{s.existsSync(i)&&r.push({name:n,fullPath:i})}catch{}}return r}static loadProjectContext(e,t=2e4){for(let r of Mo){if(r==="SOUL.md"||r.includes("*"))continue;let s=Vt.join(e,r);try{if(!Ke.existsSync(s))continue;let n=Ke.readFileSync(s,"utf-8").trim();if(!n)continue;return n=yt.stripFrontmatter(n).trim(),n.length>t&&(n=`${n.slice(0,t)}
...
`;return Qe.writeFileSync(r,s,"utf-8"),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3}).code!==0?!1:b.runSyncResult("loginctl enable-linger "+pr.userInfo().username,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=Ur.join(pr.homedir(),".config","systemd","user",`${e}.service`);return Qe.existsSync(t)&&(b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:5e3}),Qe.unlinkSync(t),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3})),!0}catch{return!1}}isServiceInstalled(e){let t=Ur.join(pr.homedir(),".config","systemd","user",`${e}.service`);return Qe.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`systemctl --user is-active ${e}`,{timeoutMs:3e3});return t.code!==0?!1:t.stdout.trim()==="active"}startService(e){return b.runSyncResult(`systemctl --user start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`systemctl --user enable ${e}`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`systemctl --user disable ${e}`,{timeoutMs:5e3}).code===0}};import*as Ze from"fs";import*as qn from"os";import*as Ct from"path";var Gn=class{platform="win32";findKexvimPids(){let e=b.runSyncResult(`powershell -Command "Get-Process node | Where-Object { $_.CommandLine -match 'Main\\.ts|dev\\.mjs|kexvim\\.js' } | Select-Object -ExpandProperty Id"`,{timeoutMs:5e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
...
npx tsx src/Main.ts
`;return Ze.writeFileSync(Ct.join(r,`${e.name}.bat`),s,"utf-8"),!0}catch{return!1}}uninstallService(e){if(!(b.runSyncResult(`nssm remove ${e} confirm`,{timeoutMs:1e4}).code===0)){let r=Ct.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup"),s=Ct.join(r,`${e}.bat`);if(Ze.existsSync(s))try{Ze.unlinkSync(s)}catch{}}return!0}isServiceInstalled(e){if(b.runSyncResult(`nssm status ${e}`,{timeoutMs:3e3}).code===0)return!0;let t=Ct.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");return Ze.existsSync(Ct.join(t,`${e}.bat`))}isServiceRunning(e){let t=b.runSyncResult(`powershell -Command "Get-Service ${e} | Select-Object -ExpandProperty Status"`,{timeoutMs:5e3});return t.code!==0?!1:t.stdout.trim().toLowerCase()==="running"}startService(e){return b.runSyncResult(`net start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`net stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`sc config ${e} start=auto`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`sc config ${e} start=disabled`,{timeoutMs:5e3}).code===0}};import*as et from"fs";import*as mr from"os";import*as Rt from"path";var Kn=class{platform="darwin";findKexvimPids(){let e=b.runSyncResult('pgrep -f "tsx.*Main\\.ts"',{timeoutMs:3e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){try{return process.kill(e,t?"SIGKILL":"SIGTERM"),!0}catch{return!1}}installService(e){try{let t=Rt.join(mr.homedir(),"Library","LaunchAgents");et.mkdirSync(t,{recursive:!0});let r=Rt.join(t,`${e.name}.plist`),n=`<?xml version="1.0" encoding="UTF-8"?>
...
</plist>`;return et.writeFileSync(r,n,"utf-8"),b.runSyncResult(`launchctl load ${r}`,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=Rt.join(mr.homedir(),"Library","LaunchAgents",`${e}.plist`);return et.existsSync(t)&&(b.runSyncResult(`launchctl unload ${t}`,{timeoutMs:5e3}),et.unlinkSync(t)),!0}catch{return!1}}isServiceInstalled(e){let t=Rt.join(mr.homedir(),"Library","LaunchAgents",`${e}.plist`);return et.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`launchctl list local.${e}`,{timeoutMs:3e3});if(t.code!==0)return!1;let r=t.stdout.trim();return r.includes(`local.${e}`)&&!r.startsWith("-")}startService(e){let t=Rt.join(mr.homedir(),"Library","LaunchAgents",`${e}.plist`);return b.runSyncResult(`launchctl load ${t}`,{timeoutMs:5e3}).code===0}stopService(e){let t=Rt.join(mr.homedir(),"Library","LaunchAgents",`${e}.plist`);return b.runSyncResult(`launchctl unload ${t}`,{timeoutMs:5e3}).code===0}enableService(e){return this.startService(e)}disableService(e){return this.stopService(e)}};function tu(){switch(typeof process<"u"?process.platform:"linux"){case"win32":return new Gn;case"darwin":return new Kn;default:return new Wn}}var Ki=null;function Mt(){return Ki||(Ki=tu()),Ki}var xe=class l{static DEV_WEB_PORT=8787;static RELEASE_WEB_PORT=8788;static isDevEntry(){let e=process.argv[1]??"";if(/dev\.mjs$/i.test(e))return!0;try{if(/kexvim(?:\.mjs)?$/i.test(Ce.basename(e)))return Fe.existsSync(Ce.join(Ce.dirname(e),"dist","dev.mjs"))}catch{}return!1}static webPort(){let e=Number(process.env.KEXVIM_WEB_PORT);return Number.isFinite(e)&&e>0?e:l.isDevEntry()?l.DEV_WEB_PORT:l.RELEASE_WEB_PORT}static findPid(){let e=l.webPort();try{if(process.platform==="win32"){let t=b.runSyncResult(`netstat -ano | findstr :${e} | findstr LISTENING`,{timeoutMs:5e3});for(let r of(t.stdout||"").split(/\r?\n/)){let s=r.trim().match(/LISTENING\s+(\d+)\s*$/);if(s)return Number(s[1])}}else{let r=(b.runSyncResult(`ss -ltnp 'sport = :${e}'`,{timeoutMs:5e3}).stdout||"").match(/pid=(\d+)/);if(r)return Number(r[1])}}catch{}return null}static webEntry(e){let t=Ce.join(e,"dist","dev.mjs");if(Fe.existsSync(t))return t;let r=Ce.join(e,"kexvim.js");return Fe.existsSync(r)?r:Ce.join(e,"src","Main.ts")}static restart(e){let t=l.findPid();if(t!==null){let r=Mt();try{process.platform==="win32"?b.runSyncResult(`taskkill /PID ${t} /F /T`,{timeoutMs:5e3}):r.killProcess(t,!0)}catch{}}return l.spawnWeb(e)}static ensureRunning(e){return l.findPid()!==null?!1:l.spawnWeb(e)}static spawnWeb(e){let t=l.webEntry(e),r=Ce.join(e,"data","log");try{Fe.mkdirSync(r,{recursive:!0})}catch{}let s=null;try{s=Fe.openSync(Ce.join(r,"web.log"),"a")}catch{}let n=b.spawn(process.execPath,[t,"web"],{cwd:e,stdio:s!==null?["ignore",s,s]:"ignore",detached:!0});return s!==null&&Fe.closeSync(s),n.unref(),!0}};var Be=class l{static resolveEntry(e){let t=It.join(e,"dist","dev.mjs");if(At.existsSync(t))return t;let r=It.join(e,"kexvim.js");return At.existsSync(r)?r:It.join(e,"src","Main.ts")}static buildEnv(e){let t={};for(let[n,i]of Object.entries(process.env))i!==void 0&&(t[n]=i);let r=It.join(e,"data",".env");if(!At.existsSync(r))return t;let s=At.readFileSync(r,"utf-8");for(let n of s.split(`
