...
`)});var jt=class{get executionGuidance(){return"none"}get staleTimeoutFloorSeconds(){}get reasoningMaxOutputTokens(){}get modelName(){}};var Ce=class{static FLOORS=[{slug:"deepseek-r1",floor:600},{slug:"deepseek-v4-flash",floor:600},{slug:"o1",floor:600},{slug:"o1-mini",floor:600},{slug:"o1-pro",floor:600},{slug:"o1-preview",floor:600},{slug:"o3",floor:600},{slug:"o3-pro",floor:600},{slug:"o3-mini",floor:300},{slug:"o4-mini",floor:300}].sort((e,t)=>t.slug.length-e.slug.length);static getStaleTimeoutFloor(e){if(typeof e!="string"||!e.trim())return;let t=e.trim().toLowerCase();if(t.includes("/")&&(t=t.split("/").pop()??t),!!t)return this.matchAny(t)}static isReasoningModel(e){return this.getStaleTimeoutFloor(e)!==void 0}static matchAny(e){for(let t of this.FLOORS)if(this.matches(e,t.slug))return t.floor}static matches(e,t){if(!e.startsWith(t))return!1;let r=e.slice(t.length);return r===""||r.startsWith("-")||r.startsWith(".")||r.startsWith("_")}};var Re=class extends Error{constructor(t,r,s,n=!1){super(t);this.statusCode=r;this.provider=s;this.retryable=n;this.name="ProviderError"}statusCode;provider;retryable},Ut=class extends Re{constructor(t,r,s){super(t,429,s,!0);this.retryAfter=r;this.name="RateLimitError"}retryAfter};var Wl=/^[\s]*<think>([\s\S]*?)<\/think>/m,ql=/^[\s]*<reason>([\s\S]*?)<\/reason>/m,Gl=[/\b(sk-[a-zA-Z0-9]{20,})\b/g,/\b(Bearer\s+)([a-zA-Z0-9_\-\.]{20,})\b/gi,/(Authorization:\s*)(?:Bearer\s+)?[a-zA-Z0-9_\-\.]{20,}/gi,/(x-api-key:\s*)[a-zA-Z0-9_\-\.]{20,}/gi,/\b(ghp_[a-zA-Z0-9]{36,})\b/g,/\b(eyJ[a-zA-Z0-9_\-]{10,}\.[a-zA-Z0-9_\-]{10,}\.[a-zA-Z0-9_\-]{10,})\b/g],rs=class{static stripThinkBlocks(e){return e?e.replace(/<think>[\s\S]*?<\/think>/g,"").replace(/<reason>[\s\S]*?<\/reason>/g,"").trim():""}static extractThinkContent(e){if(!e)return null;let t=e.match(Wl);if(t)return t[1].trim();let r=e.match(ql);return r?r[1].trim():null}static stripThinkTagsOnly(e){return e?e.replace(/<\/?(?:think|reason)>/g,"").trim():""}static stripThinkTagsFromReasoning(e){return e&&e.replace(/<think>[\s\S]*?<\/think>/g,"").trim()||null}static renderContent(e,t,r={}){let s={stripMode:"no_strip",contentRender:"content_only",stripThinkTagsFromReasoning:!0,...r},n=e??"",i=t??null,o=n;s.stripMode==="strip_think_content"?o=this.stripThinkBlocks(n):s.stripMode==="strip_think_tags"&&(o=this.stripThinkTagsOnly(n));let a=null;s.contentRender!=="content_only"&&(a=this.extractThinkContent(n));let c="";if(s.contentRender==="content_only")c=o;else if(s.contentRender==="content_and_think"){let u=[];a&&u.push(a),o&&u.push(o),c=u.join(`
...
\u8F93\u5165 goal_text \u4E3A\u81EA\u7136\u8BED\u8A00\u76EE\u6807\u63CF\u8FF0\uFF0C\u7CFB\u7EDF\u4F1A\u81EA\u52A8\u89C4\u5212\u3001\u62C6\u89E3\u3001\u5E76\u884C\u6267\u884C\u3002
\u793A\u4F8B\uFF1A\u8C03\u7528 delegate_task(goal_text='\u7ED9\u652F\u4ED8\u6A21\u5757\u52A0\u9000\u6B3E\u529F\u80FD\uFF0C\u652F\u6301\u5FAE\u4FE1\u548C\u652F\u4ED8\u5B9D')
\u7B80\u5355\u67E5\u8BE2\uFF08\u95EE\u95EE\u9898\u3001\u67E5\u72B6\u6001\u3001\u5355\u6B65\u64CD\u4F5C\uFF09\u4E0D\u9700\u8981\u7528\u6B64\u5DE5\u5177\uFF0C\u76F4\u63A5\u56DE\u590D\u5373\u53EF\u3002`;static PLATFORM_HINTS={}},Po=["AGENTS.md",".cursorrules",".cursor/rules/wildcard.mdc",".cursor/rules/*.mdc","SOUL.md",".zk.md","ZK.md",".github/copilot-instructions.md",".windsurfrules"],yt=class{static stripFrontmatter(e){if(e.startsWith("---")){let t=e.indexOf(`
...
`)}catch{return""}}static discoverContextFiles(e,t){let r=[],s=t||Ke;for(let n of Po){if(n.includes("*"))continue;let i=`${e}/${n}`;try{s.existsSync(i)&&r.push({name:n,fullPath:i})}catch{}}return r}static loadProjectContext(e,t=2e4){for(let r of Po){if(r==="SOUL.md"||r.includes("*"))continue;let s=Yt.join(e,r);try{if(!Ke.existsSync(s))continue;let n=Ke.readFileSync(s,"utf-8").trim();if(!n)continue;return n=yt.stripFrontmatter(n).trim(),n.length>t&&(n=`${n.slice(0,t)}
...
Only proceed without loading a skill if genuinely none are relevant to the task.`}_shouldInjectEnforcement(e){return e!==!1}};var _c=`Review the conversation above and update two things:
...
${p.join(`
`)}`,t.state.lastRunAt=new Date(e).toISOString(),t.state.lastRunSummary=n,t.saveState(),t.onSummary&&t.onSummary(n),n}};var xs=class{tools=new Map;all(){return[...this.tools.values()]}names(){return[...this.tools.keys()]}get(e){return this.tools.get(e)}has(e){return this.tools.has(e)}add(e){this.tools.set(e.name,e)}remove(e){this.tools.delete(e)}};import*as qo from"node:fs";var Ts=class{name="read_file";description="Read the contents of a file. Returns line-numbered output. Use for code, config, markdown, and log files.";parameters={type:"object",properties:{path:{type:"string",description:"Absolute or relative file path to read"},offset:{type:"number",description:"Line number to start from (1-indexed, optional, default 1)"},limit:{type:"number",description:"Max lines to return (optional, default 500)"}},required:["path"]};async execute(e,t,r){let s=String(e.path||"").trim();if(!s)return"Error: path is required";try{let n=Number(e.offset)||1,i=Number(e.limit)||500,a=qo.readFileSync(s,"utf-8").split(`
...
... (truncated at 50K chars)`:m}catch(n){let i=n instanceof Error?n.message:String(n);return`Error reading file '${s}': ${i}`}}};import*as Cs from"node:fs";import*as Go from"node:path";var Es=class{name="write_file";description="Write content to a file, creating parent directories if needed. OVERWRITES the entire file.";parameters={type:"object",properties:{path:{type:"string",description:"Absolute or relative file path to write"},content:{type:"string",description:"Complete content to write to the file"}},required:["path","content"]};async execute(e){let t=String(e.path||"").trim(),r=String(e.content||"");if(!t)return"Error: path is required";try{let s=Go.dirname(t);return Cs.mkdirSync(s,{recursive:!0}),Cs.writeFileSync(t,r,"utf-8"),`Written ${Buffer.byteLength(r,"utf-8")} bytes to ${t}`}catch(s){let n=s instanceof Error?s.message:String(s);return`Error writing file '${t}': ${n}`}}};import*as As from"node:fs";var Rs=class{name="patch";description="Targeted find-and-replace in a file. Replaces a unique text segment with new content. Use replace_all=true to replace every occurrence.";parameters={type:"object",properties:{path:{type:"string",description:"File path to edit (absolute or relative)"},old_string:{type:"string",description:"Text to find \u2014 must be unique in the file unless replace_all=true"},new_string:{type:"string",description:"Replacement text (can be empty string to delete the matched text)"},replace_all:{type:"boolean",description:"Replace all occurrences instead of requiring a unique match",default:!1}},required:["path","old_string","new_string"]};async execute(e,t,r){let s=String(e.path||"").trim(),n=String(e.old_string??""),i=String(e.new_string??""),o=!!e.replace_all;if(!s)return"Error: path is required";let a;try{a=As.readFileSync(s,"utf-8")}catch(p){let m=p instanceof Error?p.message:String(p);return`Error reading file '${s}': ${m}`}let c=(a.match(new RegExp(jc(n),"g"))||[]).length;if(c===0)return`Error: string not found in '${s}'. Make sure the old_string matches exactly (including whitespace).`;if(!o&&c>1)return`Error: Found ${c} occurrences. Use replace_all=true to replace all, or provide more context to make old_string unique.`;let d=o?a.replaceAll(n,i):a.replace(n,i);try{As.writeFileSync(s,d,"utf-8")}catch(p){let m=p instanceof Error?p.message:String(p);return`Error writing file '${s}': ${m}`}let u=d.split(`
`);return`Replaced ${c} occurrence(s) in ${s} (${u.length} lines).`}};function jc(l){return l.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}import*as St from"node:fs";import*as Ko from"node:path";var Ms=class{name="search_files";description="Search file contents or find files by name. Uses pattern matching (not full regex).";parameters={type:"object",properties:{pattern:{type:"string",description:"Search pattern (text or glob like *.py)"},path:{type:"string",description:"File or directory to search in (default: current working directory)"},file_glob:{type:"string",description:"Filter by file pattern (e.g. *.ts, *.py)"},limit:{type:"number",description:"Max results (default 20)"}},required:["pattern"]};searchFile(e,t,r,s,n){if(r){let i=e.split("/").pop()||e.split("\\").pop()||"",o=r.replace("*","");if(!i.endsWith(o))return""}try{let i=St.readFileSync(e,"utf-8");if(!i.includes(t))return"";let o=i.split(`
...
4. On ready the clip is downloaded; deliver it with MEDIA:<path> in your response.`}};import*as _a from"node:fs";import*as _i from"node:path";import*as Si from"node:dns";var Md=600*1e3,ba=500,ka=15e3,dn=null;async function Id(){if(dn)return dn;try{dn=await import("playwright")}catch(l){throw new Error(`playwright \u672A\u5B89\u88C5\uFF1A\u8BF7\u5148\u6267\u884C npm i playwright && npx playwright install chromium\uFF08\u5F53\u524D\uFF1A${l instanceof Error?l.message:String(l)}\uFF09`)}return dn}var Z=class l{static sessions=new Map;static consoleLogs=new Map;static dialogLogs=new Map;static async ensure(e,t){let r=e||"default",s=l.sessions.get(r);if(s)return s;let n=await Id(),i;if(t){let o=await n.chromium.connectOverCDP(t),a=o.contexts()[0]||await o.newContext(),c=a.pages()[0]||await a.newPage();i={browser:o,context:a,page:c,cdp:!0,lastActivity:Date.now()}}else{let o=await n.chromium.launch({headless:!0}),a=await o.newContext({viewport:{width:1280,height:800}}),c=await a.newPage();i={browser:o,context:a,page:c,cdp:!1,lastActivity:Date.now()}}return l.dialogLogs.set(r,[]),i.page.on("dialog",async o=>{(l.dialogLogs.get(r)||[]).push(`${o.type()}: ${o.message().slice(0,300)}`);try{await o.accept()}catch{}}),l.consoleLogs.set(r,{messages:[],errors:[]}),i.page.on("console",o=>{let a=l.consoleLogs.get(r);if(!a)return;let c=o.text().slice(0,500);o.type()==="error"?a.errors.push(c):a.messages.push(c)}),i.page.on("pageerror",o=>{l.consoleLogs.get(r)?.errors.push(`pageerror: ${o.message.slice(0,500)}`)}),l.sessions.set(r,i),i}static touch(e){let t=l.sessions.get(e||"default");t&&(t.lastActivity=Date.now())}static async reapIdle(){let e=Date.now();for(let[t,r]of[...l.sessions.entries()])if(e-r.lastActivity>Md){try{await r.browser.close()}catch{}l.sessions.delete(t),l.consoleLogs.delete(t),l.dialogLogs.delete(t)}}static get(e){let t=l.sessions.get(e||"default");return t&&(t.lastActivity=Date.now()),t}static consoleState(e){return l.consoleLogs.get(e||"default")||{messages:[],errors:[]}}static dialogs(e){return l.dialogLogs.get(e||"default")||[]}static async closeAll(){for(let e of l.sessions.values())try{await e.browser.close()}catch{}l.sessions.clear()}};async function wa(l,e){if(e)return(await l.evaluate(()=>document.body?.innerText||"")).slice(0,ka*2);let r=l.locator('a[href], button, input, textarea, select, [role="button"], [role="link"], [role="checkbox"], [role="radio"], [role="tab"], [role="menuitem"], [contenteditable="true"], [tabindex]:not([tabindex="-1"])'),s=Math.min(await r.count(),120),n=[],i=await l.evaluate(()=>document.title||"");n.push(`URL: ${l.url()}`),n.push(`Title: ${i}`),n.push(""),n.push(`Interactive elements (${s}):`);for(let a=0;a<s;a++){let c=r.nth(a),d=await c.evaluate(m=>{let g=m.tagName.toLowerCase(),f=(m.textContent||"").trim().replace(/\s+/g," ").slice(0,80),h=m.type||"";return{tag:g,txt:f,type:h}}).catch(()=>({tag:"?",txt:"",type:""})),u=await c.getAttribute("href").catch(()=>null),p=`[ref=${a+1}] <${d.tag}${d.type?` type="${d.type}"`:""}> ${d.txt||d.tag}`;n.push(u?`${p} href="${String(u).slice(0,100)}"`:p)}let o=await l.evaluate(()=>document.body?.innerText||"").catch(()=>"");return n.push(""),n.push(`Page text (${o.length} chars):`),n.push(o.replace(/\n{3,}/g,`

`).slice(0,ka)),n.join(`
`)}async function xa(l,e){let t=0,r=/^@?e(\d+)$/i.exec(e.trim());if(r)t=Number(r[1])-1;else{let n=/^\d+$/.exec(e.trim());if(n)t=Number(n)-1;else throw new Error(`ref \u5FC5\u987B\u662F @eN \u5F62\u5F0F\uFF08\u5982 @e5\uFF09\uFF0C\u6536\u5230 '${e}'\u3002\u5148 browser_snapshot \u83B7\u53D6 ref\u3002`)}return l.locator('a[href], button, input, textarea, select, [role="button"], [role="link"], [role="checkbox"], [role="radio"], [role="tab"], [role="menuitem"], [contenteditable="true"], [tabindex]:not([tabindex="-1"])').nth(t)}async function Pd(l){let e=l.toLowerCase().replace(/\.$/,"");if(e==="localhost"||e.endsWith(".localhost"))return!0;if(/^\d+\.\d+\.\d+\.\d+$/.test(e))return Sa(e);try{let t=[];try{t.push(...await Si.promises.resolve4(e))}catch{}try{t.push(...await Si.promises.resolve6(e))}catch{}return t.some(Sa)}catch{return!1}}function Sa(l){if(l==="::1"||l.startsWith("fc")||l.startsWith("fd")||l.startsWith("fe80"))return!0;let e=l.split(".").map(Number);if(e.length!==4)return!1;let[t,r]=e;return t===10||t===127||t===172&&r>=16&&r<=31||t===192&&r===168||t===0||t===169&&r===254}async function wi(l){try{let e=new URL(l);return await Pd(e.hostname)}catch{return!0}}async function wn(l,e){return await wi(l.url())?`browser_${e} \u88AB\u963B\u6B62\uFF1A\u5F53\u524D\u9875\u9762\u662F\u79C1\u6709/\u672C\u5730\u5730\u5740\uFF08${l.url()}\uFF09\uFF0C\u4E0D\u5141\u8BB8\u5728\u79C1\u6709\u9875\u9762\u4E0A\u64CD\u4F5C\u3002`:null}function Ld(l){let e=[],t=/https?:\/\/[^\s'"`)\]<>]+/g;for(let r of l.matchAll(t))e.push(r[0].replace(/[.,;]$/,""));return e}var Nd=[[/document\s*\.\s*cookie/i,"document.cookie"],[/\b(?:localStorage|sessionStorage)\b/i,"web storage"],[/\bindexedDB\b/i,"IndexedDB"],[/\bcaches\s*\.\s*(?:open|match|keys)\b/i,"Cache Storage"],[/\bnavigator\s*\.\s*(?:clipboard|credentials|serviceWorker)\b/i,"navigator sensitive API"],[/\b(?:fetch|XMLHttpRequest|WebSocket|EventSource|sendBeacon)\s*\(/i,"network request"]];function Od(){return process.env.KEXVIM_BROWSER_RESTRICT_EVALUATE==="1"}function $d(l){if(Od()){for(let[e,t]of Nd)if(e.test(l))return`Blocked: browser_console(expression=...) tried to use sensitive browser primitive: ${t} (KEXVIM_BROWSER_RESTRICT_EVALUATE=1).`}return null}function Dd(){let l=I.findProjectRoot(),e=_i.join(l||process.cwd(),"data","media");return _a.mkdirSync(e,{recursive:!0}),e}var un=class{name="browser_navigate";description="Open a URL in the browser. Returns the page title, final URL and a compact snapshot of interactive elements (refs like [ref=1] for browser_click / browser_type). First call starts a headless browser session for this task.";parameters={type:"object",properties:{url:{type:"string",description:"The URL to navigate to (http/https)."}},required:["url"]};async execute(e,t,r){let s=String(e.url||"").trim();if(!s)return JSON.stringify({success:!1,error:"url is required"});let n=s;if(/^https?:\/\//i.test(n)||(n=`https://${n}`),await wi(n))return JSON.stringify({success:!1,error:`\u5BFC\u822A\u88AB\u963B\u6B62\uFF1A${n} \u662F\u79C1\u6709/\u672C\u5730\u5730\u5740\uFF0C\u6D4F\u89C8\u5668\u4E0D\u5141\u8BB8\u8BBF\u95EE\u79C1\u7F51\u3002`});try{let i=await Z.ensure("default");await i.page.goto(n,{waitUntil:"domcontentloaded",timeout:6e4}),await i.page.waitForTimeout(800);let o=await wa(i.page,!1);return JSON.stringify({success:!0,url:i.page.url(),title:await i.page.title(),snapshot:o})}catch(i){return JSON.stringify({success:!1,error:i instanceof Error?i.message:String(i)})}}},pn=class{name="browser_snapshot";description="Get a snapshot of the current page: interactive elements with ref IDs ([ref=1], [ref=2]...) for browser_click and browser_type, plus page text. full=true returns complete page content (may be truncated at ~30k chars). Requires browser_navigate first.";parameters={type:"object",properties:{full:{type:"boolean",default:!1,description:"true = full page text; false (default) = compact view with interactive element refs."}}};async execute(e,t,r){let s=Z.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{let n=await wa(s.page,e.full===!0);return JSON.stringify({success:!0,snapshot:n})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},mn=class{name="browser_click";description="Click an interactive element by ref (from browser_snapshot, e.g. @e5 or 5).";parameters={type:"object",properties:{ref:{type:"string",description:"Element ref like @e5 (or plain number)."}},required:["ref"]};async execute(e,t,r){let s=Z.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});let n=await wn(s.page,"click");if(n)return JSON.stringify({success:!1,error:n});try{let i=await xa(s.page,String(e.ref||""));return await i.scrollIntoViewIfNeeded({timeout:5e3}).catch(()=>{}),await i.click({timeout:1e4}),await s.page.waitForTimeout(500),JSON.stringify({success:!0,message:`Clicked ${e.ref}.`})}catch(i){return JSON.stringify({success:!1,error:i instanceof Error?i.message:String(i)})}}},gn=class{name="browser_type";description="Fill an input element with text by ref (from browser_snapshot, e.g. @e3).";parameters={type:"object",properties:{ref:{type:"string",description:"Element ref like @e3."},text:{type:"string",description:"Text to type into the field."}},required:["ref","text"]};async execute(e,t,r){let s=Z.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});let n=await wn(s.page,"type");if(n)return JSON.stringify({success:!1,error:n});try{return await(await xa(s.page,String(e.ref||""))).fill(String(e.text??""),{timeout:1e4}),JSON.stringify({success:!0,message:`Filled ${e.ref}.`})}catch(i){return JSON.stringify({success:!1,error:i instanceof Error?i.message:String(i)})}}},fn=class{name="browser_scroll";description="Scroll the page. direction: down (default) or up. Scrolls ~500px per call; call repeatedly to page through.";parameters={type:"object",properties:{direction:{type:"string",enum:["down","up"],default:"down"}}};async execute(e,t,r){let s=Z.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{let n=e.direction==="up"?-ba:ba;return await s.page.evaluate(i=>window.scrollBy(0,i),n),await s.page.waitForTimeout(300),JSON.stringify({success:!0,message:`Scrolled ${e.direction||"down"} ${Math.abs(n)}px.`})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},hn=class{name="browser_back";description="Go back to the previous page in the browser history.";parameters={type:"object",properties:{}};async execute(e,t,r){let s=Z.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{return await s.page.goBack({timeout:3e4}),await s.page.waitForTimeout(500),JSON.stringify({success:!0,url:s.page.url()})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},yn=class{name="browser_press";description="Press a keyboard key in the page (e.g. Enter, Escape, ArrowDown, Tab).";parameters={type:"object",properties:{key:{type:"string",description:"Key name (Enter, Escape, ArrowDown, Tab, ...)."}},required:["key"]};async execute(e,t,r){let s=Z.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});let n=await wn(s.page,"press");if(n)return JSON.stringify({success:!1,error:n});try{return await s.page.keyboard.press(String(e.key||"")),JSON.stringify({success:!0,message:`Pressed ${e.key}.`})}catch(i){return JSON.stringify({success:!1,error:i instanceof Error?i.message:String(i)})}}},vn=class{name="browser_get_images";description="List image URLs and alt texts currently on the page.";parameters={type:"object",properties:{}};async execute(e,t,r){let s=Z.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{let n=await s.page.evaluate(()=>Array.from(document.querySelectorAll("img")).slice(0,50).map(i=>({src:i.src.slice(0,300),alt:i.alt.slice(0,100),width:i.naturalWidth,height:i.naturalHeight})).filter(i=>i.src));return JSON.stringify({success:!0,count:n.length,images:n})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},bn=class{name="browser_vision";description="Take a screenshot of the current page so you can inspect it visually. Returns a screenshot path the model can analyze (needs a vision-capable model). Use for CAPTCHAs, visual layouts, or when the text snapshot misses something.";parameters={type:"object",properties:{question:{type:"string",description:"Optional: what to look for (used as analysis prompt)."},annotate:{type:"boolean",default:!1,description:"Reserved for future annotated snapshot; currently ignored."}}};async execute(e,t,r){let s=Z.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{let n=_i.join(Dd(),`browser_shot_${Date.now()}.png`);return await s.page.screenshot({path:n,fullPage:!1}),JSON.stringify({type:"vision",path:n,mimeType:"image/png",prompt:String(e.question||"Describe what you see on this page, focusing on layout, text and interactive elements."),url:s.page.url()})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},kn=class l{name="browser_console";description="Read browser console messages (clear=true clears the log), or evaluate a JavaScript expression in the page (expression=...). Evaluation with sensitive primitives (cookies, storage, network) is blocked when KEXVIM_BROWSER_RESTRICT_EVALUATE=1; navigation to private/local addresses is always blocked.";parameters={type:"object",properties:{clear:{type:"boolean",default:!1,description:"Clear the console log (only meaningful without expression)."},expression:{type:"string",description:"Optional JS expression to evaluate in the page (result JSON-serialized)."}}};async execute(e,t,r){let s=Z.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});let n=e.expression!==void 0?String(e.expression):void 0;if(n!==void 0&&n.trim()){let o=$d(n);if(o)return JSON.stringify({success:!1,error:o});let a=await wn(s.page,"console");if(a)return JSON.stringify({success:!1,error:a});for(let c of Ld(n))if(await wi(c))return JSON.stringify({success:!1,error:`browser_console eval \u88AB\u963B\u6B62\uFF1A\u8868\u8FBE\u5F0F\u5305\u542B\u79C1\u6709/\u672C\u5730\u5730\u5740 ${c}\u3002`});try{let c=await s.page.evaluate(d=>{try{return new Function(`return (${d});`)()}catch{return new Function(d)()}},n);return JSON.stringify({success:!0,result:l.serialize(c)})}catch(c){return JSON.stringify({success:!1,error:c instanceof Error?c.message:String(c)})}}let i=Z.consoleState("default");return e.clear===!0?(i.messages=[],i.errors=[],JSON.stringify({success:!0,messages:[],errors:[],cleared:!0})):JSON.stringify({success:!0,messages:i.messages.slice(-50),errors:i.errors.slice(-50)})}static serialize(e){if(e===void 0)return"undefined";try{return JSON.parse(JSON.stringify(e))}catch{return String(e)}}},Sn=class{name="browser_cdp";description="Attach the browser session to an existing Chrome/Chromium via its CDP WebSocket URL (e.g. from chrome://inspect, or a remote debugging port). Subsequent browser_* calls operate on that browser. Session is per-task; existing session is replaced.";parameters={type:"object",properties:{url:{type:"string",description:"CDP WebSocket debugger URL (ws://host:port/devtools/browser/...)."}},required:["url"]};async execute(e,t,r){let s=String(e.url||"").trim();if(!/^ws:\/\//i.test(s))return JSON.stringify({success:!1,error:"url \u5FC5\u987B\u662F CDP WebSocket \u5730\u5740\uFF08ws://...\uFF09\uFF0C\u5982 ws://127.0.0.1:9222/devtools/browser/xxx"});try{let n=Z.get("default");if(n)try{await n.browser.close()}catch{}let o=(await Z.ensure("default",s)).context.pages().map(a=>a.url());return JSON.stringify({success:!0,attached:!0,pages:o})}catch(n){return JSON.stringify({success:!1,error:`CDP \u8FDE\u63A5\u5931\u8D25\uFF1A${n instanceof Error?n.message:String(n)}`})}}},_n=class{name="browser_dialog";description="Show dialogs (alert/confirm/prompt) that appeared on the page. Dialogs are auto-accepted and recorded; this tool lists them so you know what the page asked. No arguments.";parameters={type:"object",properties:{}};async execute(e,t,r){let s=Z.dialogs("default");return s.length?JSON.stringify({success:!0,dialogs:s}):JSON.stringify({success:!0,dialogs:[],message:"No dialogs were shown."})}};process.on("exit",()=>{Z.closeAll()});var Fd=setInterval(()=>{Z.reapIdle()},6e4);Fd.unref();import*as Ta from"node:crypto";var xn=class l{static build(e,t,r,s,n,i){let o=Ta.randomUUID(),a="";try{a=n.prefetchAll(e,s)}catch(d){console.warn(S.t("memory.tcb_prefetch_failed",{error:d instanceof Error?d.message:String(d)}))}let c=l._findLastUserIndex(r);return{userMessage:e,messages:r,activeSystemPrompt:t,turnId:o,extPrefetchCache:a,currentTurnUserIdx:c}}static _findLastUserIndex(e){for(let t=e.length-1;t>=0;t--)if(e[t].role==="user")return t;return e.length}};var Ca=5e3,Ea=new Set(["clarify","delegate_task","memory","read_file","write_file","edit","bash","search_files","terminal","execute_code"]),xi=class{queue=[];_running=!1;submit(e){this.queue.push(e),this._running||this.drain()}drain(){this._running=!0;let e=()=>{if(this.queue.length===0){this._running=!1;return}let t=this.queue.shift();try{t()}catch{}setImmediate(e)};setImmediate(e)}flush(e=Ca){return new Promise(t=>{if(this.queue.length===0){t(!0);return}let r=setTimeout(()=>t(!1),e);this.submit(()=>{clearTimeout(r),t(!0)})})}shutdown(){this.queue=[],this._running=!1}};var xr=class l{static sanitizeContext(e){return e.replace(/<memory-context>[\s\S]*?<\/memory-context>/gi,"").replace(/\[System note:\s*The following is recalled memory context,\s*NOT new user input\.\s*Treat as [^\]]*\]\s*/gi,"").replace(/<\/?\s*memory-context\s*>/gi,"")}static buildMemoryContextBlock(e){return!e||!e.trim()?"":`<memory-context>
...
`+l.sanitizeContext(e)+`
</memory-context>`}},Tr=class l{providers=[];toolToProvider=new Map;hasExternal=!1;bg=new xi;addProvider(e){if(!(e.name==="builtin")){if(this.hasExternal){console.warn(S.t("memory.memmgr_rejected_provider",{name:e.name}));return}this.hasExternal=!0}this.providers.push(e);for(let r of e.getToolSchemas()){let s=l.normalizeToolSchema(r);if(!s)continue;let n=s.name;if(!(!n||typeof n!="string")){if(Ea.has(n)){console.warn(S.t("memory.memmgr_shadowed_tool",{providerName:e.name,toolName:n}));continue}this.toolToProvider.has(n)?console.warn(`Memory tool name conflict: '${n}' already registered by ${this.toolToProvider.get(n).name}`):this.toolToProvider.set(n,e)}}}get providersList(){return[...this.providers]}getProvider(e){return this.providers.find(t=>t.name===e)}buildSystemPrompt(){let e=[];for(let t of this.providers)try{let r=t.systemPromptBlock();r&&r.trim()&&e.push(r)}catch{}return e.join(`
...
`,lr=class{db;constructor(e){this.db=new tu(e),this.db.exec("PRAGMA busy_timeout = 15000"),this.db.exec(ru)}async upsert(e){let t=this.db.prepare(`
...
`;return Qe.writeFileSync(r,s,"utf-8"),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3}).code!==0?!1:b.runSyncResult("loginctl enable-linger "+ur.userInfo().username,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=Hr.join(ur.homedir(),".config","systemd","user",`${e}.service`);return Qe.existsSync(t)&&(b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:5e3}),Qe.unlinkSync(t),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3})),!0}catch{return!1}}isServiceInstalled(e){let t=Hr.join(ur.homedir(),".config","systemd","user",`${e}.service`);return Qe.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`systemctl --user is-active ${e}`,{timeoutMs:3e3});return t.code!==0?!1:t.stdout.trim()==="active"}startService(e){return b.runSyncResult(`systemctl --user start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`systemctl --user enable ${e}`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`systemctl --user disable ${e}`,{timeoutMs:5e3}).code===0}};import*as Ze from"fs";import*as Gn from"os";import*as Ct from"path";var Kn=class{platform="win32";findKexvimPids(){let e=b.runSyncResult(`powershell -Command "Get-Process node | Where-Object { $_.CommandLine -match 'dev\\.mjs|kexvim\\.js' } | Select-Object -ExpandProperty Id"`,{timeoutMs:5e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){let r=t?"/F":"";return b.runSyncResult(`taskkill ${r} /PID ${e}`,{timeoutMs:5e3}).code===0}installService(e){if(b.runSyncResult("nssm --version",{timeoutMs:3e3}).code===0&&b.runSyncResult(`nssm install ${e.name} "${e.execPath}" "${e.workDir}"`,{timeoutMs:1e4}).code===0)return!0;try{let r=Ct.join(process.env.APPDATA||Gn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");Ze.mkdirSync(r,{recursive:!0});let s=`@echo off
cd /d "${e.workDir}"
...
`;return Ze.writeFileSync(Ct.join(r,`${e.name}.bat`),s,"utf-8"),!0}catch{return!1}}uninstallService(e){if(!(b.runSyncResult(`nssm remove ${e} confirm`,{timeoutMs:1e4}).code===0)){let r=Ct.join(process.env.APPDATA||Gn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup"),s=Ct.join(r,`${e}.bat`);if(Ze.existsSync(s))try{Ze.unlinkSync(s)}catch{}}return!0}isServiceInstalled(e){if(b.runSyncResult(`nssm status ${e}`,{timeoutMs:3e3}).code===0)return!0;let t=Ct.join(process.env.APPDATA||Gn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");return Ze.existsSync(Ct.join(t,`${e}.bat`))}isServiceRunning(e){let t=b.runSyncResult(`powershell -Command "Get-Service ${e} | Select-Object -ExpandProperty Status"`,{timeoutMs:5e3});return t.code!==0?!1:t.stdout.trim().toLowerCase()==="running"}startService(e){return b.runSyncResult(`net start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`net stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`sc config ${e} start=auto`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`sc config ${e} start=disabled`,{timeoutMs:5e3}).code===0}};import*as et from"fs";import*as pr from"os";import*as Rt from"path";var zn=class{platform="darwin";findKexvimPids(){let e=b.runSyncResult('pgrep -af "dev\\.mjs|kexvim\\.js" | grep -v " web"',{timeoutMs:3e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
...
platform:
  ...
  adapters:
    qq:
      app_id: ""
      client_secret: ""
`;function vl(l){let e=process.platform==="win32",t=ee.join(l,e?"kexvim.cmd":"kexvim");j.existsSync(t)||(e?j.writeFileSync(t,`@echo off\r
...
`),console.log(`[~] \u5DF2\u5C06 ${l} \u52A0\u5165 ${ee.basename(r)}\uFF0C\u91CD\u542F\u7EC8\u7AEF\u540E\u751F\u6548`));else if(e)try{let n=`$h = ${"'"+l.replace(/'/g,"''")+"'"}; $p = [Environment]::GetEnvironmentVariable('PATH','User'); if ($p -and $p.Contains($h)) { Write-Output 'exists' } elseif ($p) { [Environment]::SetEnvironmentVariable('PATH', $p + ';' + $h, 'User'); Write-Output 'added' } else { [Environment]::SetEnvironmentVariable('PATH', $h, 'User'); Write-Output 'added' }`,i=b.runFileSync("powershell",["-NoProfile","-Command",n],{maxBuffer:1024*1024});console.log(i.includes("exists")?"[~] PATH \u5DF2\u914D\u7F6E":"[~] \u5DF2\u5C06 Kexvim \u52A0\u5165 PATH\uFF0C\u65B0\u7EC8\u7AEF\u751F\u6548")}catch{console.log("[~] \u672A\u80FD\u81EA\u52A8\u8BBE\u7F6E PATH\uFF0C\u8BF7\u624B\u52A8\u5C06\u542F\u52A8\u5668\u76EE\u5F55\u52A0\u5165\u73AF\u5883\u53D8\u91CF PATH")}else console.log(`[~] \u624B\u52A8\u5C06\u4EE5\u4E0B\u884C\u52A0\u5165 shell \u914D\u7F6E\u6587\u4EF6 (~/.bashrc / ~/.zshrc):
    export PATH="$PATH:${l}"`)}async function Sl(l,e){let t=i=>{try{return j.accessSync(i),!0}catch{return!1}};if(t(ee.join(l,"data",".env"))){console.log("Kexvim \u5DF2\u521D\u59CB\u5316(data/.env \u5DF2\u5B58\u5728)\u3002"),vl(l);return}if(t(ee.join(l,"kexvim.js")))console.log("[~] kexvim.js \u5DF2\u5B58\u5728\uFF0C\u8DF3\u8FC7\u4E0B\u8F7D");else if(j.mkdirSync(l,{recursive:!0}),bl("git")){console.log("[~] \u514B\u9686\u4ED3\u5E93...");try{b.runFileSync("git",["clone","--depth","1",e,l],{timeoutMs:12e4})}catch{console.error("[\u2717] \u514B\u9686\u5931\u8D25"),process.exit(1)}}else{console.log("[~] \u4E0B\u8F7D kexvim.js...");let i=ee.join(Ft.tmpdir(),"kexvim-init.js"),o=`${e}/raw/main/kexvim.js`;try{bl("wget")?b.runFileSync("wget",["-q","-O",i,o],{timeoutMs:6e4}):b.runFileSync("curl",["-fsSL","-o",i,o],{timeoutMs:6e4})}catch{console.error("[\u2717] \u4E0B\u8F7D\u5931\u8D25"),process.exit(1)}t(i)&&(j.copyFileSync(i,ee.join(l,"kexvim.js")),j.rmSync(i,{force:!0}))}let r=ee.join(l,"data",".env");if(j.mkdirSync(ee.join(l,"data"),{recursive:!0}),!t(r)){let i=kl.createInterface({input:process.stdin,output:process.stdout}),o=await new Promise(a=>{i.question("\u8BF7\u8F93\u5165 DeepSeek API Key: ",a)});i.close(),o&&(j.writeFileSync(r,`DEEPSEEK_API_KEY=${o}
KEXVIM_HOME=${l}
`),console.log("[\u2713] API Key \u5DF2\u4FDD\u5B58"))}let s=ee.join(l,"data","config.yaml");t(s)||(j.writeFileSync(s,cp),console.log("[\u2713] config.yaml \u5DF2\u521B\u5EFA\uFF08\u8BF7\u6309\u9700\u8865\u5145\u914D\u7F6E\uFF09"));let n=ee.join(l,"skills");if(j.existsSync(n))console.log("[~] skills \u5DF2\u5B58\u5728\uFF0C\u8DF3\u8FC7");else{console.log("[~] \u4E0B\u8F7D\u516C\u5171\u6280\u80FD...");let i=j.mkdtempSync(ee.join(Ft.tmpdir(),"kexvim-skills-"));try{let o=b.isWindows?"nul":"/dev/null";b.runSync(`git clone --depth 1 ${e}.git "${i}" --single-branch 2>${o}`,{timeoutMs:6e4}),j.existsSync(ee.join(i,"skills"))?(j.cpSync(ee.join(i,"skills"),n,{recursive:!0}),console.log("[\u2713] \u6280\u80FD\u5DF2\u4E0B\u8F7D")):console.warn("[~] \u8FDC\u7A0B\u4ED3\u5E93\u4E2D\u6CA1\u6709 skills \u76EE\u5F55\uFF0C\u8DF3\u8FC7")}catch{console.warn("[~] \u6280\u80FD\u4E0B\u8F7D\u5931\u8D25\uFF08git \u672A\u5B89\u88C5\u6216\u65E0\u7F51\u7EDC\uFF09\uFF0C\u53EF\u624B\u52A8 clone \u540E\u590D\u5236 skills/ \u76EE\u5F55")}finally{j.rmSync(i,{recursive:!0,force:!0})}}vl(l),console.log("[\u2713] \u5B89\u88C5\u5B8C\u6210\u3002\u73B0\u5728\u53EF\u4EE5\u76F4\u63A5\u5728\u7EC8\u7AEF\u8F93\u5165: kexvim restart")}function bl(l){try{return b.runFileSync(process.platform==="win32"?"where":"which",[l]),!0}catch{return!1}}import*as yr from"node:fs";import*as Zr from"node:path";import*as _l from"node:os";function dp(l){try{return b.runFileSync(process.platform==="win32"?"where":"which",[l]),!0}catch{return!1}}async function wl(l,e){let t=r=>{try{return yr.accessSync(r),!0}catch{return!1}};if(t(Zr.join(l,"kexvim.js"))||(console.error("[\u2717] \u672A\u5B89\u88C5 Kexvim\uFF0C\u5148\u8FD0\u884C node kexvim.js init"),process.exit(1)),t(Zr.join(l,".git"))){console.log("[~] git pull...");try{b.runFileSync("git",["pull","--ff-only"],{cwd:l,timeoutMs:12e4})}catch{console.error("[\u2717] \u62C9\u53D6\u5931\u8D25"),process.exit(1)}}else{console.log("[~] \u4E0B\u8F7D\u6700\u65B0 kexvim.js...");let r=Zr.join(_l.tmpdir(),"kexvim-update.js");try{let s=`${e}/raw/main/kexvim.js`;dp("wget")?b.runFileSync("wget",["-q","-O",r,s],{timeoutMs:6e4}):b.runFileSync("curl",["-fsSL","-o",r,s],{timeoutMs:6e4})}catch{console.error("[\u2717] \u4E0B\u8F7D\u5931\u8D25"),process.exit(1)}t(r)&&(yr.copyFileSync(r,Zr.join(l,"kexvim.js")),yr.rmSync(r,{force:!0}))}console.log("[\u2713] \u66F4\u65B0\u5B8C\u6210\uFF0C\u6B63\u5728\u91CD\u542F...")}import*as H from"fs";import*as Se from"path";var xl=At();function up(l,e){if(!H.existsSync(Se.join(l,"src")))return!1;if(!H.existsSync(e))return!0;let t=H.statSync(e).mtimeMs,r=t,s=n=>{let i;try{i=H.readdirSync(n,{withFileTypes:!0})}catch{return}for(let o of i){let a=Se.join(n,o.name);if(o.isDirectory()){if(o.name==="node_modules"||o.name==="dist"||o.name==="data"||o.name===".git")continue;s(a)}else if(o.name.endsWith(".ts"))try{H.statSync(a).mtimeMs>r&&(r=H.statSync(a).mtimeMs)}catch{}}};return s(Se.join(l,"src")),s(Se.join(l,"packages")),r>t}function Qi(l){if(process.platform!=="win32")return[];let e=b.runSyncResult(`wmic process where "name='node.exe'" get ProcessId,CommandLine /format:list`,{timeoutMs:5e3});if(e.code!==0)return[];let t=e.stdout.replace(/\u0000/g,""),r=Se.resolve(l).toLowerCase(),s=o=>{let a=Se.resolve(o).toLowerCase();return a===r||a.startsWith(r+Se.sep)},n=[],i=null;for(let o of t.split(/\r?\n/)){let a=o.trim();if(a.startsWith("CommandLine="))i=a.slice(12).replace(/^"|"$/g,"");else if(a.startsWith("ProcessId=")){let c=Number(a.slice(10));if(i&&c>0&&c!==process.pid&&!/\sweb(\s|$)/.test(i)){let d=i.match(/(\S*(?:dev\.mjs|kexvim\.js))/);d&&s(d[1])&&n.push({pid:c,entry:d[1]})}i=null}}return n}function pp(l,e=64*1024){try{let t=H.statSync(l);if(!t.isFile()||t.size===0)return"";if(t.size<=e)return H.readFileSync(l,"utf-8");let r=H.openSync(l,"r");try{let s=Buffer.alloc(e),n=H.readSync(r,s,0,e,t.size-e);return s.toString("utf-8",0,n)}finally{H.closeSync(r)}}catch{return""}}function mp(l,e){return new Promise(t=>{let r=0,s=setInterval(()=>{r++;try{if(H.existsSync(l)){let n=pp(l);if(n.includes("Guardian agent \u5DF2\u5C31\u7EEA")||n.includes("kexvim \u5DF2\u5C31\u7EEA")){clearInterval(s),console.log(`[kexvim] \u2705 kexvim \u5DF2\u5C31\u7EEA\uFF08${r}s\uFF09`),t(!0);return}}}catch{}r>=e&&(clearInterval(s),t(!1))},1e3)})}async function Tl(l){let e=l,t=Se.join(e,"data","log");try{H.mkdirSync(t,{recursive:!0})}catch{}let r=Se.join(t,"kexvim.log"),s=Se.join(e,"dist","dev.mjs");if(up(e,s)){console.log("[kexvim] \u68C0\u6D4B\u5230\u6E90\u7801\u53D8\u66F4\uFF0C\u81EA\u52A8\u6784\u5EFA...");let d=b.runSyncResult("npm run build:dev",{cwd:e,timeoutMs:12e4});d.code!==0&&(console.error(`[kexvim] \u274C \u6784\u5EFA\u5931\u8D25\uFF0C\u4E2D\u6B62\u91CD\u542F\uFF1A${d.stderr?.trim()||d.stdout?.trim()}`),process.exit(1)),console.log("[kexvim] \u2705 \u6784\u5EFA\u5B8C\u6210")}if(process.platform==="linux")try{let d=b.runSyncResult("systemctl --user is-active kexvim.service",{timeoutMs:5e3});if(d.code===0&&d.stdout.trim()==="active"){console.log("[kexvim] systemd \u6258\u7BA1\uFF08kexvim.service active\uFF09\uFF0C\u8D70 systemctl --user restart");let u=b.runSyncResult("systemctl --user restart kexvim.service",{timeoutMs:3e4});u.code!==0&&(console.error(`[kexvim] \u274C systemctl restart \u5931\u8D25: ${u.stderr?.trim()||u.stdout?.trim()}`),process.exit(1)),console.log("[kexvim] \u2705 systemctl restart \u5B8C\u6210");return}}catch{}let n=Qi(e),i=n.length>0?n.map(d=>d.pid):xl.findKexvimPids(),o=n.length>0?n[0].entry:Be.resolveEntry(e);if(console.log(`[kexvim] \u65E7\u8FDB\u7A0B: ${i.length} \u4E2A; \u5165\u53E3: ${o}`),H.writeFileSync(r,"","utf-8"),i.length>0){console.log("[kexvim] \u6E05\u7406\u65E7\u8FDB\u7A0B...");for(let d of i)process.platform==="win32"?b.runSyncResult(`taskkill /PID ${d} /F`,{timeoutMs:5e3}):xl.killProcess(d,!0);console.log("[kexvim] \u2705 \u65E7\u8FDB\u7A0B\u5DF2\u6E05\u7406")}console.log("[kexvim] \u6B63\u5728\u542F\u52A8\u65B0 kexvim...");let a=Be.spawnFreshDaemon(e,o);a===null&&(console.error("[kexvim] \u274C \u65E0\u6CD5\u542F\u52A8\u65B0\u8FDB\u7A0B"),process.exit(1)),console.log(`[kexvim] \u65B0 PID: ${a}\uFF0C\u7B49\u5F85\u5C31\u7EEA...`),await mp(r,30)||console.warn("[kexvim] \u26A0\uFE0F \u7B49\u5F85\u5C31\u7EEA\u8D85\u65F6\uFF0830s\uFF09\uFF0C\u8BF7\u68C0\u67E5 data/log/kexvim.log");try{xe.restart(e),console.log("[kexvim] \u2705 web \u8FDB\u7A0B\u5DF2\u5C31\u7EEA\uFF08\u5DF2\u786E\u4FDD\u62C9\u8D77\uFF09")}catch(d){console.error(`[kexvim] \u26A0\uFE0F web \u91CD\u542F\u5931\u8D25: ${d instanceof Error?d.message:String(d)}`)}console.log("[kexvim] \u65E5\u5FD7:",r),process.exit(0)}import*as El from"fs";import*as Cl from"path";async function Rl(l){let e=Qi(l);for(let r of e)r.pid!==process.pid&&(process.platform==="win32"?b.runSyncResult(`taskkill /PID ${r.pid} /F /T`,{timeoutMs:5e3}):At().killProcess(r.pid,!0),console.log(`[kexvim] \u5DF2\u505C\u6B62 PID ${r.pid} (${r.entry})`));let t=!1;try{let r=xe.findPid();r!==null&&(process.platform==="win32"?b.runSyncResult(`taskkill /PID ${r} /F /T`,{timeoutMs:5e3}):At().killProcess(r,!0),console.log(`[kexvim] \u5DF2\u505C\u6B62 web \u8FDB\u7A0B PID ${r}`),t=!0)}catch{}try{El.rmSync(Cl.join(l,"data","kexvim.pid"),{force:!0})}catch{}e.length===0&&!t?console.log("[kexvim] \u6CA1\u6709\u8FD0\u884C\u4E2D\u7684 kexvim \u8FDB\u7A0B"):console.log("[kexvim] \u2705 kexvim \u5DF2\u505C\u6B62")}import*as K from"node:fs";import*as es from"node:os";import*as te from"node:path";import{fileURLToPath as gp}from"node:url";var ie=class l{static SERVICE_NAME="kexvim";static AUTOSTART_DIR="autostart";static KEEPALIVE_INTERVAL_MIN=1;static SYSTEMD_TEMPLATE=(e,t,r)=>`[Unit]
...
`;static platform=process.platform;static _findRoot(){let e=te.dirname(gp(import.meta.url));for(let t=0;t<8;t++){if(K.existsSync(te.join(e,"package.json")))return e;let r=te.dirname(e);if(r===e)break;e=r}throw new Error("[kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF08\u65E0 package.json\uFF09\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002")}static _systemPath(e){if(l.platform==="win32")return"";let t=es.homedir();return l.platform==="darwin"?te.join(t,"Library","LaunchAgents","com.kexvim.daemon.plist"):te.join(t,".config","systemd","user","kexvim.service")}static _dataPath(e){let t=te.join(e,"data",l.AUTOSTART_DIR);return K.mkdirSync(t,{recursive:!0}),l.platform==="win32"?te.join(t,"keepalive.cmd"):l.platform==="darwin"?te.join(t,"com.kexvim.daemon.plist"):te.join(t,"kexvim.service")}static _devEntry(e){let t=te.join(e,"dist","dev.mjs");if(K.existsSync(t))return t;let r=te.join(e,"kexvim.js");if(K.existsSync(r))return r;throw new Error("[kexvim] \u672A\u627E\u5230 daemon \u5165\u53E3\uFF08dist/dev.mjs \u6216 kexvim.js\uFF09\u3002")}static _template(e){let t=process.execPath,r=l._devEntry(e);return l.platform==="win32"?`@echo off\r
set NODE_NO_WARNINGS=1\r
cd /d "${e}"\r
${t} "${r}" keepalive\r
`:l.platform==="darwin"?l.LAUNCHD_TEMPLATE(t,e,r):l.SYSTEMD_TEMPLATE(t,e,r)}static writeTemplate(e){let t=e??l._findRoot(),r=l._dataPath(t);return K.writeFileSync(r,l._template(t)),r}static install(e){let t=e??l._findRoot(),r=l.writeTemplate(t),s=l._systemPath(t),n=[`[\u2713] \u6A21\u677F\u5DF2\u751F\u6210: ${r}`];if(l.platform==="win32")l._winInstall(t,n);else if(l.platform==="darwin"){K.mkdirSync(te.dirname(s),{recursive:!0}),K.copyFileSync(r,s),n.push(`[\u2713] \u5DF2\u5199\u5165: ${s}`);try{b.runFileSync("launchctl",["load","-w",s]),n.push("[\u2713] launchctl load \u5B8C\u6210")}catch{n.push("[\u2717] launchctl load \u5931\u8D25\uFF08\u9700\u5F53\u524D\u7528\u6237\u6709 GUI \u4F1A\u8BDD\uFF09")}}else{K.mkdirSync(te.dirname(s),{recursive:!0}),K.copyFileSync(r,s),n.push(`[\u2713] \u5DF2\u5199\u5165: ${s}`);try{b.runFileSync("systemctl",["--user","daemon-reload"]),b.runFileSync("systemctl",["--user","enable","kexvim.service"]),n.push("[\u2713] systemd user \u5355\u5143\u5DF2 enable")}catch(i){n.push(`[\u2717] systemctl \u6CE8\u518C\u5931\u8D25: ${i instanceof Error?i.message:String(i)}`)}try{b.runFileSync("loginctl",["enable-linger",es.userInfo().username]),n.push("[\u2713] linger \u5DF2\u542F\u7528\uFF08\u5F00\u673A\u81EA\u542F\uFF09")}catch{n.push("[~] linger \u542F\u7528\u5931\u8D25\uFF08\u90E8\u5206\u7CFB\u7EDF\u9700 root\uFF09\uFF0C\u81EA\u542F\u53EF\u80FD\u53EA\u5728\u767B\u5F55\u540E\u751F\u6548")}}return n.join(`
...
Register-ScheduledTask -TaskName 'KexvimKeepAlive' -Action (New-ScheduledTaskAction -Execute '${r}' -Argument '"--no-warnings" "${s}" keepalive' -WorkingDirectory '${e}') -Trigger (New-ScheduledTaskTrigger -Once -At (Get-Date).AddMinutes(1) -RepetitionInterval (New-TimeSpan -Minutes ${l.KEEPALIVE_INTERVAL_MIN})) -Principal $p -Force | Out-Null`;try{b.runFileSync("powershell",["-NoProfile","-Command",n]),t.push("[\u2713] \u8BA1\u5212\u4EFB\u52A1 KexvimDaemon\uFF08\u5F00\u673A\u5EF6\u8FDF 1 \u5206\u949F\u62C9\u8D77\uFF0CS4U \u65E0\u7A97\u53E3\uFF09\u5DF2\u521B\u5EFA"),t.push(`[\u2713] \u8BA1\u5212\u4EFB\u52A1 KexvimKeepAlive\uFF08\u6BCF ${l.KEEPALIVE_INTERVAL_MIN} \u5206\u949F\u4FDD\u6D3B daemon+web\uFF0CS4U \u65E0\u7A97\u53E3\uFF09\u5DF2\u521B\u5EFA`)}catch(i){t.push(`[\u2717] \u521B\u5EFA\u8BA1\u5212\u4EFB\u52A1\u5931\u8D25: ${i instanceof Error?i.message:String(i)}`)}}static isProcessRunning(e){let t=e??l._findRoot(),r=te.join(t,"data","kexvim.pid");try{let s=Number.parseInt(K.readFileSync(r,"utf-8").trim(),10);return!Number.isFinite(s)||s<=0?!1:(process.kill(s,0),!0)}catch{return!1}}static resolveRoot(){return l._findRoot()}};function Al(){let l=ie.resolveRoot();console.log(ie.install(l));let e=ie.status(l);console.log(`[~] \u72B6\u6001: platform=${e.platform} installed=${e.installed} consistent=${e.consistent} active=${e.serviceActive}`),process.exit(0)}function Ml(){console.log(ie.uninstall(ie.resolveRoot())),process.exit(0)}function Il(){let l=ie.resolveRoot(),e=ie.status(l),t=ie.isProcessRunning(l);console.log(`kexvim \u5E73\u53F0:        ${e.platform}`),console.log(`\u8FDB\u7A0B:               ${t?"\u8FD0\u884C\u4E2D":"\u672A\u8FD0\u884C"} (PID \u6587\u4EF6: data/kexvim.pid)`),console.log(`\u81EA\u542F\u914D\u7F6E:           ${e.installed?"\u5DF2\u5B89\u88C5":"\u672A\u5B89\u88C5"} @ ${e.configPath||"(Windows \u8BA1\u5212\u4EFB\u52A1)"}`),console.log(`\u914D\u7F6E\u4E00\u81F4\u6027(\u7EDF\u4E00):    ${e.consistent?"\u4E00\u81F4(\u6A21\u677F)":"\u4E0D\u4E00\u81F4(\u5C06\u88AB install \u8986\u76D6)"}`),console.log(`\u670D\u52A1\u72B6\u6001:           ${e.serviceActive?"active":"inactive"}`),console.log(`\u91CD\u542F\u5FAA\u73AF\u9632\u62A4:       ${ke.isTripped(l)?"\u26A0\uFE0F TRIPPED\uFF08\u7528 kexvim clear-loop \u89E3\u9664\uFF09":`\u6B63\u5E38 (${ke.bootCount(l)}/${ke.MAX_RESTARTS} in ${ke.WINDOW_SECONDS}s)`}`),process.exit(t?0:1)}function Pl(){ke.clear(ie.resolveRoot()),console.log("[\u2713] restart_loop \u72B6\u6001\u5DF2\u6E05\u9664"),process.exit(0)}import*as vr from"node:fs";import*as Zi from"node:path";var fp=9e4;function hp(l){let e=Zi.join(l,"data","daemon.heartbeat");try{let t=vr.statSync(e);if(Date.now()-t.mtimeMs>fp)return null;let r=Number.parseInt(vr.readFileSync(e,"utf-8").trim(),10);return Number.isFinite(r)&&r>0?r:null}catch{return null}}function Ll(){let l=ie.resolveRoot(),e=hp(l);if(e===null)Be.spawnFreshDaemon(l)===null&&process.exit(1);else try{vr.writeFileSync(Zi.join(l,"data","kexvim.pid"),String(e))}catch{}try{xe.ensureRunning(l)}catch{}process.exit(0)}import*as ts from"node:fs";import*as eo from"node:path";import{DatabaseSync as yp}from"node:sqlite";function Nl(l){let e=eo.join(l,"kexvim.db");ts.existsSync(e)||(console.error("[kexvim] \u274C \u627E\u4E0D\u5230\u4F1A\u8BDD\u5E93:",e),process.exit(1));let t=new yp(e,{readOnly:!0});try{let r=t.prepare(`
      ...
      WHERE s.is_test = 0
      ORDER BY s.updated_at DESC
    `).all();if(r.length===0){console.log("\u6682\u65E0\u5386\u53F2\u4F1A\u8BDD\u3002");return}let s="";try{let n=eo.join(l,".current_session");ts.existsSync(n)&&(s=ts.readFileSync(n,"utf-8").trim())}catch{}console.log(`\u5171 ${r.length} \u4E2A\u4F1A\u8BDD\uFF1A
...
\u7528\u6CD5: kexvim session <\u524D8\u4F4DID>  \u5207\u6362/\u6062\u590D\u8BE5\u4F1A\u8BDD`)}finally{t.close()}}import*as Ol from"node:fs";import*as to from"node:path";import{DatabaseSync as vp}from"node:sqlite";var bp="session-switch.json";function $l(l,e){let t=to.join(l,"kexvim.db"),r=new vp(t,{readOnly:!0}),s;try{s=r.prepare(`
      SELECT id, COALESCE(summary, '') AS summary,
             (SELECT COUNT(*) FROM messages m WHERE m.session_id = s.id) AS msgCount
      FROM sessions s
      WHERE s.is_test = 0 AND s.id LIKE ?
    `).get(`${e}%`)}finally{r.close()}s||(console.error(`[kexvim] \u274C \u627E\u4E0D\u5230\u4F1A\u8BDD "${e}"\u3002\u7528 kexvim sessions \u67E5\u770B\u5168\u90E8\u4F1A\u8BDD\u3002`),process.exit(1)),Ol.writeFileSync(to.join(l,bp),JSON.stringify({sessionId:s.id,requestedAt:Date.now()/1e3}),"utf-8");let n=(s.summary||"(\u65E0\u6807\u9898)").slice(0,30);console.log(`\u2705 \u5DF2\u6807\u8BB0\u5207\u6362\u5230\u4F1A\u8BDD ${s.id.slice(0,8)}\uFF08${s.msgCount} \u6761\u6D88\u606F\uFF0C${n}\uFF09`),console.log("   \u53D1\u9001\u4E0B\u4E00\u6761\u6D88\u606F\u540E daemon \u5C06\u6062\u590D\u8BE5\u4F1A\u8BDD\u7EE7\u7EED\u5BF9\u8BDD\u3002")}function ro(){console.log(`\u7528\u6CD5: kexvim <command>
...
\u5165\u53E3: kexvim \u547D\u4EE4\u7EDF\u4E00\u8F6C\u53D1\u4E3A node dev.js\uFF08\u5F00\u53D1\uFF09\u6216 node kexvim.js\uFF08\u5B89\u88C5\u7248\uFF09\uFF0C
      \u547D\u4EE4\u8FDB\u7A0B\u5B8C\u5168\u72EC\u7ACB\uFF0C\u4E0D\u4F9D\u8D56\u8FD0\u884C\u4E2D\u7684 kexvim \u5B9E\u4F8B\u3002`),process.exit(0)}function Dl(){console.log("kexvim (\u5F00\u53D1\u5165\u53E3 dist/dev.mjs / \u5B89\u88C5\u5165\u53E3 kexvim.js)"),process.exit(0)}import*as br from"node:fs";import*as no from"node:path";import*as Bl from"node:readline";import{load as kp,dump as Sp}from"js-yaml";var so={qq:{name:"QQ \u673A\u5668\u4EBA",desc:"QQ \u5F00\u653E\u5E73\u53F0\u673A\u5668\u4EBA\uFF08app_id / client_secret\uFF09",fields:[{key:"app_id",label:"app_id"},{key:"client_secret",label:"client_secret"},{key:"api_base",label:"api_base\uFF08\u53EF\u9009\uFF0C\u9ED8\u8BA4\u5B98\u65B9\u5730\u5740\uFF09",optional:!0},{key:"markdown_support",label:"markdown_support\uFF08\u53EF\u9009\uFF0Ctrue/false\uFF09",optional:!0}]},api_server:{name:"HTTP API \u670D\u52A1",desc:"OpenAI \u517C\u5BB9 HTTP \u670D\u52A1\uFF08port / host / api_key\uFF09",fields:[{key:"port",label:"port\uFF08\u53EF\u9009\uFF0C\u9ED8\u8BA4 3000\uFF09",optional:!0},{key:"host",label:"host\uFF08\u53EF\u9009\uFF0C\u9ED8\u8BA4 0.0.0.0\uFF09",optional:!0},{key:"api_key",label:"api_key\uFF08\u53EF\u9009\uFF09",optional:!0}]},telegram:{name:"Telegram Bot",desc:"Telegram \u673A\u5668\u4EBA\uFF08token\uFF09",fields:[{key:"token",label:"token\uFF08BotFather \u83B7\u53D6\uFF09"},{key:"poll_timeout",label:"poll_timeout\uFF08\u53EF\u9009\uFF0C\u79D2\uFF09",optional:!0},{key:"group_mention_only",label:"group_mention_only\uFF08\u53EF\u9009\uFF0Ctrue/false\uFF09",optional:!0}]},discord:{name:"Discord Bot",desc:"Discord \u673A\u5668\u4EBA\uFF08token\uFF09",fields:[{key:"token",label:"token"},{key:"group_mention_only",label:"group_mention_only\uFF08\u53EF\u9009\uFF0Ctrue/false\uFF09",optional:!0}]},weixin:{name:"\u5FAE\u4FE1",desc:"\u5FAE\u4FE1\uFF08token / account_id\uFF09",fields:[{key:"token",label:"token"},{key:"account_id",label:"account_id"},{key:"group_mention_only",label:"group_mention_only\uFF08\u53EF\u9009\uFF0Ctrue/false\uFF09",optional:!0}]},dingtalk:{name:"\u9489\u9489",desc:"\u9489\u9489\uFF08client_id / client_secret\uFF09",fields:[{key:"client_id",label:"client_id"},{key:"client_secret",label:"client_secret"},{key:"group_mention_only",label:"group_mention_only\uFF08\u53EF\u9009\uFF0Ctrue/false\uFF09",optional:!0}]},feishu:{name:"\u98DE\u4E66",desc:"\u98DE\u4E66\uFF08app_id / app_secret\uFF09",fields:[{key:"app_id",label:"app_id"},{key:"app_secret",label:"app_secret"},{key:"group_mention_only",label:"group_mention_only\uFF08\u53EF\u9009\uFF0Ctrue/false\uFF09",optional:!0}]},ws:{name:"WebSocket \u670D\u52A1",desc:"WebSocket \u670D\u52A1\uFF08port / host / api_key\uFF09",fields:[{key:"port",label:"port\uFF08\u53EF\u9009\uFF0C\u9ED8\u8BA4 3001\uFF09",optional:!0},{key:"host",label:"host\uFF08\u53EF\u9009\uFF0C\u9ED8\u8BA4 0.0.0.0\uFF09",optional:!0},{key:"api_key",label:"api_key\uFF08\u53EF\u9009\uFF09",optional:!0}]}};function _p(l){let e=br.readFileSync(l,"utf-8"),t=kp(e);if(!t||typeof t!="object"||Array.isArray(t))throw new Error(`config.yaml \u683C\u5F0F\u5F02\u5E38: ${l}`);return t}function Fl(l,e){return new Promise(t=>{l.question(e,t)})}function wp(l,e){let t=!!l?.enabled,r=l?.adapters??{};console.log(`\u914D\u7F6E\u6587\u4EF6: ${no.join(e,"config.yaml")}`),console.log(`platform.enabled: ${t}`);let s=Object.keys(r);if(s.length===0)console.log("\u5DF2\u914D\u7F6E\u5E73\u53F0: \u65E0");else{console.log("\u5DF2\u914D\u7F6E\u5E73\u53F0:");for(let n of s){let i=r[n]??{},o=Object.entries(i).map(([a,c])=>`${a}=${c===void 0?"":String(c)}`).join(", ");console.log(`  ${n}: ${o||"(\u7A7A)"}`)}}console.log()}async function jl(l){let e=no.join(l,"config.yaml");br.existsSync(e)||(console.error("[\u2717] \u672A\u627E\u5230 config.yaml\uFF0C\u8BF7\u5148\u6267\u884C: kexvim init"),process.exit(1));let t=_p(e),r=t.platform??{enabled:!0,adapters:{}};wp(r,l);let s=Object.keys(so);console.log("\u652F\u6301\u7684\u5E73\u53F0:"),s.forEach((i,o)=>{let a=so[i];console.log(`  ${o+1}) ${a.name.padEnd(16)} ${a.desc}`)}),console.log("  0) \u9000\u51FA"),console.log();let n=Bl.createInterface({input:process.stdin,output:process.stdout});try{let i=(await Fl(n,"\u8BF7\u9009\u62E9\u5E73\u53F0\u7F16\u53F7: ")).trim(),o=parseInt(i,10);if(!i||i==="0"||isNaN(o)||o<1||o>s.length){console.log("\u5DF2\u9000\u51FA");return}let a=s[o-1],c=so[a];console.log(),console.log(`\u914D\u7F6E ${c.name}\uFF08${a}\uFF09:`);let d={},u=r.adapters??{},p=u[a]??{};for(let m of c.fields){let g=p[m.key]!==void 0?` (\u5F53\u524D: ${String(p[m.key])})`:"",f=(await Fl(n,`  ${m.label}${g}: `)).trim();if(f===""){if(!m.optional){console.error(`[\u2717] ${m.key} \u5FC5\u586B\uFF0C\u5DF2\u53D6\u6D88\uFF08\u672A\u4FEE\u6539\u914D\u7F6E\uFF09`);return}p[m.key]!==void 0&&(d[m.key]=p[m.key]);continue}f==="true"||f==="false"?d[m.key]=f==="true":/^\d+$/.test(f)?d[m.key]=Number(f):d[m.key]=f}u[a]=d,r.enabled=!0,r.adapters=u,t.platform=r,br.writeFileSync(e,Sp(t,{indent:2,lineWidth:-1})),console.log(),console.log(`[\u2713] \u5DF2\u5199\u5165 platform.adapters.${a}:`);for(let[m,g]of Object.entries(d))console.log(`    ${m}: ${String(g)}`);console.log("[\u2713] platform.enabled = true"),console.log(),console.log("\u91CD\u542F\u540E\u751F\u6548: kexvim restart")}finally{n.close()}}var Qn=class l{static resolveUserDataDir(){let e=process.env.KEXVIM_USER_DATA_DIR;if(e)return e;let t=I.findProjectRoot()??I.findProjectRoot(Bt.dirname(process.argv[1]??""));if(!t)throw new Error("[kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\uFF0C\u6216\u8BBE\u7F6E KEXVIM_USER_DATA_DIR");return Bt.join(t,"data")}static async dispatchFromArgv(){let e=process.argv[2];if(!e)return!1;let t=e.toLowerCase(),r=t==="--help"||t==="-h"?"help":t==="--version"||t==="-v"?"version":t;return new Set(["init","update","restart","stop","install","uninstall","status","keepalive","clear-loop","sessions","session","platform","help","version"]).has(r)?(await l.handleCliCommand(r),!0):!1}static async handleCliCommand(e,t){let r=Bt.resolve(t??l.resolveUserDataDir(),".."),s=Bt.join(r,"data"),n="https://gitee.com/moscowzk/kexvim";switch(e){case"init":await Sl(r,n);return;case"update":await wl(r,n);return;case"install":Al();return;case"uninstall":Ml();return;case"status":Il();return;case"keepalive":Ll();return;case"clear-loop":Pl();return;case"restart":await Tl(r),process.exit(0);return;case"stop":await Rl(r),process.exit(0);return;case"sessions":Nl(s);return;case"platform":await jl(s);return;case"session":{let i=process.argv[3];i||(console.error("\u7528\u6CD5: kexvim session <\u524D8\u4F4DID>\uFF08\u7528 kexvim sessions \u67E5\u770B\u5217\u8868\uFF09"),process.exit(1)),$l(s,i);return}case"help":ro();return;case"version":Dl();return;default:ro();return}}};import*as _e from"node:fs";import*as io from"node:path";import*as Hl from"node:worker_threads";var oo=()=>{let l=I.findProjectRoot();if(!l)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D kexvim.log\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let e=io.join(l,"data","log");try{_e.mkdirSync(e,{recursive:!0})}catch{}return io.join(e,"kexvim.log")},xp=100*1024*1024;try{let l=oo();_e.existsSync(l)&&_e.statSync(l).size>xp&&(_e.rmSync(`${l}.1`,{force:!0}),_e.renameSync(l,`${l}.1`))}catch{}process.on("uncaughtException",l=>{try{_e.appendFileSync(oo(),`[kexvim] uncaughtException at ${new Date().toISOString()}:
${l.stack??l.message}
`)}catch{}});process.on("unhandledRejection",l=>{try{let e=l instanceof Error?l.stack??l.message:String(l);_e.appendFileSync(oo(),`[kexvim] unhandledRejection at ${new Date().toISOString()}:
${e}
`)}catch{}try{console.error("[kexvim] unhandledRejection:",l)}catch{}});async function Ul(){try{await Xn.main()}catch(l){console.error(l instanceof Error?l.message:String(l)),process.exit(1)}}Hl.isMainThread?Qn.dispatchFromArgv().then(l=>{l?process.exit(0):Ul()}):Ul();
