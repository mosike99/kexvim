...
Background:
${e.context}`:""}`,{statusCallback:n,signal:i?.signal}),p=null,m=new Promise((k,y)=>{p=setTimeout(()=>{y(new Error(`subagent timeout after ${c/1e3}s`))},c)}),g=await Promise.race([u,m]);p&&clearTimeout(p);let f=Number(((Date.now()-o)/1e3).toFixed(1)),h=!!(g.usage&&(g.usage.promptTokens>0||g.usage.completionTokens>0));return{task_index:t,status:"completed",summary:g.content||"(no response)",api_calls:h?1:0,duration_seconds:f}}catch(d){let u=d instanceof Error?d.message:String(d),p=u.startsWith("subagent timeout");return{task_index:t,status:p?"timeout":"error",summary:"",error:u,api_calls:0,duration_seconds:Number(((Date.now()-o)/1e3).toFixed(1))}}}};import{DatabaseSync as Jl}from"node:sqlite";import*as mo from"node:fs";import*as go from"node:path";var Me=class l{static instance=null;db;_dbPath;static init(e){return l.instance||(l.instance=new l(e)),l.instance}static get instanceOrNull(){return l.instance}constructor(e){this._dbPath=e,mo.mkdirSync(go.dirname(e),{recursive:!0}),this.db=new Jl(e),this.db.exec("PRAGMA busy_timeout = 15000"),this._initializeSchema()}_initializeSchema(){this.db.exec(`
...
`+Pc;g||m.push({role:f,content:y,[Uo]:!0});for(let _=d;_<a.length;_++){let M={...a[_]};g&&_===d&&(M.content=l._appendText(M.content,`
...
`,Hr=class{db;enableMemoryLogging=!1;constructor(e){this.db=new su(e),this.db.exec("PRAGMA busy_timeout = 15000"),this.db.exec(nu)}async insert(e){let t=this.db.prepare(`
...
\u{1F44B} \u518D\u89C1 / Bye`)}}};import*as jt from"node:path";import*as j from"node:fs";import*as ee from"node:path";import*as Bt from"node:os";var pp=`# Kexvim \u914D\u7F6E
llm:
...
`;static platform=process.platform;static _findRoot(){let e=te.dirname(yp(import.meta.url));for(let t=0;t<8;t++){if(z.existsSync(te.join(e,"package.json")))return e;let r=te.dirname(e);if(r===e)break;e=r}throw new Error("[kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF08\u65E0 package.json\uFF09\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002")}static _systemPath(e){if(l.platform==="win32")return"";let t=rs.homedir();return l.platform==="darwin"?te.join(t,"Library","LaunchAgents","com.kexvim.daemon.plist"):te.join(t,".config","systemd","user","kexvim.service")}static _dataPath(e){let t=te.join(e,"data",l.AUTOSTART_DIR);return z.mkdirSync(t,{recursive:!0}),l.platform==="win32"?te.join(t,"keepalive.cmd"):l.platform==="darwin"?te.join(t,"com.kexvim.daemon.plist"):te.join(t,"kexvim.service")}static _devEntry(e){let t=te.join(e,"dist","dev.mjs");if(z.existsSync(t))return t;let r=te.join(e,"kexvim.js");if(z.existsSync(r))return r;throw new Error("[kexvim] \u672A\u627E\u5230 daemon \u5165\u53E3\uFF08dist/dev.mjs \u6216 kexvim.js\uFF09\u3002")}static _template(e){let t=process.execPath,r=l._devEntry(e);return l.platform==="win32"?`@echo off\r
set NODE_NO_WARNINGS=1\r
cd /d "${e}"\r
${t} "${r}" keepalive\r
...
\u7528\u6CD5: kexvim session <\u524D8\u4F4DID>  \u5207\u6362/\u6062\u590D\u8BE5\u4F1A\u8BDD`)}finally{t.close()}}import*as Ol from"node:fs";import*as so from"node:path";import{DatabaseSync as Sp}from"node:sqlite";var _p="session-switch.json";function $l(l,e){let t=so.join(l,"kexvim.db"),r=new Sp(t,{readOnly:!0}),s;try{s=r.prepare(`
      SELECT id, COALESCE(summary, '') AS summary,
             (SELECT COUNT(*) FROM messages m WHERE m.session_id = s.id) AS msgCount
      FROM sessions s
      WHERE s.is_test = 0 AND s.id LIKE ?
    `).get(`${e}%`)}finally{r.close()}s||(console.error(`[kexvim] \u274C \u627E\u4E0D\u5230\u4F1A\u8BDD "${e}"\u3002\u7528 kexvim sessions \u67E5\u770B\u5168\u90E8\u4F1A\u8BDD\u3002`),process.exit(1)),Ol.writeFileSync(so.join(l,_p),JSON.stringify({sessionId:s.id,requestedAt:Date.now()/1e3}),"utf-8");let n=(s.summary||"(\u65E0\u6807\u9898)").slice(0,30);console.log(`\u2705 \u5DF2\u6807\u8BB0\u5207\u6362\u5230\u4F1A\u8BDD ${s.id.slice(0,8)}\uFF08${s.msgCount} \u6761\u6D88\u606F\uFF0C${n}\uFF09`),console.log("   \u53D1\u9001\u4E0B\u4E00\u6761\u6D88\u606F\u540E daemon \u5C06\u6062\u590D\u8BE5\u4F1A\u8BDD\u7EE7\u7EED\u5BF9\u8BDD\u3002")}function no(){console.log(`\u7528\u6CD5: kexvim <command>
...
${e}
`)}catch{}try{console.error("[kexvim] unhandledRejection:",l)}catch{}});async function Ul(){try{await Zn.main()}catch(l){console.error(l instanceof Error?l.message:String(l)),process.exit(1)}}Hl.isMainThread?ei.dispatchFromArgv().then(l=>{l?process.exit(0):Ul()}):Ul();
