#!/usr/bin/env node
var Bt=class{get executionGuidance(){return"none"}get staleTimeoutFloorSeconds(){}get reasoningMaxOutputTokens(){}get modelName(){}};var Te=class{static FLOORS=[{slug:"deepseek-r1",floor:600},{slug:"deepseek-v4-flash",floor:600},{slug:"o1",floor:600},{slug:"o1-mini",floor:600},{slug:"o1-pro",floor:600},{slug:"o1-preview",floor:600},{slug:"o3",floor:600},{slug:"o3-pro",floor:600},{slug:"o3-mini",floor:300},{slug:"o4-mini",floor:300}].sort((e,t)=>t.slug.length-e.slug.length);static getStaleTimeoutFloor(e){if(typeof e!="string"||!e.trim())return;let t=e.trim().toLowerCase();if(t.includes("/")&&(t=t.split("/").pop()??t),!!t)return this.matchAny(t)}static isReasoningModel(e){return this.getStaleTimeoutFloor(e)!==void 0}static matchAny(e){for(let t of this.FLOORS)if(this.matches(e,t.slug))return t.floor}static matches(e,t){if(!e.startsWith(t))return!1;let r=e.slice(t.length);return r===""||r.startsWith("-")||r.startsWith(".")||r.startsWith("_")}};var Ee=class extends Error{constructor(t,r,s,n=!1){super(t);this.statusCode=r;this.provider=s;this.retryable=n;this.name="ProviderError"}statusCode;provider;retryable},jt=class extends Ee{constructor(t,r,s){super(t,429,s,!0);this.retryAfter=r;this.name="RateLimitError"}retryAfter};var Nl=/^[\s]*<think>([\s\S]*?)<\/think>/m,Ol=/^[\s]*<reason>([\s\S]*?)<\/reason>/m,$l=[/\b(sk-[a-zA-Z0-9]{20,})\b/g,/\b(Bearer\s+)([a-zA-Z0-9_\-\.]{20,})\b/gi,/(Authorization:\s*)(?:Bearer\s+)?[a-zA-Z0-9_\-\.]{20,}/gi,/(x-api-key:\s*)[a-zA-Z0-9_\-\.]{20,}/gi,/\b(ghp_[a-zA-Z0-9]{36,})\b/g,/\b(eyJ[a-zA-Z0-9_\-]{10,}\.[a-zA-Z0-9_\-]{10,}\.[a-zA-Z0-9_\-]{10,})\b/g],es=class{static stripThinkBlocks(e){return e?e.replace(/<think>[\s\S]*?<\/think>/g,"").replace(/<reason>[\s\S]*?<\/reason>/g,"").trim():""}static extractThinkContent(e){if(!e)return null;let t=e.match(Nl);if(t)return t[1].trim();let r=e.match(Ol);return r?r[1].trim():null}static stripThinkTagsOnly(e){return e?e.replace(/<\/?(?:think|reason)>/g,"").trim():""}static stripThinkTagsFromReasoning(e){return e&&e.replace(/<think>[\s\S]*?<\/think>/g,"").trim()||null}static renderContent(e,t,r={}){let s={stripMode:"no_strip",contentRender:"content_only",stripThinkTagsFromReasoning:!0,...r},n=e??"",i=t??null,o=n;s.stripMode==="strip_think_content"?o=this.stripThinkBlocks(n):s.stripMode==="strip_think_tags"&&(o=this.stripThinkTagsOnly(n));let a=null;s.contentRender!=="content_only"&&(a=this.extractThinkContent(n));let c="";if(s.contentRender==="content_only")c=o;else if(s.contentRender==="content_and_think"){let u=[];a&&u.push(a),o&&u.push(o),c=u.join(`
...
\u5982\u679C\u6CA1\u6709\u76F8\u5173\u5185\u5BB9\uFF0C\u5BF9\u5E94\u90E8\u5206\u5199\u300C\u65E0\u300D\u3002`;_filepath;_llm;_summaries={};_loaded=!1;constructor(e,t){this._filepath=ds.join(e,"session_summaries.json"),this._llm=t}async consolidate(e,t){if(e.length===0||!t)return console.log("consolidate: skipped \u2014 empty messages or session_id"),null;this._loaded||this._load();let r=await this._extract(e,t);return r===null?null:(this._summaries[t]=r,this._save(),console.log(`Session consolidated: ${t.slice(0,24)} \u2014 ${r.title??"?"}`),r)}get(e){return this._loaded||this._load(),this._summaries[e]??null}search(e){this._loaded||this._load();let t=e.toLowerCase(),r=[];for(let s of Object.values(this._summaries))JSON.stringify(s).toLowerCase().includes(t)&&r.push(s);return r}allSummaries(){return this._loaded||this._load(),Object.values(this._summaries).sort((e,t)=>(t.timestamp??"").localeCompare(e.timestamp??""))}_load(){if(this._loaded=!0,!!tt.existsSync(this._filepath))try{let e=tt.readFileSync(this._filepath,"utf-8");this._summaries=JSON.parse(e)}catch(e){console.warn(`session_summaries load failed: ${e.message}`)}}_save(){try{tt.mkdirSync(ds.dirname(this._filepath),{recursive:!0}),tt.writeFileSync(this._filepath,JSON.stringify(this._summaries,null,2),"utf-8")}catch(e){console.warn(`session_summaries save failed: ${e.message}`)}}async _extract(e,t){let r=l.sampleMessages(e);if(r.length===0)return null;let s=l.formatMessagesForLlm(r),i=[{role:"system",content:l._CONSOLIDATE_PROMPT.replace("{max_title}",String(l.MAX_TITLE_CHARS)).replace("{max_summary}",String(l.MAX_SUMMARY_CHARS))},{role:"user",content:`\u5BF9\u8BDD\u8BB0\u5F55\uFF1A
...
\u8F93\u5165 goal_text \u4E3A\u81EA\u7136\u8BED\u8A00\u76EE\u6807\u63CF\u8FF0\uFF0C\u7CFB\u7EDF\u4F1A\u81EA\u52A8\u89C4\u5212\u3001\u62C6\u89E3\u3001\u5E76\u884C\u6267\u884C\u3002
\u793A\u4F8B\uFF1A\u8C03\u7528 delegate_task(goal_text='\u7ED9\u652F\u4ED8\u6A21\u5757\u52A0\u9000\u6B3E\u529F\u80FD\uFF0C\u652F\u6301\u5FAE\u4FE1\u548C\u652F\u4ED8\u5B9D')
\u7B80\u5355\u67E5\u8BE2\uFF08\u95EE\u95EE\u9898\u3001\u67E5\u72B6\u6001\u3001\u5355\u6B65\u64CD\u4F5C\uFF09\u4E0D\u9700\u8981\u7528\u6B64\u5DE5\u5177\uFF0C\u76F4\u63A5\u56DE\u590D\u5373\u53EF\u3002`;static PLATFORM_HINTS={}},Co=["AGENTS.md",".cursorrules",".cursor/rules/wildcard.mdc",".cursor/rules/*.mdc","SOUL.md",".zk.md","ZK.md",".github/copilot-instructions.md",".windsurfrules"],ft=class{static stripFrontmatter(e){if(e.startsWith("---")){let t=e.indexOf(`
---`,3);if(t!==-1)return e.slice(t+4).replace(/^\n+/,"")||e}return e}static buildTimestampLine(e,t,r,s){let i=[`Conversation started: ${e.toISOString()}`];return r&&i.push(`Model: ${r}`),s&&i.push(`Provider: ${s}`),t&&i.push(`Session: ${t}`),i.join(" | ")}},st=class l{identity;constructor(e){this.identity=e?.identity||oe.DEFAULT_AGENT_IDENTITY}static loadSoulIdentity(){try{let e=I.findProjectRoot();if(!e)return null;let t=Jt.join(e,"data","SOUL.md");if(!Ue.existsSync(t))return null;let r=Ue.readFileSync(t,"utf-8").trim();if(!r)return null;let s=rt(r,"context");return s.length>0?(console.warn(`[PromptBuilder] SOUL.md ignored: contains threat pattern(s) [${s.join(", ")}]. Falling back to default identity.`),null):r}catch(e){return console.warn(`[PromptBuilder] Failed to load SOUL.md: ${e.message}`),null}}build(e={}){let t=[],r=e.identity===void 0?l.loadSoulIdentity():null;if(t.push(r||this.identity),t.push(oe.HERMES_AGENT_HELP_GUIDANCE),e.toolUseEnforcement!==!1&&this._shouldInjectEnforcement(e.toolUseEnforcement)&&(t.push(oe.TOOL_USE_ENFORCEMENT_GUIDANCE),e.executionGuidance==="openai"?t.push(oe.OPENAI_MODEL_EXECUTION_GUIDANCE):e.executionGuidance==="google"&&t.push(oe.GOOGLE_MODEL_OPERATIONAL_GUIDANCE)),e.taskCompletionGuidance!==!1&&t.push(oe.TASK_COMPLETION_GUIDANCE),e.parallelToolCallGuidance!==!1&&t.push(oe.PARALLEL_TOOL_CALL_GUIDANCE),e.approvalGuidance!==!1&&t.push(oe.APPROVAL_GATE_GUIDANCE),e.memoryGuidance&&e.validToolNames?.includes("memory")&&t.push(oe.MEMORY_GUIDANCE),e.skillsGuidance&&e.validToolNames?.includes("skill_manage")&&t.push(oe.SKILLS_GUIDANCE),e.sessionSearchGuidance&&e.validToolNames?.includes("session_search")&&t.push(oe.SESSION_SEARCH_GUIDANCE),e.steerGuidance&&t.push(oe.STEER_CHANNEL_NOTE),e.skillsPrompt&&t.push(e.skillsPrompt),e.tools&&e.tools.length>0){let o=this.buildToolsSection(e.tools);o&&t.push(o)}let s=e.envHints||l.detectEnvironment();if(s){let o=this.buildEnvironmentHints(s);o&&t.push(o)}if(e.codingGuidance){let o=l.buildCodingWorkspaceBlock();o&&t.push(o)}let n=e.activeProfile||"default";if(n==="default"?t.push("Active kexvim profile: default. Other profiles (if any) live under ~/.zk/profiles/<name>/. Each profile has its own skills/, plugins/, cron/, and memories/ that affect a different session than this one. Do not modify another profile's skills/plugins/cron/memories unless the user explicitly directs you to."):t.push(`Active kexvim profile: ${n}. This session reads and writes ~/.zk/profiles/${n}/. The default profile's data lives at ~/.zk/skills/, ~/.zk/plugins/, ~/.zk/cron/, ~/.zk/memories/ \u2014 those belong to a different session run from a different shell. Do NOT modify another profile's skills/plugins/cron/memories unless the user explicitly directs you to. The cross-profile write guard will refuse such writes by default; pass cross_profile=true only after explicit direction.`),e.customPlatformHint&&t.push(e.customPlatformHint),e.systemMessage&&t.push(e.systemMessage),e.contextFiles&&e.contextFiles.length>0){let o=e.contextFiles.filter(c=>{let d=rt(c.content,"context");return d.length>0?(console.warn(`[PromptBuilder] Context file '${c.name}' skipped: contains threat pattern(s) [${d.join(", ")}].`),!1):!0}),a=this.buildContextFilesSection(o);a&&t.push(a)}e.memorySnapshot&&t.push(e.memorySnapshot),e.userProfile&&t.push(e.userProfile),e.timestamp&&t.push(e.timestamp);let i=e.plannerMode??0;return i===4||i===5?t.push(oe.TASK_DECOMPOSITION_GUIDANCE_FORCE):i>=2&&t.push(oe.TASK_DECOMPOSITION_GUIDANCE_SUGGEST),t.join(`
...
`)}static detectEnvironment(){try{let e={},t=(He.type()||"").toLowerCase();e.os=`${He.type()} (${He.release()})`;let r=He.platform();r==="win32"?e.platform="win32":r==="darwin"?e.platform="macos":e.platform=r;try{e.cwd=process.cwd()}catch{}try{e.home=He.homedir()}catch{}try{let s=Ue.readFileSync("/proc/version","utf-8");e.isWsl=/microsoft|wsl/i.test(s)}catch{e.isWsl=!1}return e}catch{return}}static buildCodingWorkspaceBlock(e){let t=e||(typeof process<"u"?process.cwd():"");if(!t)return"";let r=null,s=t;for(let n=0;n<10;n++){try{if(Ue.existsSync(Jt.join(s,".git"))){r=s;break}}catch{}let i=Jt.dirname(s);if(i===s)break;s=i}if(!r)return"";try{let n=["## Coding Workspace"];n.push(`- Root: ${r}`);let i=b.runSync("git branch --show-current",{cwd:r,timeoutMs:5e3}).trim();if(i){n.push(`- Branch: ${i}`);try{let c=b.runSync("git rev-parse --abbrev-ref --symbolic-full-name @{upstream}",{cwd:r,timeoutMs:3e3}).trim();if(c){let d=b.runSync(`git rev-list --left-right --count ${i}...${c}`,{cwd:r,timeoutMs:3e3}).trim(),[u,p]=d.split("	");(u!=="0"||p!=="0")&&n.push(`- Sync: ahead ${u}, behind ${p}`)}}catch{}}let o=b.runSync("git status --porcelain",{cwd:r,timeoutMs:5e3}).trim();if(o){let c=o.split(`
...
`)}catch{return""}}static discoverContextFiles(e,t){let r=[],s=t||Ue;for(let n of Co){if(n.includes("*"))continue;let i=`${e}/${n}`;try{s.existsSync(i)&&r.push({name:n,fullPath:i})}catch{}}return r}static loadProjectContext(e,t=2e4){for(let r of Co){if(r==="SOUL.md"||r.includes("*"))continue;let s=Jt.join(e,r);try{if(!Ue.existsSync(s))continue;let n=Ue.readFileSync(s,"utf-8").trim();if(!n)continue;return n=ft.stripFrontmatter(n).trim(),n.length>t&&(n=`${n.slice(0,t)}
...
Only proceed without loading a skill if genuinely none are relevant to the task.`}_shouldInjectEnforcement(e){return e!==!1}};var mc=`Review the conversation above and update two things:
...
`);return j.writeFileSync(r,`${o}

${e.content}`,"utf-8"),this.bumpUsage(e.name,"create"),!0}get(e){return this.list().find(t=>t.name===e)??null}update(e,t){let r=this.get(e);if(!r)return;let s={...r,...t,name:r.name,tags:t.tags??r.tags,relatedSkills:t.relatedSkills??r.relatedSkills,version:r.version,author:r.author},n=s.category?D.join(this.skillsDir,s.category,s.name):D.join(this.skillsDir,s.name);j.mkdirSync(n,{recursive:!0});let i=(s.tags??[]).length>0?`    tags: [${s.tags.map(d=>`"${d}"`).join(", ")}]
...
`);j.writeFileSync(D.join(n,"SKILL.md"),`${c}

${s.content}`,"utf-8"),this.bumpUsage(e,"patch")}loadUsage(){try{return JSON.parse(j.readFileSync(this.usagePath,"utf-8"))}catch{return{}}}saveUsage(e){j.writeFileSync(this.usagePath,JSON.stringify(e,null,2),"utf-8")}bumpUsage(e,t){let r=this.loadUsage(),s=r[e],n=new Date().toISOString();s?r[e]={...s,useCount:t==="create"?s.useCount+1:s.useCount,...t==="patch"||t==="edit"?{lastPatchedAt:n}:{},...t==="view"?{lastViewedAt:n}:{},lastUsedAt:n}:r[e]={useCount:t==="create"?1:0,viewCount:t==="view"?1:0,patchCount:t==="patch"||t==="edit"?1:0,lastUsedAt:n,pinned:!1,state:"active",createdAt:n},this.saveUsage(r)}list(){let e=[],t=new Set;if(this.sharedSkillsDir&&this.walkDir(this.sharedSkillsDir,e,t),this.marketSkillsDir){let r=this.loadDisabledMarketSkills();this.walkDir(this.marketSkillsDir,e,t,r)}return this.walkDir(this.skillsDir,e,t),e}walkDir(e,t,r,s){try{let n=j.readdirSync(e,{withFileTypes:!0});for(let i of n)i.isDirectory()&&this.walkDir(D.join(e,i.name),t,r,s);if(j.existsSync(D.join(e,"SKILL.md"))){let i=this.parseSkill(e);if(i){if(s?.has(i.name)||r?.has(i.name))return;r?.add(i.name),t.push(i)}}}catch{}}listCatalog(){let e=[];if(this.sharedSkillsDir&&this.walkCatalog(this.sharedSkillsDir,this.sharedSkillsDir,"public",e),this.marketSkillsDir){let t=this.loadDisabledMarketSkills();this.walkCatalog(this.marketSkillsDir,this.marketSkillsDir,"market",e,t)}return e}walkCatalog(e,t,r,s,n){let i;try{i=j.readdirSync(e,{withFileTypes:!0})}catch{return}for(let o of i){if(!o.isDirectory()||o.name===".git")continue;let a=D.join(e,o.name);if(j.existsSync(D.join(a,"SKILL.md"))){let c=this.parseCatalog(a,t);c&&s.push({name:c.name,title:c.title,summary:c.summary,description:c.description,category:c.category,source:r,disabled:r==="market"?n?.has(c.name)??!1:!1})}else this.walkCatalog(a,t,r,s,n)}}parseCatalog(e,t){try{let s=j.readFileSync(D.join(e,"SKILL.md"),"utf-8").match(di);if(!s)return null;let n=this.extract(s[1],/^name:\s*(.+)$/m);if(!n)return null;let i=this.extract(s[1],/^title:\s*(.+)$/m)?.replace(/^["']|["']$/g,"")||void 0,o=this.extract(s[1],/^summary:\s*(.+)$/m)?.replace(/^["']|["']$/g,"")||void 0,a=this.extract(s[1],/^category:\s*(.+)$/m)?.replace(/^["']|["']$/g,""),c=D.relative(t,e),d=c.includes(D.sep)?D.dirname(c):void 0;return{name:n,title:i,summary:o,description:this.extractDescription(s[1]).replace(/^["']|["']$/g,"").slice(0,90),category:a||d}}catch{return null}}parseFrontmatter(e){let t=e.match(di);return t?{name:this.extract(t[1],/^name:\s*(.+)$/m)??void 0,title:this.extract(t[1],/^title:\s*(.+)$/m)?.replace(/^["']|["']$/g,"")||void 0,summary:this.extract(t[1],/^summary:\s*(.+)$/m)?.replace(/^["']|["']$/g,"")||void 0,description:this.extractDescription(t[1]).replace(/^["']|["']$/g,"").slice(0,90),category:this.extract(t[1],/^category:\s*(.+)$/m)?.replace(/^["']|["']$/g,"")}:null}installMarket(e,t){return!this.marketSkillsDir||!j.existsSync(D.join(e,"SKILL.md"))?!1:(j.cpSync(e,D.join(this.marketSkillsDir,t),{recursive:!0,force:!0}),!0)}uninstallMarket(e){if(!this.marketSkillsDir)return!1;let t=D.join(this.marketSkillsDir,e);if(!j.existsSync(D.join(t,"SKILL.md")))return!1;try{j.rmSync(t,{recursive:!0,force:!0})}catch{return!1}let r=this.loadDisabledMarketSkills();return r.delete(e),this.saveDisabledMarketSkills(r),!0}toggleMarket(e,t){if(!this.marketSkillsDir)return;let r=this.loadDisabledMarketSkills();t?r.delete(e):r.add(e),this.saveDisabledMarketSkills(r)}delete(e){let t=this.get(e);if(!t)return!1;let r=t.category?D.join(this.skillsDir,t.category,t.name):D.join(this.skillsDir,t.name);j.rmSync(r,{recursive:!0,force:!0});let s=this.loadUsage();return delete s[e],this.saveUsage(s),!0}getUsage(e){return this.loadUsage()[e]||null}setState(e,t){let r=this.loadUsage();r[e]&&(r[e].state=t,this.saveUsage(r))}getByState(e){return this.list().filter(t=>t.state===e)}transitionState(e,t){return this.get(e)?(this.update(e,{state:t}),!0):!1}};import*as Ss from"node:fs";var ks=class{llm;manager;intervalMs;minIdleMs;staleAfterMs;archiveAfterMs;consolidate;onSummary;state;constructor(e){this.llm=e.llm,this.manager=e.manager,this.intervalMs=(e.intervalHours??168)*60*60*1e3,this.minIdleMs=(e.minIdleHours??1)*60*60*1e3,this.staleAfterMs=(e.staleAfterDays??30)*24*60*60*1e3,this.archiveAfterMs=(e.archiveAfterDays??90)*24*60*60*1e3,this.consolidate=e.consolidate??!1,this.onSummary=e.onSummary,this.state=this.loadState()}stateFile(){return`${this.manager.skillsDirPath}/.curatorstate`}loadState(){try{let e=Ss.readFileSync(this.stateFile(),"utf-8");return JSON.parse(e)}catch{return{lastRunAt:null,lastRunSummary:null,paused:!1,runCount:0}}}saveState(){try{Ss.writeFileSync(this.stateFile(),JSON.stringify(this.state,null,2),"utf-8")}catch{}}pause(){this.state.paused=!0,this.saveState()}resume(){this.state.paused=!1,this.saveState()}get paused(){return this.state.paused}shouldRun(e=Date.now()){let t=this;if(t.state.paused)return!1;if(t.state.lastRunAt===null)return t.state.lastRunAt=new Date(e).toISOString(),t.state.lastRunSummary="deferred first run \u2014 curator seeded, will run after one interval",t.saveState(),!1;let r=new Date(t.state.lastRunAt).getTime();return e-r>=t.intervalMs}applyAutoTransitions(e=Date.now()){let t=this,r={markedStale:0,archived:0,reactivated:0,checked:0};for(let s of t.manager.list()){r.checked++;let n=t.manager.getUsage(s.name);if(n?.pinned)continue;let i=n?.lastUsedAt?new Date(n.lastUsedAt).getTime():s.createdAt||e,o=e-i;o>=t.archiveAfterMs&&n?.state!==ae.STATE_ARCHIVED?(t.manager.setState(s.name,ae.STATE_ARCHIVED),r.archived++):o>=t.staleAfterMs&&n?.state===ae.STATE_ACTIVE?(t.manager.setState(s.name,ae.STATE_STALE),r.markedStale++):o<t.staleAfterMs&&n?.state===ae.STATE_STALE&&(t.manager.setState(s.name,ae.STATE_ACTIVE),r.reactivated++)}return r}async run(e=Date.now()){let t=this,r=Date.now(),s=t.applyAutoTransitions(e),n;if(!t.consolidate)return n=`Curator run #${++this.state.runCount} (${((Date.now()-r)/1e3).toFixed(1)}s): ${s.markedStale} stale, ${s.archived} archived, ${s.reactivated} reactivated`,t.state.lastRunAt=new Date(e).toISOString(),t.state.lastRunSummary=n,t.saveState(),t.onSummary&&t.onSummary(n),n;let i=t.manager.list().filter(g=>{let f=t.manager.getUsage(g.name);return!f?.pinned&&f?.state!==ae.STATE_ARCHIVED});if(i.length===0){let g="No active skills to review.";return t.state.lastRunAt=new Date(e).toISOString(),t.state.lastRunSummary=g,t.state.runCount++,t.saveState(),g}let c={systemPrompt:"You are a skill curator. Analyze skills and recommend consolidations. Output ONLY valid JSON.",messages:[{role:"user",content:`You are kexvim's skill curator. Review the following skills and recommend consolidations.
...
${g.reason||""}`;t.manager.update(f,{content:h.content+k}),p.push(`Updated umbrella skill '${f}'`)}if(g.absorb&&Array.isArray(g.absorb))for(let k of g.absorb)t.manager.setState(k,ae.STATE_ARCHIVED),p.push(`  Archived absorbed skill '${k}'`)}let m=((Date.now()-r)/1e3).toFixed(1);return n=`Curator run #${++this.state.runCount} (${m}s):
${p.join(`
`)}`,t.state.lastRunAt=new Date(e).toISOString(),t.state.lastRunSummary=n,t.saveState(),t.onSummary&&t.onSummary(n),n}};var _s=class{tools=new Map;all(){return[...this.tools.values()]}names(){return[...this.tools.keys()]}get(e){return this.tools.get(e)}has(e){return this.tools.has(e)}add(e){this.tools.set(e.name,e)}remove(e){this.tools.delete(e)}};import*as Bo from"node:fs";var ws=class{name="read_file";description="Read the contents of a file. Returns line-numbered output. Use for code, config, markdown, and log files.";parameters={type:"object",properties:{path:{type:"string",description:"Absolute or relative file path to read"},offset:{type:"number",description:"Line number to start from (1-indexed, optional, default 1)"},limit:{type:"number",description:"Max lines to return (optional, default 500)"}},required:["path"]};async execute(e,t,r){let s=String(e.path||"").trim();if(!s)return"Error: path is required";try{let n=Number(e.offset)||1,i=Number(e.limit)||500,a=Bo.readFileSync(s,"utf-8").split(`
...
... (truncated at 50K chars)`:m}catch(n){let i=n instanceof Error?n.message:String(n);return`Error reading file '${s}': ${i}`}}};import*as Ts from"node:fs";import*as jo from"node:path";var xs=class{name="write_file";description="Write content to a file, creating parent directories if needed. OVERWRITES the entire file.";parameters={type:"object",properties:{path:{type:"string",description:"Absolute or relative file path to write"},content:{type:"string",description:"Complete content to write to the file"}},required:["path","content"]};async execute(e){let t=String(e.path||"").trim(),r=String(e.content||"");if(!t)return"Error: path is required";try{let s=jo.dirname(t);return Ts.mkdirSync(s,{recursive:!0}),Ts.writeFileSync(t,r,"utf-8"),`Written ${Buffer.byteLength(r,"utf-8")} bytes to ${t}`}catch(s){let n=s instanceof Error?s.message:String(s);return`Error writing file '${t}': ${n}`}}};import*as Cs from"node:fs";var Es=class{name="patch";description="Targeted find-and-replace in a file. Replaces a unique text segment with new content. Use replace_all=true to replace every occurrence.";parameters={type:"object",properties:{path:{type:"string",description:"File path to edit (absolute or relative)"},old_string:{type:"string",description:"Text to find \u2014 must be unique in the file unless replace_all=true"},new_string:{type:"string",description:"Replacement text (can be empty string to delete the matched text)"},replace_all:{type:"boolean",description:"Replace all occurrences instead of requiring a unique match",default:!1}},required:["path","old_string","new_string"]};async execute(e,t,r){let s=String(e.path||"").trim(),n=String(e.old_string??""),i=String(e.new_string??""),o=!!e.replace_all;if(!s)return"Error: path is required";let a;try{a=Cs.readFileSync(s,"utf-8")}catch(p){let m=p instanceof Error?p.message:String(p);return`Error reading file '${s}': ${m}`}let c=(a.match(new RegExp(Ic(n),"g"))||[]).length;if(c===0)return`Error: string not found in '${s}'. Make sure the old_string matches exactly (including whitespace).`;if(!o&&c>1)return`Error: Found ${c} occurrences. Use replace_all=true to replace all, or provide more context to make old_string unique.`;let d=o?a.replaceAll(n,i):a.replace(n,i);try{Cs.writeFileSync(s,d,"utf-8")}catch(p){let m=p instanceof Error?p.message:String(p);return`Error writing file '${s}': ${m}`}let u=d.split(`
`);return`Replaced ${c} occurrence(s) in ${s} (${u.length} lines).`}};function Ic(l){return l.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}import*as bt from"node:fs";import*as Uo from"node:path";var Rs=class{name="search_files";description="Search file contents or find files by name. Uses pattern matching (not full regex).";parameters={type:"object",properties:{pattern:{type:"string",description:"Search pattern (text or glob like *.py)"},path:{type:"string",description:"File or directory to search in (default: current working directory)"},file_glob:{type:"string",description:"Filter by file pattern (e.g. *.ts, *.py)"},limit:{type:"number",description:"Max results (default 20)"}},required:["pattern"]};searchFile(e,t,r,s,n){if(r){let i=e.split("/").pop()||e.split("\\").pop()||"",o=r.replace("*","");if(!i.endsWith(o))return""}try{let i=bt.readFileSync(e,"utf-8");if(!i.includes(t))return"";let o=i.split(`
...
`):""}async execute(e,t,r){let s=String(e.pattern||"").trim(),n=String(e.path||".").trim(),i=String(e.file_glob||"").trim(),o=Number(e.limit)||20;if(!s)return"Error: pattern is required";try{let a=[],c=[n],d=new Set;try{if(bt.statSync(n).isFile())return this.searchFile(n,s,i,a,o)}catch{}for(;c.length>0&&a.length<o;){let u=c.shift();if(d.has(u))continue;d.add(u);let p;try{p=bt.readdirSync(u,{withFileTypes:!0})}catch{continue}for(let m of p){if(m.name.startsWith(".")&&m.name!=="."||m.name==="node_modules")continue;let g=Uo.join(u,m.name);if(m.isDirectory()){c.push(g);continue}if(!(i&&!m.name.endsWith(i.replace("*",""))))try{let f=bt.readFileSync(g,"utf-8");if(f.includes(s)){let h=f.split(`
...
`)}_create(e,t){let r=String(e.name||"").trim(),s=String(e.schedule||"").trim(),n=String(e.command||"").trim(),i=String(e.prompt||"").trim(),o=String(e.blueprint||"").trim();if(o){let k=Sr.get(o);if(!k)return`Error: unknown blueprint '${o}'. Use action=blueprints to list templates.`;s||(s=k.schedule),!n&&!i&&(k.mode==="agent"&&k.prompt?i=k.prompt:n=k.command??""),r||(r=k.name)}if(!r||!s)return"Error: name and schedule are required";if(!n&&!i)return"Error: either command (script mode) or prompt (agent mode) is required";let a;try{a=le.parseSchedule(s)}catch(k){return`Error: ${k?.message||k}`}let c=`cron_${Date.now()}`,d=new Date().toISOString(),u=le.computeNextRun(a);if(a.kind==="once"&&!u)return`Error: once \u4EFB\u52A1\u7684\u6267\u884C\u65F6\u95F4\u5DF2\u8FC7\uFF08${a.display}\uFF09\uFF0C\u8BF7\u4F7F\u7528\u672A\u6765\u65F6\u95F4`;let p=t?.platform&&t?.chatId?`${t.platform}:${t.chatId}`:void 0,m=String(e.deliver||"origin").trim(),g={id:c,name:r,schedule:s,command:n,...i?{prompt:i}:{},deliver:m,originChat:p,status:"active",createdAt:d,lastRunAt:null,nextRunAt:u,lastStatus:null,lastError:null};l._store.create(g),ne.instance.startJob(g);let f=i?`agent:${i.slice(0,40)}`:n,h=m==="origin"&&!p;return`Created cron job '${r}' (${c}): ${a.display} \u2192 ${f} | deliver=${m}${p?` | origin=${p}`:""} | next run ${g.nextRunAt??"-"}`+(h?" | \u26A0\uFE0F \u5F53\u524D\u4F1A\u8BDD\u65E0\u6295\u9012\u901A\u9053\uFF0C\u6267\u884C\u7ED3\u679C\u4EC5\u8BB0\u5F55\u4E8E\u6267\u884C\u5386\u53F2\uFF08cronjob history \u53EF\u67E5\uFF09":"")}_remove(e){let t=String(e.job_id||"").trim();return t?l._store.get(t)?(ne.instance.stopJob(t),l._store.remove(t),`Removed job '${t}'`):`Error: job '${t}' not found`:"Error: job_id is required"}_lifecycle(e,t){let r=String(t.job_id||"").trim();if(!r)return"Error: job_id is required";let s=l._store.get(r);if(!s)return`Error: job '${r}' not found`;let n=ne.instance;if(e==="pause")return n.pause(r),`Paused job '${r}' (${s.name})`;if(e==="resume"){n.resume(r);let i=l._store.get(r);return`Resumed job '${r}' (${s.name}) | next run ${i?.nextRunAt??"-"}`}return n.runNow(r),`Triggered run of job '${r}' (${s.name})`}_history(e){let t=String(e.job_id||"").trim(),r=Math.min(Math.max(parseInt(String(e.limit??"10"),10)||10,1),50),s=ne.instance.executions.history(t||void 0,r);return s.length===0?t?`No executions for job '${t}'.`:"No executions yet.":s.map(n=>`  ${n.startedAt.slice(0,16)} | ${n.status==="success"?"\u2705":"\u274C"} | ${n.jobName} | ${(n.output||n.error||"").slice(0,60)}`).join(`
`)}};var Hs=class{name="vision_analyze";description="Analyze an image (local file path or http(s) URL) and return a text description. Requires a configured vision provider (provider with `vision: true` in config).";parameters={type:"object",properties:{image_url:{type:"string",description:"Path or URL of the image to analyze"},prompt:{type:"string",description:"Optional instruction for what to look for in the image"}},required:["image_url"]};visionLlm;constructor(e){this.visionLlm=e}async execute(e){let t=String(e.image_url||"").trim();if(!t)return"Error: image_url is required";if(!this.visionLlm)return"Error: no vision provider configured. Add a provider with `vision: true` in config.yaml (e.g. GLM) to enable image analysis.";try{let r=String(e.prompt||yt.DEFAULT_ANALYSIS_PROMPT),s=await yt.analyzeImage(this.visionLlm,t,r);return JSON.stringify({success:!0,analysis:s})}catch(r){return JSON.stringify({success:!1,analysis:`Error analyzing image: ${r.message}`})}}};import*as Gs from"node:fs";var zo=3,Jo=1,Yo=50,Vo=!0,Xo=3;function qs(){try{let l=process.cwd()+"/config.json";if(Gs.existsSync(l)){let t=JSON.parse(Gs.readFileSync(l,"utf-8")).delegation||{};return{maxChildren:t.max_concurrent_children??zo,maxDepth:t.max_spawn_depth??Jo,maxIterations:t.max_iterations??Yo,orchestratorEnabled:t.orchestrator_enabled??Vo,maxAsyncChildren:t.max_async_children??Xo}}}catch{}return{maxChildren:zo,maxDepth:Jo,maxIterations:Yo,orchestratorEnabled:Vo,maxAsyncChildren:Xo}}var Kc=!1;function Qo(l){let e={};for(let[t,r]of Object.entries(l))t==="acp_command"||t==="acp_args"||(e[t]=r);return e}var Ws=class{name="delegate_task";description="Spawn one or more subagents in isolated contexts. Each subagent gets its own LLM session and toolset. Use for complex multi-step work, parallel research, or self-contained subtasks.";parameters={type:"object",properties:{goal:{type:"string",description:"What the subagent should accomplish. Be specific and self-contained \u2014 the subagent knows nothing about your conversation history."},context:{type:"string",description:"Background information the subagent needs: file paths, error messages, project structure, constraints."},tasks:{type:"array",items:{type:"object",properties:{goal:{type:"string",description:"Task goal"},context:{type:"string",description:"Task-specific context"},role:{type:"string",enum:["leaf","orchestrator"],description:"Per-task role override."}},required:["goal"]},description:`Batch mode: up to ${qs().maxChildren} tasks to run in parallel. When provided, top-level goal/context are ignored.`},role:{type:"string",enum:["leaf","orchestrator"],description:"Role of the child agent. 'leaf' (default) = focused worker, cannot delegate further. 'orchestrator' = can use delegate_task to spawn its own workers."},background:{type:"boolean",description:"DEPRECATED / IGNORED. Top-level delegations always run in the background automatically."}},required:[]};parent;currentSessionId="";currentDepth=0;constructor(e){this.parent=e}async execute(e,t,r){if(Kc)return JSON.stringify({status:"error",error:"Subagent spawning is paused. Try again later."});let s=qs(),n=String(e.goal||""),i=String(e.context||""),o=e.tasks,a=String(e.role||"leaf");s.orchestratorEnabled||(a="leaf");let c=this.currentDepth;if(c>=s.maxDepth)return JSON.stringify({status:"error",error:`Delegation depth limit reached (depth=${c}, max_spawn_depth=${s.maxDepth}). Cannot spawn deeper subagents.`});let d;if(o&&Array.isArray(o)){if(o.length>s.maxChildren)return JSON.stringify({status:"error",error:`Error: max ${s.maxChildren} concurrent tasks supported. Got ${o.length}.`});d=o.map(p=>{let m=Qo(p);return{goal:String(m.goal||""),context:String(m.context||""),role:m.role||a}}).filter(p=>p.goal.trim())}else if(typeof o=="string")try{let p=JSON.parse(o);if(!Array.isArray(p))return JSON.stringify({status:"error",error:"Error: tasks must be a JSON array of task objects."});if(p.length>s.maxChildren)return JSON.stringify({status:"error",error:`Error: max ${s.maxChildren} concurrent tasks supported. Got ${p.length}.`});d=p.map(m=>{let g=Qo(m);return{goal:String(g.goal||""),context:String(g.context||""),role:g.role||a}}).filter(m=>m.goal.trim())}catch{return JSON.stringify({status:"error",error:"Error: tasks must be a JSON array of task objects; received a string that could not be parsed as JSON."})}else if(n)d=[{goal:n,context:i,role:a}];else return JSON.stringify({status:"error",error:"Error: provide 'goal' (single task) or 'tasks' array (batch mode)."});if(d.length===0)return JSON.stringify({status:"error",error:"Error: at least one valid task is required."});if(c===0&&d.length<=s.maxChildren){let m=ie.getInstance().executeBatch(d.map(g=>({goal:g.goal,context:g.context})),c+1,this.parent,this.currentSessionId,a,null,s.maxAsyncChildren);return m.status==="rejected"?JSON.stringify({status:"rejected",error:m.error}):JSON.stringify({status:"dispatched",delegation_id:m.delegationId,child_count:d.length})}let u=await Promise.all(d.map((p,m)=>Qe.runSubagent(p,m,c+1,this.parent)));return JSON.stringify({status:u.every(p=>p.status==="completed")?"completed":"error",results:u.map(p=>({task_index:p.task_index,status:p.status,summary:p.summary,error:p.error??null,api_calls:p.api_calls,duration_seconds:p.duration_seconds}))},null,2)}};async function zc(l,e){let t=`https://lite.duckduckgo.com/lite/?q=${encodeURIComponent(l)}`,r=await fetch(t,{headers:{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",Accept:"text/html"},signal:AbortSignal.timeout(1e4)});if(!r.ok)throw new Error(`DuckDuckGo returned ${r.status}: ${r.statusText}`);let s=await r.text(),n=[],i=/<a[^>]*href="([^"]*)"[^>]*class="result-link"[^>]*>([\s\S]*?)<\/a>/gi,o=/<td[^>]*class="result-snippet"[^>]*>([\s\S]*?)<\/td>/gi,a=/<td[^>]*class="url"[^>]*>([\s\S]*?)<\/td>/gi,c=[],d;for(;(d=i.exec(s))!==null;)c.push({href:d[1].trim(),text:d[2].replace(/<[^>]*>/g,"").trim()});let u=[];for(;(d=o.exec(s))!==null;)u.push(d[1].replace(/<[^>]*>/g,"").trim());let p=[];for(;(d=a.exec(s))!==null;)p.push(d[1].replace(/<[^>]*>/g,"").trim());for(let m=0;m<Math.min(c.length,e);m++){let g=c[m];!g.href||g.href.startsWith("/")||n.push({title:g.text||"(No title)",url:g.href,description:u[m]||p[m]||"",position:m+1})}return n}async function Jc(l,e){let t=process.env.GOOGLE_API_KEY||"",r=process.env.GOOGLE_CSE_ID||"";if(!t||!r)throw new Error("GOOGLE_API_KEY and GOOGLE_CSE_ID must be set for Google Custom Search");let s=`https://www.googleapis.com/customsearch/v1?key=${t}&cx=${r}&q=${encodeURIComponent(l)}&num=${Math.min(e,10)}`,n=await fetch(s,{signal:AbortSignal.timeout(1e4)});if(!n.ok){let a=await n.text();throw new Error(`Google Search returned ${n.status}: ${a.slice(0,200)}`)}return((await n.json()).items||[]).slice(0,e).map((a,c)=>({title:a.title||"",url:a.link||"",description:a.snippet||"",position:c+1}))}function Yc(){return(process.env.KEXVIM_WEB_SEARCH_BACKEND||"duckduckgo").toLowerCase().trim()==="google"?"google":"duckduckgo"}var Ks=class{name="web_search";description="Search the web for information. Returns up to 5 results by default with titles, URLs, and descriptions. The query is passed to DuckDuckGo (default, no API key needed) or Google (set GOOGLE_API_KEY + GOOGLE_CSE_ID + KEXVIM_WEB_SEARCH_BACKEND=google).";parameters={type:"object",properties:{query:{type:"string",description:"The search query to look up on the web."},limit:{type:"number",description:"Maximum number of results to return. Defaults to 5.",minimum:1,maximum:20,default:5}},required:["query"]};async execute(e,t,r){let s=String(e.query||"").trim();if(!s)return JSON.stringify({success:!1,error:"query is required"});let n=Math.min(Math.max(Number(e.limit)||5,1),20);try{let i=Yc(),o;return i==="google"?o=await Jc(s,n):o=await zc(s,n),o.length===0?JSON.stringify({success:!0,data:{web:[]},message:"No results found."},null,2):JSON.stringify({success:!0,data:{web:o}},null,2)}catch(i){return JSON.stringify({success:!1,error:`Search failed: ${i?.message||String(i)}`},null,2)}}};import*as ea from"node:dns";var Zo=ea.promises;async function Vc(l){try{let t=new URL(l).hostname;if(t==="localhost"||t==="127.0.0.1"||t==="0.0.0.0")return!1;let r=[];try{let s=await Zo.resolve4(t);r.push(...s)}catch{}try{let s=await Zo.resolve6(t);r.push(...s)}catch{}for(let s of r)if(s.startsWith("10.")||s.startsWith("172.16.")||s.startsWith("192.168.")||s.startsWith("127.")||s.startsWith("169.254.")||s==="::1"||s.startsWith("fd")||s.startsWith("fc"))return!0;return!1}catch{return!1}}function Xc(l,e){let t=l;if(t=t.replace(/<script[^>]*>[\s\S]*?<\/script>/gi,""),t=t.replace(/<style[^>]*>[\s\S]*?<\/style>/gi,""),t=t.replace(/<nav[^>]*>[\s\S]*?<\/nav>/gi,""),t=t.replace(/<footer[^>]*>[\s\S]*?<\/footer>/gi,""),t=t.replace(/<header[^>]*>[\s\S]*?<\/header>/gi,""),t=t.replace(/<h[1-6][^>]*>(.*?)<\/h[1-6]>/gi,(r,s)=>`
## ${s.replace(/<[^>]*>/g,"")}
...
[... Content truncated at ${e.toLocaleString()} chars \u2014 full content available at source URL ...]`}return t}var zs=class{name="web_fetch";description="Fetch and extract content from a web page URL. Returns page content in plain text/markdown format. Supports any public HTTP/HTTPS URL. Pages over 5000 chars are truncated. SSRF protection blocks private/internal network addresses.";parameters={type:"object",properties:{url:{type:"string",description:"The URL to fetch content from (must be http:// or https://)."},max_length:{type:"number",description:"Maximum characters to return. Defaults to 5000.",minimum:500,maximum:5e4,default:5e3}},required:["url"]};async execute(e,t,r){let s=String(e.url||"").trim();if(!s)return JSON.stringify({success:!1,error:"url is required"});let n=Math.min(Math.max(Number(e.max_length)||5e3,500),5e4),i;try{if(i=new URL(s),i.protocol!=="http:"&&i.protocol!=="https:")return JSON.stringify({success:!1,error:"Only http:// and https:// URLs are supported"})}catch{return JSON.stringify({success:!1,error:`Invalid URL: ${s}`})}try{if(await Vc(s))return JSON.stringify({success:!1,error:"Blocked: URL targets a private or internal network address (SSRF protection)."});let a=await fetch(s,{headers:{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"},signal:AbortSignal.timeout(3e4),redirect:"follow"}),c=a.headers.get("content-type")||"",d=c.includes("text/html")||c.includes("application/xhtml")||c.includes("text/plain"),u=c.includes("application/pdf")||s.endsWith(".pdf");if(!a.ok)return JSON.stringify({success:!1,error:`HTTP ${a.status}: ${a.statusText}`});if(u)return JSON.stringify({success:!0,data:{url:s,title:i.hostname,content:`[PDF document at ${s} \u2014 PDF parsing not yet supported. Use a browser tool to view this document.]`}});let p=await a.text(),m=Qc(p,i.hostname),g;return d?g=Xc(p,n):(g=p.slice(0,n),p.length>n&&(g+=`

[... Content truncated at ${n.toLocaleString()} chars \u2014 full content available at source URL ...]`)),JSON.stringify({success:!0,data:{url:s,title:m,content:g}},null,2)}catch(o){let a=(o instanceof Error?o.message:String(o))||String(o);return a.includes("timed out")||a.includes("timeout")||a.includes("abort")?JSON.stringify({success:!1,error:`Request timed out: ${s}. The page may be too slow or unreachable.`}):a.includes("fetch")||a.includes("ENOTFOUND")||a.includes("ECONNREFUSED")||a.includes("ECONNRESET")?JSON.stringify({success:!1,error:`Could not connect to ${i.hostname}: ${a.slice(0,150)}`}):JSON.stringify({success:!1,error:`Failed to fetch ${s}: ${a.slice(0,200)}`})}}};function Qc(l,e){let t=l.match(/<title[^>]*>([\s\S]*?)<\/title>/i);if(t)return t[1].replace(/<[^>]*>/g,"").trim();let r=l.match(/<h1[^>]*>(.*?)<\/h1>/i);return r?r[1].replace(/<[^>]*>/g,"").trim():e}import*as Zt from"node:fs";import*as ta from"node:os";import*as er from"node:path";import{randomUUID as Zc}from"node:crypto";var ed="https://queue.fal.run",td=55*1024*1024,mi=6e4,z=class l{static getKey(){return process.env.FAL_KEY?.trim()||void 0}static isConfigured(){return!!l.getKey()}static noKeyMessage(e){return`${e} \u9700\u8981 FAL.ai \u51ED\u636E\uFF1A\u8BBE\u7F6E\u73AF\u5883\u53D8\u91CF FAL_KEY\uFF08export FAL_KEY=...\uFF09\u540E\u91CD\u542F\u751F\u6548\u3002`}static async submit(e,t){let r=l.getKey();if(!r)throw new Error(l.noKeyMessage("\u56FE\u50CF/\u89C6\u9891\u751F\u6210"));let s;try{s=await fetch(`${ed}/${e}`,{method:"POST",headers:{Authorization:`Key ${r}`,"Content-Type":"application/json","x-idempotency-key":Zc()},body:JSON.stringify(t),signal:AbortSignal.timeout(mi)})}catch(i){throw new Error(`\u65E0\u6CD5\u8FDE\u63A5 FAL.ai\uFF1A${i instanceof Error?i.message:String(i)}`)}if(!s.ok){let i=await s.text().catch(()=>"");throw new Error(`FAL \u63D0\u4EA4\u5931\u8D25 (HTTP ${s.status})\uFF1A${i.slice(0,500)}`)}let n=await s.json();if(!n.request_id||!n.status_url)throw new Error(`FAL \u54CD\u5E94\u7F3A\u5C11 request_id/status_url\uFF1A${JSON.stringify(n).slice(0,300)}`);return{requestId:String(n.request_id),statusUrl:String(n.status_url),responseUrl:String(n.response_url||""),cancelUrl:n.cancel_url?String(n.cancel_url):void 0}}static async pollStatus(e,t=0,r=3e3){let s=t>0?Date.now()+t:0;for(;;){let n=await l.queryStatus(e);if(n.status==="COMPLETED"||n.status==="ERROR"||n.status==="CANCELED")return n;if(s>0&&Date.now()>=s)return{status:"TIMEOUT"};await new Promise(i=>setTimeout(i,r))}}static async queryStatus(e){let t=l.getKey();if(!t)throw new Error(l.noKeyMessage("\u4EFB\u52A1\u72B6\u6001\u67E5\u8BE2"));let r;try{r=await fetch(e,{headers:{Authorization:`Key ${t}`},signal:AbortSignal.timeout(mi)})}catch(i){throw new Error(`\u65E0\u6CD5\u8FDE\u63A5 FAL.ai\uFF1A${i instanceof Error?i.message:String(i)}`)}if(!r.ok)throw new Error(`FAL \u72B6\u6001\u67E5\u8BE2\u5931\u8D25 (HTTP ${r.status})`);let s=await r.json(),n=String(s.status||"");if(n==="COMPLETED")return{status:n,responseUrl:s.response_url?String(s.response_url):void 0};if(n==="ERROR"||n==="CANCELED"){let i=s.error||s.detail,o=typeof i=="string"?i:i?.message;return{status:n,error:o?String(o):`FAL \u4EFB\u52A1 ${n}`}}return{status:n}}static async getResult(e){let t=l.getKey();if(!t)throw new Error(l.noKeyMessage("\u4EFB\u52A1\u7ED3\u679C\u83B7\u53D6"));let r;try{r=await fetch(e,{headers:{Authorization:`Key ${t}`},signal:AbortSignal.timeout(mi)})}catch(s){throw new Error(`\u65E0\u6CD5\u8FDE\u63A5 FAL.ai\uFF1A${s instanceof Error?s.message:String(s)}`)}if(!r.ok)throw new Error(`FAL \u7ED3\u679C\u83B7\u53D6\u5931\u8D25 (HTTP ${r.status})`);return await r.json()}static async resolveMedia(e,t=10*1024*1024){let r=(e||"").trim();if(!r||/^https?:\/\//i.test(r)||/^data:/i.test(r)||!(r==="~"||r.startsWith("~/")||r.startsWith("~\\")||r.startsWith("/")||r.startsWith("./")||r.startsWith("../")||r.startsWith(".\\")||r.startsWith("..\\")||/^[A-Za-z]:[\\/]/.test(r)||r.startsWith("\\\\")))return r;let n=r.startsWith("~")?er.join(ta.homedir(),r.slice(1)):er.resolve(r);if(!Zt.existsSync(n))throw new Error(`\u5A92\u4F53\u6587\u4EF6\u4E0D\u5B58\u5728\uFF1A${n}`);let i=Zt.statSync(n).size;if(i>t)throw new Error(`\u5A92\u4F53\u6587\u4EF6\u8FC7\u5927\uFF1A${n} (${(i/1024/1024).toFixed(1)}MB > ${(t/1024/1024).toFixed(0)}MB)`);let o=l.mimeForPath(n),a=Zt.readFileSync(n).toString("base64");if(a.length>td)throw new Error(`\u5A92\u4F53\u6587\u4EF6\u8FC7\u5927\uFF08base64 \u540E ${(a.length/1024/1024).toFixed(1)}MB \u8D85\u51FA\u8BF7\u6C42\u4F53\u4E0A\u9650\uFF09`);return`data:${o};base64,${a}`}static mimeForPath(e){switch(er.extname(e).toLowerCase()){case".png":return"image/png";case".jpg":case".jpeg":return"image/jpeg";case".webp":return"image/webp";case".gif":return"image/gif";case".mp4":return"video/mp4";case".webm":return"video/webm";case".mov":return"video/quicktime";default:return"application/octet-stream"}}};import*as Ys from"node:fs";import*as fi from"node:path";var gi={"fal-ai/flux-2/klein/9b":{display:"FLUX 2 Klein 9B",speed:"<1s",strengths:"Fast, crisp text",price:"$0.006/MP",size_style:"image_size_preset",sizes:{landscape:"landscape_16_9",square:"square_hd",portrait:"portrait_16_9"},defaults:{num_inference_steps:4,output_format:"png",enable_safety_checker:!1},supports:["prompt","image_size","num_inference_steps","seed","output_format","enable_safety_checker"],upscale:!1,edit_endpoint:"fal-ai/flux-2/klein/9b/edit",edit_supports:["prompt","image_urls","num_inference_steps","seed","output_format","enable_safety_checker"],max_reference_images:9},"fal-ai/flux-2-pro":{display:"FLUX 2 Pro",speed:"~6s",strengths:"Studio photorealism",price:"$0.03/MP",size_style:"image_size_preset",sizes:{landscape:"landscape_16_9",square:"square_hd",portrait:"portrait_16_9"},defaults:{num_inference_steps:50,guidance_scale:4.5,num_images:1,output_format:"png",enable_safety_checker:!1,safety_tolerance:"5",sync_mode:!0},supports:["prompt","image_size","num_inference_steps","guidance_scale","num_images","output_format","enable_safety_checker","safety_tolerance","sync_mode","seed"],upscale:!0,edit_endpoint:"fal-ai/flux-2-pro/edit",edit_supports:["prompt","image_urls","num_inference_steps","guidance_scale","num_images","output_format","enable_safety_checker","safety_tolerance","sync_mode","seed"],max_reference_images:9},"fal-ai/z-image/turbo":{display:"Z-Image Turbo",speed:"~2s",strengths:"Bilingual EN/CN, 6B",price:"$0.005/MP",size_style:"image_size_preset",sizes:{landscape:"landscape_16_9",square:"square_hd",portrait:"portrait_16_9"},defaults:{num_inference_steps:8,num_images:1,output_format:"png",enable_safety_checker:!1,enable_prompt_expansion:!1},supports:["prompt","image_size","num_inference_steps","num_images","seed","output_format","enable_safety_checker","enable_prompt_expansion"],upscale:!1},"fal-ai/nano-banana-pro":{display:"Nano Banana Pro (Gemini 3 Pro Image)",speed:"~8s",strengths:"Gemini 3 Pro, reasoning depth, text rendering",price:"$0.15/image (1K)",size_style:"aspect_ratio",sizes:{landscape:"16:9",square:"1:1",portrait:"9:16"},defaults:{num_images:1,output_format:"png",safety_tolerance:"5",resolution:"1K"},supports:["prompt","aspect_ratio","num_images","output_format","safety_tolerance","seed","sync_mode","resolution","enable_web_search","limit_generations"],upscale:!1,edit_endpoint:"fal-ai/nano-banana-pro/edit",edit_supports:["prompt","image_urls","aspect_ratio","num_images","output_format","safety_tolerance","seed","sync_mode","resolution","enable_web_search","limit_generations"],max_reference_images:2},"fal-ai/gpt-image-1.5":{display:"GPT Image 1.5",speed:"~15s",strengths:"Prompt adherence",price:"$0.034/image",size_style:"gpt_literal",sizes:{landscape:"1536x1024",square:"1024x1024",portrait:"1024x1536"},defaults:{quality:"medium",num_images:1,output_format:"png"},supports:["prompt","image_size","quality","num_images","output_format","background","sync_mode"],upscale:!1,edit_endpoint:"fal-ai/gpt-image-1.5/edit",edit_supports:["prompt","image_urls","image_size","quality","num_images","output_format","sync_mode"],max_reference_images:16},"fal-ai/gpt-image-2":{display:"GPT Image 2",speed:"~20s",strengths:"SOTA text rendering + CJK, world-aware photorealism",price:"$0.04\u20130.06/image",size_style:"image_size_preset",sizes:{landscape:"landscape_4_3",square:"square_hd",portrait:"portrait_4_3"},defaults:{quality:"medium",num_images:1,output_format:"png"},supports:["prompt","image_size","quality","num_images","output_format","sync_mode"],upscale:!1,edit_endpoint:"openai/gpt-image-2/edit",edit_supports:["prompt","image_urls","quality","num_images","output_format","sync_mode","mask_image_url"],max_reference_images:16},"fal-ai/ideogram/v3":{display:"Ideogram V3",speed:"~5s",strengths:"Best typography",price:"$0.03-0.09/image",size_style:"image_size_preset",sizes:{landscape:"landscape_16_9",square:"square_hd",portrait:"portrait_16_9"},defaults:{rendering_speed:"BALANCED",expand_prompt:!0,style:"AUTO"},supports:["prompt","image_size","rendering_speed","expand_prompt","style","seed"],upscale:!1,edit_endpoint:"fal-ai/ideogram/v3/edit",edit_supports:["prompt","image_urls","rendering_speed","expand_prompt","style","seed"],max_reference_images:1},"fal-ai/recraft/v4/pro/text-to-image":{display:"Recraft V4 Pro",speed:"~8s",strengths:"Design, brand systems, production-ready",price:"$0.25/image",size_style:"image_size_preset",sizes:{landscape:"landscape_16_9",square:"square_hd",portrait:"portrait_16_9"},defaults:{enable_safety_checker:!1},supports:["prompt","image_size","enable_safety_checker","colors","background_color"],upscale:!1},"fal-ai/qwen-image":{display:"Qwen Image",speed:"~12s",strengths:"LLM-based, complex text",price:"$0.02/MP",size_style:"image_size_preset",sizes:{landscape:"landscape_16_9",square:"square_hd",portrait:"portrait_16_9"},defaults:{num_inference_steps:30,guidance_scale:2.5,num_images:1,output_format:"png",acceleration:"regular"},supports:["prompt","image_size","num_inference_steps","guidance_scale","num_images","output_format","acceleration","seed","sync_mode"],upscale:!1,edit_endpoint:"fal-ai/qwen-image-2/pro/edit",edit_supports:["prompt","image_urls","num_inference_steps","guidance_scale","num_images","output_format","acceleration","seed","sync_mode"],max_reference_images:3},"fal-ai/krea/v2/medium/text-to-image":{display:"Krea 2 Medium",speed:"~15-25s",strengths:"Illustration, anime, painting, expressive/artistic styles",price:"$0.030 (text) / $0.035 (style refs)",size_style:"aspect_ratio",sizes:{landscape:"16:9",square:"1:1",portrait:"9:16"},defaults:{creativity:"medium"},supports:["prompt","aspect_ratio","creativity","seed","image_style_references"],upscale:!1},"fal-ai/krea/v2/large/text-to-image":{display:"Krea 2 Large",speed:"~25-60s",strengths:"Photorealism, raw textured looks (motion blur, grain, film)",price:"$0.060 (text) / $0.065 (style refs)",size_style:"aspect_ratio",sizes:{landscape:"16:9",square:"1:1",portrait:"9:16"},defaults:{creativity:"medium"},supports:["prompt","aspect_ratio","creativity","seed","image_style_references"],upscale:!1}},na="fal-ai/flux-2/klein/9b",ra="landscape",sa=["landscape","square","portrait"],rd="fal-ai/clarity-upscaler",sd="masterpiece, best quality, highres",nd="(worst quality, low quality, normal quality:2)",id=.35,od=.6,ad=4,ld=18;function cd(l){let e=(l||process.env.FAL_IMAGE_MODEL||"").trim(),t=e&&gi[e]?e:na;return{model:t,meta:gi[t]}}function dd(){let l=I.findProjectRoot(),e=fi.join(l||process.cwd(),"data","media");return Ys.mkdirSync(e,{recursive:!0}),e}async function ud(l,e){try{let t=await fetch(l,{signal:AbortSignal.timeout(6e4)});if(!t.ok)return l;let r=Buffer.from(await t.arrayBuffer()),s=`fal_${Date.now()}_${Math.floor(Math.random()*1e6)}.${e||"bin"}`,n=fi.join(dd(),s);return Ys.writeFileSync(n,r),n}catch{return l}}function pd(l,e){let t=/image\/(\w+)/.exec(l||"");if(t)return t[1]==="jpeg"?"jpg":t[1];let r=/\.(png|jpg|jpeg|webp|gif)(?:$|\?)/i.exec(e);return r?r[1]==="jpeg"?"jpg":r[1].toLowerCase():"png"}var Js=class l{name="image_generate";description="Generate an image from a text prompt (or edit a source image) via FAL.ai. Requires FAL_KEY. aspect_ratio: landscape (default), square, portrait. Pass image_url or reference_image_urls (local paths or URLs) for image-to-image editing. Result includes a local file path you can share.";parameters={type:"object",properties:{prompt:{type:"string",description:"Image description / edit instruction. Required."},aspect_ratio:{type:"string",enum:sa,default:ra,description:"Output aspect ratio."},image_url:{type:"string",description:"Source image for editing: local file path or URL."},reference_image_urls:{type:"array",items:{type:"string"},description:"Reference images for editing (model-dependent cap)."},seed:{type:"integer",minimum:0,maximum:4294967295,description:"Optional reproducibility seed."},model:{type:"string",description:`Model override. Default: ${na}. Available: ${Object.keys(gi).join(", ")}`}},required:["prompt"]};async execute(e,t,r){let s=String(e.prompt||"").trim();if(!s)return JSON.stringify({success:!1,image:null,error:"prompt is required for image generation",error_type:"ValueError"});let{model:n,meta:i}=cd(e.model?String(e.model):void 0),o=sa.includes(String(e.aspect_ratio||"").toLowerCase())?String(e.aspect_ratio).toLowerCase():ra,a=[];if(typeof e.image_url=="string"&&e.image_url.trim()&&a.push(e.image_url.trim()),Array.isArray(e.reference_image_urls))for(let p of e.reference_image_urls)typeof p=="string"&&p.trim()&&a.push(p.trim());if(!z.isConfigured())return JSON.stringify({success:!1,image:null,error:z.noKeyMessage("\u56FE\u50CF\u751F\u6210"),error_type:"MissingCredentials"});let c=i.edit_endpoint,d=a.length>0&&!!c,u=d?"image":"text";if(a.length>0&&!c)return JSON.stringify({success:!1,image:null,error:`Model '${i.display}' (${n}) \u4E0D\u652F\u6301\u56FE\u751F\u56FE/\u7F16\u8F91\u3002\u8BF7\u63D0\u4F9B\u7EAF\u6587\u672C prompt\uFF08\u7701\u7565 image_url\uFF09\uFF0C\u6216\u6362\u7528\u652F\u6301\u7F16\u8F91\u7684\u6A21\u578B\u3002`,error_type:"ValueError"});try{let p={...i.defaults};if(p.prompt=s,d){let _=i.max_reference_images&&i.max_reference_images>0?i.max_reference_images:1,M=a.slice(0,_);p.image_urls=await Promise.all(M.map(L=>z.resolveMedia(L))),e.seed!==void 0&&i.edit_supports?.includes("seed")&&(p.seed=e.seed);for(let L of["num_inference_steps","guidance_scale","num_images","output_format"])e[L]!==void 0&&i.edit_supports?.includes(L)&&(p[L]=e[L])}else{let _=i.sizes[o];i.size_style==="aspect_ratio"?p.aspect_ratio=_:p.image_size=_,e.seed!==void 0&&i.supports.includes("seed")&&(p.seed=e.seed);for(let M of["num_inference_steps","guidance_scale","num_images","output_format"])e[M]!==void 0&&i.supports.includes(M)&&(p[M]=e[M])}let m=d&&c?c:n,g=await z.submit(m,p),f=await z.pollStatus(g.statusUrl,3e5);if(f.status!=="COMPLETED"||!f.responseUrl){let _=f.status==="TIMEOUT"?"\u751F\u6210\u8D85\u65F6\uFF085 \u5206\u949F\uFF09\u3002\u8BF7\u91CD\u8BD5\u6216\u66F4\u6362\u6A21\u578B\u3002":f.error||`\u4EFB\u52A1 ${f.status}`;return JSON.stringify({success:!1,image:null,error:_,error_type:f.status==="TIMEOUT"?"Timeout":"FalError"})}let k=(await z.getResult(f.responseUrl)).images||[];if(!k.length)return JSON.stringify({success:!1,image:null,error:"FAL.ai \u672A\u8FD4\u56DE\u56FE\u7247",error_type:"FalError"});let y=i.upscale&&!d,v=[];for(let _ of k){let M=String(_.url||"");if(!M)continue;let L={url:M,width:_.width||0,height:_.height||0,upscaled:!1};if(y){let he=await l.upscale(M,s);if(he){v.push({...he,upscaled:!0});continue}}v.push(L)}if(!v.length)return JSON.stringify({success:!1,image:null,error:"FAL.ai \u672A\u8FD4\u56DE\u6709\u6548\u56FE\u7247 URL",error_type:"FalError"});let w=v[0],R=await ud(String(w.url),pd(String(w.content_type||""),String(w.url)));return JSON.stringify({success:!0,image:R,url:w.url,width:w.width,height:w.height,upscaled:w.upscaled,modality:u,count:v.length})}catch(p){let m=p instanceof Error?p.message:String(p);return JSON.stringify({success:!1,image:null,error:m,error_type:p instanceof Error?p.constructor.name:"Error"})}}static async upscale(e,t){try{let r={image_url:e,prompt:t||sd,negative_prompt:nd,creativity:id,resemblance:od,guidance_scale:ad,num_inference_steps:ld,enable_safety_checker:!1},s=await z.submit(rd,r),n=await z.pollStatus(s.statusUrl,3e5);if(n.status!=="COMPLETED"||!n.responseUrl)return null;let o=(await z.getResult(n.responseUrl)).image;return!o||!o.url?null:{url:String(o.url),width:Number(o.width||0),height:Number(o.height||0)}}catch{return null}}};import*as tr from"node:fs";import*as da from"node:os";import*as hi from"node:path";import{randomUUID as md}from"node:crypto";var gd=300,ia=5e4,oa=1e4;function aa(l){let e={head:Buffer.alloc(0),tail:Buffer.alloc(0),total:0},t=Math.floor(l*.4),r=l-t;return{get head(){return e.head},get tail(){return e.tail},get total(){return e.total},onChunk(s){if(e.total+=s.length,e.total<=l){e.head=Buffer.concat([e.head,s]);return}if(e.head.length<t){let n=t-e.head.length;e.head=Buffer.concat([e.head,s.subarray(0,n)])}e.tail=Buffer.concat([e.tail,s]),e.tail.length>r&&(e.tail=e.tail.subarray(e.tail.length-r))}}}function la(l,e){let t=l.head.length+l.tail.length,r=Math.max(l.total,t),s=r>t,n=Math.max(0,r-t),i;return s?i=l.head.toString("utf8",0,l.head.length).replace(/\uFFFD/g,"")+`

... [OUTPUT TRUNCATED - ${n.toLocaleString()} bytes omitted out of ${r.toLocaleString()} total] ...

`+l.tail.toString("utf8",0,l.tail.length).replace(/\uFFFD/g,""):i=Buffer.concat([l.head,l.tail]).toString("utf8").replace(/\uFFFD/g,""),{text:i,truncated:s,captured:t,omitted:n}}function ca(l){if(l.pid)if(process.platform==="win32")b.spawn("taskkill",["/pid",String(l.pid),"/T","/F"],{stdio:"ignore"});else try{process.kill(-l.pid,"SIGKILL")}catch{try{l.kill("SIGKILL")}catch{}}}var Vs=class l{name="execute_code";description="Run a Python script in an isolated sandbox subprocess (NOT your terminal). Use for data crunching, prototypes, one-off scripts. The script runs in a temp cwd with a 5-minute timeout; stdout is capped at 50KB (head/tail). Pure-Python only \u2014 to interact with the environment (files, web, shell) use the regular tools instead.";parameters={type:"object",properties:{code:{type:"string",description:"Python source code to execute. Must be self-contained."}},required:["code"]};static sanitizeEnv(e){let t={};for(let[r,s]of Object.entries(e)){if(s===void 0)continue;let n=r.toUpperCase();/(API|TOKEN|SECRET|PASSWORD|PASSWD|CREDENTIAL|AUTH|KEY)\b|_KEY$|_TOKEN$|_SECRET$/.test(n)||(t[r]=s)}return t}async execute(e,t,r){let s=String(e.code||"");if(!s.trim())return JSON.stringify({status:"error",error:"No code provided.",duration_seconds:0});let n=`${r?.platform||"chat"}:${r?.chatId||"default"}`,i=await it().checkCommand(s,n,{source:r?.platform,isPython:!0});if(i.decision==="deny")return JSON.stringify({status:"error",error:i.reason,duration_seconds:0,note:"Code was NOT executed."});if(i.decision==="approval_required")return JSON.stringify({type:"approval_required",command:s.slice(0,500),description:i.description,message:i.message,instruction:"Ask the user to approve this code (they reply \u6279\u51C6/approve). The code has NOT been executed. If approved, retry with the exact same code."});let o=Math.max(1,Number(e.timeout||process.env.EXECUTE_CODE_TIMEOUT||gd)),a=tr.mkdtempSync(hi.join(da.tmpdir(),`kexvim_sandbox_${md().slice(0,8)}_`)),c=hi.join(a,"script.py");try{tr.writeFileSync(c,s,"utf8");let d=aa(ia),u=aa(oa),p=Date.now(),m=b.spawn("python",["-u",c],{cwd:a,env:{...l.sanitizeEnv(process.env),PYTHONIOENCODING:"utf-8",PYTHONUTF8:"1",KEXVIM_SANDBOX:"1"},stdio:["ignore","pipe","pipe"],detached:process.platform!=="win32"});m.stdout.on("data",w=>d.onChunk(w)),m.stderr.on("data",w=>u.onChunk(w));let g=setTimeout(()=>ca(m),o*1e3),f=!1;t&&t.addEventListener("abort",()=>{f=!0,ca(m)},{once:!0});let h=await new Promise(w=>{m.on("close",R=>w(R)),m.on("error",R=>{u.onChunk(Buffer.from(`Failed to spawn python: ${R.message}`)),w(null)})});clearTimeout(g);let k=(Date.now()-p)/1e3,y=la(d,ia),v=la(u,oa);return f||h===null&&!v.text?JSON.stringify({status:"error",error:`Timed out after ${o} seconds`,stdout:y.text,stderr:v.text,duration_seconds:Math.round(k*10)/10,exit_code:h,stdout_truncated:y.truncated,stderr_truncated:v.truncated}):JSON.stringify({status:h===0?"success":"error",stdout:y.text,stderr:v.text,duration_seconds:Math.round(k*10)/10,exit_code:h,stdout_bytes:y.captured,stdout_bytes_total:d.total,stdout_truncated:y.truncated,stderr_bytes:v.captured,stderr_bytes_total:u.total,stderr_truncated:v.truncated})}catch(d){return JSON.stringify({status:"error",error:d instanceof Error?d.message:String(d),duration_seconds:0})}finally{try{tr.rmSync(a,{recursive:!0,force:!0})}catch{}}}};import*as Le from"node:fs";import*as qe from"node:path";import*as ua from"node:os";function fd(){let l=process.env.FAL_VIDEO_MODEL?.trim();return l?{textToVideo:l,imageToVideo:l,keyframes:l,continuation:l}:{textToVideo:"fal-ai/bfl-flux3/text-to-video",imageToVideo:"fal-ai/bfl-flux3/image-to-video",keyframes:"fal-ai/bfl-flux3/keyframes-to-video",continuation:"fal-ai/bfl-flux3/video-continuation"}}var hd=["auto","21:9","16:9","4:3","1:1","3:4","9:16","9:21"],yd=["720p"],yi=10;function pa(){let l=I.findProjectRoot();return qe.join(l||process.cwd(),"data","fal-jobs.json")}function ma(){try{let l=Le.readFileSync(pa(),"utf8");return JSON.parse(l)}catch{return{}}}function vd(l){let e=ma();e[l.requestId]=l;let t=Object.keys(e);for(;t.length>200;)delete e[t.shift()];try{let r=pa();Le.mkdirSync(qe.dirname(r),{recursive:!0}),Le.writeFileSync(r,JSON.stringify(e,null,2))}catch{}}function bd(l){return ma()[l]}function sn(){return{prompt:{type:"string",minLength:1,description:'Generation brief in plain prose. Order it subject, visual specifics, action, camera, lighting, environment, audio, style. Audio is generated by default \u2014 say "no music" if unwanted.'},aspect_ratio:{type:"string",enum:hd,default:"auto",description:'Output aspect ratio. "auto" lets the model choose.'},duration:{oneOf:[{type:"integer",minimum:5,maximum:20},{type:"string",const:"auto"}],default:"auto",description:'Clip duration in whole seconds (5-20), or "auto".'},resolution:{type:"string",enum:yd,default:"720p",description:"Output resolution bin."},generate_audio:{type:"boolean",default:!0,description:"Generate synchronized audio."},grounding:{type:"boolean",default:!0,description:"Allow a short research pass before generation."},seed:{type:"integer",minimum:0,maximum:4294967295,description:"Optional reproducibility seed."},version:{type:"string",description:'Model version pin. Defaults to "latest".'}}}var nn="Read bfl_flux3_prompting_guide before your first generation. ",on="All guidance is defaults: explicit user instructions override it.",vi="Media fields accept a local file path (uploaded automatically to FAL) or a URL. ";async function an(l,e,t){if(!z.isConfigured())return JSON.stringify({error:z.noKeyMessage("\u89C6\u9891\u751F\u6210")});let r=fd(),s;switch(l){case"text_to_video":s=r.textToVideo;break;case"image_to_video":s=r.imageToVideo;break;case"keyframes":s=r.keyframes;break;case"continuation":s=r.continuation;break;default:s=r.textToVideo}let n={};for(let i of["prompt","aspect_ratio","duration","resolution","generate_audio","grounding","seed","version"])e[i]!==void 0&&(n[i]=e[i]);try{l==="image_to_video"&&typeof e.input_image=="string"&&(n.input_image=await z.resolveMedia(e.input_image)),l==="keyframes"&&Array.isArray(e.input_images)&&(n.input_images=await Promise.all(e.input_images.slice(0,yi).map(o=>z.resolveMedia(o))),Array.isArray(e.keyframe_indices)&&(n.keyframe_indices=e.keyframe_indices)),l==="continuation"&&typeof e.input_video=="string"&&(n.input_video=await z.resolveMedia(e.input_video,55*1024*1024));let i=await z.submit(s,n);return vd({requestId:i.requestId,model:s,statusUrl:i.statusUrl,responseUrl:i.responseUrl,mode:l,createdAt:Date.now()}),JSON.stringify({job_id:i.requestId,status:"submitted",model:s,message:"Generation takes several minutes. Poll bfl_flux3_get_result with this job id."})}catch(i){return JSON.stringify({error:i instanceof Error?i.message:String(i)})}}async function kd(l,e,t){let r=await fetch(l,{signal:AbortSignal.timeout(3e5)});if(!r.ok)throw new Error(`\u89C6\u9891\u4E0B\u8F7D\u5931\u8D25 (HTTP ${r.status})`);let s=Buffer.from(await r.arrayBuffer()),n=`bfl_flux3_${new Date().toISOString().replace(/[:.]/g,"-").slice(0,19)}.mp4`,i;if(e){let d=qe.resolve(e);i=/\.mp4$/i.test(d)?d:qe.join(d,n)}else i=qe.join(ua.homedir(),"Downloads",n);let o=qe.dirname(i);Le.mkdirSync(o,{recursive:!0});let a=i,c=1;for(;Le.existsSync(a);)a=i.replace(/\.mp4$/i,"")+`-${c}.mp4`,c++;return Le.writeFileSync(a,s),{target:a,size:s.length}}var Xs=class{name="bfl_flux3_text_to_video";description=nn+"FLUX 3 text-to-video: generates a clip (with audio) from the prompt alone. Nothing but the prompt anchors the subject here, so research anything with a real, checkable appearance before writing it. Generation takes several minutes: this returns a job id immediately; poll bfl_flux3_get_result. "+on;parameters={type:"object",properties:sn(),required:["prompt"],additionalProperties:!1};async execute(e,t,r){let s=String(e.prompt||"").trim();return s?an("text_to_video",{...e,prompt:s},r?.chatId):JSON.stringify({error:"prompt is required"})}},Qs=class{name="bfl_flux3_image_to_video";description=nn+"FLUX 3 image-to-video: animates one image as the literal opening frame \u2014 those pixels are frame 0 and the clip moves from there. "+vi+"Returns a job id; poll bfl_flux3_get_result. "+on;parameters={type:"object",properties:{...sn(),input_image:{type:"string",minLength:1,description:"Exactly one opening-frame image: a local file path or a URL (PNG/JPEG/WebP, up to 10MB)."}},required:["prompt","input_image"],additionalProperties:!1};async execute(e,t,r){let s=String(e.prompt||"").trim();return s?typeof e.input_image!="string"||!e.input_image.trim()?JSON.stringify({error:"input_image is required"}):an("image_to_video",{...e,prompt:s},r?.chatId):JSON.stringify({error:"prompt is required"})}},Zs=class{name="bfl_flux3_keyframes_to_video";description=nn+"FLUX 3 keyframe video: a storyboard of 1-10 images pinned at chosen frame positions (24fps). Name the subject and describe the motion that carries it between the pins, and keep the subject consistent across every pinned image. "+vi+"Returns a job id; poll bfl_flux3_get_result. "+on;parameters={type:"object",properties:{...sn(),input_images:{type:"array",items:{type:"string",minLength:1},minItems:1,maxItems:yi,description:"1-10 keyframe images: local file paths or URLs (PNG/JPEG/WebP, up to 10MB each)."},keyframe_indices:{type:"array",items:{type:"integer",minimum:0,maximum:480},minItems:1,maxItems:yi,description:'One unique non-negative frame index per image (24fps). Each must be at most duration\xD724, so set an explicit duration rather than "auto" whenever you pin indices.'}},required:["prompt","input_images","keyframe_indices"],additionalProperties:!1};async execute(e,t,r){let s=String(e.prompt||"").trim();return s?!Array.isArray(e.input_images)||e.input_images.length===0?JSON.stringify({error:"input_images is required"}):an("keyframes",{...e,prompt:s},r?.chatId):JSON.stringify({error:"prompt is required"})}},en=class{name="bfl_flux3_video_continuation";description=nn+`FLUX 3 video continuation: the new generation picks up from the input clip's final frames. Open the prompt with "Continue this video from its final frames:", re-establish the subject and the moment it ended on, then describe what happens next. input_video must be an mp4 of at most 50MB and 15 seconds; the generated segment tops out at 15s too \u2014 chain a second continuation for a longer sequence. duration is the new segment only. `+vi+"Returns a job id; poll bfl_flux3_get_result. "+on;parameters={type:"object",properties:{...sn(),input_video:{type:"string",minLength:1,description:"The clip to continue: a local file path or a URL. mp4 only, at most 50MB and 15 seconds."}},required:["prompt","input_video"],additionalProperties:!1};async execute(e,t,r){let s=String(e.prompt||"").trim();return s?typeof e.input_video!="string"||!e.input_video.trim()?JSON.stringify({error:"input_video is required"}):an("continuation",{...e,prompt:s},r?.chatId):JSON.stringify({error:"prompt is required"})}},tn=class{name="bfl_flux3_get_result";description="Poll a FLUX 3 video job by the job id a generate tool returned. Generation takes minutes and a long Generating phase is normal. This call waits for you while the job runs (up to ~5 minutes), so it may run for several minutes; if it returns still generating, just call it again. On Ready the clip is downloaded for you and the response gives its local path.";parameters={type:"object",properties:{id:{type:"string",minLength:1,description:"Job id from a previous bfl_flux3_* generate call."},save_to:{type:"string",description:"Where to save the finished clip: a directory or a full file path. Set this only when the user asked for a particular location; the default is ~/Downloads. An existing file is never overwritten."}},required:["id"],additionalProperties:!1};async execute(e,t,r){let s=String(e.id||"").trim();if(!s)return JSON.stringify({error:"id is required"});let n=bd(s);if(!n)return JSON.stringify({error:`Unknown job id '${s}'. Generate a video first with a bfl_flux3_* tool (job records persist in data/fal-jobs.json).`});try{let i=await z.pollStatus(n.statusUrl,3e5,5e3);if(i.status==="TIMEOUT")return JSON.stringify({job_id:s,status:"still_generating",message:"Job is still generating. Call bfl_flux3_get_result again with the same id."});if(i.status!=="COMPLETED"||!i.responseUrl)return JSON.stringify({job_id:s,status:"failed",error:i.error||`Task ${i.status}`});let o=await z.getResult(i.responseUrl),c=(o.video??(Array.isArray(o.videos)?o.videos[0]:void 0))||{},d=String(c.url||o.url||"");if(!d)return JSON.stringify({job_id:s,status:"failed",error:"\u5B8C\u6210\u4F46\u7ED3\u679C\u65E0\u89C6\u9891 URL",details:String(JSON.stringify(o)).slice(0,500)});let u=await kd(d,e.save_to?String(e.save_to):void 0,Date.now());return JSON.stringify({job_id:s,status:"ready",video_path:u.target,size_bytes:u.size,url:d,duration_seconds:c.duration_seconds,width:c.width,height:c.height,message:`Deliver the video to the user with MEDIA:${u.target}`})}catch(i){return JSON.stringify({job_id:s,status:"error",error:i instanceof Error?i.message:String(i)})}}},rn=class{name="bfl_flux3_prompting_guide";description="Read this before your first FLUX 3 generation. The prompting and grounding guide: how to research a subject so it renders as itself, how to assemble a prompt, which generate tool fits, and how to save and deliver the finished clip. Takes no arguments and spends no generation budget.";parameters={type:"object",properties:{},additionalProperties:!1};async execute(e,t,r){return`# FLUX 3 video generation \u2014 how to get the best results
...
4. On ready the clip is downloaded; deliver it with MEDIA:<path> in your response.`}};import*as ya from"node:fs";import*as ki from"node:path";import*as bi from"node:dns";var Sd=600*1e3,ga=500,fa=15e3,ln=null;async function _d(){if(ln)return ln;try{ln=await import("playwright")}catch(l){throw new Error(`playwright \u672A\u5B89\u88C5\uFF1A\u8BF7\u5148\u6267\u884C npm i playwright && npx playwright install chromium\uFF08\u5F53\u524D\uFF1A${l instanceof Error?l.message:String(l)}\uFF09`)}return ln}var ee=class l{static sessions=new Map;static consoleLogs=new Map;static dialogLogs=new Map;static async ensure(e,t){let r=e||"default",s=l.sessions.get(r);if(s)return s;let n=await _d(),i;if(t){let o=await n.chromium.connectOverCDP(t),a=o.contexts()[0]||await o.newContext(),c=a.pages()[0]||await a.newPage();i={browser:o,context:a,page:c,cdp:!0,lastActivity:Date.now()}}else{let o=await n.chromium.launch({headless:!0}),a=await o.newContext({viewport:{width:1280,height:800}}),c=await a.newPage();i={browser:o,context:a,page:c,cdp:!1,lastActivity:Date.now()}}return l.dialogLogs.set(r,[]),i.page.on("dialog",async o=>{(l.dialogLogs.get(r)||[]).push(`${o.type()}: ${o.message().slice(0,300)}`);try{await o.accept()}catch{}}),l.consoleLogs.set(r,{messages:[],errors:[]}),i.page.on("console",o=>{let a=l.consoleLogs.get(r);if(!a)return;let c=o.text().slice(0,500);o.type()==="error"?a.errors.push(c):a.messages.push(c)}),i.page.on("pageerror",o=>{l.consoleLogs.get(r)?.errors.push(`pageerror: ${o.message.slice(0,500)}`)}),l.sessions.set(r,i),i}static touch(e){let t=l.sessions.get(e||"default");t&&(t.lastActivity=Date.now())}static async reapIdle(){let e=Date.now();for(let[t,r]of[...l.sessions.entries()])if(e-r.lastActivity>Sd){try{await r.browser.close()}catch{}l.sessions.delete(t),l.consoleLogs.delete(t),l.dialogLogs.delete(t)}}static get(e){let t=l.sessions.get(e||"default");return t&&(t.lastActivity=Date.now()),t}static consoleState(e){return l.consoleLogs.get(e||"default")||{messages:[],errors:[]}}static dialogs(e){return l.dialogLogs.get(e||"default")||[]}static async closeAll(){for(let e of l.sessions.values())try{await e.browser.close()}catch{}l.sessions.clear()}};async function va(l,e){if(e)return(await l.evaluate(()=>document.body?.innerText||"")).slice(0,fa*2);let r=l.locator('a[href], button, input, textarea, select, [role="button"], [role="link"], [role="checkbox"], [role="radio"], [role="tab"], [role="menuitem"], [contenteditable="true"], [tabindex]:not([tabindex="-1"])'),s=Math.min(await r.count(),120),n=[],i=await l.evaluate(()=>document.title||"");n.push(`URL: ${l.url()}`),n.push(`Title: ${i}`),n.push(""),n.push(`Interactive elements (${s}):`);for(let a=0;a<s;a++){let c=r.nth(a),d=await c.evaluate(m=>{let g=m.tagName.toLowerCase(),f=(m.textContent||"").trim().replace(/\s+/g," ").slice(0,80),h=m.type||"";return{tag:g,txt:f,type:h}}).catch(()=>({tag:"?",txt:"",type:""})),u=await c.getAttribute("href").catch(()=>null),p=`[ref=${a+1}] <${d.tag}${d.type?` type="${d.type}"`:""}> ${d.txt||d.tag}`;n.push(u?`${p} href="${String(u).slice(0,100)}"`:p)}let o=await l.evaluate(()=>document.body?.innerText||"").catch(()=>"");return n.push(""),n.push(`Page text (${o.length} chars):`),n.push(o.replace(/\n{3,}/g,`

`).slice(0,fa)),n.join(`
`)}async function ba(l,e){let t=0,r=/^@?e(\d+)$/i.exec(e.trim());if(r)t=Number(r[1])-1;else{let n=/^\d+$/.exec(e.trim());if(n)t=Number(n)-1;else throw new Error(`ref \u5FC5\u987B\u662F @eN \u5F62\u5F0F\uFF08\u5982 @e5\uFF09\uFF0C\u6536\u5230 '${e}'\u3002\u5148 browser_snapshot \u83B7\u53D6 ref\u3002`)}return l.locator('a[href], button, input, textarea, select, [role="button"], [role="link"], [role="checkbox"], [role="radio"], [role="tab"], [role="menuitem"], [contenteditable="true"], [tabindex]:not([tabindex="-1"])').nth(t)}async function wd(l){let e=l.toLowerCase().replace(/\.$/,"");if(e==="localhost"||e.endsWith(".localhost"))return!0;if(/^\d+\.\d+\.\d+\.\d+$/.test(e))return ha(e);try{let t=[];try{t.push(...await bi.promises.resolve4(e))}catch{}try{t.push(...await bi.promises.resolve6(e))}catch{}return t.some(ha)}catch{return!1}}function ha(l){if(l==="::1"||l.startsWith("fc")||l.startsWith("fd")||l.startsWith("fe80"))return!0;let e=l.split(".").map(Number);if(e.length!==4)return!1;let[t,r]=e;return t===10||t===127||t===172&&r>=16&&r<=31||t===192&&r===168||t===0||t===169&&r===254}async function Si(l){try{let e=new URL(l);return await wd(e.hostname)}catch{return!0}}async function Sn(l,e){return await Si(l.url())?`browser_${e} \u88AB\u963B\u6B62\uFF1A\u5F53\u524D\u9875\u9762\u662F\u79C1\u6709/\u672C\u5730\u5730\u5740\uFF08${l.url()}\uFF09\uFF0C\u4E0D\u5141\u8BB8\u5728\u79C1\u6709\u9875\u9762\u4E0A\u64CD\u4F5C\u3002`:null}function xd(l){let e=[],t=/https?:\/\/[^\s'"`)\]<>]+/g;for(let r of l.matchAll(t))e.push(r[0].replace(/[.,;]$/,""));return e}var Td=[[/document\s*\.\s*cookie/i,"document.cookie"],[/\b(?:localStorage|sessionStorage)\b/i,"web storage"],[/\bindexedDB\b/i,"IndexedDB"],[/\bcaches\s*\.\s*(?:open|match|keys)\b/i,"Cache Storage"],[/\bnavigator\s*\.\s*(?:clipboard|credentials|serviceWorker)\b/i,"navigator sensitive API"],[/\b(?:fetch|XMLHttpRequest|WebSocket|EventSource|sendBeacon)\s*\(/i,"network request"]];function Ed(){return process.env.KEXVIM_BROWSER_RESTRICT_EVALUATE==="1"}function Cd(l){if(Ed()){for(let[e,t]of Td)if(e.test(l))return`Blocked: browser_console(expression=...) tried to use sensitive browser primitive: ${t} (KEXVIM_BROWSER_RESTRICT_EVALUATE=1).`}return null}function Rd(){let l=I.findProjectRoot(),e=ki.join(l||process.cwd(),"data","media");return ya.mkdirSync(e,{recursive:!0}),e}var cn=class{name="browser_navigate";description="Open a URL in the browser. Returns the page title, final URL and a compact snapshot of interactive elements (refs like [ref=1] for browser_click / browser_type). First call starts a headless browser session for this task.";parameters={type:"object",properties:{url:{type:"string",description:"The URL to navigate to (http/https)."}},required:["url"]};async execute(e,t,r){let s=String(e.url||"").trim();if(!s)return JSON.stringify({success:!1,error:"url is required"});let n=s;if(/^https?:\/\//i.test(n)||(n=`https://${n}`),await Si(n))return JSON.stringify({success:!1,error:`\u5BFC\u822A\u88AB\u963B\u6B62\uFF1A${n} \u662F\u79C1\u6709/\u672C\u5730\u5730\u5740\uFF0C\u6D4F\u89C8\u5668\u4E0D\u5141\u8BB8\u8BBF\u95EE\u79C1\u7F51\u3002`});try{let i=await ee.ensure("default");await i.page.goto(n,{waitUntil:"domcontentloaded",timeout:6e4}),await i.page.waitForTimeout(800);let o=await va(i.page,!1);return JSON.stringify({success:!0,url:i.page.url(),title:await i.page.title(),snapshot:o})}catch(i){return JSON.stringify({success:!1,error:i instanceof Error?i.message:String(i)})}}},dn=class{name="browser_snapshot";description="Get a snapshot of the current page: interactive elements with ref IDs ([ref=1], [ref=2]...) for browser_click and browser_type, plus page text. full=true returns complete page content (may be truncated at ~30k chars). Requires browser_navigate first.";parameters={type:"object",properties:{full:{type:"boolean",default:!1,description:"true = full page text; false (default) = compact view with interactive element refs."}}};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{let n=await va(s.page,e.full===!0);return JSON.stringify({success:!0,snapshot:n})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},un=class{name="browser_click";description="Click an interactive element by ref (from browser_snapshot, e.g. @e5 or 5).";parameters={type:"object",properties:{ref:{type:"string",description:"Element ref like @e5 (or plain number)."}},required:["ref"]};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});let n=await Sn(s.page,"click");if(n)return JSON.stringify({success:!1,error:n});try{let i=await ba(s.page,String(e.ref||""));return await i.scrollIntoViewIfNeeded({timeout:5e3}).catch(()=>{}),await i.click({timeout:1e4}),await s.page.waitForTimeout(500),JSON.stringify({success:!0,message:`Clicked ${e.ref}.`})}catch(i){return JSON.stringify({success:!1,error:i instanceof Error?i.message:String(i)})}}},pn=class{name="browser_type";description="Fill an input element with text by ref (from browser_snapshot, e.g. @e3).";parameters={type:"object",properties:{ref:{type:"string",description:"Element ref like @e3."},text:{type:"string",description:"Text to type into the field."}},required:["ref","text"]};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});let n=await Sn(s.page,"type");if(n)return JSON.stringify({success:!1,error:n});try{return await(await ba(s.page,String(e.ref||""))).fill(String(e.text??""),{timeout:1e4}),JSON.stringify({success:!0,message:`Filled ${e.ref}.`})}catch(i){return JSON.stringify({success:!1,error:i instanceof Error?i.message:String(i)})}}},mn=class{name="browser_scroll";description="Scroll the page. direction: down (default) or up. Scrolls ~500px per call; call repeatedly to page through.";parameters={type:"object",properties:{direction:{type:"string",enum:["down","up"],default:"down"}}};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{let n=e.direction==="up"?-ga:ga;return await s.page.evaluate(i=>window.scrollBy(0,i),n),await s.page.waitForTimeout(300),JSON.stringify({success:!0,message:`Scrolled ${e.direction||"down"} ${Math.abs(n)}px.`})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},gn=class{name="browser_back";description="Go back to the previous page in the browser history.";parameters={type:"object",properties:{}};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{return await s.page.goBack({timeout:3e4}),await s.page.waitForTimeout(500),JSON.stringify({success:!0,url:s.page.url()})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},fn=class{name="browser_press";description="Press a keyboard key in the page (e.g. Enter, Escape, ArrowDown, Tab).";parameters={type:"object",properties:{key:{type:"string",description:"Key name (Enter, Escape, ArrowDown, Tab, ...)."}},required:["key"]};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});let n=await Sn(s.page,"press");if(n)return JSON.stringify({success:!1,error:n});try{return await s.page.keyboard.press(String(e.key||"")),JSON.stringify({success:!0,message:`Pressed ${e.key}.`})}catch(i){return JSON.stringify({success:!1,error:i instanceof Error?i.message:String(i)})}}},hn=class{name="browser_get_images";description="List image URLs and alt texts currently on the page.";parameters={type:"object",properties:{}};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{let n=await s.page.evaluate(()=>Array.from(document.querySelectorAll("img")).slice(0,50).map(i=>({src:i.src.slice(0,300),alt:i.alt.slice(0,100),width:i.naturalWidth,height:i.naturalHeight})).filter(i=>i.src));return JSON.stringify({success:!0,count:n.length,images:n})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},yn=class{name="browser_vision";description="Take a screenshot of the current page so you can inspect it visually. Returns a screenshot path the model can analyze (needs a vision-capable model). Use for CAPTCHAs, visual layouts, or when the text snapshot misses something.";parameters={type:"object",properties:{question:{type:"string",description:"Optional: what to look for (used as analysis prompt)."},annotate:{type:"boolean",default:!1,description:"Reserved for future annotated snapshot; currently ignored."}}};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{let n=ki.join(Rd(),`browser_shot_${Date.now()}.png`);return await s.page.screenshot({path:n,fullPage:!1}),JSON.stringify({type:"vision",path:n,mimeType:"image/png",prompt:String(e.question||"Describe what you see on this page, focusing on layout, text and interactive elements."),url:s.page.url()})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},vn=class l{name="browser_console";description="Read browser console messages (clear=true clears the log), or evaluate a JavaScript expression in the page (expression=...). Evaluation with sensitive primitives (cookies, storage, network) is blocked when KEXVIM_BROWSER_RESTRICT_EVALUATE=1; navigation to private/local addresses is always blocked.";parameters={type:"object",properties:{clear:{type:"boolean",default:!1,description:"Clear the console log (only meaningful without expression)."},expression:{type:"string",description:"Optional JS expression to evaluate in the page (result JSON-serialized)."}}};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});let n=e.expression!==void 0?String(e.expression):void 0;if(n!==void 0&&n.trim()){let o=Cd(n);if(o)return JSON.stringify({success:!1,error:o});let a=await Sn(s.page,"console");if(a)return JSON.stringify({success:!1,error:a});for(let c of xd(n))if(await Si(c))return JSON.stringify({success:!1,error:`browser_console eval \u88AB\u963B\u6B62\uFF1A\u8868\u8FBE\u5F0F\u5305\u542B\u79C1\u6709/\u672C\u5730\u5730\u5740 ${c}\u3002`});try{let c=await s.page.evaluate(d=>{try{return new Function(`return (${d});`)()}catch{return new Function(d)()}},n);return JSON.stringify({success:!0,result:l.serialize(c)})}catch(c){return JSON.stringify({success:!1,error:c instanceof Error?c.message:String(c)})}}let i=ee.consoleState("default");return e.clear===!0?(i.messages=[],i.errors=[],JSON.stringify({success:!0,messages:[],errors:[],cleared:!0})):JSON.stringify({success:!0,messages:i.messages.slice(-50),errors:i.errors.slice(-50)})}static serialize(e){if(e===void 0)return"undefined";try{return JSON.parse(JSON.stringify(e))}catch{return String(e)}}},bn=class{name="browser_cdp";description="Attach the browser session to an existing Chrome/Chromium via its CDP WebSocket URL (e.g. from chrome://inspect, or a remote debugging port). Subsequent browser_* calls operate on that browser. Session is per-task; existing session is replaced.";parameters={type:"object",properties:{url:{type:"string",description:"CDP WebSocket debugger URL (ws://host:port/devtools/browser/...)."}},required:["url"]};async execute(e,t,r){let s=String(e.url||"").trim();if(!/^ws:\/\//i.test(s))return JSON.stringify({success:!1,error:"url \u5FC5\u987B\u662F CDP WebSocket \u5730\u5740\uFF08ws://...\uFF09\uFF0C\u5982 ws://127.0.0.1:9222/devtools/browser/xxx"});try{let n=ee.get("default");if(n)try{await n.browser.close()}catch{}let o=(await ee.ensure("default",s)).context.pages().map(a=>a.url());return JSON.stringify({success:!0,attached:!0,pages:o})}catch(n){return JSON.stringify({success:!1,error:`CDP \u8FDE\u63A5\u5931\u8D25\uFF1A${n instanceof Error?n.message:String(n)}`})}}},kn=class{name="browser_dialog";description="Show dialogs (alert/confirm/prompt) that appeared on the page. Dialogs are auto-accepted and recorded; this tool lists them so you know what the page asked. No arguments.";parameters={type:"object",properties:{}};async execute(e,t,r){let s=ee.dialogs("default");return s.length?JSON.stringify({success:!0,dialogs:s}):JSON.stringify({success:!0,dialogs:[],message:"No dialogs were shown."})}};process.on("exit",()=>{ee.closeAll()});var Md=setInterval(()=>{ee.reapIdle()},6e4);Md.unref();import*as ka from"node:crypto";var _n=class l{static build(e,t,r,s,n,i){let o=ka.randomUUID(),a="";try{a=n.prefetchAll(e,s)}catch(d){console.warn(S.t("memory.tcb_prefetch_failed",{error:d instanceof Error?d.message:String(d)}))}let c=l._findLastUserIndex(r);return{userMessage:e,messages:r,activeSystemPrompt:t,turnId:o,extPrefetchCache:a,currentTurnUserIdx:c}}static _findLastUserIndex(e){for(let t=e.length-1;t>=0;t--)if(e[t].role==="user")return t;return e.length}};var _a=5e3,Sa=new Set(["clarify","delegate_task","memory","read_file","write_file","edit","bash","search_files","terminal","execute_code"]),_i=class{queue=[];_running=!1;submit(e){this.queue.push(e),this._running||this.drain()}drain(){this._running=!0;let e=()=>{if(this.queue.length===0){this._running=!1;return}let t=this.queue.shift();try{t()}catch{}setImmediate(e)};setImmediate(e)}flush(e=_a){return new Promise(t=>{if(this.queue.length===0){t(!0);return}let r=setTimeout(()=>t(!1),e);this.submit(()=>{clearTimeout(r),t(!0)})})}shutdown(){this.queue=[],this._running=!1}};var _r=class l{static sanitizeContext(e){return e.replace(/<memory-context>[\s\S]*?<\/memory-context>/gi,"").replace(/\[System note:\s*The following is recalled memory context,\s*NOT new user input\.\s*Treat as [^\]]*\]\s*/gi,"").replace(/<\/?\s*memory-context\s*>/gi,"")}static buildMemoryContextBlock(e){return!e||!e.trim()?"":`<memory-context>
...
7. IMPORTANT: Always respond in the language the user wrote in. If the user writes in Chinese, respond in Chinese. / \u91CD\u8981\uFF1A\u59CB\u7EC8\u7528\u7528\u6237\u4F7F\u7528\u7684\u8BED\u8A00\u56DE\u590D\u3002\u5982\u679C\u7528\u6237\u5199\u4E2D\u6587\uFF0C\u5C31\u7528\u4E2D\u6587\u56DE\u590D\u3002`}var wt=class{llm;_memory;messages=[];maxTurns=20;systemPrompt;constructor(e,t){this.llm=new Ar(e),this._memory=new Ir(e.memoryPath),this.systemPrompt=t||Od(),this.resetConversation()}get memory(){return this._memory}get llmClient(){return this.llm}async process(e){if(!e||!e.trim())return"";let t=e.trim().toLowerCase();if(t==="/reset"||t==="/new"||t==="/clear")return this.resetConversation(),"\u5BF9\u8BDD\u5DF2\u91CD\u7F6E\u3002";this.messages.push({role:"user",content:e});let r=0;for(;r<this.maxTurns;){r++;let n;try{n=await this.llm.chat(this.messages,or.GUARDIAN_TOOL_DEFINITIONS,4096,.7)}catch(p){let m=`LLM \u8C03\u7528\u5931\u8D25: ${p.message}`;return this.messages.push({role:"assistant",content:m}),m}let a=((n.choices||[{}])[0]||{}).message||{},c=a.content||"",d=a.tool_calls;if(!d||d.length===0){let p=c||"";return this.messages.push({role:"assistant",content:p}),p}let u={role:"assistant",content:c||"",tool_calls:d};this.messages.push(u);for(let p of d){let m=p.function,g=m?.name||"",f={};try{f=JSON.parse(m?.arguments||"{}")}catch{f={}}let h=or.executeGuardianTool(g,f,this._memory);this.messages.push({role:"tool",tool_call_id:p.id,content:h})}if(this.messages.length>40){let p=[this.messages[0],...this.messages.slice(-20)];for(this.messages=p;this.messages.length>1&&this.messages[this.messages.length-1]?.role==="tool";)this.messages.pop();let m=this.messages[this.messages.length-1],g=m?m.tool_calls:void 0;for(m?.role==="assistant"&&Array.isArray(g)&&g.length>0&&this.messages.pop();this.messages.length>2&&this.messages[1]?.role==="tool";)this.messages.splice(1,1)}}let s="\u5DF2\u8FBE\u5230\u6700\u5927\u5DE5\u5177\u8C03\u7528\u8F6E\u6B21\uFF0C\u5DF2\u622A\u65AD\u3002\u8BF7\u63D0\u4E0B\u4E00\u4E2A\u95EE\u9898\u3002";return this.messages.push({role:"assistant",content:s}),s}resetConversation(){this.messages=[{role:"system",content:this.systemPrompt}]}};import{StdioClientTransport as Fd}from"@modelcontextprotocol/sdk/client/stdio.js";import{SSEClientTransport as Pa}from"@modelcontextprotocol/sdk/client/sse.js";var de=class l{static SENSITIVE_PATTERNS=[/token/i,/secret/i,/password/i,/passwd/i,/api[_-]?key/i,/apikey/i,/auth/i,/credential/i,/private[_-]?key/i,/access[_-]?key/i,/secret[_-]?key/i,/session/i,/jwt/i];static SAFE_ENV_VARS=new Set(["PATH","HOME","USER","USERNAME","SHELL","TERM","LANG","LC_ALL","TMPDIR","TEMP","TMP","DISPLAY","XDG_RUNTIME_DIR"]);static buildSafeEnv(e){let t={};for(let r of l.SAFE_ENV_VARS){let s=typeof process<"u"?process.env[r]:void 0;s&&(t[r]=s)}if(e)for(let[r,s]of Object.entries(e))t[r]=s;return t}static sanitizeErrorMessage(e){let t=e;t=t.replace(/Bearer\s+[A-Za-z0-9\-._~+/]+=*/gi,"Bearer ***"),t=t.replace(/(Authorization:\s*)(\S+)/gi,"$1***"),t=t.replace(/([?&](?:api[_-]?key|token|secret|key)=)[^&]+/gi,"$1***"),t=t.replace(/"(token|secret|api_key|apikey|password|credential)"\s*:\s*"[^"]+"/gi,'"$1": "***"');for(let r of l.SENSITIVE_PATTERNS)t=t.replace(new RegExp(`(${r.source}\\s*[:=]\\s*)\\S+`,"gi"),"$1***");return t}static isValidUrl(e){try{let t=new URL(e);return t.protocol==="http:"||t.protocol==="https:"||t.protocol==="ws:"||t.protocol==="wss:"}catch{return!1}}static delay(e){return new Promise(t=>setTimeout(t,e))}static backoffDelay(e,t=2e3,r=3e4){let s=Math.min(t*Math.pow(2,e),r);return s+Math.random()*s*.25}};import*as be from"node:fs";import*as On from"node:path";import*as Ca from"node:http";import*as Ra from"node:crypto";var ji=class{server=null;port=0;pending=new Map;async start(e){if(this.server)return this.callbackUrl(e);this.server=Ca.createServer((r,s)=>this._handle(r,s)),await new Promise((r,s)=>{this.server.once("error",s),this.server.listen(0,e,()=>r())});let t=this.server.address();return this.port=typeof t=="object"&&t?t.port:0,this.callbackUrl(e)}callbackUrl(e){return`http://${e}:${this.port}/callback`}waitForCode(e,t){return new Promise((r,s)=>{let n=setTimeout(()=>{this.pending.delete(e),s(new Error(`OAuth authorization timed out after ${Math.round(t/1e3)}s`))},t);this.pending.set(e,{resolve:r,reject:s,timer:n})})}_handle(e,t){let r=new URL(e.url||"/","http://localhost");if(r.pathname==="/callback"){let s=r.searchParams.get("code"),n=r.searchParams.get("state"),i=r.searchParams.get("error"),o=n?this.pending.get(n):void 0;if(t.writeHead(200,{"Content-Type":"text/html; charset=utf-8"}),i||!s||!o){t.end(`<html><body><h3>OAuth \u6388\u6743\u5931\u8D25${i?`\uFF1A${i}`:"\uFF08\u65E0\u6548\u56DE\u8C03\uFF09"}</h3><p>\u8BF7\u8FD4\u56DE\u804A\u5929\u7A97\u53E3\u67E5\u770B\u9519\u8BEF\u4FE1\u606F\u3002</p></body></html>`),o&&(clearTimeout(o.timer),this.pending.delete(n),o.reject(new Error(i||"OAuth callback missing code")));return}clearTimeout(o.timer),this.pending.delete(n),o.resolve(s),t.end("<html><body><h3>\u2705 OAuth \u6388\u6743\u6210\u529F</h3><p>\u53EF\u4EE5\u5173\u95ED\u6B64\u7A97\u53E3\uFF0C\u8FD4\u56DE\u804A\u5929\u7A97\u53E3\u7EE7\u7EED\u3002</p></body></html>");return}t.writeHead(404),t.end("Not found")}stop(){for(let{timer:e,reject:t}of this.pending.values())clearTimeout(e),t(new Error("OAuth callback server stopped"));if(this.pending.clear(),this.server){try{this.server.close()}catch{}this.server=null}}},xt=class l{static filePath(e){let t=I.findProjectRoot();if(!t)throw new Error("Cannot locate project root (package.json) \u2014 data/mcp_oauth unavailable");return On.join(t,"data","mcp_oauth",`${e.replace(/[^a-zA-Z0-9_-]/g,"_")}.json`)}static load(e){try{let t=l.filePath(e);return be.existsSync(t)?JSON.parse(be.readFileSync(t,"utf-8")):{}}catch(t){return console.warn(`[MCP OAuth] failed to load token store for '${e}': ${t.message}`),{}}}static save(e,t){try{let r=l.filePath(e);be.mkdirSync(On.dirname(r),{recursive:!0}),be.writeFileSync(r,JSON.stringify(t,null,2),"utf-8");try{be.chmodSync(r,384)}catch{}}catch(r){console.warn(`[MCP OAuth] failed to persist token store for '${e}': ${r.message}`)}}static clear(e){try{let t=l.filePath(e);be.existsSync(t)&&be.rmSync(t)}catch(t){console.warn(`[MCP OAuth] failed to clear token store for '${e}': ${t.message}`)}}},$d=600*1e3,Ma=new Map;function Aa(l,e){Ma.set(l,e)}function Ia(l){return Ma.get(l)}var Nn=class{serverName;config;callback;_redirectUrl;_clientInformation;_tokens;_codeVerifier;_pendingState;openBrowser;constructor(e,t={},r){this.serverName=e,this.config=t,this.callback=new ji,this.openBrowser=r?.openBrowser||Dd;let s=xt.load(e);this._clientInformation=s.clientInformation,this._tokens=s.tokens}get redirectUrl(){return this._redirectUrl}get clientMetadata(){let e=this.config.redirectHost||"localhost",t=this._redirectUrl||`http://${e}:0/callback`,r={client_name:this.config.clientName||"Kexvim Agent",redirect_uris:[t],grant_types:["authorization_code","refresh_token"],response_types:["code"],token_endpoint_auth_method:this.config.clientSecret?"client_secret_post":"none"};return this.config.scope&&(r.scope=this.config.scope),r}state(){return this._pendingState=Ra.randomBytes(16).toString("hex"),this._pendingState}clientInformation(){return this._clientInformation}async saveClientInformation(e){this._clientInformation=e,xt.save(this.serverName,{clientInformation:this._clientInformation,tokens:this._tokens})}tokens(){return this._tokens}async saveTokens(e){this._tokens=e,xt.save(this.serverName,{clientInformation:this._clientInformation,tokens:this._tokens})}async redirectToAuthorization(e){let t=this.config.redirectHost||"localhost";this._redirectUrl=await this.callback.start(t),console.log(`[MCP OAuth] '${this.serverName}' \u9700\u8981\u6388\u6743\uFF0C\u8BF7\u5728\u6D4F\u89C8\u5668\u4E2D\u5B8C\u6210\uFF1A${e.toString()}`),this.openBrowser(e.toString())}waitForAuthorizationCode(e=$d){return this._pendingState?this.callback.waitForCode(this._pendingState,e):Promise.reject(new Error("No pending OAuth state \u2014 call redirectToAuthorization first"))}saveCodeVerifier(e){this._codeVerifier=e}codeVerifier(){if(!this._codeVerifier)throw new Error("No code verifier saved");return this._codeVerifier}async invalidateCredentials(e){(e==="all"||e==="tokens")&&(this._tokens=void 0,xt.save(this.serverName,{clientInformation:this._clientInformation})),(e==="all"||e==="client")&&(this._clientInformation=void 0,xt.save(this.serverName,{})),(e==="all"||e==="verifier")&&(this._codeVerifier=void 0)}dispose(){this.callback.stop()}};function Dd(l){let e=process.platform,t;e==="win32"?t=`start "" "${l}"`:e==="darwin"?t=`open "${l}"`:t=`xdg-open "${l}"`,b.runAsync(t).catch(()=>{console.log(`[MCP OAuth] \u8BF7\u5728\u6D4F\u89C8\u5668\u4E2D\u6253\u5F00\u5B8C\u6210\u6388\u6743\uFF1A
${l}`)})}var Ke=class l{static async createTransport(e,t){let r=e.transport||"streamable-http";switch(r){case"stdio":return l._createStdio(e,t);case"sse":case"streamable-http":return l._createSSE(e,t);default:throw new Error(`[MCP:${t}] Unsupported transport: ${r}`)}}static async _createStdio(e,t){if(!e.command)throw new Error(`[MCP:${t}] 'command' is required for stdio transport`);return new Fd({command:e.command,args:e.args??[],env:de.buildSafeEnv(e.env),stderr:"pipe"})}static async _createSSE(e,t){if(!e.url)throw new Error(`[MCP:${t}] 'url' is required for ${e.transport||"streamable-http"} transport`);if(!de.isValidUrl(e.url))throw new Error(`[MCP:${t}] Invalid URL: ${e.url}`);if(e.auth==="oauth"){let s=new Nn(t,e.oauth||{});return Aa(t,s),new Pa(new URL(e.url),{authProvider:s})}let r=e.url;if(e.headers?.Authorization&&!r.includes("?")){let s=e.headers.Authorization.replace(/^Bearer\s+/i,"");r+=(r.includes("?")?"&":"?")+`token=${encodeURIComponent(s)}`}return new Pa(new URL(r))}};var ue={Disconnected:"disconnected",Connecting:"connecting",Connected:"connected",Error:"error"};import{Client as jd}from"@modelcontextprotocol/sdk/client/index.js";import{UnauthorizedError as Ud}from"@modelcontextprotocol/sdk/client/auth.js";function at(l){return typeof l=="object"&&l!==null&&!Array.isArray(l)}function Bd(l){return Array.isArray(l.anyOf)&&l.anyOf.length>0?{key:"anyOf",branches:l.anyOf}:Array.isArray(l.oneOf)&&l.oneOf.length>0?{key:"oneOf",branches:l.oneOf}:null}var Lr=class l{static normalizeSchema(e){if(!e||typeof e!="object")return{type:"object"};let t={},r=Bd(e);for(let[s,n]of Object.entries(e))if(!(s==="$ref"||s==="$schema"||s==="definitions"||s==="$id")&&!(s==="anyOf"||s==="oneOf")){if(s==="title"){typeof n=="string"&&n.length<=64&&!/[{}()$@]/.test(n)&&(t[s]=n);continue}if(s!=="default"){if(at(n)){t[s]=l.normalizeSchema(n);continue}if(Array.isArray(n)&&n.length>0&&at(n[0])){t[s]=n.map(i=>at(i)?l.normalizeSchema(i):i);continue}if(s==="type"){if(Array.isArray(n)){let i=n.filter(o=>o!=="null");t[s]=i.length===1?i[0]:i.length>1?i:"string"}else n==="null"?t[s]="string":t[s]=n;continue}if(s==="additionalProperties"){n==="object"||n===!0?t[s]=!0:n===!1||n===null?t[s]=!1:at(n)?t[s]=l.normalizeSchema(n):t[s]=!0;continue}if(["minimum","maximum","minLength","maxLength","minItems","maxItems"].includes(s)){typeof n=="number"&&(t[s]=n);continue}if(s==="enum"&&Array.isArray(n)){t[s]=n;continue}if(s==="properties"&&at(n)){let i={};for(let[o,a]of Object.entries(n))i[o]=at(a)?l.normalizeSchema(a):a;t[s]=i;continue}if(s==="items"&&at(n)){t[s]=l.normalizeSchema(n);continue}if(s==="description"&&typeof n=="string"){t[s]=n.length>2048?n.slice(0,2048)+"\u2026":n;continue}if(s==="required"&&Array.isArray(n)){t[s]=n.filter(i=>typeof i=="string");continue}t[s]=n}}if(r){let{branches:s,key:n}=r,i=s.filter(o=>at(o)&&o.type!=="null").map(o=>l.normalizeSchema(o));if(i.length===1)for(let[o,a]of Object.entries(i[0]))o in t||(t[o]=a);else i.length>1&&(t[n]=i)}return!t.type&&!t.properties&&!t.$ref&&!t.enum&&!t.anyOf&&!t.oneOf&&t.items&&(t.type="array"),t}static normalizeToolList(e){return e.map(t=>({...t,description:t.description?t.description.length>1024?t.description.slice(0,1024)+"\u2026":t.description:t.name,inputSchema:l.normalizeSchema(t.inputSchema)}))}};var $n=5,Hd=1e4,Nr=class{name;config;client=null;transport=null;_state=ue.Disconnected;_connectedAt=0;_reconnectAttempts=0;_tools=[];_lastError;_keepaliveTimer=null;_disconnectRequested=!1;_stateHandlers=[];_reconnectHandlers=[];constructor(e){this.name=e.name,this.config={timeout:300,connectTimeout:60,keepaliveInterval:180,supportsParallelToolCalls:!1,autoConnect:!0,...e}}get state(){return this._state}get toolCount(){return this._tools.length}get connected(){return this._state===ue.Connected}get supportsParallel(){return this.config.supportsParallelToolCalls??!1}getTools(){return this._tools}async connect(e){if(this._state!==ue.Connected){this._setState(ue.Connecting),this._lastError=void 0,this._disconnectRequested=!1;try{let t={capabilities:{sampling:this.config.sampling?.enabled!==!1?{}:void 0}};this.client=new jd({name:"kexvim-mcp-client",version:"1.0.0"},t),this.transport=e,this.client.onclose=()=>{this._disconnectRequested||this._handleTransportClose()},await this.client.connect(e),this._setState(ue.Connected),this._connectedAt=Date.now(),this._reconnectAttempts=0,await this._discoverTools(),this._startKeepalive()}catch(t){if(this.config.auth==="oauth"&&t instanceof Ud){let r=Ia(this.name);if(r)throw console.log(`[MCP:${this.name}] OAuth \u6388\u6743\u5DF2\u89E6\u53D1\uFF1A\u8BF7\u5728\u6D4F\u89C8\u5668\u4E2D\u5B8C\u6210\u6388\u6743\uFF0C\u5B8C\u6210\u540E\u5C06\u81EA\u52A8\u91CD\u8FDE\u3002`),this._setState(ue.Error),this._lastError="OAuth authorization required \u2014 waiting for browser authorization",this._cleanup(),this._startOAuthReconnect(e,r),new Error(`[MCP:${this.name}] OAuth \u6388\u6743\u4E2D\uFF1A\u8BF7\u5728\u6D4F\u89C8\u5668\u5B8C\u6210\u6388\u6743\uFF08\u89C1\u65E5\u5FD7 URL\uFF09\u3002\u6388\u6743\u5B8C\u6210\u540E\u81EA\u52A8\u91CD\u8FDE\u3002`)}throw this._setState(ue.Error),this._lastError=de.sanitizeErrorMessage(String(t)),this._cleanup(),t}}}async _startOAuthReconnect(e,t){try{let r=await t.waitForAuthorizationCode(),s=e;s?.finishAuth&&await s.finishAuth(r);let n=await Ke.createTransport(this.config,this.name);await this.connect(n),console.log(`[MCP:${this.name}] OAuth \u6388\u6743\u5B8C\u6210\uFF0C\u5DF2\u91CD\u8FDE\u3002`)}catch(r){console.warn(`[MCP:${this.name}] OAuth \u6388\u6743/\u91CD\u8FDE\u5931\u8D25\uFF1A${r.message}`)}}async _discoverTools(){if(this.client)try{let e=await this.client.listTools({},{timeout:(this.config.connectTimeout??60)*1e3});this._tools=(e.tools??[]).map(t=>({name:t.name,description:t.description??t.name,inputSchema:Lr.normalizeSchema(t.inputSchema)}))}catch(e){console.warn(`[MCP:${this.name}] listTools failed:`,String(e)),this._tools=[]}}_startKeepalive(){let e=this.config.keepaliveInterval??180;e<=0||(this._keepaliveTimer=setInterval(async()=>{if(!(this._state!==ue.Connected||!this.client))try{await this.client.ping({timeout:Hd})}catch{console.warn(`[MCP:${this.name}] ping failed, reconnecting...`),this._handleTransportClose()}},e*1e3))}_handleTransportClose(){this._disconnectRequested||(this._setState(ue.Disconnected),this._cleanup(),this._scheduleReconnect().catch(()=>{}))}async _scheduleReconnect(){if(this._disconnectRequested||this._reconnectAttempts>=$n){this._reconnectAttempts>=$n&&(this._setState(ue.Error),this._lastError=`\u91CD\u8FDE\u5931\u8D25\uFF0C\u5DF2\u5C1D\u8BD5 ${$n} \u6B21`);return}this._reconnectAttempts++;let e=de.backoffDelay(this._reconnectAttempts-1);console.log(`[MCP:${this.name}] reconnecting in ${Math.round(e/1e3)}s (attempt ${this._reconnectAttempts}/${$n})...`),await new Promise(t=>setTimeout(t,e)),!this._disconnectRequested&&this._emitReconnectNeeded()}async callTool(e,t,r){if(!this.client)throw new Error(`[MCP:${this.name}] not connected`);let s=(this.config.timeout??300)*1e3;try{let n=await this.client.callTool({name:e,arguments:t},void 0,{timeout:s,signal:r});return this._formatResult(n)}catch(n){let i=n instanceof Error?n.message:String(n);throw new Error(`[MCP:${this.name}:${e}] ${de.sanitizeErrorMessage(i)}`)}}_formatResult(e){if(!e.content||e.content.length===0)return e.isError?"Error: empty response":"(empty result)";let t=[];for(let r of e.content)if(r.type==="text"&&"text"in r)t.push(r.text);else if(r.type==="resource"){let s=r;t.push(s.resource?.text??`[Resource: ${s.resource?.uri??"unknown"}]`)}else t.push(`[${r.type} data]`);return t.join(`
`)||"(empty)"}getStatus(){return{name:this.name,state:this._state,toolCount:this._tools.length,uptime:this._state===ue.Connected&&this._connectedAt>0?Math.floor((Date.now()-this._connectedAt)/1e3):0,lastError:this._lastError}}onStateChange(e){this._stateHandlers.push(e)}onReconnectNeeded(e){this._reconnectHandlers.push(e)}_setState(e){this._state=e;for(let t of this._stateHandlers)try{t(this.getStatus())}catch{}}_emitReconnectNeeded(){for(let e of this._reconnectHandlers)try{e()}catch{}}async disconnect(){this._disconnectRequested=!0,this._cleanup();try{await this.client?.close()}catch{}try{await this.transport?.close()}catch{}this.client=null,this.transport=null,this._tools=[],this._setState(ue.Disconnected)}_cleanup(){this._keepaliveTimer&&(clearInterval(this._keepaliveTimer),this._keepaliveTimer=null)}};var Or=class{tasks=new Map;_bridgeTools=[];register(e){if(this.tasks.has(e.name))throw new Error(`[MCP] Server '${e.name}' already registered`);let t=new Nr(e);t.onStateChange(r=>{r.state===ue.Error&&console.warn(`[MCP:${r.name}] state: ${r.state}${r.lastError?` \u2014 ${r.lastError}`:""}`)}),this.tasks.set(e.name,t)}registerAll(e){for(let t of e)this.register(t)}async connectAll(e=!0){console.log(`[MCP] ${this.tasks.size} servers registered, waiting for connect calls`)}async connect(e,t){let r=this.tasks.get(e);if(!r)throw new Error(`[MCP] Server '${e}' not found`);await r.connect(t)}discoverAndBridge(){let e=[];for(let[t,r]of this.tasks)if(r.connected)for(let s of r.getTools())e.push(new $r(this,t,s));return this._bridgeTools=e,e}discoverAndBridgeServer(e){let t=this.tasks.get(e);if(!t)throw new Error(`[MCP] Server '${e}' not found`);let r=t.getTools().map(s=>new $r(this,e,s));return this._bridgeTools=this._bridgeTools.filter(s=>s.serverName!==e),this._bridgeTools.push(...r),r}getBridgeTools(){return[...this._bridgeTools]}async callTool(e,t,r,s){let n=this.tasks.get(e);if(!n)throw new Error(`[MCP] Server '${e}' not found`);return n.callTool(t,r,s)}getStatus(){return Array.from(this.tasks.values()).map(e=>e.getStatus())}isAllConnected(){for(let e of this.tasks.values())if(!e.connected)return!1;return!0}get serverCount(){return this.tasks.size}async disconnectAll(){await Promise.allSettled(Array.from(this.tasks.values()).map(e=>e.disconnect())),this.tasks.clear(),this._bridgeTools=[]}async disconnect(e){let t=this.tasks.get(e);t&&(await t.disconnect(),this.tasks.delete(e),this._bridgeTools=this._bridgeTools.filter(r=>r.serverName!==e))}},$r=class{constructor(e,t,r){this.manager=e;this.serverName=t,this.toolName=r.name,this.name=`mcp__${t}__${r.name}`,this.description=r.description??`${t}: ${r.name}`,this.parameters=r.inputSchema}manager;name;description;parameters;serverName;toolName;async execute(e,t){return this.manager.callTool(this.serverName,this.toolName,e,t)}toJSON(){return{name:this.name,serverName:this.serverName,toolName:this.toolName,description:this.description,parameters:this.parameters}}};var sv=Ke.createTransport.bind(Ke),nv=de.sanitizeErrorMessage.bind(de),iv=de.buildSafeEnv.bind(de),ov=de.isValidUrl.bind(de);import*as Ot from"node:fs";import*as Xe from"node:path";import{Worker as qd}from"worker_threads";import Ui from"path";import Wd from"fs";import{fileURLToPath as Gd}from"url";var Dr=class{worker;pending=new Map;nextId=0;ready;readyResolve;constructor(e){this.ready=new Promise(i=>{this.readyResolve=i});let t=Ui.dirname(Gd(import.meta.url)),r=Ui.join(t,"StoreWorkerEntry.js"),s=Ui.join(t,"StoreWorkerEntry.ts"),n=Wd.existsSync(r)?r:s;this.worker=new qd(n,{workerData:e}),this.worker.on("message",this.onMessage.bind(this)),this.worker.on("error",i=>console.error(S.t("memory.storeworker_error"),i)),this.worker.on("exit",i=>{i!==0&&console.error(S.t("memory.storeworker_exit",{code:i}));for(let[o,a]of this.pending)a.reject(new Error(`StoreWorker exited with code ${i}`)),this.pending.delete(o)})}async call(e,t){return await this.ready,new Promise((r,s)=>{let n=this.nextId++;this.pending.set(n,{resolve:r,reject:s}),this.worker.postMessage({id:n,type:e,data:t})})}send(e,t){if(!this.worker)return;let r=()=>{try{this.worker.postMessage({id:-1,type:e,data:t})}catch{}};this.ready.then(()=>r())}async shutdown(){try{await this.call("shutdown")}catch{}await new Promise(e=>setImmediate(e)),this.worker.terminate().catch(()=>{}),this.worker=null,this.pending.clear()}createSessionStore(){let e=this;return{findByQuery(t){return e.call("findByQuery",t)},getById(t){return e.call("getById",t)},create(t){return e.call("createSession",{session:t})},update(t){return e.call("updateSession",{update:t})},delete(t){return e.call("deleteSession",t)},listRecent(t,r){return e.call("listRecentSessions",{profile:t,limit:r})},countSessions(){return e.call("countSessions")},appendMessage(t,r,s,n,i){e.call("appendMessage",{sessionId:t,role:r,content:s,metadata:n,parentId:i}).catch(()=>{})},getLastActiveMessageId(t){return e.call("getLastActiveMessageId",t)},getMessagesAsConversation(t,r){return e.call("getMessagesAsConversation",{sessionId:t,limit:r})},switchBranch(t,r){return e.call("switchBranch",{sessionId:t,parentId:r})},appendBranchSummary(t,r,s){e.call("appendBranchSummary",{sessionId:t,parentId:r,summaryText:s}).catch(()=>{})},getMessageTree(t){return e.call("getMessageTree",t)},startTaskNode(t,r,s){return e.call("startTaskNode",{sessionId:t,startMsgId:r,title:s})},completeTaskNode(t,r,s){return e.call("completeTaskNode",{sessionId:t,nodeId:r,status:s})},getTaskTree(t){return e.call("getTaskTree",t)},setTaskLastChild(t,r,s){return e.call("setTaskLastChild",{sessionId:t,taskId:r,childId:s})},setTaskSummary(t,r,s){return e.call("setTaskSummary",{sessionId:t,nodeId:r,summary:s})},saveTaskGraph(t,r){return e.call("saveTaskGraph",{chatId:t,graphJson:r})},getTaskGraph(t){return e.call("getTaskGraph",{chatId:t})},appendSystemNotice(t){return e.call("appendSystemNotice",{text:t})},listSystemNotices(t){return e.call("listSystemNotices",{limit:t})}}}createMemoryStore(){let e=this;return{insert(t){return e.call("insertMemory",{entry:t})},query(t){return e.call("queryMemory",t)},search(t,r=20){return e.call("searchMemory",{text:t,limit:r})},update(t){return e.call("updateMemory",{update:t})},delete(t){return e.call("deleteMemory",t)}}}createEntityStore(){let e=this;return{upsert(t){return e.call("upsertEntity",t)},get(t){return e.call("getEntity",t)},delete(t){return e.call("deleteEntity",t)},list(){return e.call("listEntities",{})}}}onMessage(e){if(e.type==="ready"){this.readyResolve();return}if(e.id<0)return;let t=this.pending.get(e.id);t&&(this.pending.delete(e.id),e.type==="error"?t.reject(new Error(e.error??"Unknown worker error")):t.resolve(e.result))}};import{DatabaseSync as Kd}from"node:sqlite";var zd=`
...
`,ar=class{db;constructor(e){this.db=new Kd(e),this.db.exec("PRAGMA busy_timeout = 15000"),this.db.exec(zd)}async upsert(e){let t=this.db.prepare(`
...
`;return ze.writeFileSync(r,s,"utf-8"),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3}).code!==0?!1:b.runSyncResult("loginctl enable-linger "+dr.userInfo().username,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=jr.join(dr.homedir(),".config","systemd","user",`${e}.service`);return ze.existsSync(t)&&(b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:5e3}),ze.unlinkSync(t),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3})),!0}catch{return!1}}isServiceInstalled(e){let t=jr.join(dr.homedir(),".config","systemd","user",`${e}.service`);return ze.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`systemctl --user is-active ${e}`,{timeoutMs:3e3});return t.code!==0?!1:t.stdout.trim()==="active"}startService(e){return b.runSyncResult(`systemctl --user start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`systemctl --user enable ${e}`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`systemctl --user disable ${e}`,{timeoutMs:5e3}).code===0}};import*as Je from"fs";import*as qn from"os";import*as Tt from"path";var Wn=class{platform="win32";findKexvimPids(){let e=b.runSyncResult(`powershell -Command "Get-Process node | Where-Object { $_.CommandLine -match 'Main\\.ts|dev\\.mjs|kexvim\\.js' } | Select-Object -ExpandProperty Id"`,{timeoutMs:5e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){let r=t?"/F":"";return b.runSyncResult(`taskkill ${r} /PID ${e}`,{timeoutMs:5e3}).code===0}installService(e){if(b.runSyncResult("nssm --version",{timeoutMs:3e3}).code===0&&b.runSyncResult(`nssm install ${e.name} "${e.execPath}" "${e.workDir}"`,{timeoutMs:1e4}).code===0)return!0;try{let r=Tt.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");Je.mkdirSync(r,{recursive:!0});let s=`@echo off
cd /d "${e.workDir}"
set KEXVIM_DIR=${e.workDir}
npx tsx src/Main.ts
`;return Je.writeFileSync(Tt.join(r,`${e.name}.bat`),s,"utf-8"),!0}catch{return!1}}uninstallService(e){if(!(b.runSyncResult(`nssm remove ${e} confirm`,{timeoutMs:1e4}).code===0)){let r=Tt.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup"),s=Tt.join(r,`${e}.bat`);if(Je.existsSync(s))try{Je.unlinkSync(s)}catch{}}return!0}isServiceInstalled(e){if(b.runSyncResult(`nssm status ${e}`,{timeoutMs:3e3}).code===0)return!0;let t=Tt.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");return Je.existsSync(Tt.join(t,`${e}.bat`))}isServiceRunning(e){let t=b.runSyncResult(`powershell -Command "Get-Service ${e} | Select-Object -ExpandProperty Status"`,{timeoutMs:5e3});return t.code!==0?!1:t.stdout.trim().toLowerCase()==="running"}startService(e){return b.runSyncResult(`net start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`net stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`sc config ${e} start=auto`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`sc config ${e} start=disabled`,{timeoutMs:5e3}).code===0}};import*as Ye from"fs";import*as ur from"os";import*as Et from"path";var Gn=class{platform="darwin";findKexvimPids(){let e=b.runSyncResult('pgrep -f "tsx.*Main\\.ts"',{timeoutMs:3e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){try{return process.kill(e,t?"SIGKILL":"SIGTERM"),!0}catch{return!1}}installService(e){try{let t=Et.join(ur.homedir(),"Library","LaunchAgents");Ye.mkdirSync(t,{recursive:!0});let r=Et.join(t,`${e.name}.plist`),n=`<?xml version="1.0" encoding="UTF-8"?>
...
</plist>`;return Ye.writeFileSync(r,n,"utf-8"),b.runSyncResult(`launchctl load ${r}`,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=Et.join(ur.homedir(),"Library","LaunchAgents",`${e}.plist`);return Ye.existsSync(t)&&(b.runSyncResult(`launchctl unload ${t}`,{timeoutMs:5e3}),Ye.unlinkSync(t)),!0}catch{return!1}}isServiceInstalled(e){let t=Et.join(ur.homedir(),"Library","LaunchAgents",`${e}.plist`);return Ye.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`launchctl list local.${e}`,{timeoutMs:3e3});if(t.code!==0)return!1;let r=t.stdout.trim();return r.includes(`local.${e}`)&&!r.startsWith("-")}startService(e){let t=Et.join(ur.homedir(),"Library","LaunchAgents",`${e}.plist`);return b.runSyncResult(`launchctl load ${t}`,{timeoutMs:5e3}).code===0}stopService(e){let t=Et.join(ur.homedir(),"Library","LaunchAgents",`${e}.plist`);return b.runSyncResult(`launchctl unload ${t}`,{timeoutMs:5e3}).code===0}enableService(e){return this.startService(e)}disableService(e){return this.stopService(e)}};function Qd(){switch(typeof process<"u"?process.platform:"linux"){case"win32":return new Wn;case"darwin":return new Gn;default:return new Hn}}var Gi=null;function Ct(){return Gi||(Gi=Qd()),Gi}var ct=class l{static webPort(){let e=Number(process.env.KEXVIM_WEB_PORT);return Number.isFinite(e)&&e>0?e:8787}static findPid(){let e=l.webPort();try{if(process.platform==="win32"){let t=b.runSyncResult(`netstat -ano | findstr :${e} | findstr LISTENING`,{timeoutMs:5e3});for(let r of(t.stdout||"").split(/\r?\n/)){let s=r.trim().match(/LISTENING\s+(\d+)\s*$/);if(s)return Number(s[1])}}else{let r=(b.runSyncResult(`ss -ltnp 'sport = :${e}'`,{timeoutMs:5e3}).stdout||"").match(/pid=(\d+)/);if(r)return Number(r[1])}}catch{}return null}static webEntry(e){let t=pr.join(e,"dist","dev.mjs");if(Ve.existsSync(t))return t;let r=pr.join(e,"kexvim.js");return Ve.existsSync(r)?r:pr.join(e,"src","Main.ts")}static restart(e){let t=l.findPid();if(t!==null){let o=Ct();try{process.platform==="win32"?b.runSyncResult(`taskkill /PID ${t} /F /T`,{timeoutMs:5e3}):o.killProcess(t,!0)}catch{}}let r=l.webEntry(e),s=pr.join(e,"data","log");try{Ve.mkdirSync(s,{recursive:!0})}catch{}let n=null;try{n=Ve.openSync(pr.join(s,"web.log"),"a")}catch{}let i=b.spawn(process.execPath,[r,"web"],{cwd:e,stdio:n!==null?["ignore",n,n]:"ignore",detached:!0});return n!==null&&Ve.closeSync(n),i.unref(),!0}};var dt=class l{static resolveEntry(e){let t=Mt.join(e,"dist","dev.mjs");if(Rt.existsSync(t))return t;let r=Mt.join(e,"kexvim.js");return Rt.existsSync(r)?r:Mt.join(e,"src","Main.ts")}static buildEnv(e){let t={};for(let[n,i]of Object.entries(process.env))i!==void 0&&(t[n]=i);let r=Mt.join(e,"data",".env");if(!Rt.existsSync(r))return t;let s=Rt.readFileSync(r,"utf-8");for(let n of s.split(`
`)){let i=n.trim();if(!i||i.startsWith("#"))continue;let o=i.indexOf("=");if(o===-1)continue;let a=i.slice(0,o).trim(),c=i.slice(o+1).trim();(c.startsWith('"')&&c.endsWith('"')||c.startsWith("'")&&c.endsWith("'"))&&(c=c.slice(1,-1)),a&&!(a in t)&&(t[a]=c)}return t}static spawnFreshDaemon(e,t){let r=t;if(!r){let n=process.argv[1];r=n&&Rt.existsSync(n)?Mt.resolve(n):l.resolveEntry(e)}let s=b.spawn(process.execPath,[r,"--daemon"],{cwd:e,stdio:"ignore",env:l.buildEnv(e),detached:!0});return s.unref(),s.pid??null}static selfRestart(e,t){try{ct.restart(e)}catch(r){t?.(`[kexvim] web \u91CD\u542F\u5931\u8D25: ${r instanceof Error?r.message:String(r)}
`)}return l.spawnFreshDaemon(e,process.argv[1])}};import*as $e from"node:worker_threads";var gr=class l{static workersLaunched=!1;static workerInstances=new Map;static async startWatchdog(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D .stop_watchdog\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let t=mr.join(e,"data",".stop_watchdog");setInterval(()=>{if(Y.existsSync(t)){try{Y.unlinkSync(t)}catch{}process.exit(0)}},3e3),process.on("SIGTERM",()=>{dt.spawnFreshDaemon(e),process.exit(0)}),await new Promise(()=>{})}static async startGuardian(){let e=I.load();S.init(e.language||"zh-CN");let t=new wt(_t.load());console.log(`[guardian] ready \u2014 ${t.llmClient.model}`);let r=$e.parentPort;if(!r){await new Promise(()=>{});return}r.on("message",async s=>{let n=s;if(!(!n||n.type!=="repair"))try{let i=await t.process(String(n.text??""));r.postMessage({type:"repair-result",reqId:n.reqId,text:i})}catch(i){r.postMessage({type:"repair-result",reqId:n.reqId,text:`[guardian error] ${i.message}`})}})}static terminateWorkers(){for(let e of l.workerInstances.values())e.terminate();l.workerInstances.clear()}static launchConsoleWorkers(){for(let t of["watchdog","agent"]){let r=new $e.Worker(process.argv[1],{workerData:{role:t},stdout:!0,stderr:!0});r.stdout.on("data",s=>process.stdout.write(`[${t[0]}] ${s}`)),r.stderr.on("data",s=>process.stderr.write(`[${t[0]}] ${s}`)),r.on("exit",s=>console.log(`[${t}] exited (${s})`)),l.workerInstances.set(t,r)}let e=()=>{l.terminateWorkers()};process.on("SIGTERM",e),process.on("SIGINT",e),process.on("exit",e)}static launchDaemon(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u542F\u52A8 daemon\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");if(ke.recordBoot(e))try{Y.appendFileSync(l.resolveLogPath(),`[kexvim] \u26A0\uFE0F restart_loop guard TRIPPED: ${ke.bootCount(e)} boots in ${ke.WINDOW_SECONDS}s. \u81EA\u52A8\u62C9\u8D77\u6682\u505C\uFF0C\u8BF7\u4EBA\u5DE5\u68C0\u67E5\uFF08kexvim status / kexvim clear-loop\uFF09
`)}catch{}try{Y.writeFileSync(mr.join(e,"data","kexvim.pid"),String(process.pid))}catch{}process.on("exit",()=>{try{Y.rmSync(mr.join(e,"data","kexvim.pid"),{force:!0})}catch{}});let r=()=>{l._shuttingDown=!0,l.terminateWorkers(),process.exit(0)};process.on("SIGTERM",r),process.on("SIGINT",r);let s=()=>{if(process.platform!=="linux")return!1;try{let i=Y.readFileSync("/proc/self/environ","utf-8");return i.includes("INVOCATION_ID")||i.includes("SYSTEMD_EXEC_PID")}catch{return!1}};process.platform!=="win32"&&process.on("SIGHUP",()=>{try{Y.appendFileSync(l.resolveLogPath(),`[kexvim] SIGHUP received \u2014 ${s()?"systemd-managed, exiting for systemd restart":"standalone, spawning fresh daemon"}
`)}catch{}s()||dt.selfRestart(e,i=>{try{Y.appendFileSync(l.resolveLogPath(),i)}catch{}}),r()});let n=l.resolveLogPath();for(let i of["watchdog","agent"]){let o=new $e.Worker(process.argv[1],{workerData:{role:i},stdout:!0,stderr:!0});l.workerInstances.set(i,o);let a=u=>{try{Y.appendFileSync(n,u)}catch{}};o.stdout.on("data",a),o.stderr.on("data",a);let c=()=>{let u=new $e.Worker(process.argv[1],{workerData:{role:i},stdout:!0,stderr:!0});l.workerInstances.set(i,u),u.stdout.on("data",a),u.stderr.on("data",a),u.on("error",p=>{try{Y.appendFileSync(n,`[kexvim] worker '${i}' error: ${p.message}
`)}catch{}}),u.on("exit",d)},d=u=>{l.workerInstances.delete(i);let p=u===0||u===null;try{Y.appendFileSync(n,`[kexvim] worker '${i}' exited (code=${u}) ${p?"":"\u2014 CRASH, restarting"} at ${new Date().toISOString()}
`)}catch{}!p&&!l._shuttingDown&&setTimeout(c,2e3)};o.on("error",u=>{try{Y.appendFileSync(n,`[kexvim] worker '${i}' error: ${u.message}
`)}catch{}}),o.on("exit",d)}}static resolveLogPath(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D kexvim.log\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let t=mr.join(e,"data","log");try{Y.mkdirSync(t,{recursive:!0})}catch{}return mr.join(t,"kexvim.log")}static _shuttingDown=!1;static async handleWorkerDispatch(){if(!$e.isMainThread){let t=$e.workerData?.role||"agent";return t==="watchdog"?(await l.startWatchdog(),!0):t==="guardian"?(await l.startGuardian(),!0):!1}return process.argv.includes("--daemon")?(l.launchDaemon(),!0):(I.load().platform?.enabled&&!process.argv.slice(2).includes("web")&&(l.launchConsoleWorkers(),l.workersLaunched=!0),!1)}};import*as pe from"node:fs";import*as Vr from"node:path";import*as ol from"node:worker_threads";var Zd=new Set(["stop","new","newchat","reset","\u65B0\u4F1A\u8BDD","\u5207\u6362\u4F1A\u8BDD","\u5207\u4F1A\u8BDD"]);function Oa(l,e){let t=process.env[l];if(t===void 0||t.trim()==="")return e;let r=parseFloat(t);return Number.isFinite(r)?r:e}function eu(l,e){let t=process.env[l];return t===void 0||t.trim()===""?e:["1","true","yes","on"].includes(t.trim().toLowerCase())}function tu(l,e){let t=process.env[l]?.trim().toLowerCase();return t==="steer"||t==="queue"||t==="interrupt"?t:e}var fr=class l{adapters=new Map;config;messageHandler=null;activeSessions=new Map;pendingMessages=new Map;debounceTimers=new Map;debounceStates=new Map;_lastBusyNotifyTime=new Map;_sessionControllers=new Map;static _BUSY_COOLDOWN=30;static splitNatural(e,t=1500){if(!e||e.length<=t)return[e];let r=[],s="",n=()=>{s.trim()&&(r.push(s.trim()),s="")},i=e.split(/\n+/).filter(o=>o.length>0);for(let o of i)if(o.length>t){n();let a=o.match(/[^。！？；.!?;]+[。！？；.!?;]?/g)||[o],c="";for(let d of a){if(d.length>t){c&&(r.push(c.trim()),c="");for(let u=0;u<d.length;u+=t)r.push(d.slice(u,u+t));continue}c&&(c+d).length>t?(r.push(c.trim()),c=d):c+=d}c.trim()&&r.push(c.trim())}else s&&(s+`
`+o).length>t?(n(),s=o):s=s?s+`
`+o:o;return n(),r.filter(o=>o.length>0)}_busyStateProvider=null;_steerProvider=null;_redirectProvider=null;constructor(e={}){this.config={timeout:e.timeout??6e4,startupWait:e.startupWait??5e3,busyTextDebounceSeconds:e.busyTextDebounceSeconds??Oa("HERMES_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS",.35),busyTextHardCapSeconds:e.busyTextHardCapSeconds??Oa("HERMES_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS",1),busyAckEnabled:e.busyAckEnabled??eu("HERMES_GATEWAY_BUSY_ACK_ENABLED",!0),steerAckEnabled:e.steerAckEnabled??!0,busyAckDetail:e.busyAckDetail??!0,busyInputMode:e.busyInputMode??tu("HERMES_GATEWAY_BUSY_INPUT_MODE","interrupt"),busyTextMode:e.busyTextMode??(process.env.HERMES_GATEWAY_BUSY_TEXT_MODE?.trim().toLowerCase()==="queue"?"queue":"interrupt")}}getSignalForMessage(e){return this._sessionControllers.get(this.getSessionKey(e))?.signal}register(e){if(this.adapters.has(e.name))throw new Error(`Adapter '${e.name}' already registered`);this.adapters.set(e.name,e)}async unregister(e){let t=this.adapters.get(e);t&&(await t.stop(),this.adapters.delete(e))}setMessageHandler(e,t){this.messageHandler=e,this._busyStateProvider=t?.busyStateProvider??null,this._steerProvider=t?.steerProvider??null,this._redirectProvider=t?.redirectProvider??null,this.adapters.forEach(r=>{r.setMessageHandler(this.dispatch)})}getSessionKey(e){return e.groupId?`${e.groupId}:${e.userId}`:e.userId}toRuntimeSessionKey(e){return e.source?`${e.source.platform}:${e.source.chatId}`:e.groupId?`qq:group:${e.groupId}`:`qq:${e.userId.replace(/^user:/,"")}`}getSenderKey(e){return e.groupId?e.userId:""}releaseSessionGuard(e){this.activeSessions.has(e)&&this.activeSessions.delete(e)}isBypassCommand(e){if(!e.trim().startsWith("/"))return null;let t=e.trim().slice(1).split(" ")[0]?.toLowerCase();return t&&Zd.has(t)?t:null}static _BUSY_QUEUE_MAX_PENDING=32;queueOrReplacePendingEvent(e,t){let r=this.pendingMessages.get(e);if(r||(r=[],this.pendingMessages.set(e,r)),(!!t.media_urls?.length||t.message_type==="photo")&&r.length>0){let n=r[0];n.text&&t.text?n.text=n.text+`
...
===== \u5206\u652F\u5BF9\u8BDD\u5185\u5BB9\u7ED3\u675F =====`}static async compact(e,t,r){let s=e.getMessagesAsConversation(t,200);if(s.length<10)return null;let n=Math.min(40,Math.floor(s.length*.4)),i=s.length-n,o=s.slice(0,i);if(o.length===0)return null;let a=o.reduce((d,u)=>d+l.estimateTokens(u),0);if(a<500)return null;let c;return r?c=await r(o,void 0):c=`\u538B\u5B9E\u4E86 ${o.length} \u6761\u6D88\u606F\uFF0C\u8282\u7701\u7EA6 ${a} tokens\u3002`,{summary:c,firstKeptEntryId:i,tokensBefore:a,estimatedTokensAfter:a}}};var Yu={qq:l=>new Ur({appId:l.app_id,clientSecret:l.client_secret,apiBase:l.api_base,markdownSupport:l.markdown_support}),api_server:l=>new Hr({port:l.port,host:l.host,apiKey:l.api_key}),telegram:l=>new qr({token:l.token,pollTimeout:l.poll_timeout,groupMentionOnly:l.group_mention_only}),discord:l=>new Wr({token:l.token,groupMentionOnly:l.group_mention_only}),weixin:l=>new Gr({token:l.token,accountId:l.account_id,groupMentionOnly:l.group_mention_only}),dingtalk:l=>new Kr({clientId:l.client_id,clientSecret:l.client_secret,groupMentionOnly:l.group_mention_only}),feishu:l=>new zr({appId:l.app_id,appSecret:l.app_secret,groupMentionOnly:l.group_mention_only}),ws:l=>new Yr({port:l.port,host:l.host,apiKey:l.api_key})};function nl(l,e){let t=[];for(let[r,s]of Object.entries(e)){let n=Yu[r];if(!n){console.warn(S.t("gateway.unknown_adapter",{name:r}));continue}if(r==="qq"){let o=s;if(!o.app_id)throw new Error("QQ Bot app_id \u672A\u914D\u7F6E\uFF08config.yaml \u4E2D platform.adapters.qq.app_id\uFF0C\u6CE8\u518C\uFF1Ahttps://q.qq.com\uFF09");if(!o.client_secret)throw new Error("QQ Bot client_secret \u672A\u914D\u7F6E\uFF08config.yaml \u4E2D platform.adapters.qq.client_secret\uFF0C\u6CE8\u518C\uFF1Ahttps://q.qq.com\uFF09")}let i=n(s);l.register(i),t.push(i),console.error(S.t("gateway.adapter_registered",{name:r}))}return t}function Yi(){let l=I.findProjectRoot();if(!l)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D .last_user\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");return Vr.join(l,"data",".last_user")}var Kn=null,Vu=0,Nt=new Map;function Xu(){if(Kn)return Kn;let l=new ol.Worker(process.argv[1],{workerData:{role:"guardian"},stdout:!0,stderr:!0});return l.stdout.on("data",e=>{try{console.error(`[guardian] ${e}`)}catch{}}),l.stderr.on("data",e=>{try{console.error(`[guardian] ${e}`)}catch{}}),l.on("message",e=>{let t=e;if(!t||t.type!=="repair-result")return;let r=Nt.get(t.reqId);r&&(clearTimeout(r.timer),Nt.delete(t.reqId),r.resolve(String(t.text??"")))}),l.on("exit",e=>{Kn=null;for(let[,t]of Nt)clearTimeout(t.timer),t.resolve(`[guardian] \u7EBF\u7A0B\u5DF2\u9000\u51FA (code=${e})`);Nt.clear()}),l.on("error",e=>{try{console.error(`[guardian] worker error: ${e.message}`)}catch{}}),Kn=l,l}function Qu(l,e=18e4){let t=Xu(),r=++Vu;return new Promise(s=>{let n=setTimeout(()=>{Nt.delete(r),s(`[guardian] \u8D85\u65F6\u65E0\u54CD\u5E94\uFF08${Math.round(e/1e3)}s\uFF09\uFF0C\u7EBF\u7A0B\u53EF\u80FD\u4ECD\u5728\u5904\u7406`)},e);Nt.set(r,{resolve:s,timer:n});try{t.postMessage({type:"repair",reqId:r,text:l})}catch(i){clearTimeout(n),Nt.delete(r),s(`[guardian] \u53D1\u9001\u5931\u8D25: ${i.message}`)}})}function il(l,e){let t=(()=>{try{return pe.readFileSync(e,"utf-8").trim()||null}catch{return null}})();if(!t)return;let r=l.find(c=>c.name!=="api_server");if(!r)return;let s=()=>r.sendText(t,"\u2705 Kexvim \u5DF2\u91CD\u65B0\u4E0A\u7EBF").catch(()=>{}),n=100,i=15e3,o=Date.now(),a=()=>{r.isConnected()||Date.now()-o>i?s():(n=Math.min(n*1.5,1e3),setTimeout(a,n))};a()}var zn=class l{static createGatewayHandler(e){let{runtime:t,sessionStore:r,registry:s,signalProvider:n,sendTyping:i}=e;return async o=>{try{console.error(S.t("gateway.msg_in",{userId:o.userId,text:o.text.slice(0,100)}));let a=Yi();if(!(o.source?o.source.chatType==="group":o.userId.startsWith("group:")))try{pe.writeFileSync(a,o.userId,"utf-8")}catch{}let d=o.text.trim(),u=d.toLowerCase();if(u.startsWith("repair/")||u.startsWith("\u4FEE\u590D/")){let C=(await Qu(d))?.trim();return C&&console.error(S.t("gateway.msg_out",{userId:o.userId,text:C.slice(0,100)})),C}let p="qq",m="c2c",g=o.userId,f,h=o.userId;if(o.source)p=o.source.platform,m=o.source.chatType,g=o.source.chatId,f=o.source.guildId??o.groupId,h=o.source.userId??o.userId;else{let A=o.userId.split(":");A.length>=2&&A[0]==="group"?(m="group",f=A[1],g=`group:${A[1]}`):A.length>=2&&A[0]==="user"&&(g=A[1]),h=A.length>=2?A[1]:o.userId}if(new Set(["/new","/newchat","/\u65B0\u4F1A\u8BDD","/\u5207\u6362\u4F1A\u8BDD","/\u5207\u4F1A\u8BDD","\u5207\u4F1A\u8BDD"]).has(d))try{let{sessionId:A,prevSessionId:C}=await t.startNewSession({source:p,chatType:m,chatId:g,userId:h}),P=C?`\u65E7\u4F1A\u8BDD\uFF08${C.slice(0,8)}\uFF09\u5DF2\u4FDD\u7559\uFF0C\u53EF\u968F\u65F6\u56DE\u6EAF\u3002`:"";return`\u2705 \u5DF2\u5207\u6362\u5230\u65B0\u4F1A\u8BDD\uFF08${A.slice(0,8)}\uFF09\u3002${P}`}catch(A){return`\u274C \u5207\u4F1A\u8BDD\u5931\u8D25: ${A.message}`}let y=/^\/resume\s+([A-Za-z0-9-]{8,})/;if(d==="/sessions"||d==="/\u4F1A\u8BDD\u5217\u8868"||y.test(d))try{let A=r.profile??"default";if(d==="/sessions"||d==="/\u4F1A\u8BDD\u5217\u8868"){let F=await r.listRecent(A,100);if(F.length===0)return"\u6682\u65E0\u5386\u53F2\u4F1A\u8BDD\u3002\u53D1\u9001 /new \u5F00\u59CB\u65B0\u4F1A\u8BDD\u3002";let re="";try{let me=I.findProjectRoot(),pt=me?Vr.join(me,"data",".current_session"):"";pt&&pe.existsSync(pt)&&(re=pe.readFileSync(pt,"utf-8").trim())}catch{}return`\u6700\u8FD1\u4F1A\u8BDD\uFF1A
${F.map((me,pt)=>{let ro=me.id===re?" \u25C0 \u5F53\u524D":"";return`${pt+1}. ${me.id.slice(0,8)} - ${Ze.displayTitle(me.summary,me.firstUserMsg)}  [${me.source}]${ro}`}).join(`
`)}

\u7528 /resume <\u524D8\u4F4DID> \u6062\u590D\u3002`}let C=d.match(y),P=await t.resumeSession(C[1],{source:p,chatType:m,chatId:g,userId:h});return P?`\u2705 \u5DF2\u6062\u590D\u4F1A\u8BDD ${P.sessionId.slice(0,8)}\uFF08${P.messageCount} \u6761\u6D88\u606F\uFF09\uFF0C\u7EE7\u7EED\u5BF9\u8BDD\u3002`:`\u274C \u627E\u4E0D\u5230\u4F1A\u8BDD ${C[1]}\u3002\u7528 /sessions \u67E5\u770B\u53EF\u6062\u590D\u7684\u4F1A\u8BDD\u3002`}catch(A){return`\u274C \u4F1A\u8BDD\u64CD\u4F5C\u5931\u8D25: ${A.message}`}try{let A=I.findProjectRoot(),C=A?Vr.join(A,"data","session-switch.json"):null;if(C&&pe.existsSync(C)){let P=JSON.parse(pe.readFileSync(C,"utf-8"));if(pe.rmSync(C,{force:!0}),P.sessionId){let F=await t.resumeSession(P.sessionId,{source:p,chatType:m,chatId:g,userId:h});return F?(console.error(`[CLI\u5207\u6362] \u5DF2\u6062\u590D\u4F1A\u8BDD ${F.sessionId.slice(0,8)}\uFF08${F.messageCount} \u6761\u6D88\u606F\uFF09`),`\u2705 \u5DF2\u5207\u6362\u5230\u5386\u53F2\u4F1A\u8BDD ${F.sessionId.slice(0,8)}\uFF08${F.messageCount} \u6761\u6D88\u606F\uFF09\uFF0C\u7EE7\u7EED\u5BF9\u8BDD\u3002`):`\u274C \u627E\u4E0D\u5230\u4F1A\u8BDD ${P.sessionId.slice(0,8)}\u3002`}}}catch(A){console.error(`[CLI\u5207\u6362] \u6807\u8BB0\u5904\u7406\u5931\u8D25: ${A.message}`);try{pe.rmSync(Vr.join(I.findProjectRoot()??".","data","session-switch.json"),{force:!0})}catch{}}let v=r.recover({chatId:g,chatType:m,source:p,userId:h}).session?.id,w=o.sendReply?A=>{o.sendReply(A).catch(()=>{})}:void 0;try{i&&i(o.userId)}catch{}let R=n?.(o),_=Number(process.env.HERMES_AGENT_NOTIFY_INTERVAL||180),M=_>0?_*1e3:0,L=null,he=()=>{L&&(clearInterval(L),L=null)};M>0&&w&&(L=setInterval(()=>{let A=t.getActivitySummary();if(!A)return;let C=Math.floor((Date.now()-A.startTime)/6e4);w(S.t("gateway.long_running",{elapsed:C,i:A.iteration,max:A.maxIterations,tool:A.currentTool?` \u2014 ${A.currentTool}`:""}))},M));let O;try{O=await t.chat(d,{source:p,chatType:m,chatId:g,userId:h,signal:R,statusCallback:w})}finally{he()}let K=O.content?.trim();if(!K)return;if(console.error(S.t("gateway.msg_out",{userId:o.userId,text:K.slice(0,100)})),O.autoResetNotice)try{await o.sendReply?.(O.autoResetNotice)}catch(A){console.warn(`[SessionReset] notice send failed: ${A?.message}`)}if(v)try{let A=r.buildContext(v);if(A.length>20&&ut.shouldCompact(A.reduce((C,P)=>C+(typeof P.content=="string"?P.content.length/4:0),0),128e3)){let C=async(P,F)=>{try{let re=F?ut.buildUpdateCompactionPrompt(P,F):ut.buildCompactionPrompt(P);return(await s.resolve(e.compactorProvider||"deepseek",e.compactorModel||"deepseek-chat").chat([{role:"system",content:ut.SUMMARIZATION_SYSTEM_PROMPT},{role:"user",content:re}],void 0,50,.3))?.choices?.[0]?.message?.content?.trim()||"\uFF08\u81EA\u52A8\u538B\u5B9E\u6458\u8981\uFF09"}catch{return"\uFF08\u81EA\u52A8\u538B\u5B9E\u6458\u8981\uFF09"}};ut.compact(r,v,C).catch(()=>{})}}catch{}return K}catch(a){let c=a.message,d=/401|403|unauthorized|forbidden|auth/i.test(c)&&!/429|rate/i.test(c),u=/429|rate.?limit|too many/i.test(c),p=/ENOTFOUND|ECONNREFUSED|ECONNRESET|ETIMEDOUT|fetch failed|socket/i.test(c),m=/5\d{2}|api.*error|server.*error|internal|timeout/i.test(c)&&!d&&!u&&!p,g;d?g="gateway.error_auth":u?g="gateway.error_rate_limit":p?g="gateway.error_network":m?g="gateway.error_api":g="gateway.error_unknown";let f=S.t(g,g==="gateway.error_api"||g==="gateway.error_unknown"?{msg:c}:{});return console.error(S.t("gateway.msg_error",{msg:c})),f}}}static installCronDeliverer(e,t){ne.instance.setDeliverer(async(r,s)=>{try{if(s==="all"){let n=t();if(!n)return;for(let i of e)if(i.name!=="api_server"){await i.sendText(n,r);break}}else{let n=s.indexOf(":");if(n<0)return;let i=s.slice(0,n),o=s.slice(n+1),a=e.find(c=>c.name===i);a&&await a.sendText(o,r)}}catch(n){console.error(`[CronScheduler] \u6295\u9012\u5931\u8D25 (${s}):`,n?.message)}})}static async startGatewayMode(e,t,r,s){let n=new fr,i=t.platform,o=Yi(),a=null;try{pe.existsSync(o)&&(a=pe.readFileSync(o,"utf-8").trim()||null)}catch{}let c=i.adapters?nl(n,i.adapters):[],d=c.find(g=>g.name==="qq");ve.notifyHandler=g=>{if(a){for(let f of c)if(f.name!=="api_server"){f.sendText(a,g).catch(()=>{});break}}try{r.appendSystemNotice(g)}catch{}},l.installCronDeliverer(c,()=>a);let u=c.find(g=>typeof g.sendTyping=="function"),p=l.createGatewayHandler({runtime:e,sessionStore:r,registry:s,compactorProvider:t.llm?.defaultProvider,compactorModel:t.llm?.defaultModel,signalProvider:g=>n.getSignalForMessage(g),sendTyping:u?g=>{u.sendTyping(g).catch(()=>{})}:void 0});n.setMessageHandler(p,{busyStateProvider:g=>e.getBusyState(g),steerProvider:(g,f)=>e.steer(f),redirectProvider:(g,f)=>e.redirect(f)}),console.error(S.t("gateway.starting"));let m=n.start();il(c,o),await Promise.race([m,new Promise(g=>{let f=async()=>{console.error(S.t("gateway.shutting_down")),await n.stop(),g()};process.on("SIGINT",f),process.on("SIGTERM",f)})])}static async startGatewayWorker(e,t,r,s){let n=new fr,i=t.platform,o=Yi(),a=i.adapters?nl(n,i.adapters):[];ve.notifyHandler=p=>{let m=(()=>{try{return pe.readFileSync(o,"utf-8").trim()||null}catch{return null}})();if(m){for(let g of a)if(g.name!=="api_server"){g.sendText(m,p).catch(()=>{});break}}},l.installCronDeliverer(a,()=>{try{return pe.readFileSync(o,"utf-8").trim()||null}catch{return null}});let c=a.find(p=>typeof p.sendTyping=="function"),d=l.createGatewayHandler({runtime:e,sessionStore:r,registry:s,compactorProvider:t.llm?.defaultProvider,compactorModel:t.llm?.defaultModel,sendTyping:c?p=>{c.sendTyping(p).catch(()=>{})}:void 0});n.setMessageHandler(d,{busyStateProvider:p=>e.getBusyState(p),steerProvider:(p,m)=>e.steer(m),redirectProvider:(p,m)=>e.redirect(m)});let u=n.start();il(a,o),await u}};import*as cl from"node:http";import*as U from"node:fs";import*as V from"node:path";import*as dl from"node:crypto";import{WebSocketServer as Zu,WebSocket as ll}from"ws";var al=`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
platform:
  ...
  adapters:
    qq:
      app_id: ""
      client_secret: ""
`;function ml(l){let e=process.platform==="win32",t=te.join(l,e?"kexvim.cmd":"kexvim");B.existsSync(t)||(e?B.writeFileSync(t,`@echo off\r
if exist "%~dp0dist\\dev.mjs" (\r
  node "%~dp0dist\\dev.mjs" %*\r
) else (\r
  node "%~dp0dist\\kexvim.mjs" %*\r
)\r
`):(B.writeFileSync(t,`#!/bin/sh
exec node "$(dirname "$0")/kexvim.js" "$@"
`),B.chmodSync(t,493)));let r=e?null:[te.join($t.homedir(),".bashrc"),te.join($t.homedir(),".zshrc"),te.join($t.homedir(),".profile")].find(s=>B.existsSync(s));if(r)B.readFileSync(r,"utf-8").includes(l)?console.log("[~] PATH \u5DF2\u914D\u7F6E"):(B.appendFileSync(r,`
export PATH="$PATH:${l}"
`),console.log(`[~] \u5DF2\u5C06 ${l} \u52A0\u5165 ${te.basename(r)}\uFF0C\u91CD\u542F\u7EC8\u7AEF\u540E\u751F\u6548`));else if(e)try{let n=`$h = ${"'"+l.replace(/'/g,"''")+"'"}; $p = [Environment]::GetEnvironmentVariable('PATH','User'); if ($p -and $p.Contains($h)) { Write-Output 'exists' } elseif ($p) { [Environment]::SetEnvironmentVariable('PATH', $p + ';' + $h, 'User'); Write-Output 'added' } else { [Environment]::SetEnvironmentVariable('PATH', $h, 'User'); Write-Output 'added' }`,i=b.runFileSync("powershell",["-NoProfile","-Command",n],{maxBuffer:1024*1024});console.log(i.includes("exists")?"[~] PATH \u5DF2\u914D\u7F6E":"[~] \u5DF2\u5C06 Kexvim \u52A0\u5165 PATH\uFF0C\u65B0\u7EC8\u7AEF\u751F\u6548")}catch{console.log("[~] \u672A\u80FD\u81EA\u52A8\u8BBE\u7F6E PATH\uFF0C\u8BF7\u624B\u52A8\u5C06\u542F\u52A8\u5668\u76EE\u5F55\u52A0\u5165\u73AF\u5883\u53D8\u91CF PATH")}else console.log(`[~] \u624B\u52A8\u5C06\u4EE5\u4E0B\u884C\u52A0\u5165 shell \u914D\u7F6E\u6587\u4EF6 (~/.bashrc / ~/.zshrc):
    export PATH="$PATH:${l}"`)}async function hl(l,e){let t=i=>{try{return B.accessSync(i),!0}catch{return!1}};if(t(te.join(l,"data",".env"))){console.log("Kexvim \u5DF2\u521D\u59CB\u5316(data/.env \u5DF2\u5B58\u5728)\u3002"),ml(l);return}if(t(te.join(l,"kexvim.js")))console.log("[~] kexvim.js \u5DF2\u5B58\u5728\uFF0C\u8DF3\u8FC7\u4E0B\u8F7D");else if(B.mkdirSync(l,{recursive:!0}),gl("git")){console.log("[~] \u514B\u9686\u4ED3\u5E93...");try{b.runFileSync("git",["clone","--depth","1",e,l],{timeoutMs:12e4})}catch{console.error("[\u2717] \u514B\u9686\u5931\u8D25"),process.exit(1)}}else{console.log("[~] \u4E0B\u8F7D kexvim.js...");let i=te.join($t.tmpdir(),"kexvim-init.js"),o=`${e}/raw/main/kexvim.js`;try{gl("wget")?b.runFileSync("wget",["-q","-O",i,o],{timeoutMs:6e4}):b.runFileSync("curl",["-fsSL","-o",i,o],{timeoutMs:6e4})}catch{console.error("[\u2717] \u4E0B\u8F7D\u5931\u8D25"),process.exit(1)}t(i)&&(B.copyFileSync(i,te.join(l,"kexvim.js")),B.rmSync(i,{force:!0}))}let r=te.join(l,"data",".env");if(B.mkdirSync(te.join(l,"data"),{recursive:!0}),!t(r)){let i=fl.createInterface({input:process.stdin,output:process.stdout}),o=await new Promise(a=>{i.question("\u8BF7\u8F93\u5165 DeepSeek API Key: ",a)});i.close(),o&&(B.writeFileSync(r,`DEEPSEEK_API_KEY=${o}
KEXVIM_HOME=${l}
`),console.log("[\u2713] API Key \u5DF2\u4FDD\u5B58"))}let s=te.join(l,"data","config.yaml");t(s)||(B.writeFileSync(s,ep),console.log("[\u2713] config.yaml \u5DF2\u521B\u5EFA\uFF08\u8BF7\u6309\u9700\u8865\u5145\u914D\u7F6E\uFF09"));let n=te.join(l,"skills");if(B.existsSync(n))console.log("[~] skills \u5DF2\u5B58\u5728\uFF0C\u8DF3\u8FC7");else{console.log("[~] \u4E0B\u8F7D\u516C\u5171\u6280\u80FD...");let i=B.mkdtempSync(te.join($t.tmpdir(),"kexvim-skills-"));try{b.runSync(`git clone --depth 1 ${e}.git "${i}" --single-branch 2>/dev/null`,{timeoutMs:6e4}),B.existsSync(te.join(i,"skills"))?(B.cpSync(te.join(i,"skills"),n,{recursive:!0}),console.log("[\u2713] \u6280\u80FD\u5DF2\u4E0B\u8F7D")):console.warn("[~] \u8FDC\u7A0B\u4ED3\u5E93\u4E2D\u6CA1\u6709 skills \u76EE\u5F55\uFF0C\u8DF3\u8FC7")}catch{console.warn("[~] \u6280\u80FD\u4E0B\u8F7D\u5931\u8D25\uFF08git \u672A\u5B89\u88C5\u6216\u65E0\u7F51\u7EDC\uFF09\uFF0C\u53EF\u624B\u52A8 clone \u540E\u590D\u5236 skills/ \u76EE\u5F55")}finally{B.rmSync(i,{recursive:!0,force:!0})}}ml(l),console.log("[\u2713] \u5B89\u88C5\u5B8C\u6210\u3002\u73B0\u5728\u53EF\u4EE5\u76F4\u63A5\u5728\u7EC8\u7AEF\u8F93\u5165: kexvim restart")}function gl(l){try{return b.runFileSync(process.platform==="win32"?"where":"which",[l]),!0}catch{return!1}}import*as yr from"node:fs";import*as Xr from"node:path";import*as yl from"node:os";function tp(l){try{return b.runFileSync(process.platform==="win32"?"where":"which",[l]),!0}catch{return!1}}async function vl(l,e){let t=r=>{try{return yr.accessSync(r),!0}catch{return!1}};if(t(Xr.join(l,"kexvim.js"))||(console.error("[\u2717] \u672A\u5B89\u88C5 Kexvim\uFF0C\u5148\u8FD0\u884C node kexvim.js init"),process.exit(1)),t(Xr.join(l,".git"))){console.log("[~] git pull...");try{b.runFileSync("git",["pull","--ff-only"],{cwd:l,timeoutMs:12e4})}catch{console.error("[\u2717] \u62C9\u53D6\u5931\u8D25"),process.exit(1)}}else{console.log("[~] \u4E0B\u8F7D\u6700\u65B0 kexvim.js...");let r=Xr.join(yl.tmpdir(),"kexvim-update.js");try{let s=`${e}/raw/main/kexvim.js`;tp("wget")?b.runFileSync("wget",["-q","-O",r,s],{timeoutMs:6e4}):b.runFileSync("curl",["-fsSL","-o",r,s],{timeoutMs:6e4})}catch{console.error("[\u2717] \u4E0B\u8F7D\u5931\u8D25"),process.exit(1)}t(r)&&(yr.copyFileSync(r,Xr.join(l,"kexvim.js")),yr.rmSync(r,{force:!0}))}console.log("[\u2713] \u66F4\u65B0\u5B8C\u6210\uFF0C\u6B63\u5728\u91CD\u542F...")}import*as W from"fs";import*as Dt from"path";var bl=Ct();function rp(l,e){if(!W.existsSync(e))return!0;let t=W.statSync(e).mtimeMs,r=t,s=n=>{let i;try{i=W.readdirSync(n,{withFileTypes:!0})}catch{return}for(let o of i){let a=Dt.join(n,o.name);if(o.isDirectory()){if(o.name==="node_modules"||o.name==="dist"||o.name==="data"||o.name===".git")continue;s(a)}else if(o.name.endsWith(".ts"))try{W.statSync(a).mtimeMs>r&&(r=W.statSync(a).mtimeMs)}catch{}}};return s(Dt.join(l,"src")),s(Dt.join(l,"packages")),r>t}function Vi(){if(process.platform!=="win32")return[];let l=b.runSyncResult(`wmic process where "name='node.exe'" get ProcessId,CommandLine /format:list`,{timeoutMs:5e3});if(l.code!==0)return[];let e=l.stdout.replace(/\u0000/g,""),t=[],r=null;for(let s of e.split(/\r?\n/)){let n=s.trim();if(n.startsWith("CommandLine="))r=n.slice(12).replace(/^"|"$/g,"");else if(n.startsWith("ProcessId=")){let i=Number(n.slice(10));if(r&&i>0&&i!==process.pid&&!/\sweb(\s|$)/.test(r)){let o=r.match(/(\S*(?:dev\.mjs|kexvim\.js|Main\.ts))/);o&&t.push({pid:i,entry:o[1]})}r=null}}return t}function sp(l,e=64*1024){try{let t=W.statSync(l);if(!t.isFile()||t.size===0)return"";if(t.size<=e)return W.readFileSync(l,"utf-8");let r=W.openSync(l,"r");try{let s=Buffer.alloc(e),n=W.readSync(r,s,0,e,t.size-e);return s.toString("utf-8",0,n)}finally{W.closeSync(r)}}catch{return""}}function np(l,e){return new Promise(t=>{let r=0,s=setInterval(()=>{r++;try{if(W.existsSync(l)){let n=sp(l);if(n.includes("Guardian agent \u5DF2\u5C31\u7EEA")||n.includes("kexvim \u5DF2\u5C31\u7EEA")){clearInterval(s),console.log(`[kexvim] \u2705 kexvim \u5DF2\u5C31\u7EEA\uFF08${r}s\uFF09`),t(!0);return}}}catch{}r>=e&&(clearInterval(s),t(!1))},1e3)})}async function kl(l){let e=l,t=Dt.join(e,"data","log");try{W.mkdirSync(t,{recursive:!0})}catch{}let r=Dt.join(t,"kexvim.log"),s=Dt.join(e,"dist","dev.mjs");if(rp(e,s)){console.log("[kexvim] \u68C0\u6D4B\u5230\u6E90\u7801\u53D8\u66F4\uFF0C\u81EA\u52A8\u6784\u5EFA...");let d=b.runSyncResult("npm run build:dev",{cwd:e,timeoutMs:12e4});d.code!==0&&(console.error(`[kexvim] \u274C \u6784\u5EFA\u5931\u8D25\uFF0C\u4E2D\u6B62\u91CD\u542F\uFF1A${d.stderr?.trim()||d.stdout?.trim()}`),process.exit(1)),console.log("[kexvim] \u2705 \u6784\u5EFA\u5B8C\u6210")}if(process.platform==="linux")try{let d=b.runSyncResult("systemctl --user is-active kexvim.service",{timeoutMs:5e3});if(d.code===0&&d.stdout.trim()==="active"){console.log("[kexvim] systemd \u6258\u7BA1\uFF08kexvim.service active\uFF09\uFF0C\u8D70 systemctl --user restart");let u=b.runSyncResult("systemctl --user restart kexvim.service",{timeoutMs:3e4});u.code!==0&&(console.error(`[kexvim] \u274C systemctl restart \u5931\u8D25: ${u.stderr?.trim()||u.stdout?.trim()}`),process.exit(1)),console.log("[kexvim] \u2705 systemctl restart \u5B8C\u6210");return}}catch{}let n=Vi(),i=n.length>0?n.map(d=>d.pid):bl.findKexvimPids(),o=n.length>0?n[0].entry:dt.resolveEntry(e);if(console.log(`[kexvim] \u65E7\u8FDB\u7A0B: ${i.length} \u4E2A; \u5165\u53E3: ${o}`),W.writeFileSync(r,"","utf-8"),i.length>0){console.log("[kexvim] \u6E05\u7406\u65E7\u8FDB\u7A0B...");for(let d of i)process.platform==="win32"?b.runSyncResult(`taskkill /PID ${d} /F`,{timeoutMs:5e3}):bl.killProcess(d,!0);console.log("[kexvim] \u2705 \u65E7\u8FDB\u7A0B\u5DF2\u6E05\u7406")}console.log("[kexvim] \u6B63\u5728\u542F\u52A8\u65B0 kexvim...");let a=dt.spawnFreshDaemon(e,o);a===null&&(console.error("[kexvim] \u274C \u65E0\u6CD5\u542F\u52A8\u65B0\u8FDB\u7A0B"),process.exit(1)),console.log(`[kexvim] \u65B0 PID: ${a}\uFF0C\u7B49\u5F85\u5C31\u7EEA...`),await np(r,30)||console.warn("[kexvim] \u26A0\uFE0F \u7B49\u5F85\u5C31\u7EEA\u8D85\u65F6\uFF0830s\uFF09\uFF0C\u8BF7\u68C0\u67E5 data/log/kexvim.log");try{ct.restart(e),console.log("[kexvim] \u2705 web \u8FDB\u7A0B\u5DF2\u5C31\u7EEA\uFF08\u5DF2\u786E\u4FDD\u62C9\u8D77\uFF09")}catch(d){console.error(`[kexvim] \u26A0\uFE0F web \u91CD\u542F\u5931\u8D25: ${d instanceof Error?d.message:String(d)}`)}console.log("[kexvim] \u65E5\u5FD7:",r),process.exit(0)}import*as Sl from"fs";import*as _l from"path";async function wl(l){let e=Vi();for(let r of e)r.pid!==process.pid&&(process.platform==="win32"?b.runSyncResult(`taskkill /PID ${r.pid} /F /T`,{timeoutMs:5e3}):Ct().killProcess(r.pid,!0),console.log(`[kexvim] \u5DF2\u505C\u6B62 PID ${r.pid} (${r.entry})`));let t=!1;try{let r=ct.findPid();r!==null&&(process.platform==="win32"?b.runSyncResult(`taskkill /PID ${r} /F /T`,{timeoutMs:5e3}):Ct().killProcess(r,!0),console.log(`[kexvim] \u5DF2\u505C\u6B62 web \u8FDB\u7A0B PID ${r}`),t=!0)}catch{}try{Sl.rmSync(_l.join(l,"data","kexvim.pid"),{force:!0})}catch{}e.length===0&&!t?console.log("[kexvim] \u6CA1\u6709\u8FD0\u884C\u4E2D\u7684 kexvim \u8FDB\u7A0B"):console.log("[kexvim] \u2705 kexvim \u5DF2\u505C\u6B62")}import*as G from"node:fs";import*as Qr from"node:os";import*as X from"node:path";import{fileURLToPath as ip}from"node:url";var fe=class l{static SERVICE_NAME="kexvim";static AUTOSTART_DIR="autostart";static KEEPALIVE_INTERVAL_MIN=1;static SYSTEMD_TEMPLATE=(e,t,r)=>`[Unit]
...
`;static platform=process.platform;static _findRoot(){let e=X.dirname(ip(import.meta.url));for(let t=0;t<8;t++){if(G.existsSync(X.join(e,"package.json")))return e;let r=X.dirname(e);if(r===e)break;e=r}throw new Error("[kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF08\u65E0 package.json\uFF09\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002")}static _systemPath(e){if(l.platform==="win32")return"";let t=Qr.homedir();return l.platform==="darwin"?X.join(t,"Library","LaunchAgents","com.kexvim.daemon.plist"):X.join(t,".config","systemd","user","kexvim.service")}static _dataPath(e){let t=X.join(e,"data",l.AUTOSTART_DIR);return G.mkdirSync(t,{recursive:!0}),l.platform==="win32"?X.join(t,"keepalive.cmd"):l.platform==="darwin"?X.join(t,"com.kexvim.daemon.plist"):X.join(t,"kexvim.service")}static _devEntry(e){let t=X.join(e,"dist","dev.mjs");if(G.existsSync(t))return t;let r=X.join(e,"kexvim.js");if(G.existsSync(r))return r;throw new Error("[kexvim] \u672A\u627E\u5230 daemon \u5165\u53E3\uFF08dist/dev.mjs \u6216 kexvim.js\uFF09\u3002")}static _template(e){let t=process.execPath,r=l._devEntry(e);return l.platform==="win32"?`@echo off\r
${t} "${X.join(e,"dist","dev.mjs")}" status >nul 2>&1 || ${t} "${X.join(e,"dist","dev.mjs")}" --daemon\r
`:l.platform==="darwin"?l.LAUNCHD_TEMPLATE(t,e,r):l.SYSTEMD_TEMPLATE(t,e,r)}static writeTemplate(e){let t=e??l._findRoot(),r=l._dataPath(t);return G.writeFileSync(r,l._template(t)),r}static install(e){let t=e??l._findRoot(),r=l.writeTemplate(t),s=l._systemPath(t),n=[`[\u2713] \u6A21\u677F\u5DF2\u751F\u6210: ${r}`];if(l.platform==="win32")l._winInstall(t,n);else if(l.platform==="darwin"){G.mkdirSync(X.dirname(s),{recursive:!0}),G.copyFileSync(r,s),n.push(`[\u2713] \u5DF2\u5199\u5165: ${s}`);try{b.runFileSync("launchctl",["load","-w",s]),n.push("[\u2713] launchctl load \u5B8C\u6210")}catch{n.push("[\u2717] launchctl load \u5931\u8D25\uFF08\u9700\u5F53\u524D\u7528\u6237\u6709 GUI \u4F1A\u8BDD\uFF09")}}else{G.mkdirSync(X.dirname(s),{recursive:!0}),G.copyFileSync(r,s),n.push(`[\u2713] \u5DF2\u5199\u5165: ${s}`);try{b.runFileSync("systemctl",["--user","daemon-reload"]),b.runFileSync("systemctl",["--user","enable","kexvim.service"]),n.push("[\u2713] systemd user \u5355\u5143\u5DF2 enable")}catch(i){n.push(`[\u2717] systemctl \u6CE8\u518C\u5931\u8D25: ${i instanceof Error?i.message:String(i)}`)}try{b.runFileSync("loginctl",["enable-linger",Qr.userInfo().username]),n.push("[\u2713] linger \u5DF2\u542F\u7528\uFF08\u5F00\u673A\u81EA\u542F\uFF09")}catch{n.push("[~] linger \u542F\u7528\u5931\u8D25\uFF08\u90E8\u5206\u7CFB\u7EDF\u9700 root\uFF09\uFF0C\u81EA\u542F\u53EF\u80FD\u53EA\u5728\u767B\u5F55\u540E\u751F\u6548")}}return n.join(`
`)}static uninstall(e){let t=e??l._findRoot(),r=l._systemPath(t),s=[];if(l.platform==="win32")for(let n of["KexvimDaemon","KexvimKeepAlive"])try{b.runFileSync("schtasks",["/Delete","/TN",n,"/F"]),s.push(`[\u2713] \u5DF2\u5220\u9664\u8BA1\u5212\u4EFB\u52A1 ${n}`)}catch{s.push(`[~] \u8BA1\u5212\u4EFB\u52A1 ${n} \u4E0D\u5B58\u5728\u6216\u5220\u9664\u5931\u8D25`)}else if(l.platform==="darwin"){try{b.runFileSync("launchctl",["unload",r])}catch{}G.existsSync(r)?(G.rmSync(r,{force:!0}),s.push(`[\u2713] \u5DF2\u5220\u9664: ${r}`)):s.push("[~] \u672A\u5B89\u88C5")}else{try{b.runFileSync("systemctl",["--user","disable","kexvim.service"]),s.push("[\u2713] systemd \u5355\u5143\u5DF2 disable")}catch{}G.existsSync(r)?(G.rmSync(r,{force:!0}),s.push(`[\u2713] \u5DF2\u5220\u9664: ${r}`)):s.push("[~] \u672A\u5B89\u88C5")}return s.join(`
`)}static status(e){let t=e??l._findRoot(),r=l._systemPath(t),s=l._dataPath(t),n=!1,i=!1,o=!1;if(l.platform==="win32")try{b.runFileSync("schtasks",["/Query","/TN","KexvimDaemon"]),n=!0}catch{}else if(n=G.existsSync(r),n)try{let a=l._template(t);i=G.readFileSync(r,"utf-8")===a}catch{i=!1}if(l.platform==="darwin")try{o=b.runFileSync("launchctl",["print",`gui/${process.getuid?.()??Qr.userInfo().uid}/com.kexvim.daemon`]).includes("state = running")}catch{o=!1}else if(l.platform==="win32")o=l.isProcessRunning(t);else try{o=b.runFileSync("systemctl",["--user","is-active","kexvim.service"]).trim()==="active"}catch{o=!1}return{platform:l.platform,installed:n,configPath:r,dataPath:s,consistent:i,serviceActive:o}}static _winInstall(e,t){let r=process.execPath,s=l._devEntry(e),n=`cmd /c "${r}" "${s}" --daemon`,i=`cmd /c "${r}" "${s}" status >nul 2>&1 || "${r}" "${s}" --daemon`;try{b.runFileSync("schtasks",["/Create","/TN","KexvimDaemon","/SC","ONLOGON","/TR",n,"/F"]),t.push("[\u2713] \u8BA1\u5212\u4EFB\u52A1 KexvimDaemon\uFF08\u767B\u5F55\u81EA\u542F\uFF09\u5DF2\u521B\u5EFA")}catch(o){t.push(`[\u2717] \u521B\u5EFA KexvimDaemon \u5931\u8D25: ${o instanceof Error?o.message:String(o)}`)}try{b.runFileSync("schtasks",["/Create","/TN","KexvimKeepAlive","/SC","MINUTE","/MO",String(l.KEEPALIVE_INTERVAL_MIN),"/TR",i,"/F"]),t.push(`[\u2713] \u8BA1\u5212\u4EFB\u52A1 KexvimKeepAlive\uFF08\u6BCF ${l.KEEPALIVE_INTERVAL_MIN} \u5206\u949F\u4FDD\u6D3B\uFF09\u5DF2\u521B\u5EFA`)}catch(o){t.push(`[\u2717] \u521B\u5EFA KexvimKeepAlive \u5931\u8D25: ${o instanceof Error?o.message:String(o)}`)}}static isProcessRunning(e){let t=e??l._findRoot(),r=X.join(t,"data","kexvim.pid");try{let s=Number.parseInt(G.readFileSync(r,"utf-8").trim(),10);return!Number.isFinite(s)||s<=0?!1:(process.kill(s,0),!0)}catch{return!1}}static resolveRoot(){return l._findRoot()}};function xl(){let l=fe.resolveRoot();console.log(fe.install(l));let e=fe.status(l);console.log(`[~] \u72B6\u6001: platform=${e.platform} installed=${e.installed} consistent=${e.consistent} active=${e.serviceActive}`),process.exit(0)}function Tl(){console.log(fe.uninstall(fe.resolveRoot())),process.exit(0)}function El(){let l=fe.resolveRoot(),e=fe.status(l),t=fe.isProcessRunning(l);console.log(`kexvim \u5E73\u53F0:        ${e.platform}`),console.log(`\u8FDB\u7A0B:               ${t?"\u8FD0\u884C\u4E2D":"\u672A\u8FD0\u884C"} (PID \u6587\u4EF6: data/kexvim.pid)`),console.log(`\u81EA\u542F\u914D\u7F6E:           ${e.installed?"\u5DF2\u5B89\u88C5":"\u672A\u5B89\u88C5"} @ ${e.configPath||"(Windows \u8BA1\u5212\u4EFB\u52A1)"}`),console.log(`\u914D\u7F6E\u4E00\u81F4\u6027(\u7EDF\u4E00):    ${e.consistent?"\u4E00\u81F4(\u6A21\u677F)":"\u4E0D\u4E00\u81F4(\u5C06\u88AB install \u8986\u76D6)"}`),console.log(`\u670D\u52A1\u72B6\u6001:           ${e.serviceActive?"active":"inactive"}`),console.log(`\u91CD\u542F\u5FAA\u73AF\u9632\u62A4:       ${ke.isTripped(l)?"\u26A0\uFE0F TRIPPED\uFF08\u7528 kexvim clear-loop \u89E3\u9664\uFF09":`\u6B63\u5E38 (${ke.bootCount(l)}/${ke.MAX_RESTARTS} in ${ke.WINDOW_SECONDS}s)`}`),process.exit(t?0:1)}function Cl(){ke.clear(fe.resolveRoot()),console.log("[\u2713] restart_loop \u72B6\u6001\u5DF2\u6E05\u9664"),process.exit(0)}import*as Zr from"node:fs";import*as Xi from"node:path";import{DatabaseSync as op}from"node:sqlite";function Rl(l){let e=Xi.join(l,"kexvim.db");Zr.existsSync(e)||(console.error("[kexvim] \u274C \u627E\u4E0D\u5230\u4F1A\u8BDD\u5E93:",e),process.exit(1));let t=new op(e,{readOnly:!0});try{let r=t.prepare(`
      ...
      WHERE s.is_test = 0
      ORDER BY s.updated_at DESC
    `).all();if(r.length===0){console.log("\u6682\u65E0\u5386\u53F2\u4F1A\u8BDD\u3002");return}let s="";try{let n=Xi.join(l,".current_session");Zr.existsSync(n)&&(s=Zr.readFileSync(n,"utf-8").trim())}catch{}console.log(`\u5171 ${r.length} \u4E2A\u4F1A\u8BDD\uFF1A
`),r.forEach((n,i)=>{let o=Ze.displayTitle(n.summary,n.firstUserMsg),a=n.id===s?"  \u25C0 \u5F53\u524D":"";console.log(`${String(i+1).padStart(2)}. ${n.id.slice(0,8)} - ${o}  [${n.source}] ${String(n.msgCount).padStart(4)}\u6761${a}`)}),console.log(`
\u7528\u6CD5: kexvim session <\u524D8\u4F4DID>  \u5207\u6362/\u6062\u590D\u8BE5\u4F1A\u8BDD`)}finally{t.close()}}import*as Ml from"node:fs";import*as Qi from"node:path";import{DatabaseSync as ap}from"node:sqlite";var lp="session-switch.json";function Al(l,e){let t=Qi.join(l,"kexvim.db"),r=new ap(t,{readOnly:!0}),s;try{s=r.prepare(`
      SELECT id, COALESCE(summary, '') AS summary,
             (SELECT COUNT(*) FROM messages m WHERE m.session_id = s.id) AS msgCount
      FROM sessions s
      WHERE s.is_test = 0 AND s.id LIKE ?
    `).get(`${e}%`)}finally{r.close()}s||(console.error(`[kexvim] \u274C \u627E\u4E0D\u5230\u4F1A\u8BDD "${e}"\u3002\u7528 kexvim sessions \u67E5\u770B\u5168\u90E8\u4F1A\u8BDD\u3002`),process.exit(1)),Ml.writeFileSync(Qi.join(l,lp),JSON.stringify({sessionId:s.id,requestedAt:Date.now()/1e3}),"utf-8");let n=(s.summary||"(\u65E0\u6807\u9898)").slice(0,30);console.log(`\u2705 \u5DF2\u6807\u8BB0\u5207\u6362\u5230\u4F1A\u8BDD ${s.id.slice(0,8)}\uFF08${s.msgCount} \u6761\u6D88\u606F\uFF0C${n}\uFF09`),console.log("   \u53D1\u9001\u4E0B\u4E00\u6761\u6D88\u606F\u540E daemon \u5C06\u6062\u590D\u8BE5\u4F1A\u8BDD\u7EE7\u7EED\u5BF9\u8BDD\u3002")}function Zi(){console.log(`\u7528\u6CD5: kexvim <command>
...
\u5165\u53E3: kexvim \u547D\u4EE4\u7EDF\u4E00\u8F6C\u53D1\u4E3A node dev.js\uFF08\u5F00\u53D1\uFF09\u6216 node kexvim.js\uFF08\u5B89\u88C5\u7248\uFF09\uFF0C
      \u547D\u4EE4\u8FDB\u7A0B\u5B8C\u5168\u72EC\u7ACB\uFF0C\u4E0D\u4F9D\u8D56\u8FD0\u884C\u4E2D\u7684 kexvim \u5B9E\u4F8B\u3002`),process.exit(0)}function Il(){console.log("kexvim (\u5F00\u53D1\u5165\u53E3 dist/dev.mjs / \u5B89\u88C5\u5165\u53E3 kexvim.js)"),process.exit(0)}var Vn=class l{static resolveUserDataDir(){let e=process.env.KEXVIM_USER_DATA_DIR;if(e)return e;let t=I.findProjectRoot()??I.findProjectRoot(Ft.dirname(process.argv[1]??""));if(!t)throw new Error("[kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\uFF0C\u6216\u8BBE\u7F6E KEXVIM_USER_DATA_DIR");return Ft.join(t,"data")}static async dispatchFromArgv(){let e=process.argv[2];if(!e)return!1;let t=e.toLowerCase(),r=t==="--help"||t==="-h"?"help":t==="--version"||t==="-v"?"version":t;return new Set(["init","update","restart","stop","install","uninstall","status","clear-loop","sessions","session","help","version"]).has(r)?(await l.handleCliCommand(r),!0):!1}static async handleCliCommand(e,t){let r=Ft.resolve(t??l.resolveUserDataDir(),".."),s=Ft.join(r,"data"),n="https://gitee.com/moscowzk/kexvim";switch(e){case"init":await hl(r,n);return;case"update":await vl(r,n);return;case"install":xl();return;case"uninstall":Tl();return;case"status":El();return;case"clear-loop":Cl();return;case"restart":await kl(r),process.exit(0);return;case"stop":await wl(r),process.exit(0);return;case"sessions":Rl(s);return;case"session":{let i=process.argv[3];i||(console.error("\u7528\u6CD5: kexvim session <\u524D8\u4F4DID>\uFF08\u7528 kexvim sessions \u67E5\u770B\u5217\u8868\uFF09"),process.exit(1)),Al(s,i);return}case"help":Zi();return;case"version":Il();return;default:Zi();return}}};import*as Se from"node:fs";import*as eo from"node:path";import*as Ll from"node:worker_threads";var to=()=>{let l=I.findProjectRoot();if(!l)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D kexvim.log\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let e=eo.join(l,"data","log");try{Se.mkdirSync(e,{recursive:!0})}catch{}return eo.join(e,"kexvim.log")},cp=100*1024*1024;try{let l=to();Se.existsSync(l)&&Se.statSync(l).size>cp&&(Se.rmSync(`${l}.1`,{force:!0}),Se.renameSync(l,`${l}.1`))}catch{}process.on("uncaughtException",l=>{try{Se.appendFileSync(to(),`[kexvim] uncaughtException at ${new Date().toISOString()}:
${l.stack??l.message}
`)}catch{}});process.on("unhandledRejection",l=>{try{let e=l instanceof Error?l.stack??l.message:String(l);Se.appendFileSync(to(),`[kexvim] unhandledRejection at ${new Date().toISOString()}:
${e}
`)}catch{}try{console.error("[kexvim] unhandledRejection:",l)}catch{}});async function Pl(){try{await Yn.main()}catch(l){console.error(l instanceof Error?l.message:String(l)),process.exit(1)}}Ll.isMainThread?Vn.dispatchFromArgv().then(l=>{l?process.exit(0):Pl()}):Pl();
