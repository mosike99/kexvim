#!/usr/bin/env node
var Qe=class{get executionGuidance(){return"none"}get staleTimeoutFloorSeconds(){}get reasoningMaxOutputTokens(){}get modelName(){}};var Te=class{static FLOORS=[{slug:"deepseek-r1",floor:600},{slug:"deepseek-v4-flash",floor:600},{slug:"o1",floor:600},{slug:"o1-mini",floor:600},{slug:"o1-pro",floor:600},{slug:"o1-preview",floor:600},{slug:"o3",floor:600},{slug:"o3-pro",floor:600},{slug:"o3-mini",floor:300},{slug:"o4-mini",floor:300}].sort((e,t)=>t.slug.length-e.slug.length);static getStaleTimeoutFloor(e){if(typeof e!="string"||!e.trim())return;let t=e.trim().toLowerCase();if(t.includes("/")&&(t=t.split("/").pop()??t),!!t)return this.matchAny(t)}static isReasoningModel(e){return this.getStaleTimeoutFloor(e)!==void 0}static matchAny(e){for(let t of this.FLOORS)if(this.matches(e,t.slug))return t.floor}static matches(e,t){if(!e.startsWith(t))return!1;let r=e.slice(t.length);return r===""||r.startsWith("-")||r.startsWith(".")||r.startsWith("_")}};var Ee=class extends Error{constructor(t,r,s,n=!1){super(t);this.statusCode=r;this.provider=s;this.retryable=n;this.name="ProviderError"}statusCode;provider;retryable},jt=class extends Ee{constructor(t,r,s){super(t,429,s,!0);this.retryAfter=r;this.name="RateLimitError"}retryAfter};var Ol=/^[\s]*<think>([\s\S]*?)<\/think>/m,$l=/^[\s]*<reason>([\s\S]*?)<\/reason>/m,Dl=[/\b(sk-[a-zA-Z0-9]{20,})\b/g,/\b(Bearer\s+)([a-zA-Z0-9_\-\.]{20,})\b/gi,/(Authorization:\s*)(?:Bearer\s+)?[a-zA-Z0-9_\-\.]{20,}/gi,/(x-api-key:\s*)[a-zA-Z0-9_\-\.]{20,}/gi,/\b(ghp_[a-zA-Z0-9]{36,})\b/g,/\b(eyJ[a-zA-Z0-9_\-]{10,}\.[a-zA-Z0-9_\-]{10,}\.[a-zA-Z0-9_\-]{10,})\b/g],es=class{static stripThinkBlocks(e){return e?e.replace(/<think>[\s\S]*?<\/think>/g,"").replace(/<reason>[\s\S]*?<\/reason>/g,"").trim():""}static extractThinkContent(e){if(!e)return null;let t=e.match(Ol);if(t)return t[1].trim();let r=e.match($l);return r?r[1].trim():null}static stripThinkTagsOnly(e){return e?e.replace(/<\/?(?:think|reason)>/g,"").trim():""}static stripThinkTagsFromReasoning(e){return e&&e.replace(/<think>[\s\S]*?<\/think>/g,"").trim()||null}static renderContent(e,t,r={}){let s={stripMode:"no_strip",contentRender:"content_only",stripThinkTagsFromReasoning:!0,...r},n=e??"",i=t??null,o=n;s.stripMode==="strip_think_content"?o=this.stripThinkBlocks(n):s.stripMode==="strip_think_tags"&&(o=this.stripThinkTagsOnly(n));let a=null;s.contentRender!=="content_only"&&(a=this.extractThinkContent(n));let c="";if(s.contentRender==="content_only")c=o;else if(s.contentRender==="content_and_think"){let u=[];a&&u.push(a),o&&u.push(o),c=u.join(`
...
\u5982\u679C\u6CA1\u6709\u76F8\u5173\u5185\u5BB9\uFF0C\u5BF9\u5E94\u90E8\u5206\u5199\u300C\u65E0\u300D\u3002`;_filepath;_llm;_summaries={};_loaded=!1;constructor(e,t){this._filepath=us.join(e,"session_summaries.json"),this._llm=t}async consolidate(e,t){if(e.length===0||!t)return console.log("consolidate: skipped \u2014 empty messages or session_id"),null;this._loaded||this._load();let r=await this._extract(e,t);return r===null?null:(this._summaries[t]=r,this._save(),console.log(`Session consolidated: ${t.slice(0,24)} \u2014 ${r.title??"?"}`),r)}get(e){return this._loaded||this._load(),this._summaries[e]??null}search(e){this._loaded||this._load();let t=e.toLowerCase(),r=[];for(let s of Object.values(this._summaries))JSON.stringify(s).toLowerCase().includes(t)&&r.push(s);return r}allSummaries(){return this._loaded||this._load(),Object.values(this._summaries).sort((e,t)=>(t.timestamp??"").localeCompare(e.timestamp??""))}_load(){if(this._loaded=!0,!!rt.existsSync(this._filepath))try{let e=rt.readFileSync(this._filepath,"utf-8");this._summaries=JSON.parse(e)}catch(e){console.warn(`session_summaries load failed: ${e.message}`)}}_save(){try{rt.mkdirSync(us.dirname(this._filepath),{recursive:!0}),rt.writeFileSync(this._filepath,JSON.stringify(this._summaries,null,2),"utf-8")}catch(e){console.warn(`session_summaries save failed: ${e.message}`)}}async _extract(e,t){let r=l.sampleMessages(e);if(r.length===0)return null;let s=l.formatMessagesForLlm(r),i=[{role:"system",content:l._CONSOLIDATE_PROMPT.replace("{max_title}",String(l.MAX_TITLE_CHARS)).replace("{max_summary}",String(l.MAX_SUMMARY_CHARS))},{role:"user",content:`\u5BF9\u8BDD\u8BB0\u5F55\uFF1A
...
${i}${d}`}_viewFile(e,t,r,s){let n=Re.join(e,t);if(!Ue.existsSync(n))return`Error: File '${t}' not found in skill '${r}'.`;if(!Re.resolve(n).startsWith(Re.resolve(e)))return"Error: file_path must be within the skill directory.";try{let o=Ue.readFileSync(n,"utf-8");return s.bumpUsage(r,"view"),`## ${r} / ${t}

${o}`}catch(o){return`Error reading file '${t}': ${o instanceof Error?o.message:String(o)}`}}_listLinkedFiles(e){let t=[],r=["references","templates","scripts","assets"];for(let s of r){let n=Re.join(e,s);if(Ue.existsSync(n))try{this._walkDir(n,s,t)}catch{}}return t.sort()}_walkDir(e,t,r){let s;try{s=Ue.readdirSync(e,{withFileTypes:!0})}catch{return}for(let n of s){let i=Re.join(e,n.name);n.isDirectory()?this._walkDir(i,`${t}/${n.name}`,r):r.push(`${t}/${n.name}`)}}};var Me=class{llm;tools=[];toolMap=new Map;systemPrompt;_maxIterations;_onStream;get _reasoningAwareMaxTokens(){return this.llm.reasoningMaxOutputTokens??4096}get maxIterations(){return this._maxIterations}get onStream(){return this._onStream}set onStream(e){this._onStream=e}constructor(e){this.llm=e.llm,this.systemPrompt=e.systemPrompt??"You are a helpful assistant.",this._maxIterations=e.maxIterations??90,this._onStream=e.onStream,this.setTools(e.tools??[])}setTools(e){this.tools=e,this.toolMap.clear();for(let t of e)this.toolMap.set(t.name,t)}setLLM(e){this.llm=e}async run(e,t){let r=this,s=t?.messages?[...t.messages]:[],n,i=!1,o=0,a=r.tools.map(d=>({name:d.name,description:d.description,input_schema:d.parameters}));for(s.push({role:"user",content:e});o<r.maxIterations;){if(o++,t?.signal?.aborted){i=!0;break}let d={systemPrompt:r.systemPrompt,messages:s,tools:a.length>0?a:void 0,maxOutputTokens:this._reasoningAwareMaxTokens},u={content:"",finishReason:"stop"};if(r.onStream){let h="",k;for await(let y of r.llm.stream(d,t?.signal))if(r.onStream(y),y.type==="text"&&(h+=y.delta),y.type==="usage"&&(k=y.usage),y.type==="done"&&(u={content:h,finishReason:y.finishReason,usage:k}),y.type==="error")throw new Error(`LLM stream error: ${y.message}`)}else u=await r.llm.chat(d,t?.signal);u.usage&&(n=u.usage);let m=u.toolCalls||[];if(s.push({role:"assistant",content:u.content||"",tool_calls:m.length>0?m.map(h=>({id:h.id||h.call_id||`call_${o}_${Math.random().toString(36).slice(2,8)}`,type:"function",function:{name:h.name||h.function?.name||"",arguments:typeof h.arguments=="string"?h.arguments:typeof h.function?.arguments=="string"?h.function.arguments:JSON.stringify(h.input||h.arguments||h.function?.arguments||{})}})):void 0}),!m||m.length===0)break;let g=[],f=m.map(h=>h.name||h.function?.name||"?").join(", ");console.log(`[Agent:run] Iter ${o}: LLM returned ${m.length} tool call(s): ${f}`),H.setCurrentTool(f);for(let h of m){if(t?.signal?.aborted){i=!0;break}let k=h.name||h.function?.name||"",y=h.input||h.arguments||h.function?.arguments||{},v;try{v=typeof y=="string"?JSON.parse(y):y}catch{console.error(`[Agent:run] Failed to parse args for tool '${k}', raw: ${String(y).slice(0,200)}`),g.push({name:k,args:{},result:"Error: failed to parse tool arguments (malformed JSON)"});continue}let w=r.toolMap.get(k);console.log(`[Agent:run] Executing tool: ${k}, args: ${JSON.stringify(v).slice(0,200)}`);let R;if(w)try{R=await w.execute(v,t?.signal)}catch(_){R=`Error: ${_ instanceof Error?_.message:String(_)}`}else{if(a.length===0)continue;let _=r.tools.map(M=>M.name).join(", ");R=`Error: Tool '${k}' does not exist. Available: ${_}`}g.push({name:k,args:v,result:R}),s.push({role:"tool",tool_call_id:h.id||h.call_id||`${k}_${o}`,content:R})}if(H.setCurrentTool(void 0),i)break}let c="";for(let d=s.length-1;d>=0;d--){let u=s[d];if(u.role==="assistant"&&typeof u.content=="string"&&u.content.trim()){c=u.content;break}}return{content:c,toolCalls:[],usage:n,interrupted:i,messages:s}}};import*as He from"fs";import*as Jt from"path";import*as qe from"os";import{spawn as cc,exec as dc,execSync as Ro,execFileSync as uc}from"node:child_process";var zt=class extends Error{code;stdout;stderr;timedOut;signal;constructor(e,t){super(e),this.name="CommandError",this.code=t.code,this.stdout=t.stdout,this.stderr=t.stderr,this.timedOut=t.timedOut,this.signal=t.signal}},ai=10*1024*1024,b=class l{static platform=process.platform;static get isWindows(){return l.platform==="win32"}static runSync(e,t={}){try{return Ro(e,l._syncOptions(t))}catch(r){throw new zt(`Command failed: ${e}`,l._syncErrorResult(r,e))}}static runSyncResult(e,t={}){try{return{code:0,stdout:Ro(e,l._syncOptions(t)),stderr:"",timedOut:!1,signal:null}}catch(r){return l._syncErrorResult(r,e)}}static async runAsync(e,t={}){let r=await l.runAsyncResult(e,t);if(r.code!==0||r.timedOut)throw new zt(`Command failed: ${e}`,r);return r.stdout}static runAsyncResult(e,t={}){return new Promise(r=>{dc(e,{cwd:t.cwd,env:t.env,timeout:t.timeoutMs,maxBuffer:t.maxBuffer??ai,encoding:"utf-8",windowsHide:t.windowsHide??!0},(s,n,i)=>{if(!s){r({code:0,stdout:n,stderr:i??"",timedOut:!1,signal:null});return}r(l._execErrorResult(s,n,i,e))})})}static runFileSync(e,t=[],r={}){return uc(e,t,{cwd:r.cwd,env:r.env,timeout:r.timeoutMs,maxBuffer:r.maxBuffer??ai,encoding:"utf-8",windowsHide:r.windowsHide??!0}).toString()}static spawn(e,t=[],r={}){return cc(e,t,{...r,windowsHide:r.windowsHide??!0})}static _syncOptions(e){let t={encoding:"utf-8",maxBuffer:e.maxBuffer??ai,windowsHide:e.windowsHide??!0};return e.cwd!==void 0&&(t.cwd=e.cwd),e.env!==void 0&&(t.env=e.env),e.timeoutMs!==void 0&&(t.timeout=e.timeoutMs),e.input!==void 0&&(t.input=e.input),t}static _syncErrorResult(e,t){let r=e,s=r.killed===!0,n=r.signal??null;return{code:r.status??null,stdout:r.stdout!==void 0?Buffer.isBuffer(r.stdout)?r.stdout.toString("utf-8"):r.stdout:"",stderr:r.stderr!==void 0?Buffer.isBuffer(r.stderr)?r.stderr.toString("utf-8"):r.stderr:"",timedOut:s,signal:n}}static _execErrorResult(e,t,r,s){let n=e.killed===!0;return{code:typeof e.code=="number"?e.code:null,stdout:t,stderr:r,timedOut:n,signal:e.signal??null}}};var pc=new Set(["\u200B","\u200C","\u200D","\u2060","\u2062","\u2063","\u2064","\uFEFF","\u202A","\u202B","\u202C","\u202D","\u202E","\u2066","\u2067","\u2068","\u2069"]),Z="(?:\\w+\\s+){0,8}",mc=[{pattern:new RegExp(`ignore\\s+${Z}(previous|all|above|prior)\\s+${Z}instructions`,"i"),name:"prompt_injection",scope:"all"},{pattern:/system\s+prompt\s+override/i,name:"sys_prompt_override",scope:"all"},{pattern:new RegExp(`disregard\\s+${Z}(your|all|any)\\s+${Z}(instructions|rules|guidelines)`,"i"),name:"disregard_rules",scope:"all"},{pattern:new RegExp(`act\\s+as\\s+(if|though)\\s+${Z}you\\s+${Z}(have\\s+no|don't\\s+have)\\s+${Z}(restrictions|limits|rules)`,"i"),name:"bypass_restrictions",scope:"all"},{pattern:/<!--[^>]{0,512}(?:ignore|override|system|secret|hidden)[^>]{0,512}-->/i,name:"html_comment_injection",scope:"all"},{pattern:/<\s*div\s+style\s*=\s*["'][^>]{0,2048}display\s*:\s*none/i,name:"hidden_div",scope:"all"},{pattern:/translate\s+[^\n]{0,512}\s+into\s+[^\n]{0,512}\s+and\s+(execute|run|eval)/i,name:"translate_execute",scope:"all"},{pattern:new RegExp(`do\\s+not\\s+${Z}tell\\s+${Z}the\\s+user`,"i"),name:"deception_hide",scope:"all"},{pattern:new RegExp(`you\\s+are\\s+${Z}now\\s+(?:a|an|the)\\s+`,"i"),name:"role_hijack",scope:"context"},{pattern:new RegExp(`pretend\\s+${Z}(you\\s+are|to\\s+be)\\s+`,"i"),name:"role_pretend",scope:"context"},{pattern:new RegExp(`output\\s+${Z}(system|initial)\\s+prompt`,"i"),name:"leak_system_prompt",scope:"context"},{pattern:new RegExp(`(respond|answer|reply)\\s+without\\s+${Z}(restrictions|limitations|filters|safety)`,"i"),name:"remove_filters",scope:"context"},{pattern:new RegExp(`you\\s+have\\s+been\\s+${Z}(updated|upgraded|patched)\\s+to`,"i"),name:"fake_update",scope:"context"},{pattern:/\bname\s+yourself\s+\w+/i,name:"identity_override",scope:"context"},{pattern:/register\s+(as\s+)?a?\s*node/i,name:"c2_node_registration",scope:"context"},{pattern:/(heartbeat|beacon|check[\s\-]?in)\s+(to|with)\s+/i,name:"c2_heartbeat",scope:"context"},{pattern:/pull\s+(down\s+)?(?:new\s+)?task(?:ing|s)?\b/i,name:"c2_task_pull",scope:"context"},{pattern:/connect\s+to\s+the\s+network\b/i,name:"c2_network_connect",scope:"context"},{pattern:new RegExp("you\\s+must\\s+(?:\\w+\\s+){0,3}(register|connect|report|beacon)\\b","i"),name:"forced_action",scope:"context"},{pattern:/only\s+use\s+one[\s\-]?liners?\b/i,name:"anti_forensic_oneliner",scope:"context"},{pattern:new RegExp(`never\\s+${Z}(?:create|write)\\s+${Z}(?:script|file)\\s+${Z}disk`,"i"),name:"anti_forensic_disk",scope:"context"},{pattern:/unset\s+\w*(?:CLAUDE|CODEX|HERMES|AGENT|OPENAI|ANTHROPIC)\w*/i,name:"env_var_unset_agent",scope:"context"},{pattern:/\b(?:cobalt\s*strike|sliver|havoc|mythic|metasploit|brainworm)\b/i,name:"known_c2_framework",scope:"context"},{pattern:/\bc2\s+(?:server|channel|infrastructure|beacon)\b/i,name:"c2_explicit",scope:"context"},{pattern:/\bcommand\s+and\s+control\b/i,name:"c2_explicit_long",scope:"context"},{pattern:/curl\s+[^\n]{0,2048}\$\{?\w*(KEY|TOKEN|SECRET|PASSWORD|CREDENTIAL|API)/i,name:"exfil_curl",scope:"all"},{pattern:/wget\s+[^\n]{0,2048}\$\{?\w*(KEY|TOKEN|SECRET|PASSWORD|CREDENTIAL|API)/i,name:"exfil_wget",scope:"all"},{pattern:/cat\s+[^\n]{0,2048}(\.env|credentials|\.netrc|\.pgpass|\.npmrc|\.pypirc)/i,name:"read_secrets",scope:"all"},{pattern:/(send|post|upload|transmit)\s+[^\n]{0,2048}\s+(to|at)\s+https?:\/\//i,name:"send_to_url",scope:"strict"},{pattern:new RegExp(`(include|output|print|share)\\s+${Z}(conversation|chat\\s+history|previous\\s+messages|full\\s+context|entire\\s+context)`,"i"),name:"context_exfil",scope:"strict"},{pattern:/authorized_keys/i,name:"ssh_backdoor",scope:"strict"},{pattern:/\$HOME\/\.ssh|\~\/\.ssh/i,name:"ssh_access",scope:"strict"},{pattern:/(update|modify|edit|write|change|append|add\s+to)\s+[^\n]{0,2048}(?:AGENTS\.md|CLAUDE\.md|\.cursorrules|\.clinerules)/i,name:"agent_config_mod",scope:"strict"},{pattern:/(update|modify|edit|write|change|append|add\s+to)\s+[^\n]{0,2048}(?:config\.yaml|SOUL\.md)/i,name:"agent_config_mod",scope:"strict"},{pattern:/(?:api[_-]?key|token|secret|password)\s*[=:]\s*["'][A-Za-z0-9+/=_-]{20,}/i,name:"hardcoded_secret",scope:"strict"},{pattern:/[A-Za-z0-9+/]{60,}(?:={0,2})/,name:"base64_encoded_data",scope:"all"},{pattern:/[A-Za-z0-9+/]{40,}={1,2}/,name:"base64_encoded_data",scope:"all"},{pattern:/\b[0-9a-fA-F]{60,}\b/,name:"hex_encoded_data",scope:"all"}],ht={all:[],context:[],strict:[]};for(let l of mc){let e={pattern:l.pattern,name:l.name};l.scope==="all"?(ht.all.push(e),ht.context.push(e),ht.strict.push(e)):(l.scope==="context"&&ht.context.push(e),ht.strict.push(e))}function st(l,e="context"){if(!l)return[];let t=[],r=l.slice(0,65536),s=new Set(r);for(let o of pc)s.has(o)&&t.push(`invisible_unicode_U+${o.codePointAt(0).toString(16).toUpperCase().padStart(4,"0")}`);let n;try{n=r.normalize("NFKC")}catch{n=r}let i=ht[e];for(let{pattern:o,name:a}of i)o.test(n)&&t.push(a);return t}function ms(l,e="strict"){let t=st(l,e);if(t.length===0)return null;let r=t[0];return r.startsWith("invisible_unicode_")?`Blocked: content contains invisible unicode character U+${r.replace("invisible_unicode_","")} (possible injection).`:`Blocked: content matches threat pattern '${r}'. Content is injected into the system prompt and must not contain injection or exfiltration payloads.`}var ie=class{static DEFAULT_AGENT_IDENTITY=`You are Kexvim \u2014 an intelligent AI agent for conversation, code, and task automation.
...
\u8F93\u5165 goal_text \u4E3A\u81EA\u7136\u8BED\u8A00\u76EE\u6807\u63CF\u8FF0\uFF0C\u7CFB\u7EDF\u4F1A\u81EA\u52A8\u89C4\u5212\u3001\u62C6\u89E3\u3001\u5E76\u884C\u6267\u884C\u3002
\u793A\u4F8B\uFF1A\u8C03\u7528 delegate_task(goal_text='\u7ED9\u652F\u4ED8\u6A21\u5757\u52A0\u9000\u6B3E\u529F\u80FD\uFF0C\u652F\u6301\u5FAE\u4FE1\u548C\u652F\u4ED8\u5B9D')
\u7B80\u5355\u67E5\u8BE2\uFF08\u95EE\u95EE\u9898\u3001\u67E5\u72B6\u6001\u3001\u5355\u6B65\u64CD\u4F5C\uFF09\u4E0D\u9700\u8981\u7528\u6B64\u5DE5\u5177\uFF0C\u76F4\u63A5\u56DE\u590D\u5373\u53EF\u3002`;static PLATFORM_HINTS={}},Mo=["AGENTS.md",".cursorrules",".cursor/rules/wildcard.mdc",".cursor/rules/*.mdc","SOUL.md",".zk.md","ZK.md",".github/copilot-instructions.md",".windsurfrules"],yt=class{static stripFrontmatter(e){if(e.startsWith("---")){let t=e.indexOf(`
---`,3);if(t!==-1)return e.slice(t+4).replace(/^\n+/,"")||e}return e}static buildTimestampLine(e,t,r,s){let i=[`Conversation started: ${e.toISOString()}`];return r&&i.push(`Model: ${r}`),s&&i.push(`Provider: ${s}`),t&&i.push(`Session: ${t}`),i.join(" | ")}},nt=class l{identity;constructor(e){this.identity=e?.identity||ie.DEFAULT_AGENT_IDENTITY}static loadSoulIdentity(){try{let e=I.findProjectRoot();if(!e)return null;let t=Jt.join(e,"data","SOUL.md");if(!He.existsSync(t))return null;let r=He.readFileSync(t,"utf-8").trim();if(!r)return null;let s=st(r,"context");return s.length>0?(console.warn(`[PromptBuilder] SOUL.md ignored: contains threat pattern(s) [${s.join(", ")}]. Falling back to default identity.`),null):r}catch(e){return console.warn(`[PromptBuilder] Failed to load SOUL.md: ${e.message}`),null}}build(e={}){let t=[],r=e.identity===void 0?l.loadSoulIdentity():null;if(t.push(r||this.identity),t.push(ie.HERMES_AGENT_HELP_GUIDANCE),e.toolUseEnforcement!==!1&&this._shouldInjectEnforcement(e.toolUseEnforcement)&&(t.push(ie.TOOL_USE_ENFORCEMENT_GUIDANCE),e.executionGuidance==="openai"?t.push(ie.OPENAI_MODEL_EXECUTION_GUIDANCE):e.executionGuidance==="google"&&t.push(ie.GOOGLE_MODEL_OPERATIONAL_GUIDANCE)),e.taskCompletionGuidance!==!1&&t.push(ie.TASK_COMPLETION_GUIDANCE),e.parallelToolCallGuidance!==!1&&t.push(ie.PARALLEL_TOOL_CALL_GUIDANCE),e.approvalGuidance!==!1&&t.push(ie.APPROVAL_GATE_GUIDANCE),e.memoryGuidance&&e.validToolNames?.includes("memory")&&t.push(ie.MEMORY_GUIDANCE),e.skillsGuidance&&e.validToolNames?.includes("skill_manage")&&t.push(ie.SKILLS_GUIDANCE),e.sessionSearchGuidance&&e.validToolNames?.includes("session_search")&&t.push(ie.SESSION_SEARCH_GUIDANCE),e.steerGuidance&&t.push(ie.STEER_CHANNEL_NOTE),e.skillsPrompt&&t.push(e.skillsPrompt),e.tools&&e.tools.length>0){let o=this.buildToolsSection(e.tools);o&&t.push(o)}let s=e.envHints||l.detectEnvironment();if(s){let o=this.buildEnvironmentHints(s);o&&t.push(o)}if(e.codingGuidance){let o=l.buildCodingWorkspaceBlock();o&&t.push(o)}let n=e.activeProfile||"default";if(n==="default"?t.push("Active kexvim profile: default. Other profiles (if any) live under ~/.zk/profiles/<name>/. Each profile has its own skills/, plugins/, cron/, and memories/ that affect a different session than this one. Do not modify another profile's skills/plugins/cron/memories unless the user explicitly directs you to."):t.push(`Active kexvim profile: ${n}. This session reads and writes ~/.zk/profiles/${n}/. The default profile's data lives at ~/.zk/skills/, ~/.zk/plugins/, ~/.zk/cron/, ~/.zk/memories/ \u2014 those belong to a different session run from a different shell. Do NOT modify another profile's skills/plugins/cron/memories unless the user explicitly directs you to. The cross-profile write guard will refuse such writes by default; pass cross_profile=true only after explicit direction.`),e.customPlatformHint&&t.push(e.customPlatformHint),e.systemMessage&&t.push(e.systemMessage),e.contextFiles&&e.contextFiles.length>0){let o=e.contextFiles.filter(c=>{let d=st(c.content,"context");return d.length>0?(console.warn(`[PromptBuilder] Context file '${c.name}' skipped: contains threat pattern(s) [${d.join(", ")}].`),!1):!0}),a=this.buildContextFilesSection(o);a&&t.push(a)}e.memorySnapshot&&t.push(e.memorySnapshot),e.userProfile&&t.push(e.userProfile),e.timestamp&&t.push(e.timestamp);let i=e.plannerMode??0;return i===4||i===5?t.push(ie.TASK_DECOMPOSITION_GUIDANCE_FORCE):i>=2&&t.push(ie.TASK_DECOMPOSITION_GUIDANCE_SUGGEST),t.join(`
...
`)}static detectEnvironment(){try{let e={},t=(qe.type()||"").toLowerCase();e.os=`${qe.type()} (${qe.release()})`;let r=qe.platform();r==="win32"?e.platform="win32":r==="darwin"?e.platform="macos":e.platform=r;try{e.cwd=process.cwd()}catch{}try{e.home=qe.homedir()}catch{}try{let s=He.readFileSync("/proc/version","utf-8");e.isWsl=/microsoft|wsl/i.test(s)}catch{e.isWsl=!1}return e}catch{return}}static buildCodingWorkspaceBlock(e){let t=e||(typeof process<"u"?process.cwd():"");if(!t)return"";let r=null,s=t;for(let n=0;n<10;n++){try{if(He.existsSync(Jt.join(s,".git"))){r=s;break}}catch{}let i=Jt.dirname(s);if(i===s)break;s=i}if(!r)return"";try{let n=["## Coding Workspace"];n.push(`- Root: ${r}`);let i=b.runSync("git branch --show-current",{cwd:r,timeoutMs:5e3}).trim();if(i){n.push(`- Branch: ${i}`);try{let c=b.runSync("git rev-parse --abbrev-ref --symbolic-full-name @{upstream}",{cwd:r,timeoutMs:3e3}).trim();if(c){let d=b.runSync(`git rev-list --left-right --count ${i}...${c}`,{cwd:r,timeoutMs:3e3}).trim(),[u,p]=d.split("	");(u!=="0"||p!=="0")&&n.push(`- Sync: ahead ${u}, behind ${p}`)}}catch{}}let o=b.runSync("git status --porcelain",{cwd:r,timeoutMs:5e3}).trim();if(o){let c=o.split(`
...
`)}catch{return""}}static discoverContextFiles(e,t){let r=[],s=t||He;for(let n of Mo){if(n.includes("*"))continue;let i=`${e}/${n}`;try{s.existsSync(i)&&r.push({name:n,fullPath:i})}catch{}}return r}static loadProjectContext(e,t=2e4){for(let r of Mo){if(r==="SOUL.md"||r.includes("*"))continue;let s=Jt.join(e,r);try{if(!He.existsSync(s))continue;let n=He.readFileSync(s,"utf-8").trim();if(!n)continue;return n=yt.stripFrontmatter(n).trim(),n.length>t&&(n=`${n.slice(0,t)}
...
Only proceed without loading a skill if genuinely none are relevant to the task.`}_shouldInjectEnforcement(e){return e!==!1}};var gc=`Review the conversation above and update two things:
...
\u5269\u4F59 ${s.verification.remainingGoalGaps} \u4E2A\u76EE\u6807\u5DEE\u8DDD + ${s.verification.remainingDrifts} \u4E2A\u6F02\u79FB`;for(let i of s.verification.details.slice(0,5))r+=`
  ${i}`}}else{let s=await this.planner.plan(e);s.shouldSplit&&s.subtasks.length>0&&(r=await this.planner.executePlan(s,this._tools.all(),{signal:t?.signal}))}return r}maybeRunBackgroundReview(e,t,r){if(r||!this._backgroundReviewEnabled||!this.skillManager||!this.memoryTool)return;let s=!1,n=!1;this._turnsSinceMemory++,this._memoryNudgeInterval>0&&this._turnsSinceMemory>=this._memoryNudgeInterval&&(s=!0,this._turnsSinceMemory=0),this._skillNudgeInterval>0&&this._itersSinceSkill>=this._skillNudgeInterval&&(n=!0,this._itersSinceSkill=0),(s||n)&&this.spawnBackgroundReview(e,s,n,t).catch(()=>{})}async spawnBackgroundReview(e,t=!1,r=!1,s){if(this.skillManager)try{let n=this.createReviewLLM?this.createReviewLLM():this.llm,o=await new gs(n,this.skillManager,t,r,s).review(e),a=(o||"").slice(0,300);o&&o!=="Nothing to save."&&console.error(`[BackgroundReview] summary: ${a}`)}catch(n){console.error("[BackgroundReview] error:",n instanceof Error?n.message:String(n))}}reset(e){e?.sessionKey?(this.sessionMessages.delete(e.sessionKey),this.sessionInstances.delete(e.sessionKey),this._activeSessionKey===e.sessionKey&&(this.messages=[],this.session=null,this._activeSessionKey="")):(this.messages=[],this.session=null,this.lastTurnContext=void 0,this.sessionMessages.clear(),this.sessionInstances.clear(),this._activeSessionKey="")}initSessionMemory(e){let t=async r=>{try{return(await this.llm.chat({systemPrompt:"",messages:r})).content??""}catch(s){return console.warn(`session memory LLM call failed: ${s.message}`),""}};this.sessionConsolidator=new ds(e,t),this.ltpPromoter=new ps(e,li.join(e,"memories","MEMORY.md"),t),this.lifecycle.on("on_session_end",r=>{!r.completed||r.interrupted||this._consolidateSession(r.sessionId)}),console.log(`[SessionMemory] LTP + consolidator initialized (${e})`)}async _consolidateSession(e){try{if(!this.sessionConsolidator||!this.ltpPromoter||!e||(this._sessionConsolidateCount++,this._sessionConsolidateCount%qt.CONSOLIDATE_INTERVAL!==0))return;let t=await this.sessionConsolidator.consolidate(this.messages,e);if(t){let r=[...t.importantFacts,...t.keyDecisions];r.length>0&&await this.ltpPromoter.processFacts(r,e)}}catch(t){console.warn(`session consolidation failed: ${t.message}`)}}_stateDispatch(){let e=this,t=3;return async r=>{let s=new Array(r.length),n=0,i=async()=>{for(;n<r.length;){let o=n++,a=r[o];if(e.statePlanner?.dispatchSignal?.aborted)throw new Error("run_state_driven_goal \u5DF2\u4E2D\u65AD\uFF08\u7528\u6237\u63D2\u8BDD\uFF09");let c=await Ze.runSubagent({goal:a.goal,context:a.context||void 0,role:"leaf"},o,1,e.getSubagentParentRuntime(),void 0,{timeoutSeconds:600,signal:e.statePlanner?.dispatchSignal});c.status==="error"?s[o]={_nodeId:a._nodeId,error:c.error,status:"error"}:s[o]={_nodeId:a._nodeId,output:e._parseSubagentOutput(c.summary)},e.statePlanner?.onNodeDone?.()}};return await Promise.all(Array.from({length:Math.min(t,r.length)},()=>i())),s}}_parseSubagentOutput(e){let t=(e??"").trim();if(!t)return{_output:""};let r=[t],s=t.match(/```(?:json)?\s*([\s\S]*?)```/);s&&r.push(s[1].trim());let n=t.indexOf("{"),i=t.lastIndexOf("}");n>=0&&i>n&&r.push(t.slice(n,i+1));for(let o of r)try{let a=JSON.parse(o);if(a&&typeof a=="object"&&!Array.isArray(a))return a}catch{}return{_output:t}}}}function No(l){return class extends l{steer(e){if(!e||!e.trim())return!1;let t=this._activeSessionKey||"";if(!t)return!1;let r=e.trim(),s=this._pendingSteer.get(t);return this._pendingSteer.set(t,s?`${s}
...
`);return B.writeFileSync(r,`${o}

${e.content}`,"utf-8"),this.bumpUsage(e.name,"create"),!0}get(e){return this.list().find(t=>t.name===e)??null}update(e,t){let r=this.get(e);if(!r)return;let s={...r,...t,name:r.name,tags:t.tags??r.tags,relatedSkills:t.relatedSkills??r.relatedSkills,version:r.version,author:r.author},n=s.category?D.join(this.skillsDir,s.category,s.name):D.join(this.skillsDir,s.name);B.mkdirSync(n,{recursive:!0});let i=(s.tags??[]).length>0?`    tags: [${s.tags.map(d=>`"${d}"`).join(", ")}]
...
`);B.writeFileSync(D.join(n,"SKILL.md"),`${c}

${s.content}`,"utf-8"),this.bumpUsage(e,"patch")}loadUsage(){try{return JSON.parse(B.readFileSync(this.usagePath,"utf-8"))}catch{return{}}}saveUsage(e){B.writeFileSync(this.usagePath,JSON.stringify(e,null,2),"utf-8")}bumpUsage(e,t){let r=this.loadUsage(),s=r[e],n=new Date().toISOString();s?r[e]={...s,useCount:t==="create"?s.useCount+1:s.useCount,...t==="patch"||t==="edit"?{lastPatchedAt:n}:{},...t==="view"?{lastViewedAt:n}:{},lastUsedAt:n}:r[e]={useCount:t==="create"?1:0,viewCount:t==="view"?1:0,patchCount:t==="patch"||t==="edit"?1:0,lastUsedAt:n,pinned:!1,state:"active",createdAt:n},this.saveUsage(r)}list(){let e=[],t=new Set;if(this.sharedSkillsDir&&this.walkDir(this.sharedSkillsDir,e,t),this.marketSkillsDir){let r=this.loadDisabledMarketSkills();this.walkDir(this.marketSkillsDir,e,t,r)}return this.walkDir(this.skillsDir,e,t),e}walkDir(e,t,r,s){try{let n=B.readdirSync(e,{withFileTypes:!0});for(let i of n)i.isDirectory()&&this.walkDir(D.join(e,i.name),t,r,s);if(B.existsSync(D.join(e,"SKILL.md"))){let i=this.parseSkill(e);if(i){if(s?.has(i.name)||r?.has(i.name))return;r?.add(i.name),t.push(i)}}}catch{}}listCatalog(){let e=[];if(this.sharedSkillsDir&&this.walkCatalog(this.sharedSkillsDir,this.sharedSkillsDir,"public",e),this.marketSkillsDir){let t=this.loadDisabledMarketSkills();this.walkCatalog(this.marketSkillsDir,this.marketSkillsDir,"market",e,t)}return e}walkCatalog(e,t,r,s,n){let i;try{i=B.readdirSync(e,{withFileTypes:!0})}catch{return}for(let o of i){if(!o.isDirectory()||o.name===".git")continue;let a=D.join(e,o.name);if(B.existsSync(D.join(a,"SKILL.md"))){let c=this.parseCatalog(a,t);c&&s.push({name:c.name,title:c.title,summary:c.summary,description:c.description,category:c.category,source:r,disabled:r==="market"?n?.has(c.name)??!1:!1})}else this.walkCatalog(a,t,r,s,n)}}parseCatalog(e,t){try{let s=B.readFileSync(D.join(e,"SKILL.md"),"utf-8").match(ui);if(!s)return null;let n=this.extract(s[1],/^name:\s*(.+)$/m);if(!n)return null;let i=this.extract(s[1],/^title:\s*(.+)$/m)?.replace(/^["']|["']$/g,"")||void 0,o=this.extract(s[1],/^summary:\s*(.+)$/m)?.replace(/^["']|["']$/g,"")||void 0,a=this.extract(s[1],/^category:\s*(.+)$/m)?.replace(/^["']|["']$/g,""),c=D.relative(t,e),d=c.includes(D.sep)?D.dirname(c):void 0;return{name:n,title:i,summary:o,description:this.extractDescription(s[1]).replace(/^["']|["']$/g,"").slice(0,90),category:a||d}}catch{return null}}parseFrontmatter(e){let t=e.match(ui);return t?{name:this.extract(t[1],/^name:\s*(.+)$/m)??void 0,title:this.extract(t[1],/^title:\s*(.+)$/m)?.replace(/^["']|["']$/g,"")||void 0,summary:this.extract(t[1],/^summary:\s*(.+)$/m)?.replace(/^["']|["']$/g,"")||void 0,description:this.extractDescription(t[1]).replace(/^["']|["']$/g,"").slice(0,90),category:this.extract(t[1],/^category:\s*(.+)$/m)?.replace(/^["']|["']$/g,"")}:null}installMarket(e,t){return!this.marketSkillsDir||!B.existsSync(D.join(e,"SKILL.md"))?!1:(B.cpSync(e,D.join(this.marketSkillsDir,t),{recursive:!0,force:!0}),!0)}uninstallMarket(e){if(!this.marketSkillsDir)return!1;let t=D.join(this.marketSkillsDir,e);if(!B.existsSync(D.join(t,"SKILL.md")))return!1;try{B.rmSync(t,{recursive:!0,force:!0})}catch{return!1}let r=this.loadDisabledMarketSkills();return r.delete(e),this.saveDisabledMarketSkills(r),!0}toggleMarket(e,t){if(!this.marketSkillsDir)return;let r=this.loadDisabledMarketSkills();t?r.delete(e):r.add(e),this.saveDisabledMarketSkills(r)}delete(e){let t=this.get(e);if(!t)return!1;let r=t.category?D.join(this.skillsDir,t.category,t.name):D.join(this.skillsDir,t.name);B.rmSync(r,{recursive:!0,force:!0});let s=this.loadUsage();return delete s[e],this.saveUsage(s),!0}getUsage(e){return this.loadUsage()[e]||null}setState(e,t){let r=this.loadUsage();r[e]&&(r[e].state=t,this.saveUsage(r))}getByState(e){return this.list().filter(t=>t.state===e)}transitionState(e,t){return this.get(e)?(this.update(e,{state:t}),!0):!1}};import*as _s from"node:fs";var Ss=class{llm;manager;intervalMs;minIdleMs;staleAfterMs;archiveAfterMs;consolidate;onSummary;state;constructor(e){this.llm=e.llm,this.manager=e.manager,this.intervalMs=(e.intervalHours??168)*60*60*1e3,this.minIdleMs=(e.minIdleHours??1)*60*60*1e3,this.staleAfterMs=(e.staleAfterDays??30)*24*60*60*1e3,this.archiveAfterMs=(e.archiveAfterDays??90)*24*60*60*1e3,this.consolidate=e.consolidate??!1,this.onSummary=e.onSummary,this.state=this.loadState()}stateFile(){return`${this.manager.skillsDirPath}/.curatorstate`}loadState(){try{let e=_s.readFileSync(this.stateFile(),"utf-8");return JSON.parse(e)}catch{return{lastRunAt:null,lastRunSummary:null,paused:!1,runCount:0}}}saveState(){try{_s.writeFileSync(this.stateFile(),JSON.stringify(this.state,null,2),"utf-8")}catch{}}pause(){this.state.paused=!0,this.saveState()}resume(){this.state.paused=!1,this.saveState()}get paused(){return this.state.paused}shouldRun(e=Date.now()){let t=this;if(t.state.paused)return!1;if(t.state.lastRunAt===null)return t.state.lastRunAt=new Date(e).toISOString(),t.state.lastRunSummary="deferred first run \u2014 curator seeded, will run after one interval",t.saveState(),!1;let r=new Date(t.state.lastRunAt).getTime();return e-r>=t.intervalMs}applyAutoTransitions(e=Date.now()){let t=this,r={markedStale:0,archived:0,reactivated:0,checked:0};for(let s of t.manager.list()){r.checked++;let n=t.manager.getUsage(s.name);if(n?.pinned)continue;let i=n?.lastUsedAt?new Date(n.lastUsedAt).getTime():s.createdAt||e,o=e-i;o>=t.archiveAfterMs&&n?.state!==oe.STATE_ARCHIVED?(t.manager.setState(s.name,oe.STATE_ARCHIVED),r.archived++):o>=t.staleAfterMs&&n?.state===oe.STATE_ACTIVE?(t.manager.setState(s.name,oe.STATE_STALE),r.markedStale++):o<t.staleAfterMs&&n?.state===oe.STATE_STALE&&(t.manager.setState(s.name,oe.STATE_ACTIVE),r.reactivated++)}return r}async run(e=Date.now()){let t=this,r=Date.now(),s=t.applyAutoTransitions(e),n;if(!t.consolidate)return n=`Curator run #${++this.state.runCount} (${((Date.now()-r)/1e3).toFixed(1)}s): ${s.markedStale} stale, ${s.archived} archived, ${s.reactivated} reactivated`,t.state.lastRunAt=new Date(e).toISOString(),t.state.lastRunSummary=n,t.saveState(),t.onSummary&&t.onSummary(n),n;let i=t.manager.list().filter(g=>{let f=t.manager.getUsage(g.name);return!f?.pinned&&f?.state!==oe.STATE_ARCHIVED});if(i.length===0){let g="No active skills to review.";return t.state.lastRunAt=new Date(e).toISOString(),t.state.lastRunSummary=g,t.state.runCount++,t.saveState(),g}let c={systemPrompt:"You are a skill curator. Analyze skills and recommend consolidations. Output ONLY valid JSON.",messages:[{role:"user",content:`You are kexvim's skill curator. Review the following skills and recommend consolidations.
...
${g.reason||""}`;t.manager.update(f,{content:h.content+k}),p.push(`Updated umbrella skill '${f}'`)}if(g.absorb&&Array.isArray(g.absorb))for(let k of g.absorb)t.manager.setState(k,oe.STATE_ARCHIVED),p.push(`  Archived absorbed skill '${k}'`)}let m=((Date.now()-r)/1e3).toFixed(1);return n=`Curator run #${++this.state.runCount} (${m}s):
${p.join(`
`)}`,t.state.lastRunAt=new Date(e).toISOString(),t.state.lastRunSummary=n,t.saveState(),t.onSummary&&t.onSummary(n),n}};var ws=class{tools=new Map;all(){return[...this.tools.values()]}names(){return[...this.tools.keys()]}get(e){return this.tools.get(e)}has(e){return this.tools.has(e)}add(e){this.tools.set(e.name,e)}remove(e){this.tools.delete(e)}};import*as Uo from"node:fs";var xs=class{name="read_file";description="Read the contents of a file. Returns line-numbered output. Use for code, config, markdown, and log files.";parameters={type:"object",properties:{path:{type:"string",description:"Absolute or relative file path to read"},offset:{type:"number",description:"Line number to start from (1-indexed, optional, default 1)"},limit:{type:"number",description:"Max lines to return (optional, default 500)"}},required:["path"]};async execute(e,t,r){let s=String(e.path||"").trim();if(!s)return"Error: path is required";try{let n=Number(e.offset)||1,i=Number(e.limit)||500,a=Uo.readFileSync(s,"utf-8").split(`
...
... (truncated at 50K chars)`:m}catch(n){let i=n instanceof Error?n.message:String(n);return`Error reading file '${s}': ${i}`}}};import*as Es from"node:fs";import*as Ho from"node:path";var Ts=class{name="write_file";description="Write content to a file, creating parent directories if needed. OVERWRITES the entire file.";parameters={type:"object",properties:{path:{type:"string",description:"Absolute or relative file path to write"},content:{type:"string",description:"Complete content to write to the file"}},required:["path","content"]};async execute(e){let t=String(e.path||"").trim(),r=String(e.content||"");if(!t)return"Error: path is required";try{let s=Ho.dirname(t);return Es.mkdirSync(s,{recursive:!0}),Es.writeFileSync(t,r,"utf-8"),`Written ${Buffer.byteLength(r,"utf-8")} bytes to ${t}`}catch(s){let n=s instanceof Error?s.message:String(s);return`Error writing file '${t}': ${n}`}}};import*as Rs from"node:fs";var Cs=class{name="patch";description="Targeted find-and-replace in a file. Replaces a unique text segment with new content. Use replace_all=true to replace every occurrence.";parameters={type:"object",properties:{path:{type:"string",description:"File path to edit (absolute or relative)"},old_string:{type:"string",description:"Text to find \u2014 must be unique in the file unless replace_all=true"},new_string:{type:"string",description:"Replacement text (can be empty string to delete the matched text)"},replace_all:{type:"boolean",description:"Replace all occurrences instead of requiring a unique match",default:!1}},required:["path","old_string","new_string"]};async execute(e,t,r){let s=String(e.path||"").trim(),n=String(e.old_string??""),i=String(e.new_string??""),o=!!e.replace_all;if(!s)return"Error: path is required";let a;try{a=Rs.readFileSync(s,"utf-8")}catch(p){let m=p instanceof Error?p.message:String(p);return`Error reading file '${s}': ${m}`}let c=(a.match(new RegExp(Pc(n),"g"))||[]).length;if(c===0)return`Error: string not found in '${s}'. Make sure the old_string matches exactly (including whitespace).`;if(!o&&c>1)return`Error: Found ${c} occurrences. Use replace_all=true to replace all, or provide more context to make old_string unique.`;let d=o?a.replaceAll(n,i):a.replace(n,i);try{Rs.writeFileSync(s,d,"utf-8")}catch(p){let m=p instanceof Error?p.message:String(p);return`Error writing file '${s}': ${m}`}let u=d.split(`
`);return`Replaced ${c} occurrence(s) in ${s} (${u.length} lines).`}};function Pc(l){return l.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}import*as St from"node:fs";import*as qo from"node:path";var Ms=class{name="search_files";description="Search file contents or find files by name. Uses pattern matching (not full regex).";parameters={type:"object",properties:{pattern:{type:"string",description:"Search pattern (text or glob like *.py)"},path:{type:"string",description:"File or directory to search in (default: current working directory)"},file_glob:{type:"string",description:"Filter by file pattern (e.g. *.ts, *.py)"},limit:{type:"number",description:"Max results (default 20)"}},required:["pattern"]};searchFile(e,t,r,s,n){if(r){let i=e.split("/").pop()||e.split("\\").pop()||"",o=r.replace("*","");if(!i.endsWith(o))return""}try{let i=St.readFileSync(e,"utf-8");if(!i.includes(t))return"";let o=i.split(`
...
${o||"(empty)"}`}if(s==="kill")return i.process.kill(),i.status="killed",`Process ${n} terminated.`;if(s==="wait"){let o=Math.max(Number(e.timeout)||60,1)*1e3;return new Promise(a=>{let c=setTimeout(()=>a(`Timeout waiting for ${n} (${o/1e3}s)`),o);i.process.on("exit",d=>{clearTimeout(c),l.setExited(n,d),a(`Process ${n} exited with code ${d}`)}),i.process.on("error",d=>{clearTimeout(c),a(`Process ${n} error: ${d.message}`)})})}return"Error: unknown action"}};import*as Ie from"fs";import*as Bs from"path";import{CronTime as Wc,validateCronExpression as Gc}from"cron";var ae=class l{static MAX_JOBS=50;_file;_jobs;constructor(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D cron-jobs.json\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");this._file=Bs.join(e,"data","cron-jobs.json"),this._jobs=this._load()}list(){return Object.values(this._jobs)}get(e){return this._jobs[e]??null}reload(){this._jobs=this._load()}create(e){this._jobs[e.id]=e,this._save()}remove(e){delete this._jobs[e],this._save()}setStatus(e,t){let r=this._jobs[e];r&&(r.status=t,this._save())}updateRunMeta(e,t){let r=this._jobs[e];r&&(r.lastRunAt=t.lastRunAt,r.lastStatus=t.lastStatus,r.lastError=t.lastError??null,t.nextRunAt!==void 0&&(r.nextRunAt=t.nextRunAt),this._save())}static parseSchedule(e){let t=e.trim();if(!t)throw new Error("schedule \u4E0D\u80FD\u4E3A\u7A7A / schedule must not be empty");if(t.toLowerCase().startsWith("every ")){let n=l.parseDuration(t.slice(6).trim());return{kind:"interval",minutes:n,display:`every ${n}m`}}if(/^\d+\s*[mhd]$/.test(t)){let n=l.parseDuration(t);return{kind:"interval",minutes:n,display:`every ${n}m`}}if(t.includes("T")||/^\d{4}-\d{2}-\d{2}/.test(t)){let n=new Date(t);if(Number.isNaN(n.getTime()))throw new Error(`\u975E\u6CD5\u65F6\u95F4\u6233 '${t}' / invalid timestamp`);return{kind:"once",runAt:n.toISOString(),display:`once at ${n.toISOString()}`}}let s=t.split(/\s+/);if(s.length>=5&&s.slice(0,5).every(n=>/^[\d*,\-/?]+$/.test(n))){let n=[[0,59],[0,23],[1,31],[1,12],[0,7]];for(let i=0;i<5;i++){let[o,a]=n[i];for(let c of s[i].split(",")){let d=c.match(/\d+/g)??[];for(let u of d){let p=parseInt(u,10);if(p<o||p>a)throw new Error(`\u975E\u6CD5 cron \u8868\u8FBE\u5F0F '${t}'\uFF1A\u7B2C ${i+1} \u5B57\u6BB5\u503C ${p} \u8D85\u51FA\u8303\u56F4 ${o}-${a}`)}}}try{Gc(t)}catch(i){throw new Error(`\u975E\u6CD5 cron \u8868\u8FBE\u5F0F '${t}': ${i?.message||i}`)}return{kind:"cron",expr:t,display:t}}throw new Error(`\u65E0\u6CD5\u89E3\u6790 schedule '${t}'\uFF1A\u652F\u6301 'every 30m'\u3001'2026-08-03T14:00'\u3001'0 9 * * *'`)}static computeNextRun(e,t=new Date){if(e.kind==="once"){let r=new Date(e.runAt);return r.getTime()>t.getTime()?r.toISOString():null}if(e.kind==="interval")return new Date(t.getTime()+e.minutes*6e4).toISOString();try{return new Wc(e.expr).sendAt().toJSDate().toISOString()}catch{return null}}static toCronExpr(e){if(e.kind==="cron")return e.expr;if(e.kind==="interval"){let t=e.minutes;return t>=60&&t%60===0?`0 */${t/60} * * *`:`*/${t} * * * *`}return null}static parseDuration(e){let t=/^(\d+)\s*([mhd])$/.exec(e.trim());if(!t)throw new Error(`\u975E\u6CD5\u65F6\u957F '${e}'\uFF08\u652F\u6301 30m/2h/1d\uFF09/ invalid duration`);let r=parseInt(t[1],10);if(r<=0)throw new Error("\u65F6\u957F\u5FC5\u987B\u4E3A\u6B63\u6570 / duration must be positive");let s=t[2];return s==="m"?r:s==="h"?r*60:r*24*60}_load(){try{if(Ie.existsSync(this._file))return JSON.parse(Ie.readFileSync(this._file,"utf-8"))}catch(e){console.error(`[CronJobStore] \u8BFB\u53D6 ${this._file} \u5931\u8D25\uFF0C\u6309\u7A7A\u4EFB\u52A1\u5904\u7406:`,e?.message)}return{}}_save(){try{let e=Bs.dirname(this._file);Ie.mkdirSync(e,{recursive:!0});let t=`${this._file}.tmp`;Ie.writeFileSync(t,JSON.stringify(this._jobs,null,2),"utf-8"),Ie.renameSync(t,this._file)}catch(e){console.error(`[CronJobStore] \u4FDD\u5B58 ${this._file} \u5931\u8D25:`,e?.message)}}};import{CronJob as Kc}from"cron";import*as Pe from"fs";import*as Us from"path";var js=class l{static MAX_RECORDS=100;_file;_records;constructor(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D cron-executions.json\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");this._file=Us.join(e,"data","cron-executions.json"),this._records=this._load()}record(e){let t=`exec_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;this._records.push({id:t,...e}),this._records.length>l.MAX_RECORDS&&(this._records=this._records.slice(-l.MAX_RECORDS)),this._save()}history(e,t=20){return(e?this._records.filter(s=>s.jobId===e):this._records).slice(-t).reverse()}_load(){try{if(Pe.existsSync(this._file)){let e=JSON.parse(Pe.readFileSync(this._file,"utf-8"));return Array.isArray(e)?e:[]}}catch(e){console.error(`[CronExecutionStore] \u8BFB\u53D6 ${this._file} \u5931\u8D25\uFF0C\u6309\u7A7A\u5386\u53F2\u5904\u7406:`,e?.message)}return[]}_save(){try{let e=Us.dirname(this._file);Pe.mkdirSync(e,{recursive:!0});let t=`${this._file}.tmp`;Pe.writeFileSync(t,JSON.stringify(this._records,null,2),"utf-8"),Pe.renameSync(t,this._file)}catch(e){console.error(`[CronExecutionStore] \u4FDD\u5B58 ${this._file} \u5931\u8D25:`,e?.message)}}};var se=class l{static _instance=null;static get instance(){return l._instance||(l._instance=new l),l._instance}static _RUN_TIMEOUT_MS=3e5;static MAX_CONSECUTIVE_FAILURES=3;static schedulingDisabled=!1;static _SYNC_INTERVAL_MS=3e4;_store;_executions;_timers=new Map;_onceTimers=new Map;_running=new Set;_syncTimer=null;_agentExecutor=null;_deliverer=null;_activityProvider=null;constructor(){this._store=new ae,this._executions=new js}setAgentExecutor(e){this._agentExecutor=e}setActivityProvider(e){this._activityProvider=e}setDeliverer(e){this._deliverer=e}static cronInactivityTimeoutSeconds(){let e=(process.env.KEXVIM_CRON_TIMEOUT??"").trim();if(!e)return 600;let t=Number(e);return Number.isFinite(t)?t:600}get store(){return this._store}get executions(){return this._executions}get runningIds(){return Array.from(this._running)}start(){if(l.schedulingDisabled)return;let e=new Date;for(let r of this._store.list())r.status==="active"&&(r.nextRunAt&&new Date(r.nextRunAt)<=e&&this._runJob(r.id),this._startTimer(r));let t=this._store.list().length;t>0&&console.log(`[CronScheduler] Restored ${t} cron job(s)`),this._syncTimer&&clearInterval(this._syncTimer),this._syncTimer=setInterval(()=>{this._syncJobs()},l._SYNC_INTERVAL_MS),this._syncTimer.unref?.()}stop(){this._syncTimer&&(clearInterval(this._syncTimer),this._syncTimer=null);for(let e of Array.from(this._timers.keys()))this._clearTimer(e)}startJob(e){this._startTimer(e)}stopJob(e){this._clearTimer(e)}runNow(e){this._runJob(e,!0)}pause(e){this._store.get(e)&&(this.stopJob(e),this._store.setStatus(e,"paused"))}resume(e){let t=this._store.get(e);t&&(this._store.setStatus(e,"active"),this._store.updateRunMeta(e,{lastRunAt:t.lastRunAt??new Date().toISOString(),lastStatus:t.lastStatus??"success",nextRunAt:this._recomputeNextRun(t)}),this._startTimer(this._store.get(e)))}_startTimer(e){if(!l.schedulingDisabled)try{this._clearTimer(e.id);let t=ae.parseSchedule(e.schedule);if(t.kind==="once"){let s=new Date(t.runAt);if(s.getTime()>Date.now()){let n=setTimeout(()=>{this._runJob(e.id)},s.getTime()-Date.now());this._onceTimers.set(e.id,n)}return}if(t.kind==="interval"){let s=t.minutes*6e4,n=()=>{this._onceTimers.delete(e.id),this._runJob(e.id),this._onceTimers.set(e.id,setTimeout(n,s))};this._onceTimers.set(e.id,setTimeout(n,s));return}let r=new Kc(t.expr,()=>{this._runJob(e.id)},null,!0);this._timers.set(e.id,r)}catch(t){console.error(`[CronScheduler] \u542F\u52A8\u4EFB\u52A1 '${e.name}' \u5931\u8D25:`,t?.message)}}_clearTimer(e){let t=this._timers.get(e);if(t){try{t.stop()}catch{}this._timers.delete(e)}let r=this._onceTimers.get(e);r&&(clearTimeout(r),this._onceTimers.delete(e))}_syncJobs(){if(!l.schedulingDisabled){try{this._store.reload()}catch(e){console.error("[CronScheduler] \u540C\u6B65\u4EFB\u52A1\u5217\u8868\u5931\u8D25:",e?.message);return}for(let e of this._store.list()){let t=this._timers.has(e.id)||this._onceTimers.has(e.id);e.status==="active"&&!t?(e.nextRunAt&&new Date(e.nextRunAt)<=new Date&&this._runJob(e.id),this._startTimer(e)):e.status!=="active"&&t&&this._clearTimer(e.id)}}}async _runJob(e,t=!1){if(this._running.has(e))return;let r=this._store.get(e);if(!r||!t&&r.status!=="active")return;this._running.add(e);let s=new Date().toISOString(),n="",i=null;try{if(r.prompt&&this._agentExecutor)n=await this._withInactivityTimeout(this._agentExecutor(r.prompt)),n=n.slice(0,4e3);else{let p=await b.runAsyncResult(r.command,{timeoutMs:l._RUN_TIMEOUT_MS,maxBuffer:1048576});p.code!==0&&(i=p.timedOut?`Command timed out after ${l._RUN_TIMEOUT_MS}ms`:`Command exited with code ${p.code}`),n=(p.stdout||"").slice(0,4e3);let m=(p.stderr||"").slice(0,2e3);m&&(n+=n?`
[stderr] ${m}`:`[stderr] ${m}`)}}catch(p){let m=p;i=m.message||String(p),n=(m.stdout||"").slice(0,2e3),m.stderr&&(n+=n?`
[stderr] ${m.stderr}`:`[stderr] ${m.stderr}`)}finally{this._running.delete(e)}let o=new Date().toISOString(),a=i?"error":"success";this._executions.record({jobId:r.id,jobName:r.name,startedAt:s,finishedAt:o,status:a,output:n,error:i});let c=r.lastErrorCount??0,d=a==="success"?0:c+1,u=a==="error"&&d>=l.MAX_CONSECUTIVE_FAILURES;if(this._store.updateRunMeta(r.id,{lastRunAt:o,lastStatus:a,lastError:i,nextRunAt:u?null:this._recomputeNextRun(r)}),d!==c){let p=this._store.get(r.id);p&&(p.lastErrorCount=d,this._store.setStatus(r.id,p.status))}n&&await this._deliver(n,r),u?(this.pause(e),console.error(`[CronScheduler] \u4EFB\u52A1 '${r.name}' \u8FDE\u7EED\u5931\u8D25 ${d} \u6B21\uFF0C\u5DF2\u81EA\u52A8\u6682\u505C\uFF08lifecycle guard\uFF09`)):a==="error"&&console.error(`[CronScheduler] \u4EFB\u52A1 '${r.name}' \u6267\u884C\u5931\u8D25: ${i}`)}_withInactivityTimeout(e){let t=l.cronInactivityTimeoutSeconds();if(t<=0||!this._activityProvider)return e;let r=t*1e3,s=!1;return new Promise((n,i)=>{let o=setInterval(()=>{if(s)return;let a=this._activityProvider();Date.now()-a>r&&(s=!0,clearInterval(o),i(new Error(`Agent inactivity timeout after ${t}s`)))},5e3);e.then(a=>{s||(s=!0,clearInterval(o),n(a))},a=>{s||(s=!0,clearInterval(o),i(a))})})}async _deliver(e,t){let r=t.deliver||"origin";if(r!=="local"){if(!this._deliverer){console.warn(`[CronScheduler] \u4EFB\u52A1 '${t.name}' \u65E0\u6295\u9012\u901A\u9053\uFF08deliverer \u672A\u6CE8\u5165\uFF09\uFF0C\u6267\u884C\u7ED3\u679C\u4EC5\u8BB0\u5F55\u4E8E\u6267\u884C\u5386\u53F2\uFF08cron-executions.json\uFF09`);return}try{r==="origin"?t.originChat?await this._deliverer(e,t.originChat):console.warn(`[CronScheduler] \u4EFB\u52A1 '${t.name}' deliver=origin \u4F46\u65E0 originChat\uFF08\u521B\u5EFA\u65F6\u65E0\u5E73\u53F0\u4E0A\u4E0B\u6587\uFF09\uFF0C\u6267\u884C\u7ED3\u679C\u4EC5\u8BB0\u5F55\u4E8E\u6267\u884C\u5386\u53F2`):r==="all"?await this._deliverer(e,"all"):await this._deliverer(e,r)}catch(s){console.error(`[CronScheduler] \u4EFB\u52A1 '${t.name}' \u6295\u9012\u5931\u8D25:`,s?.message)}}}_recomputeNextRun(e){try{let t=ae.parseSchedule(e.schedule);return ae.computeNextRun(t)}catch{return null}}};var Sr=class l{static CATALOG=[{key:"daily_report",name:"\u6BCF\u65E5\u65E5\u62A5",description:"\u6BCF\u5929\u65E9\u4E0A\u751F\u6210\u8FC7\u53BB 24 \u5C0F\u65F6\u7684\u4E2D\u6587\u6458\u8981\u65E5\u62A5\uFF08agent \u6A21\u5F0F\uFF09",schedule:"0 9 * * *",mode:"agent",prompt:"\u603B\u7ED3\u8FC7\u53BB 24 \u5C0F\u65F6\u7684\u91CD\u8981\u4E8B\u4EF6\u548C\u52A8\u6001\uFF0C\u8F93\u51FA\u4E00\u4EFD\u7B80\u6D01\u7684\u4E2D\u6587\u65E5\u62A5\uFF08\u6309\u4E3B\u9898\u5206\u70B9\uFF0C\u4E0D\u8D85\u8FC7 15 \u884C\uFF09"},{key:"daily_summary",name:"\u6BCF\u65E5\u603B\u7ED3",description:"\u6BCF\u665A\u751F\u6210\u5F53\u65E5\u5DE5\u4F5C\u603B\u7ED3\uFF08agent \u6A21\u5F0F\uFF09",schedule:"0 21 * * *",mode:"agent",prompt:"\u5BF9\u4ECA\u5929\u7684\u5DE5\u4F5C\u548C\u5BF9\u8BDD\u505A\u4E00\u4E2A\u7B80\u6D01\u7684\u4E2D\u6587\u603B\u7ED3\uFF1A\u5B8C\u6210\u7684\u4E8B\u9879\u3001\u5F85\u529E\u3001\u503C\u5F97\u6CE8\u610F\u7684\u70B9\u3002\u4E0D\u8D85\u8FC7 15 \u884C\u3002"},{key:"health_check",name:"\u670D\u52A1\u5065\u5EB7\u68C0\u67E5",description:"\u6BCF 30 \u5206\u949F\u68C0\u67E5 kexvim \u670D\u52A1\u662F\u5426\u5B58\u6D3B\uFF08script \u6A21\u5F0F\uFF09",schedule:"*/30 * * * *",mode:"script",command:'systemctl --user is-active kexvim && echo "kexvim OK" || echo "kexvim DOWN"'},{key:"reminder",name:"\u6BCF\u65E5\u63D0\u9192",description:"\u6BCF\u5929\u65E9\u4E0A 8 \u70B9\u53D1\u9001\u63D0\u9192\uFF08script \u6A21\u5F0F\uFF0C\u6539 command \u5B9A\u5236\u5185\u5BB9\uFF09",schedule:"0 8 * * *",mode:"script",command:'echo "\u65E9\u4E0A\u597D\uFF0C\u8BB0\u5F97\u67E5\u770B\u4ECA\u5929\u7684\u5F85\u529E"'},{key:"weekly_report",name:"\u6BCF\u5468\u62A5\u544A",description:"\u6BCF\u5468\u4E00\u65E9\u4E0A\u751F\u6210\u4E0A\u5468\u56DE\u987E\uFF08agent \u6A21\u5F0F\uFF09",schedule:"0 9 * * 1",mode:"agent",prompt:"\u56DE\u987E\u4E0A\u5468\u7684\u91CD\u8981\u4E8B\u4EF6\u548C\u8FDB\u5C55\uFF0C\u8F93\u51FA\u4E00\u4EFD\u7B80\u6D01\u7684\u4E2D\u6587\u5468\u62A5\uFF08\u6309\u4E3B\u9898\u5206\u70B9\uFF0C\u4E0D\u8D85\u8FC7 20 \u884C\uFF09"}];static get(e){return l.CATALOG.find(t=>t.key===e)??null}static formatList(){return l.CATALOG.map(e=>`  ${e.key} | ${e.name} | ${e.schedule} | ${e.description}`).join(`
`)}};var Hs=class l{name="cronjob";description="Schedule recurring tasks. Create, list, pause, resume, run, and remove cron jobs. blueprints lists preset templates. Cross-platform, no system crontab needed.";parameters={type:"object",properties:{action:{type:"string",enum:["create","list","remove","pause","resume","run","history","blueprints"],description:"create=schedule new job, list=show jobs, remove=delete job, pause=stop timer, resume=restart timer, run=run once now, history=show execution history, blueprints=list preset templates"},name:{type:"string",description:"Human-friendly name for the job (required for create)"},schedule:{type:"string",description:"Cron expression ('0 9 * * *'), interval ('every 30m' / '30m' / 'every 2h'), or ISO timestamp ('2026-08-03T14:00') for one-shot. Required for create."},command:{type:"string",description:"Shell command to run (script mode). Either command or prompt is required for create."},prompt:{type:"string",description:"Agent-mode prompt: run this prompt through the LLM instead of a shell command (either command or prompt required)."},deliver:{type:"string",description:"Delivery target for output: origin=back to the chat where created (default), all=every connected platform, local=history only, or 'platform:chatId' for a specific target"},blueprint:{type:"string",description:"Preset template key (see blueprints action); fills schedule/prompt/command automatically"},job_id:{type:"string",description:"Job ID (required for remove/pause/resume/run/history)"},limit:{type:"number",description:"History entries to show (default 10, max 50)"}},required:["action"]};static get _store(){return se.instance.store}async execute(e,t,r){let s=String(e.action||"");return s==="list"?this._list():s==="create"?this._create(e,r):s==="remove"?this._remove(e):s==="pause"||s==="resume"||s==="run"?this._lifecycle(s,e):s==="blueprints"?`Preset cron templates (create with blueprint=<key>):
${Sr.formatList()}`:s==="history"?this._history(e):"Error: action must be create, list, remove, pause, resume, run, history, or blueprints"}_list(){let e=l._store.list();return e.length===0?"No cron jobs.":e.map(t=>`  ${t.id} | ${t.name} | ${t.schedule} | status=${t.status} | next=${t.nextRunAt?t.nextRunAt.slice(0,16):"-"} | last=${t.lastStatus??"-"}${t.lastError?` (${t.lastError.slice(0,60)})`:""}`).join(`
`)}_create(e,t){let r=String(e.name||"").trim(),s=String(e.schedule||"").trim(),n=String(e.command||"").trim(),i=String(e.prompt||"").trim(),o=String(e.blueprint||"").trim();if(o){let k=Sr.get(o);if(!k)return`Error: unknown blueprint '${o}'. Use action=blueprints to list templates.`;s||(s=k.schedule),!n&&!i&&(k.mode==="agent"&&k.prompt?i=k.prompt:n=k.command??""),r||(r=k.name)}if(!r||!s)return"Error: name and schedule are required";if(!n&&!i)return"Error: either command (script mode) or prompt (agent mode) is required";let a;try{a=ae.parseSchedule(s)}catch(k){return`Error: ${k?.message||k}`}let c=`cron_${Date.now()}`,d=new Date().toISOString(),u=ae.computeNextRun(a);if(a.kind==="once"&&!u)return`Error: once \u4EFB\u52A1\u7684\u6267\u884C\u65F6\u95F4\u5DF2\u8FC7\uFF08${a.display}\uFF09\uFF0C\u8BF7\u4F7F\u7528\u672A\u6765\u65F6\u95F4`;let p=t?.platform&&t?.chatId?`${t.platform}:${t.chatId}`:void 0,m=String(e.deliver||"origin").trim(),g={id:c,name:r,schedule:s,command:n,...i?{prompt:i}:{},deliver:m,originChat:p,status:"active",createdAt:d,lastRunAt:null,nextRunAt:u,lastStatus:null,lastError:null};l._store.create(g),se.instance.startJob(g);let f=i?`agent:${i.slice(0,40)}`:n,h=m==="origin"&&!p;return`Created cron job '${r}' (${c}): ${a.display} \u2192 ${f} | deliver=${m}${p?` | origin=${p}`:""} | next run ${g.nextRunAt??"-"}`+(h?" | \u26A0\uFE0F \u5F53\u524D\u4F1A\u8BDD\u65E0\u6295\u9012\u901A\u9053\uFF0C\u6267\u884C\u7ED3\u679C\u4EC5\u8BB0\u5F55\u4E8E\u6267\u884C\u5386\u53F2\uFF08cronjob history \u53EF\u67E5\uFF09":"")}_remove(e){let t=String(e.job_id||"").trim();return t?l._store.get(t)?(se.instance.stopJob(t),l._store.remove(t),`Removed job '${t}'`):`Error: job '${t}' not found`:"Error: job_id is required"}_lifecycle(e,t){let r=String(t.job_id||"").trim();if(!r)return"Error: job_id is required";let s=l._store.get(r);if(!s)return`Error: job '${r}' not found`;let n=se.instance;if(e==="pause")return n.pause(r),`Paused job '${r}' (${s.name})`;if(e==="resume"){n.resume(r);let i=l._store.get(r);return`Resumed job '${r}' (${s.name}) | next run ${i?.nextRunAt??"-"}`}return n.runNow(r),`Triggered run of job '${r}' (${s.name})`}_history(e){let t=String(e.job_id||"").trim(),r=Math.min(Math.max(parseInt(String(e.limit??"10"),10)||10,1),50),s=se.instance.executions.history(t||void 0,r);return s.length===0?t?`No executions for job '${t}'.`:"No executions yet.":s.map(n=>`  ${n.startedAt.slice(0,16)} | ${n.status==="success"?"\u2705":"\u274C"} | ${n.jobName} | ${(n.output||n.error||"").slice(0,60)}`).join(`
`)}};var qs=class{name="vision_analyze";description="Analyze an image (local file path or http(s) URL) and return a text description. Requires a configured vision provider (provider with `vision: true` in config).";parameters={type:"object",properties:{image_url:{type:"string",description:"Path or URL of the image to analyze"},prompt:{type:"string",description:"Optional instruction for what to look for in the image"}},required:["image_url"]};visionLlm;constructor(e){this.visionLlm=e}async execute(e){let t=String(e.image_url||"").trim();if(!t)return"Error: image_url is required";if(!this.visionLlm)return"Error: no vision provider configured. Add a provider with `vision: true` in config.yaml (e.g. GLM) to enable image analysis.";try{let r=String(e.prompt||bt.DEFAULT_ANALYSIS_PROMPT),s=await bt.analyzeImage(this.visionLlm,t,r);return JSON.stringify({success:!0,analysis:s})}catch(r){return JSON.stringify({success:!1,analysis:`Error analyzing image: ${r.message}`})}}};import*as Ks from"node:fs";var Yo=3,Vo=1,Xo=50,Qo=!0,Zo=3;function Ws(){try{let l=process.cwd()+"/config.json";if(Ks.existsSync(l)){let t=JSON.parse(Ks.readFileSync(l,"utf-8")).delegation||{};return{maxChildren:t.max_concurrent_children??Yo,maxDepth:t.max_spawn_depth??Vo,maxIterations:t.max_iterations??Xo,orchestratorEnabled:t.orchestrator_enabled??Qo,maxAsyncChildren:t.max_async_children??Zo}}}catch{}return{maxChildren:Yo,maxDepth:Vo,maxIterations:Xo,orchestratorEnabled:Qo,maxAsyncChildren:Zo}}var zc=!1;function ea(l){let e={};for(let[t,r]of Object.entries(l))t==="acp_command"||t==="acp_args"||(e[t]=r);return e}var Gs=class{name="delegate_task";description="Spawn one or more subagents in isolated contexts. Each subagent gets its own LLM session and toolset. Use for complex multi-step work, parallel research, or self-contained subtasks.";parameters={type:"object",properties:{goal:{type:"string",description:"What the subagent should accomplish. Be specific and self-contained \u2014 the subagent knows nothing about your conversation history."},context:{type:"string",description:"Background information the subagent needs: file paths, error messages, project structure, constraints."},tasks:{type:"array",items:{type:"object",properties:{goal:{type:"string",description:"Task goal"},context:{type:"string",description:"Task-specific context"},role:{type:"string",enum:["leaf","orchestrator"],description:"Per-task role override."}},required:["goal"]},description:`Batch mode: up to ${Ws().maxChildren} tasks to run in parallel. When provided, top-level goal/context are ignored.`},role:{type:"string",enum:["leaf","orchestrator"],description:"Role of the child agent. 'leaf' (default) = focused worker, cannot delegate further. 'orchestrator' = can use delegate_task to spawn its own workers."},background:{type:"boolean",description:"DEPRECATED / IGNORED. Top-level delegations always run in the background automatically."}},required:[]};parent;currentSessionId="";currentDepth=0;constructor(e){this.parent=e}async execute(e,t,r){if(zc)return JSON.stringify({status:"error",error:"Subagent spawning is paused. Try again later."});let s=Ws(),n=String(e.goal||""),i=String(e.context||""),o=e.tasks,a=String(e.role||"leaf");s.orchestratorEnabled||(a="leaf");let c=this.currentDepth;if(c>=s.maxDepth)return JSON.stringify({status:"error",error:`Delegation depth limit reached (depth=${c}, max_spawn_depth=${s.maxDepth}). Cannot spawn deeper subagents.`});let d;if(o&&Array.isArray(o)){if(o.length>s.maxChildren)return JSON.stringify({status:"error",error:`Error: max ${s.maxChildren} concurrent tasks supported. Got ${o.length}.`});d=o.map(p=>{let m=ea(p);return{goal:String(m.goal||""),context:String(m.context||""),role:m.role||a}}).filter(p=>p.goal.trim())}else if(typeof o=="string")try{let p=JSON.parse(o);if(!Array.isArray(p))return JSON.stringify({status:"error",error:"Error: tasks must be a JSON array of task objects."});if(p.length>s.maxChildren)return JSON.stringify({status:"error",error:`Error: max ${s.maxChildren} concurrent tasks supported. Got ${p.length}.`});d=p.map(m=>{let g=ea(m);return{goal:String(g.goal||""),context:String(g.context||""),role:g.role||a}}).filter(m=>m.goal.trim())}catch{return JSON.stringify({status:"error",error:"Error: tasks must be a JSON array of task objects; received a string that could not be parsed as JSON."})}else if(n)d=[{goal:n,context:i,role:a}];else return JSON.stringify({status:"error",error:"Error: provide 'goal' (single task) or 'tasks' array (batch mode)."});if(d.length===0)return JSON.stringify({status:"error",error:"Error: at least one valid task is required."});if(c===0&&d.length<=s.maxChildren){let m=ne.getInstance().executeBatch(d.map(g=>({goal:g.goal,context:g.context})),c+1,this.parent,this.currentSessionId,a,null,s.maxAsyncChildren);return m.status==="rejected"?JSON.stringify({status:"rejected",error:m.error}):JSON.stringify({status:"dispatched",delegation_id:m.delegationId,child_count:d.length})}let u=await Promise.all(d.map((p,m)=>Ze.runSubagent(p,m,c+1,this.parent)));return JSON.stringify({status:u.every(p=>p.status==="completed")?"completed":"error",results:u.map(p=>({task_index:p.task_index,status:p.status,summary:p.summary,error:p.error??null,api_calls:p.api_calls,duration_seconds:p.duration_seconds}))},null,2)}};async function Jc(l,e){let t=`https://lite.duckduckgo.com/lite/?q=${encodeURIComponent(l)}`,r=await fetch(t,{headers:{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",Accept:"text/html"},signal:AbortSignal.timeout(1e4)});if(!r.ok)throw new Error(`DuckDuckGo returned ${r.status}: ${r.statusText}`);let s=await r.text(),n=[],i=/<a[^>]*href="([^"]*)"[^>]*class="result-link"[^>]*>([\s\S]*?)<\/a>/gi,o=/<td[^>]*class="result-snippet"[^>]*>([\s\S]*?)<\/td>/gi,a=/<td[^>]*class="url"[^>]*>([\s\S]*?)<\/td>/gi,c=[],d;for(;(d=i.exec(s))!==null;)c.push({href:d[1].trim(),text:d[2].replace(/<[^>]*>/g,"").trim()});let u=[];for(;(d=o.exec(s))!==null;)u.push(d[1].replace(/<[^>]*>/g,"").trim());let p=[];for(;(d=a.exec(s))!==null;)p.push(d[1].replace(/<[^>]*>/g,"").trim());for(let m=0;m<Math.min(c.length,e);m++){let g=c[m];!g.href||g.href.startsWith("/")||n.push({title:g.text||"(No title)",url:g.href,description:u[m]||p[m]||"",position:m+1})}return n}async function Yc(l,e){let t=process.env.GOOGLE_API_KEY||"",r=process.env.GOOGLE_CSE_ID||"";if(!t||!r)throw new Error("GOOGLE_API_KEY and GOOGLE_CSE_ID must be set for Google Custom Search");let s=`https://www.googleapis.com/customsearch/v1?key=${t}&cx=${r}&q=${encodeURIComponent(l)}&num=${Math.min(e,10)}`,n=await fetch(s,{signal:AbortSignal.timeout(1e4)});if(!n.ok){let a=await n.text();throw new Error(`Google Search returned ${n.status}: ${a.slice(0,200)}`)}return((await n.json()).items||[]).slice(0,e).map((a,c)=>({title:a.title||"",url:a.link||"",description:a.snippet||"",position:c+1}))}function Vc(){return(process.env.KEXVIM_WEB_SEARCH_BACKEND||"duckduckgo").toLowerCase().trim()==="google"?"google":"duckduckgo"}var zs=class{name="web_search";description="Search the web for information. Returns up to 5 results by default with titles, URLs, and descriptions. The query is passed to DuckDuckGo (default, no API key needed) or Google (set GOOGLE_API_KEY + GOOGLE_CSE_ID + KEXVIM_WEB_SEARCH_BACKEND=google).";parameters={type:"object",properties:{query:{type:"string",description:"The search query to look up on the web."},limit:{type:"number",description:"Maximum number of results to return. Defaults to 5.",minimum:1,maximum:20,default:5}},required:["query"]};async execute(e,t,r){let s=String(e.query||"").trim();if(!s)return JSON.stringify({success:!1,error:"query is required"});let n=Math.min(Math.max(Number(e.limit)||5,1),20);try{let i=Vc(),o;return i==="google"?o=await Yc(s,n):o=await Jc(s,n),o.length===0?JSON.stringify({success:!0,data:{web:[]},message:"No results found."},null,2):JSON.stringify({success:!0,data:{web:o}},null,2)}catch(i){return JSON.stringify({success:!1,error:`Search failed: ${i?.message||String(i)}`},null,2)}}};import*as ra from"node:dns";var ta=ra.promises;async function Xc(l){try{let t=new URL(l).hostname;if(t==="localhost"||t==="127.0.0.1"||t==="0.0.0.0")return!1;let r=[];try{let s=await ta.resolve4(t);r.push(...s)}catch{}try{let s=await ta.resolve6(t);r.push(...s)}catch{}for(let s of r)if(s.startsWith("10.")||s.startsWith("172.16.")||s.startsWith("192.168.")||s.startsWith("127.")||s.startsWith("169.254.")||s==="::1"||s.startsWith("fd")||s.startsWith("fc"))return!0;return!1}catch{return!1}}function Qc(l,e){let t=l;if(t=t.replace(/<script[^>]*>[\s\S]*?<\/script>/gi,""),t=t.replace(/<style[^>]*>[\s\S]*?<\/style>/gi,""),t=t.replace(/<nav[^>]*>[\s\S]*?<\/nav>/gi,""),t=t.replace(/<footer[^>]*>[\s\S]*?<\/footer>/gi,""),t=t.replace(/<header[^>]*>[\s\S]*?<\/header>/gi,""),t=t.replace(/<h[1-6][^>]*>(.*?)<\/h[1-6]>/gi,(r,s)=>`
## ${s.replace(/<[^>]*>/g,"")}
...
[... Content truncated at ${e.toLocaleString()} chars \u2014 full content available at source URL ...]`}return t}var Js=class{name="web_fetch";description="Fetch and extract content from a web page URL. Returns page content in plain text/markdown format. Supports any public HTTP/HTTPS URL. Pages over 5000 chars are truncated. SSRF protection blocks private/internal network addresses.";parameters={type:"object",properties:{url:{type:"string",description:"The URL to fetch content from (must be http:// or https://)."},max_length:{type:"number",description:"Maximum characters to return. Defaults to 5000.",minimum:500,maximum:5e4,default:5e3}},required:["url"]};async execute(e,t,r){let s=String(e.url||"").trim();if(!s)return JSON.stringify({success:!1,error:"url is required"});let n=Math.min(Math.max(Number(e.max_length)||5e3,500),5e4),i;try{if(i=new URL(s),i.protocol!=="http:"&&i.protocol!=="https:")return JSON.stringify({success:!1,error:"Only http:// and https:// URLs are supported"})}catch{return JSON.stringify({success:!1,error:`Invalid URL: ${s}`})}try{if(await Xc(s))return JSON.stringify({success:!1,error:"Blocked: URL targets a private or internal network address (SSRF protection)."});let a=await fetch(s,{headers:{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"},signal:AbortSignal.timeout(3e4),redirect:"follow"}),c=a.headers.get("content-type")||"",d=c.includes("text/html")||c.includes("application/xhtml")||c.includes("text/plain"),u=c.includes("application/pdf")||s.endsWith(".pdf");if(!a.ok)return JSON.stringify({success:!1,error:`HTTP ${a.status}: ${a.statusText}`});if(u)return JSON.stringify({success:!0,data:{url:s,title:i.hostname,content:`[PDF document at ${s} \u2014 PDF parsing not yet supported. Use a browser tool to view this document.]`}});let p=await a.text(),m=Zc(p,i.hostname),g;return d?g=Qc(p,n):(g=p.slice(0,n),p.length>n&&(g+=`

[... Content truncated at ${n.toLocaleString()} chars \u2014 full content available at source URL ...]`)),JSON.stringify({success:!0,data:{url:s,title:m,content:g}},null,2)}catch(o){let a=(o instanceof Error?o.message:String(o))||String(o);return a.includes("timed out")||a.includes("timeout")||a.includes("abort")?JSON.stringify({success:!1,error:`Request timed out: ${s}. The page may be too slow or unreachable.`}):a.includes("fetch")||a.includes("ENOTFOUND")||a.includes("ECONNREFUSED")||a.includes("ECONNRESET")?JSON.stringify({success:!1,error:`Could not connect to ${i.hostname}: ${a.slice(0,150)}`}):JSON.stringify({success:!1,error:`Failed to fetch ${s}: ${a.slice(0,200)}`})}}};function Zc(l,e){let t=l.match(/<title[^>]*>([\s\S]*?)<\/title>/i);if(t)return t[1].replace(/<[^>]*>/g,"").trim();let r=l.match(/<h1[^>]*>(.*?)<\/h1>/i);return r?r[1].replace(/<[^>]*>/g,"").trim():e}import*as Zt from"node:fs";import*as sa from"node:os";import*as er from"node:path";import{randomUUID as ed}from"node:crypto";var td="https://queue.fal.run",rd=55*1024*1024,gi=6e4,J=class l{static getKey(){return process.env.FAL_KEY?.trim()||void 0}static isConfigured(){return!!l.getKey()}static noKeyMessage(e){return`${e} \u9700\u8981 FAL.ai \u51ED\u636E\uFF1A\u8BBE\u7F6E\u73AF\u5883\u53D8\u91CF FAL_KEY\uFF08export FAL_KEY=...\uFF09\u540E\u91CD\u542F\u751F\u6548\u3002`}static async submit(e,t){let r=l.getKey();if(!r)throw new Error(l.noKeyMessage("\u56FE\u50CF/\u89C6\u9891\u751F\u6210"));let s;try{s=await fetch(`${td}/${e}`,{method:"POST",headers:{Authorization:`Key ${r}`,"Content-Type":"application/json","x-idempotency-key":ed()},body:JSON.stringify(t),signal:AbortSignal.timeout(gi)})}catch(i){throw new Error(`\u65E0\u6CD5\u8FDE\u63A5 FAL.ai\uFF1A${i instanceof Error?i.message:String(i)}`)}if(!s.ok){let i=await s.text().catch(()=>"");throw new Error(`FAL \u63D0\u4EA4\u5931\u8D25 (HTTP ${s.status})\uFF1A${i.slice(0,500)}`)}let n=await s.json();if(!n.request_id||!n.status_url)throw new Error(`FAL \u54CD\u5E94\u7F3A\u5C11 request_id/status_url\uFF1A${JSON.stringify(n).slice(0,300)}`);return{requestId:String(n.request_id),statusUrl:String(n.status_url),responseUrl:String(n.response_url||""),cancelUrl:n.cancel_url?String(n.cancel_url):void 0}}static async pollStatus(e,t=0,r=3e3){let s=t>0?Date.now()+t:0;for(;;){let n=await l.queryStatus(e);if(n.status==="COMPLETED"||n.status==="ERROR"||n.status==="CANCELED")return n;if(s>0&&Date.now()>=s)return{status:"TIMEOUT"};await new Promise(i=>setTimeout(i,r))}}static async queryStatus(e){let t=l.getKey();if(!t)throw new Error(l.noKeyMessage("\u4EFB\u52A1\u72B6\u6001\u67E5\u8BE2"));let r;try{r=await fetch(e,{headers:{Authorization:`Key ${t}`},signal:AbortSignal.timeout(gi)})}catch(i){throw new Error(`\u65E0\u6CD5\u8FDE\u63A5 FAL.ai\uFF1A${i instanceof Error?i.message:String(i)}`)}if(!r.ok)throw new Error(`FAL \u72B6\u6001\u67E5\u8BE2\u5931\u8D25 (HTTP ${r.status})`);let s=await r.json(),n=String(s.status||"");if(n==="COMPLETED")return{status:n,responseUrl:s.response_url?String(s.response_url):void 0};if(n==="ERROR"||n==="CANCELED"){let i=s.error||s.detail,o=typeof i=="string"?i:i?.message;return{status:n,error:o?String(o):`FAL \u4EFB\u52A1 ${n}`}}return{status:n}}static async getResult(e){let t=l.getKey();if(!t)throw new Error(l.noKeyMessage("\u4EFB\u52A1\u7ED3\u679C\u83B7\u53D6"));let r;try{r=await fetch(e,{headers:{Authorization:`Key ${t}`},signal:AbortSignal.timeout(gi)})}catch(s){throw new Error(`\u65E0\u6CD5\u8FDE\u63A5 FAL.ai\uFF1A${s instanceof Error?s.message:String(s)}`)}if(!r.ok)throw new Error(`FAL \u7ED3\u679C\u83B7\u53D6\u5931\u8D25 (HTTP ${r.status})`);return await r.json()}static async resolveMedia(e,t=10*1024*1024){let r=(e||"").trim();if(!r||/^https?:\/\//i.test(r)||/^data:/i.test(r)||!(r==="~"||r.startsWith("~/")||r.startsWith("~\\")||r.startsWith("/")||r.startsWith("./")||r.startsWith("../")||r.startsWith(".\\")||r.startsWith("..\\")||/^[A-Za-z]:[\\/]/.test(r)||r.startsWith("\\\\")))return r;let n=r.startsWith("~")?er.join(sa.homedir(),r.slice(1)):er.resolve(r);if(!Zt.existsSync(n))throw new Error(`\u5A92\u4F53\u6587\u4EF6\u4E0D\u5B58\u5728\uFF1A${n}`);let i=Zt.statSync(n).size;if(i>t)throw new Error(`\u5A92\u4F53\u6587\u4EF6\u8FC7\u5927\uFF1A${n} (${(i/1024/1024).toFixed(1)}MB > ${(t/1024/1024).toFixed(0)}MB)`);let o=l.mimeForPath(n),a=Zt.readFileSync(n).toString("base64");if(a.length>rd)throw new Error(`\u5A92\u4F53\u6587\u4EF6\u8FC7\u5927\uFF08base64 \u540E ${(a.length/1024/1024).toFixed(1)}MB \u8D85\u51FA\u8BF7\u6C42\u4F53\u4E0A\u9650\uFF09`);return`data:${o};base64,${a}`}static mimeForPath(e){switch(er.extname(e).toLowerCase()){case".png":return"image/png";case".jpg":case".jpeg":return"image/jpeg";case".webp":return"image/webp";case".gif":return"image/gif";case".mp4":return"video/mp4";case".webm":return"video/webm";case".mov":return"video/quicktime";default:return"application/octet-stream"}}};import*as Vs from"node:fs";import*as hi from"node:path";var fi={"fal-ai/flux-2/klein/9b":{display:"FLUX 2 Klein 9B",speed:"<1s",strengths:"Fast, crisp text",price:"$0.006/MP",size_style:"image_size_preset",sizes:{landscape:"landscape_16_9",square:"square_hd",portrait:"portrait_16_9"},defaults:{num_inference_steps:4,output_format:"png",enable_safety_checker:!1},supports:["prompt","image_size","num_inference_steps","seed","output_format","enable_safety_checker"],upscale:!1,edit_endpoint:"fal-ai/flux-2/klein/9b/edit",edit_supports:["prompt","image_urls","num_inference_steps","seed","output_format","enable_safety_checker"],max_reference_images:9},"fal-ai/flux-2-pro":{display:"FLUX 2 Pro",speed:"~6s",strengths:"Studio photorealism",price:"$0.03/MP",size_style:"image_size_preset",sizes:{landscape:"landscape_16_9",square:"square_hd",portrait:"portrait_16_9"},defaults:{num_inference_steps:50,guidance_scale:4.5,num_images:1,output_format:"png",enable_safety_checker:!1,safety_tolerance:"5",sync_mode:!0},supports:["prompt","image_size","num_inference_steps","guidance_scale","num_images","output_format","enable_safety_checker","safety_tolerance","sync_mode","seed"],upscale:!0,edit_endpoint:"fal-ai/flux-2-pro/edit",edit_supports:["prompt","image_urls","num_inference_steps","guidance_scale","num_images","output_format","enable_safety_checker","safety_tolerance","sync_mode","seed"],max_reference_images:9},"fal-ai/z-image/turbo":{display:"Z-Image Turbo",speed:"~2s",strengths:"Bilingual EN/CN, 6B",price:"$0.005/MP",size_style:"image_size_preset",sizes:{landscape:"landscape_16_9",square:"square_hd",portrait:"portrait_16_9"},defaults:{num_inference_steps:8,num_images:1,output_format:"png",enable_safety_checker:!1,enable_prompt_expansion:!1},supports:["prompt","image_size","num_inference_steps","num_images","seed","output_format","enable_safety_checker","enable_prompt_expansion"],upscale:!1},"fal-ai/nano-banana-pro":{display:"Nano Banana Pro (Gemini 3 Pro Image)",speed:"~8s",strengths:"Gemini 3 Pro, reasoning depth, text rendering",price:"$0.15/image (1K)",size_style:"aspect_ratio",sizes:{landscape:"16:9",square:"1:1",portrait:"9:16"},defaults:{num_images:1,output_format:"png",safety_tolerance:"5",resolution:"1K"},supports:["prompt","aspect_ratio","num_images","output_format","safety_tolerance","seed","sync_mode","resolution","enable_web_search","limit_generations"],upscale:!1,edit_endpoint:"fal-ai/nano-banana-pro/edit",edit_supports:["prompt","image_urls","aspect_ratio","num_images","output_format","safety_tolerance","seed","sync_mode","resolution","enable_web_search","limit_generations"],max_reference_images:2},"fal-ai/gpt-image-1.5":{display:"GPT Image 1.5",speed:"~15s",strengths:"Prompt adherence",price:"$0.034/image",size_style:"gpt_literal",sizes:{landscape:"1536x1024",square:"1024x1024",portrait:"1024x1536"},defaults:{quality:"medium",num_images:1,output_format:"png"},supports:["prompt","image_size","quality","num_images","output_format","background","sync_mode"],upscale:!1,edit_endpoint:"fal-ai/gpt-image-1.5/edit",edit_supports:["prompt","image_urls","image_size","quality","num_images","output_format","sync_mode"],max_reference_images:16},"fal-ai/gpt-image-2":{display:"GPT Image 2",speed:"~20s",strengths:"SOTA text rendering + CJK, world-aware photorealism",price:"$0.04\u20130.06/image",size_style:"image_size_preset",sizes:{landscape:"landscape_4_3",square:"square_hd",portrait:"portrait_4_3"},defaults:{quality:"medium",num_images:1,output_format:"png"},supports:["prompt","image_size","quality","num_images","output_format","sync_mode"],upscale:!1,edit_endpoint:"openai/gpt-image-2/edit",edit_supports:["prompt","image_urls","quality","num_images","output_format","sync_mode","mask_image_url"],max_reference_images:16},"fal-ai/ideogram/v3":{display:"Ideogram V3",speed:"~5s",strengths:"Best typography",price:"$0.03-0.09/image",size_style:"image_size_preset",sizes:{landscape:"landscape_16_9",square:"square_hd",portrait:"portrait_16_9"},defaults:{rendering_speed:"BALANCED",expand_prompt:!0,style:"AUTO"},supports:["prompt","image_size","rendering_speed","expand_prompt","style","seed"],upscale:!1,edit_endpoint:"fal-ai/ideogram/v3/edit",edit_supports:["prompt","image_urls","rendering_speed","expand_prompt","style","seed"],max_reference_images:1},"fal-ai/recraft/v4/pro/text-to-image":{display:"Recraft V4 Pro",speed:"~8s",strengths:"Design, brand systems, production-ready",price:"$0.25/image",size_style:"image_size_preset",sizes:{landscape:"landscape_16_9",square:"square_hd",portrait:"portrait_16_9"},defaults:{enable_safety_checker:!1},supports:["prompt","image_size","enable_safety_checker","colors","background_color"],upscale:!1},"fal-ai/qwen-image":{display:"Qwen Image",speed:"~12s",strengths:"LLM-based, complex text",price:"$0.02/MP",size_style:"image_size_preset",sizes:{landscape:"landscape_16_9",square:"square_hd",portrait:"portrait_16_9"},defaults:{num_inference_steps:30,guidance_scale:2.5,num_images:1,output_format:"png",acceleration:"regular"},supports:["prompt","image_size","num_inference_steps","guidance_scale","num_images","output_format","acceleration","seed","sync_mode"],upscale:!1,edit_endpoint:"fal-ai/qwen-image-2/pro/edit",edit_supports:["prompt","image_urls","num_inference_steps","guidance_scale","num_images","output_format","acceleration","seed","sync_mode"],max_reference_images:3},"fal-ai/krea/v2/medium/text-to-image":{display:"Krea 2 Medium",speed:"~15-25s",strengths:"Illustration, anime, painting, expressive/artistic styles",price:"$0.030 (text) / $0.035 (style refs)",size_style:"aspect_ratio",sizes:{landscape:"16:9",square:"1:1",portrait:"9:16"},defaults:{creativity:"medium"},supports:["prompt","aspect_ratio","creativity","seed","image_style_references"],upscale:!1},"fal-ai/krea/v2/large/text-to-image":{display:"Krea 2 Large",speed:"~25-60s",strengths:"Photorealism, raw textured looks (motion blur, grain, film)",price:"$0.060 (text) / $0.065 (style refs)",size_style:"aspect_ratio",sizes:{landscape:"16:9",square:"1:1",portrait:"9:16"},defaults:{creativity:"medium"},supports:["prompt","aspect_ratio","creativity","seed","image_style_references"],upscale:!1}},oa="fal-ai/flux-2/klein/9b",na="landscape",ia=["landscape","square","portrait"],sd="fal-ai/clarity-upscaler",nd="masterpiece, best quality, highres",id="(worst quality, low quality, normal quality:2)",od=.35,ad=.6,ld=4,cd=18;function dd(l){let e=(l||process.env.FAL_IMAGE_MODEL||"").trim(),t=e&&fi[e]?e:oa;return{model:t,meta:fi[t]}}function ud(){let l=I.findProjectRoot(),e=hi.join(l||process.cwd(),"data","media");return Vs.mkdirSync(e,{recursive:!0}),e}async function pd(l,e){try{let t=await fetch(l,{signal:AbortSignal.timeout(6e4)});if(!t.ok)return l;let r=Buffer.from(await t.arrayBuffer()),s=`fal_${Date.now()}_${Math.floor(Math.random()*1e6)}.${e||"bin"}`,n=hi.join(ud(),s);return Vs.writeFileSync(n,r),n}catch{return l}}function md(l,e){let t=/image\/(\w+)/.exec(l||"");if(t)return t[1]==="jpeg"?"jpg":t[1];let r=/\.(png|jpg|jpeg|webp|gif)(?:$|\?)/i.exec(e);return r?r[1]==="jpeg"?"jpg":r[1].toLowerCase():"png"}var Ys=class l{name="image_generate";description="Generate an image from a text prompt (or edit a source image) via FAL.ai. Requires FAL_KEY. aspect_ratio: landscape (default), square, portrait. Pass image_url or reference_image_urls (local paths or URLs) for image-to-image editing. Result includes a local file path you can share.";parameters={type:"object",properties:{prompt:{type:"string",description:"Image description / edit instruction. Required."},aspect_ratio:{type:"string",enum:ia,default:na,description:"Output aspect ratio."},image_url:{type:"string",description:"Source image for editing: local file path or URL."},reference_image_urls:{type:"array",items:{type:"string"},description:"Reference images for editing (model-dependent cap)."},seed:{type:"integer",minimum:0,maximum:4294967295,description:"Optional reproducibility seed."},model:{type:"string",description:`Model override. Default: ${oa}. Available: ${Object.keys(fi).join(", ")}`}},required:["prompt"]};async execute(e,t,r){let s=String(e.prompt||"").trim();if(!s)return JSON.stringify({success:!1,image:null,error:"prompt is required for image generation",error_type:"ValueError"});let{model:n,meta:i}=dd(e.model?String(e.model):void 0),o=ia.includes(String(e.aspect_ratio||"").toLowerCase())?String(e.aspect_ratio).toLowerCase():na,a=[];if(typeof e.image_url=="string"&&e.image_url.trim()&&a.push(e.image_url.trim()),Array.isArray(e.reference_image_urls))for(let p of e.reference_image_urls)typeof p=="string"&&p.trim()&&a.push(p.trim());if(!J.isConfigured())return JSON.stringify({success:!1,image:null,error:J.noKeyMessage("\u56FE\u50CF\u751F\u6210"),error_type:"MissingCredentials"});let c=i.edit_endpoint,d=a.length>0&&!!c,u=d?"image":"text";if(a.length>0&&!c)return JSON.stringify({success:!1,image:null,error:`Model '${i.display}' (${n}) \u4E0D\u652F\u6301\u56FE\u751F\u56FE/\u7F16\u8F91\u3002\u8BF7\u63D0\u4F9B\u7EAF\u6587\u672C prompt\uFF08\u7701\u7565 image_url\uFF09\uFF0C\u6216\u6362\u7528\u652F\u6301\u7F16\u8F91\u7684\u6A21\u578B\u3002`,error_type:"ValueError"});try{let p={...i.defaults};if(p.prompt=s,d){let _=i.max_reference_images&&i.max_reference_images>0?i.max_reference_images:1,M=a.slice(0,_);p.image_urls=await Promise.all(M.map(L=>J.resolveMedia(L))),e.seed!==void 0&&i.edit_supports?.includes("seed")&&(p.seed=e.seed);for(let L of["num_inference_steps","guidance_scale","num_images","output_format"])e[L]!==void 0&&i.edit_supports?.includes(L)&&(p[L]=e[L])}else{let _=i.sizes[o];i.size_style==="aspect_ratio"?p.aspect_ratio=_:p.image_size=_,e.seed!==void 0&&i.supports.includes("seed")&&(p.seed=e.seed);for(let M of["num_inference_steps","guidance_scale","num_images","output_format"])e[M]!==void 0&&i.supports.includes(M)&&(p[M]=e[M])}let m=d&&c?c:n,g=await J.submit(m,p),f=await J.pollStatus(g.statusUrl,3e5);if(f.status!=="COMPLETED"||!f.responseUrl){let _=f.status==="TIMEOUT"?"\u751F\u6210\u8D85\u65F6\uFF085 \u5206\u949F\uFF09\u3002\u8BF7\u91CD\u8BD5\u6216\u66F4\u6362\u6A21\u578B\u3002":f.error||`\u4EFB\u52A1 ${f.status}`;return JSON.stringify({success:!1,image:null,error:_,error_type:f.status==="TIMEOUT"?"Timeout":"FalError"})}let k=(await J.getResult(f.responseUrl)).images||[];if(!k.length)return JSON.stringify({success:!1,image:null,error:"FAL.ai \u672A\u8FD4\u56DE\u56FE\u7247",error_type:"FalError"});let y=i.upscale&&!d,v=[];for(let _ of k){let M=String(_.url||"");if(!M)continue;let L={url:M,width:_.width||0,height:_.height||0,upscaled:!1};if(y){let he=await l.upscale(M,s);if(he){v.push({...he,upscaled:!0});continue}}v.push(L)}if(!v.length)return JSON.stringify({success:!1,image:null,error:"FAL.ai \u672A\u8FD4\u56DE\u6709\u6548\u56FE\u7247 URL",error_type:"FalError"});let w=v[0],R=await pd(String(w.url),md(String(w.content_type||""),String(w.url)));return JSON.stringify({success:!0,image:R,url:w.url,width:w.width,height:w.height,upscaled:w.upscaled,modality:u,count:v.length})}catch(p){let m=p instanceof Error?p.message:String(p);return JSON.stringify({success:!1,image:null,error:m,error_type:p instanceof Error?p.constructor.name:"Error"})}}static async upscale(e,t){try{let r={image_url:e,prompt:t||nd,negative_prompt:id,creativity:od,resemblance:ad,guidance_scale:ld,num_inference_steps:cd,enable_safety_checker:!1},s=await J.submit(sd,r),n=await J.pollStatus(s.statusUrl,3e5);if(n.status!=="COMPLETED"||!n.responseUrl)return null;let o=(await J.getResult(n.responseUrl)).image;return!o||!o.url?null:{url:String(o.url),width:Number(o.width||0),height:Number(o.height||0)}}catch{return null}}};import*as tr from"node:fs";import*as pa from"node:os";import*as yi from"node:path";import{randomUUID as gd}from"node:crypto";var fd=300,aa=5e4,la=1e4;function ca(l){let e={head:Buffer.alloc(0),tail:Buffer.alloc(0),total:0},t=Math.floor(l*.4),r=l-t;return{get head(){return e.head},get tail(){return e.tail},get total(){return e.total},onChunk(s){if(e.total+=s.length,e.total<=l){e.head=Buffer.concat([e.head,s]);return}if(e.head.length<t){let n=t-e.head.length;e.head=Buffer.concat([e.head,s.subarray(0,n)])}e.tail=Buffer.concat([e.tail,s]),e.tail.length>r&&(e.tail=e.tail.subarray(e.tail.length-r))}}}function da(l,e){let t=l.head.length+l.tail.length,r=Math.max(l.total,t),s=r>t,n=Math.max(0,r-t),i;return s?i=l.head.toString("utf8",0,l.head.length).replace(/\uFFFD/g,"")+`

... [OUTPUT TRUNCATED - ${n.toLocaleString()} bytes omitted out of ${r.toLocaleString()} total] ...

`+l.tail.toString("utf8",0,l.tail.length).replace(/\uFFFD/g,""):i=Buffer.concat([l.head,l.tail]).toString("utf8").replace(/\uFFFD/g,""),{text:i,truncated:s,captured:t,omitted:n}}function ua(l){if(l.pid)if(process.platform==="win32")b.spawn("taskkill",["/pid",String(l.pid),"/T","/F"],{stdio:"ignore"});else try{process.kill(-l.pid,"SIGKILL")}catch{try{l.kill("SIGKILL")}catch{}}}var Xs=class l{name="execute_code";description="Run a Python script in an isolated sandbox subprocess (NOT your terminal). Use for data crunching, prototypes, one-off scripts. The script runs in a temp cwd with a 5-minute timeout; stdout is capped at 50KB (head/tail). Pure-Python only \u2014 to interact with the environment (files, web, shell) use the regular tools instead.";parameters={type:"object",properties:{code:{type:"string",description:"Python source code to execute. Must be self-contained."}},required:["code"]};static sanitizeEnv(e){let t={};for(let[r,s]of Object.entries(e)){if(s===void 0)continue;let n=r.toUpperCase();/(API|TOKEN|SECRET|PASSWORD|PASSWD|CREDENTIAL|AUTH|KEY)\b|_KEY$|_TOKEN$|_SECRET$/.test(n)||(t[r]=s)}return t}async execute(e,t,r){let s=String(e.code||"");if(!s.trim())return JSON.stringify({status:"error",error:"No code provided.",duration_seconds:0});let n=`${r?.platform||"chat"}:${r?.chatId||"default"}`,i=await ot().checkCommand(s,n,{source:r?.platform,isPython:!0});if(i.decision==="deny")return JSON.stringify({status:"error",error:i.reason,duration_seconds:0,note:"Code was NOT executed."});if(i.decision==="approval_required")return JSON.stringify({type:"approval_required",command:s.slice(0,500),description:i.description,message:i.message,instruction:"Ask the user to approve this code (they reply \u6279\u51C6/approve). The code has NOT been executed. If approved, retry with the exact same code."});let o=Math.max(1,Number(e.timeout||process.env.EXECUTE_CODE_TIMEOUT||fd)),a=tr.mkdtempSync(yi.join(pa.tmpdir(),`kexvim_sandbox_${gd().slice(0,8)}_`)),c=yi.join(a,"script.py");try{tr.writeFileSync(c,s,"utf8");let d=ca(aa),u=ca(la),p=Date.now(),m=b.spawn("python",["-u",c],{cwd:a,env:{...l.sanitizeEnv(process.env),PYTHONIOENCODING:"utf-8",PYTHONUTF8:"1",KEXVIM_SANDBOX:"1"},stdio:["ignore","pipe","pipe"],detached:process.platform!=="win32"});m.stdout.on("data",w=>d.onChunk(w)),m.stderr.on("data",w=>u.onChunk(w));let g=setTimeout(()=>ua(m),o*1e3),f=!1;t&&t.addEventListener("abort",()=>{f=!0,ua(m)},{once:!0});let h=await new Promise(w=>{m.on("close",R=>w(R)),m.on("error",R=>{u.onChunk(Buffer.from(`Failed to spawn python: ${R.message}`)),w(null)})});clearTimeout(g);let k=(Date.now()-p)/1e3,y=da(d,aa),v=da(u,la);return f||h===null&&!v.text?JSON.stringify({status:"error",error:`Timed out after ${o} seconds`,stdout:y.text,stderr:v.text,duration_seconds:Math.round(k*10)/10,exit_code:h,stdout_truncated:y.truncated,stderr_truncated:v.truncated}):JSON.stringify({status:h===0?"success":"error",stdout:y.text,stderr:v.text,duration_seconds:Math.round(k*10)/10,exit_code:h,stdout_bytes:y.captured,stdout_bytes_total:d.total,stdout_truncated:y.truncated,stderr_bytes:v.captured,stderr_bytes_total:u.total,stderr_truncated:v.truncated})}catch(d){return JSON.stringify({status:"error",error:d instanceof Error?d.message:String(d),duration_seconds:0})}finally{try{tr.rmSync(a,{recursive:!0,force:!0})}catch{}}}};import*as Le from"node:fs";import*as We from"node:path";import*as ma from"node:os";function hd(){let l=process.env.FAL_VIDEO_MODEL?.trim();return l?{textToVideo:l,imageToVideo:l,keyframes:l,continuation:l}:{textToVideo:"fal-ai/bfl-flux3/text-to-video",imageToVideo:"fal-ai/bfl-flux3/image-to-video",keyframes:"fal-ai/bfl-flux3/keyframes-to-video",continuation:"fal-ai/bfl-flux3/video-continuation"}}var yd=["auto","21:9","16:9","4:3","1:1","3:4","9:16","9:21"],vd=["720p"],vi=10;function ga(){let l=I.findProjectRoot();return We.join(l||process.cwd(),"data","fal-jobs.json")}function fa(){try{let l=Le.readFileSync(ga(),"utf8");return JSON.parse(l)}catch{return{}}}function bd(l){let e=fa();e[l.requestId]=l;let t=Object.keys(e);for(;t.length>200;)delete e[t.shift()];try{let r=ga();Le.mkdirSync(We.dirname(r),{recursive:!0}),Le.writeFileSync(r,JSON.stringify(e,null,2))}catch{}}function kd(l){return fa()[l]}function nn(){return{prompt:{type:"string",minLength:1,description:'Generation brief in plain prose. Order it subject, visual specifics, action, camera, lighting, environment, audio, style. Audio is generated by default \u2014 say "no music" if unwanted.'},aspect_ratio:{type:"string",enum:yd,default:"auto",description:'Output aspect ratio. "auto" lets the model choose.'},duration:{oneOf:[{type:"integer",minimum:5,maximum:20},{type:"string",const:"auto"}],default:"auto",description:'Clip duration in whole seconds (5-20), or "auto".'},resolution:{type:"string",enum:vd,default:"720p",description:"Output resolution bin."},generate_audio:{type:"boolean",default:!0,description:"Generate synchronized audio."},grounding:{type:"boolean",default:!0,description:"Allow a short research pass before generation."},seed:{type:"integer",minimum:0,maximum:4294967295,description:"Optional reproducibility seed."},version:{type:"string",description:'Model version pin. Defaults to "latest".'}}}var on="Read bfl_flux3_prompting_guide before your first generation. ",an="All guidance is defaults: explicit user instructions override it.",bi="Media fields accept a local file path (uploaded automatically to FAL) or a URL. ";async function ln(l,e,t){if(!J.isConfigured())return JSON.stringify({error:J.noKeyMessage("\u89C6\u9891\u751F\u6210")});let r=hd(),s;switch(l){case"text_to_video":s=r.textToVideo;break;case"image_to_video":s=r.imageToVideo;break;case"keyframes":s=r.keyframes;break;case"continuation":s=r.continuation;break;default:s=r.textToVideo}let n={};for(let i of["prompt","aspect_ratio","duration","resolution","generate_audio","grounding","seed","version"])e[i]!==void 0&&(n[i]=e[i]);try{l==="image_to_video"&&typeof e.input_image=="string"&&(n.input_image=await J.resolveMedia(e.input_image)),l==="keyframes"&&Array.isArray(e.input_images)&&(n.input_images=await Promise.all(e.input_images.slice(0,vi).map(o=>J.resolveMedia(o))),Array.isArray(e.keyframe_indices)&&(n.keyframe_indices=e.keyframe_indices)),l==="continuation"&&typeof e.input_video=="string"&&(n.input_video=await J.resolveMedia(e.input_video,55*1024*1024));let i=await J.submit(s,n);return bd({requestId:i.requestId,model:s,statusUrl:i.statusUrl,responseUrl:i.responseUrl,mode:l,createdAt:Date.now()}),JSON.stringify({job_id:i.requestId,status:"submitted",model:s,message:"Generation takes several minutes. Poll bfl_flux3_get_result with this job id."})}catch(i){return JSON.stringify({error:i instanceof Error?i.message:String(i)})}}async function Sd(l,e,t){let r=await fetch(l,{signal:AbortSignal.timeout(3e5)});if(!r.ok)throw new Error(`\u89C6\u9891\u4E0B\u8F7D\u5931\u8D25 (HTTP ${r.status})`);let s=Buffer.from(await r.arrayBuffer()),n=`bfl_flux3_${new Date().toISOString().replace(/[:.]/g,"-").slice(0,19)}.mp4`,i;if(e){let d=We.resolve(e);i=/\.mp4$/i.test(d)?d:We.join(d,n)}else i=We.join(ma.homedir(),"Downloads",n);let o=We.dirname(i);Le.mkdirSync(o,{recursive:!0});let a=i,c=1;for(;Le.existsSync(a);)a=i.replace(/\.mp4$/i,"")+`-${c}.mp4`,c++;return Le.writeFileSync(a,s),{target:a,size:s.length}}var Qs=class{name="bfl_flux3_text_to_video";description=on+"FLUX 3 text-to-video: generates a clip (with audio) from the prompt alone. Nothing but the prompt anchors the subject here, so research anything with a real, checkable appearance before writing it. Generation takes several minutes: this returns a job id immediately; poll bfl_flux3_get_result. "+an;parameters={type:"object",properties:nn(),required:["prompt"],additionalProperties:!1};async execute(e,t,r){let s=String(e.prompt||"").trim();return s?ln("text_to_video",{...e,prompt:s},r?.chatId):JSON.stringify({error:"prompt is required"})}},Zs=class{name="bfl_flux3_image_to_video";description=on+"FLUX 3 image-to-video: animates one image as the literal opening frame \u2014 those pixels are frame 0 and the clip moves from there. "+bi+"Returns a job id; poll bfl_flux3_get_result. "+an;parameters={type:"object",properties:{...nn(),input_image:{type:"string",minLength:1,description:"Exactly one opening-frame image: a local file path or a URL (PNG/JPEG/WebP, up to 10MB)."}},required:["prompt","input_image"],additionalProperties:!1};async execute(e,t,r){let s=String(e.prompt||"").trim();return s?typeof e.input_image!="string"||!e.input_image.trim()?JSON.stringify({error:"input_image is required"}):ln("image_to_video",{...e,prompt:s},r?.chatId):JSON.stringify({error:"prompt is required"})}},en=class{name="bfl_flux3_keyframes_to_video";description=on+"FLUX 3 keyframe video: a storyboard of 1-10 images pinned at chosen frame positions (24fps). Name the subject and describe the motion that carries it between the pins, and keep the subject consistent across every pinned image. "+bi+"Returns a job id; poll bfl_flux3_get_result. "+an;parameters={type:"object",properties:{...nn(),input_images:{type:"array",items:{type:"string",minLength:1},minItems:1,maxItems:vi,description:"1-10 keyframe images: local file paths or URLs (PNG/JPEG/WebP, up to 10MB each)."},keyframe_indices:{type:"array",items:{type:"integer",minimum:0,maximum:480},minItems:1,maxItems:vi,description:'One unique non-negative frame index per image (24fps). Each must be at most duration\xD724, so set an explicit duration rather than "auto" whenever you pin indices.'}},required:["prompt","input_images","keyframe_indices"],additionalProperties:!1};async execute(e,t,r){let s=String(e.prompt||"").trim();return s?!Array.isArray(e.input_images)||e.input_images.length===0?JSON.stringify({error:"input_images is required"}):ln("keyframes",{...e,prompt:s},r?.chatId):JSON.stringify({error:"prompt is required"})}},tn=class{name="bfl_flux3_video_continuation";description=on+`FLUX 3 video continuation: the new generation picks up from the input clip's final frames. Open the prompt with "Continue this video from its final frames:", re-establish the subject and the moment it ended on, then describe what happens next. input_video must be an mp4 of at most 50MB and 15 seconds; the generated segment tops out at 15s too \u2014 chain a second continuation for a longer sequence. duration is the new segment only. `+bi+"Returns a job id; poll bfl_flux3_get_result. "+an;parameters={type:"object",properties:{...nn(),input_video:{type:"string",minLength:1,description:"The clip to continue: a local file path or a URL. mp4 only, at most 50MB and 15 seconds."}},required:["prompt","input_video"],additionalProperties:!1};async execute(e,t,r){let s=String(e.prompt||"").trim();return s?typeof e.input_video!="string"||!e.input_video.trim()?JSON.stringify({error:"input_video is required"}):ln("continuation",{...e,prompt:s},r?.chatId):JSON.stringify({error:"prompt is required"})}},rn=class{name="bfl_flux3_get_result";description="Poll a FLUX 3 video job by the job id a generate tool returned. Generation takes minutes and a long Generating phase is normal. This call waits for you while the job runs (up to ~5 minutes), so it may run for several minutes; if it returns still generating, just call it again. On Ready the clip is downloaded for you and the response gives its local path.";parameters={type:"object",properties:{id:{type:"string",minLength:1,description:"Job id from a previous bfl_flux3_* generate call."},save_to:{type:"string",description:"Where to save the finished clip: a directory or a full file path. Set this only when the user asked for a particular location; the default is ~/Downloads. An existing file is never overwritten."}},required:["id"],additionalProperties:!1};async execute(e,t,r){let s=String(e.id||"").trim();if(!s)return JSON.stringify({error:"id is required"});let n=kd(s);if(!n)return JSON.stringify({error:`Unknown job id '${s}'. Generate a video first with a bfl_flux3_* tool (job records persist in data/fal-jobs.json).`});try{let i=await J.pollStatus(n.statusUrl,3e5,5e3);if(i.status==="TIMEOUT")return JSON.stringify({job_id:s,status:"still_generating",message:"Job is still generating. Call bfl_flux3_get_result again with the same id."});if(i.status!=="COMPLETED"||!i.responseUrl)return JSON.stringify({job_id:s,status:"failed",error:i.error||`Task ${i.status}`});let o=await J.getResult(i.responseUrl),c=(o.video??(Array.isArray(o.videos)?o.videos[0]:void 0))||{},d=String(c.url||o.url||"");if(!d)return JSON.stringify({job_id:s,status:"failed",error:"\u5B8C\u6210\u4F46\u7ED3\u679C\u65E0\u89C6\u9891 URL",details:String(JSON.stringify(o)).slice(0,500)});let u=await Sd(d,e.save_to?String(e.save_to):void 0,Date.now());return JSON.stringify({job_id:s,status:"ready",video_path:u.target,size_bytes:u.size,url:d,duration_seconds:c.duration_seconds,width:c.width,height:c.height,message:`Deliver the video to the user with MEDIA:${u.target}`})}catch(i){return JSON.stringify({job_id:s,status:"error",error:i instanceof Error?i.message:String(i)})}}},sn=class{name="bfl_flux3_prompting_guide";description="Read this before your first FLUX 3 generation. The prompting and grounding guide: how to research a subject so it renders as itself, how to assemble a prompt, which generate tool fits, and how to save and deliver the finished clip. Takes no arguments and spends no generation budget.";parameters={type:"object",properties:{},additionalProperties:!1};async execute(e,t,r){return`# FLUX 3 video generation \u2014 how to get the best results
...
4. On ready the clip is downloaded; deliver it with MEDIA:<path> in your response.`}};import*as ba from"node:fs";import*as Si from"node:path";import*as ki from"node:dns";var _d=600*1e3,ha=500,ya=15e3,cn=null;async function wd(){if(cn)return cn;try{cn=await import("playwright")}catch(l){throw new Error(`playwright \u672A\u5B89\u88C5\uFF1A\u8BF7\u5148\u6267\u884C npm i playwright && npx playwright install chromium\uFF08\u5F53\u524D\uFF1A${l instanceof Error?l.message:String(l)}\uFF09`)}return cn}var ee=class l{static sessions=new Map;static consoleLogs=new Map;static dialogLogs=new Map;static async ensure(e,t){let r=e||"default",s=l.sessions.get(r);if(s)return s;let n=await wd(),i;if(t){let o=await n.chromium.connectOverCDP(t),a=o.contexts()[0]||await o.newContext(),c=a.pages()[0]||await a.newPage();i={browser:o,context:a,page:c,cdp:!0,lastActivity:Date.now()}}else{let o=await n.chromium.launch({headless:!0}),a=await o.newContext({viewport:{width:1280,height:800}}),c=await a.newPage();i={browser:o,context:a,page:c,cdp:!1,lastActivity:Date.now()}}return l.dialogLogs.set(r,[]),i.page.on("dialog",async o=>{(l.dialogLogs.get(r)||[]).push(`${o.type()}: ${o.message().slice(0,300)}`);try{await o.accept()}catch{}}),l.consoleLogs.set(r,{messages:[],errors:[]}),i.page.on("console",o=>{let a=l.consoleLogs.get(r);if(!a)return;let c=o.text().slice(0,500);o.type()==="error"?a.errors.push(c):a.messages.push(c)}),i.page.on("pageerror",o=>{l.consoleLogs.get(r)?.errors.push(`pageerror: ${o.message.slice(0,500)}`)}),l.sessions.set(r,i),i}static touch(e){let t=l.sessions.get(e||"default");t&&(t.lastActivity=Date.now())}static async reapIdle(){let e=Date.now();for(let[t,r]of[...l.sessions.entries()])if(e-r.lastActivity>_d){try{await r.browser.close()}catch{}l.sessions.delete(t),l.consoleLogs.delete(t),l.dialogLogs.delete(t)}}static get(e){let t=l.sessions.get(e||"default");return t&&(t.lastActivity=Date.now()),t}static consoleState(e){return l.consoleLogs.get(e||"default")||{messages:[],errors:[]}}static dialogs(e){return l.dialogLogs.get(e||"default")||[]}static async closeAll(){for(let e of l.sessions.values())try{await e.browser.close()}catch{}l.sessions.clear()}};async function ka(l,e){if(e)return(await l.evaluate(()=>document.body?.innerText||"")).slice(0,ya*2);let r=l.locator('a[href], button, input, textarea, select, [role="button"], [role="link"], [role="checkbox"], [role="radio"], [role="tab"], [role="menuitem"], [contenteditable="true"], [tabindex]:not([tabindex="-1"])'),s=Math.min(await r.count(),120),n=[],i=await l.evaluate(()=>document.title||"");n.push(`URL: ${l.url()}`),n.push(`Title: ${i}`),n.push(""),n.push(`Interactive elements (${s}):`);for(let a=0;a<s;a++){let c=r.nth(a),d=await c.evaluate(m=>{let g=m.tagName.toLowerCase(),f=(m.textContent||"").trim().replace(/\s+/g," ").slice(0,80),h=m.type||"";return{tag:g,txt:f,type:h}}).catch(()=>({tag:"?",txt:"",type:""})),u=await c.getAttribute("href").catch(()=>null),p=`[ref=${a+1}] <${d.tag}${d.type?` type="${d.type}"`:""}> ${d.txt||d.tag}`;n.push(u?`${p} href="${String(u).slice(0,100)}"`:p)}let o=await l.evaluate(()=>document.body?.innerText||"").catch(()=>"");return n.push(""),n.push(`Page text (${o.length} chars):`),n.push(o.replace(/\n{3,}/g,`

`).slice(0,ya)),n.join(`
`)}async function Sa(l,e){let t=0,r=/^@?e(\d+)$/i.exec(e.trim());if(r)t=Number(r[1])-1;else{let n=/^\d+$/.exec(e.trim());if(n)t=Number(n)-1;else throw new Error(`ref \u5FC5\u987B\u662F @eN \u5F62\u5F0F\uFF08\u5982 @e5\uFF09\uFF0C\u6536\u5230 '${e}'\u3002\u5148 browser_snapshot \u83B7\u53D6 ref\u3002`)}return l.locator('a[href], button, input, textarea, select, [role="button"], [role="link"], [role="checkbox"], [role="radio"], [role="tab"], [role="menuitem"], [contenteditable="true"], [tabindex]:not([tabindex="-1"])').nth(t)}async function xd(l){let e=l.toLowerCase().replace(/\.$/,"");if(e==="localhost"||e.endsWith(".localhost"))return!0;if(/^\d+\.\d+\.\d+\.\d+$/.test(e))return va(e);try{let t=[];try{t.push(...await ki.promises.resolve4(e))}catch{}try{t.push(...await ki.promises.resolve6(e))}catch{}return t.some(va)}catch{return!1}}function va(l){if(l==="::1"||l.startsWith("fc")||l.startsWith("fd")||l.startsWith("fe80"))return!0;let e=l.split(".").map(Number);if(e.length!==4)return!1;let[t,r]=e;return t===10||t===127||t===172&&r>=16&&r<=31||t===192&&r===168||t===0||t===169&&r===254}async function _i(l){try{let e=new URL(l);return await xd(e.hostname)}catch{return!0}}async function _n(l,e){return await _i(l.url())?`browser_${e} \u88AB\u963B\u6B62\uFF1A\u5F53\u524D\u9875\u9762\u662F\u79C1\u6709/\u672C\u5730\u5730\u5740\uFF08${l.url()}\uFF09\uFF0C\u4E0D\u5141\u8BB8\u5728\u79C1\u6709\u9875\u9762\u4E0A\u64CD\u4F5C\u3002`:null}function Td(l){let e=[],t=/https?:\/\/[^\s'"`)\]<>]+/g;for(let r of l.matchAll(t))e.push(r[0].replace(/[.,;]$/,""));return e}var Ed=[[/document\s*\.\s*cookie/i,"document.cookie"],[/\b(?:localStorage|sessionStorage)\b/i,"web storage"],[/\bindexedDB\b/i,"IndexedDB"],[/\bcaches\s*\.\s*(?:open|match|keys)\b/i,"Cache Storage"],[/\bnavigator\s*\.\s*(?:clipboard|credentials|serviceWorker)\b/i,"navigator sensitive API"],[/\b(?:fetch|XMLHttpRequest|WebSocket|EventSource|sendBeacon)\s*\(/i,"network request"]];function Cd(){return process.env.KEXVIM_BROWSER_RESTRICT_EVALUATE==="1"}function Rd(l){if(Cd()){for(let[e,t]of Ed)if(e.test(l))return`Blocked: browser_console(expression=...) tried to use sensitive browser primitive: ${t} (KEXVIM_BROWSER_RESTRICT_EVALUATE=1).`}return null}function Md(){let l=I.findProjectRoot(),e=Si.join(l||process.cwd(),"data","media");return ba.mkdirSync(e,{recursive:!0}),e}var dn=class{name="browser_navigate";description="Open a URL in the browser. Returns the page title, final URL and a compact snapshot of interactive elements (refs like [ref=1] for browser_click / browser_type). First call starts a headless browser session for this task.";parameters={type:"object",properties:{url:{type:"string",description:"The URL to navigate to (http/https)."}},required:["url"]};async execute(e,t,r){let s=String(e.url||"").trim();if(!s)return JSON.stringify({success:!1,error:"url is required"});let n=s;if(/^https?:\/\//i.test(n)||(n=`https://${n}`),await _i(n))return JSON.stringify({success:!1,error:`\u5BFC\u822A\u88AB\u963B\u6B62\uFF1A${n} \u662F\u79C1\u6709/\u672C\u5730\u5730\u5740\uFF0C\u6D4F\u89C8\u5668\u4E0D\u5141\u8BB8\u8BBF\u95EE\u79C1\u7F51\u3002`});try{let i=await ee.ensure("default");await i.page.goto(n,{waitUntil:"domcontentloaded",timeout:6e4}),await i.page.waitForTimeout(800);let o=await ka(i.page,!1);return JSON.stringify({success:!0,url:i.page.url(),title:await i.page.title(),snapshot:o})}catch(i){return JSON.stringify({success:!1,error:i instanceof Error?i.message:String(i)})}}},un=class{name="browser_snapshot";description="Get a snapshot of the current page: interactive elements with ref IDs ([ref=1], [ref=2]...) for browser_click and browser_type, plus page text. full=true returns complete page content (may be truncated at ~30k chars). Requires browser_navigate first.";parameters={type:"object",properties:{full:{type:"boolean",default:!1,description:"true = full page text; false (default) = compact view with interactive element refs."}}};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{let n=await ka(s.page,e.full===!0);return JSON.stringify({success:!0,snapshot:n})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},pn=class{name="browser_click";description="Click an interactive element by ref (from browser_snapshot, e.g. @e5 or 5).";parameters={type:"object",properties:{ref:{type:"string",description:"Element ref like @e5 (or plain number)."}},required:["ref"]};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});let n=await _n(s.page,"click");if(n)return JSON.stringify({success:!1,error:n});try{let i=await Sa(s.page,String(e.ref||""));return await i.scrollIntoViewIfNeeded({timeout:5e3}).catch(()=>{}),await i.click({timeout:1e4}),await s.page.waitForTimeout(500),JSON.stringify({success:!0,message:`Clicked ${e.ref}.`})}catch(i){return JSON.stringify({success:!1,error:i instanceof Error?i.message:String(i)})}}},mn=class{name="browser_type";description="Fill an input element with text by ref (from browser_snapshot, e.g. @e3).";parameters={type:"object",properties:{ref:{type:"string",description:"Element ref like @e3."},text:{type:"string",description:"Text to type into the field."}},required:["ref","text"]};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});let n=await _n(s.page,"type");if(n)return JSON.stringify({success:!1,error:n});try{return await(await Sa(s.page,String(e.ref||""))).fill(String(e.text??""),{timeout:1e4}),JSON.stringify({success:!0,message:`Filled ${e.ref}.`})}catch(i){return JSON.stringify({success:!1,error:i instanceof Error?i.message:String(i)})}}},gn=class{name="browser_scroll";description="Scroll the page. direction: down (default) or up. Scrolls ~500px per call; call repeatedly to page through.";parameters={type:"object",properties:{direction:{type:"string",enum:["down","up"],default:"down"}}};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{let n=e.direction==="up"?-ha:ha;return await s.page.evaluate(i=>window.scrollBy(0,i),n),await s.page.waitForTimeout(300),JSON.stringify({success:!0,message:`Scrolled ${e.direction||"down"} ${Math.abs(n)}px.`})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},fn=class{name="browser_back";description="Go back to the previous page in the browser history.";parameters={type:"object",properties:{}};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{return await s.page.goBack({timeout:3e4}),await s.page.waitForTimeout(500),JSON.stringify({success:!0,url:s.page.url()})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},hn=class{name="browser_press";description="Press a keyboard key in the page (e.g. Enter, Escape, ArrowDown, Tab).";parameters={type:"object",properties:{key:{type:"string",description:"Key name (Enter, Escape, ArrowDown, Tab, ...)."}},required:["key"]};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});let n=await _n(s.page,"press");if(n)return JSON.stringify({success:!1,error:n});try{return await s.page.keyboard.press(String(e.key||"")),JSON.stringify({success:!0,message:`Pressed ${e.key}.`})}catch(i){return JSON.stringify({success:!1,error:i instanceof Error?i.message:String(i)})}}},yn=class{name="browser_get_images";description="List image URLs and alt texts currently on the page.";parameters={type:"object",properties:{}};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{let n=await s.page.evaluate(()=>Array.from(document.querySelectorAll("img")).slice(0,50).map(i=>({src:i.src.slice(0,300),alt:i.alt.slice(0,100),width:i.naturalWidth,height:i.naturalHeight})).filter(i=>i.src));return JSON.stringify({success:!0,count:n.length,images:n})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},vn=class{name="browser_vision";description="Take a screenshot of the current page so you can inspect it visually. Returns a screenshot path the model can analyze (needs a vision-capable model). Use for CAPTCHAs, visual layouts, or when the text snapshot misses something.";parameters={type:"object",properties:{question:{type:"string",description:"Optional: what to look for (used as analysis prompt)."},annotate:{type:"boolean",default:!1,description:"Reserved for future annotated snapshot; currently ignored."}}};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});try{let n=Si.join(Md(),`browser_shot_${Date.now()}.png`);return await s.page.screenshot({path:n,fullPage:!1}),JSON.stringify({type:"vision",path:n,mimeType:"image/png",prompt:String(e.question||"Describe what you see on this page, focusing on layout, text and interactive elements."),url:s.page.url()})}catch(n){return JSON.stringify({success:!1,error:n instanceof Error?n.message:String(n)})}}},bn=class l{name="browser_console";description="Read browser console messages (clear=true clears the log), or evaluate a JavaScript expression in the page (expression=...). Evaluation with sensitive primitives (cookies, storage, network) is blocked when KEXVIM_BROWSER_RESTRICT_EVALUATE=1; navigation to private/local addresses is always blocked.";parameters={type:"object",properties:{clear:{type:"boolean",default:!1,description:"Clear the console log (only meaningful without expression)."},expression:{type:"string",description:"Optional JS expression to evaluate in the page (result JSON-serialized)."}}};async execute(e,t,r){let s=ee.get("default");if(!s)return JSON.stringify({success:!1,error:"No active browser session. Call browser_navigate first."});let n=e.expression!==void 0?String(e.expression):void 0;if(n!==void 0&&n.trim()){let o=Rd(n);if(o)return JSON.stringify({success:!1,error:o});let a=await _n(s.page,"console");if(a)return JSON.stringify({success:!1,error:a});for(let c of Td(n))if(await _i(c))return JSON.stringify({success:!1,error:`browser_console eval \u88AB\u963B\u6B62\uFF1A\u8868\u8FBE\u5F0F\u5305\u542B\u79C1\u6709/\u672C\u5730\u5730\u5740 ${c}\u3002`});try{let c=await s.page.evaluate(d=>{try{return new Function(`return (${d});`)()}catch{return new Function(d)()}},n);return JSON.stringify({success:!0,result:l.serialize(c)})}catch(c){return JSON.stringify({success:!1,error:c instanceof Error?c.message:String(c)})}}let i=ee.consoleState("default");return e.clear===!0?(i.messages=[],i.errors=[],JSON.stringify({success:!0,messages:[],errors:[],cleared:!0})):JSON.stringify({success:!0,messages:i.messages.slice(-50),errors:i.errors.slice(-50)})}static serialize(e){if(e===void 0)return"undefined";try{return JSON.parse(JSON.stringify(e))}catch{return String(e)}}},kn=class{name="browser_cdp";description="Attach the browser session to an existing Chrome/Chromium via its CDP WebSocket URL (e.g. from chrome://inspect, or a remote debugging port). Subsequent browser_* calls operate on that browser. Session is per-task; existing session is replaced.";parameters={type:"object",properties:{url:{type:"string",description:"CDP WebSocket debugger URL (ws://host:port/devtools/browser/...)."}},required:["url"]};async execute(e,t,r){let s=String(e.url||"").trim();if(!/^ws:\/\//i.test(s))return JSON.stringify({success:!1,error:"url \u5FC5\u987B\u662F CDP WebSocket \u5730\u5740\uFF08ws://...\uFF09\uFF0C\u5982 ws://127.0.0.1:9222/devtools/browser/xxx"});try{let n=ee.get("default");if(n)try{await n.browser.close()}catch{}let o=(await ee.ensure("default",s)).context.pages().map(a=>a.url());return JSON.stringify({success:!0,attached:!0,pages:o})}catch(n){return JSON.stringify({success:!1,error:`CDP \u8FDE\u63A5\u5931\u8D25\uFF1A${n instanceof Error?n.message:String(n)}`})}}},Sn=class{name="browser_dialog";description="Show dialogs (alert/confirm/prompt) that appeared on the page. Dialogs are auto-accepted and recorded; this tool lists them so you know what the page asked. No arguments.";parameters={type:"object",properties:{}};async execute(e,t,r){let s=ee.dialogs("default");return s.length?JSON.stringify({success:!0,dialogs:s}):JSON.stringify({success:!0,dialogs:[],message:"No dialogs were shown."})}};process.on("exit",()=>{ee.closeAll()});var Ad=setInterval(()=>{ee.reapIdle()},6e4);Ad.unref();import*as _a from"node:crypto";var wn=class l{static build(e,t,r,s,n,i){let o=_a.randomUUID(),a="";try{a=n.prefetchAll(e,s)}catch(d){console.warn(S.t("memory.tcb_prefetch_failed",{error:d instanceof Error?d.message:String(d)}))}let c=l._findLastUserIndex(r);return{userMessage:e,messages:r,activeSystemPrompt:t,turnId:o,extPrefetchCache:a,currentTurnUserIdx:c}}static _findLastUserIndex(e){for(let t=e.length-1;t>=0;t--)if(e[t].role==="user")return t;return e.length}};var xa=5e3,wa=new Set(["clarify","delegate_task","memory","read_file","write_file","edit","bash","search_files","terminal","execute_code"]),wi=class{queue=[];_running=!1;submit(e){this.queue.push(e),this._running||this.drain()}drain(){this._running=!0;let e=()=>{if(this.queue.length===0){this._running=!1;return}let t=this.queue.shift();try{t()}catch{}setImmediate(e)};setImmediate(e)}flush(e=xa){return new Promise(t=>{if(this.queue.length===0){t(!0);return}let r=setTimeout(()=>t(!1),e);this.submit(()=>{clearTimeout(r),t(!0)})})}shutdown(){this.queue=[],this._running=!1}};var _r=class l{static sanitizeContext(e){return e.replace(/<memory-context>[\s\S]*?<\/memory-context>/gi,"").replace(/\[System note:\s*The following is recalled memory context,\s*NOT new user input\.\s*Treat as [^\]]*\]\s*/gi,"").replace(/<\/?\s*memory-context\s*>/gi,"")}static buildMemoryContextBlock(e){return!e||!e.trim()?"":`<memory-context>
...
`+l.sanitizeContext(e)+`
</memory-context>`}},wr=class l{providers=[];toolToProvider=new Map;hasExternal=!1;bg=new wi;addProvider(e){if(!(e.name==="builtin")){if(this.hasExternal){console.warn(S.t("memory.memmgr_rejected_provider",{name:e.name}));return}this.hasExternal=!0}this.providers.push(e);for(let r of e.getToolSchemas()){let s=l.normalizeToolSchema(r);if(!s)continue;let n=s.name;if(!(!n||typeof n!="string")){if(wa.has(n)){console.warn(S.t("memory.memmgr_shadowed_tool",{providerName:e.name,toolName:n}));continue}this.toolToProvider.has(n)?console.warn(`Memory tool name conflict: '${n}' already registered by ${this.toolToProvider.get(n).name}`):this.toolToProvider.set(n,e)}}}get providersList(){return[...this.providers]}getProvider(e){return this.providers.find(t=>t.name===e)}buildSystemPrompt(){let e=[];for(let t of this.providers)try{let r=t.systemPromptBlock();r&&r.trim()&&e.push(r)}catch{}return e.join(`
...
`)}static depthOf(e,t){let r=0,s=e.findParent(t);for(;s&&s!==e.root;)r+=1,s=e.findParent(s.id);return r}_markBatchStarted(e){for(let t of e)this.graph.markStarted(t.id)}_handleNodeFailure(e,t){this.graph.markFailed(e.id,t),this._pruneChildren(e),this.strategy===Ge.Dfs&&(this._dfsCursor+=1)}_pruneChildren(e){for(let t of e.children)t.status===E.Pending&&this.graph.markSkipped(t.id),this._pruneChildren(t)}_computeDfsPaths(){let e=[],t=(r,s)=>{let n=[...s,r.id];if(r.children.length>0)for(let i of r.children)t(i,n);else e.push(n)};for(let r of this.graph.root.children)t(r,[]);return e}async _dfsStep(){for(;this._dfsCursor<this._dfsPaths.length;){let e=this._dfsPaths[this._dfsCursor];for(let t=this._dfsNodeIdx;t<e.length;t++){let r=e[t],s=this.graph.getNode(r);if(s!==null){if(s.status===E.Failed||s.status===E.Skipped)break;if(s.status!==E.Completed&&s.status===E.Pending){if(!s.dependsOn.every(i=>{let o=this.graph.getNode(i);return o!==null&&o.status===E.Completed}))break;return this._dfsNodeIdx=t,this.graph.markStarted(s.id),[await l.buildDelegationTask(this.graph,s,this.config,this._assembler)]}}}this._dfsCursor+=1,this._dfsNodeIdx=0}return null}};var x={Success:"success",Failure:"failure",Running:"running",Error:"error"},Ti=class{tree={};_nodes={};node(e,t=!1){let r=this._nodes[e];return r===void 0&&t&&(r={},this._nodes[e]=r),r===void 0?{}:r}},Er=class{_global={};_treeMem={};get(e,t,r){return this._resolve(t,r)[e]}set(e,t,r,s){let n=this._resolve(r,s,!0);n[e]=t}_resolve(e,t,r=!1){if(e===void 0)return this._global;let s=this._treeMem[e];if(s===void 0){if(!r)return{};s=new Ti,this._treeMem[e]=s}return t===void 0?s.tree:s.node(t,r)}},Ei=class{tree=null;target=null;blackboard=null;openNodes=[];nodeCount=0;enterNode(e){this.nodeCount+=1,this.openNodes.push(e)}openNode(e){}tickNode(e){}closeNode(e){this.openNodes.pop()}exitNode(e){}reset(){this.tree=null,this.target=null,this.blackboard=null,this.openNodes=[],this.nodeCount=0}},Cr=class{id;name;title;properties;child=null;children=[];constructor(e,t,r){this.id=q.newNodeId(),this.name=e||this.constructor.name,this.title=t||this.name,this.properties=r??{}}execute(e){this._enter(e),e.blackboard.get("isOpen",e.tree.id,this.id)||this._open(e);let t=this._tick(e);return t!==x.Running&&this._close(e),this._exit(e),t}_enter(e){e.enterNode(this),this.enter(e)}_open(e){e.openNode(this),e.blackboard.set("isOpen",!0,e.tree.id,this.id),this.open(e)}_tick(e){return e.tickNode(this),this.tick(e)}_close(e){e.closeNode(this),e.blackboard.set("isOpen",!1,e.tree.id,this.id),this.close(e)}_exit(e){e.exitNode(this),this.exit(e)}enter(e){}open(e){}close(e){}exit(e){}},wt=class extends Cr{constructor(e,t,r,s){super(e??"Composite",r,s),this.children=t?[...t]:[]}},Ci=class extends wt{constructor(e,t){super("Sequence",e,t)}tick(e){for(let t of this.children){let r=t.execute(e);if(r!==x.Success)return r}return x.Success}},Ri=class extends wt{constructor(e,t){super("Priority",e,t)}tick(e){for(let t of this.children){let r=t.execute(e);if(r!==x.Failure)return r}return x.Failure}},Rr=class extends wt{constructor(e,t){super("MemSequence",e,t)}open(e){e.blackboard.set("runningChild",0,e.tree.id,this.id)}tick(e){let t=e.blackboard.get("runningChild",e.tree.id,this.id)??0;for(let r=t;r<this.children.length;r++){let s=this.children[r].execute(e);if(s!==x.Success)return s===x.Running&&e.blackboard.set("runningChild",r,e.tree.id,this.id),s}return x.Success}},Mi=class extends wt{constructor(e,t){super("MemPriority",e,t)}open(e){e.blackboard.set("runningChild",0,e.tree.id,this.id)}tick(e){let t=e.blackboard.get("runningChild",e.tree.id,this.id)??0;for(let r=t;r<this.children.length;r++){let s=this.children[r].execute(e);if(s!==x.Failure)return s===x.Running&&e.blackboard.set("runningChild",r,e.tree.id,this.id),s}return x.Failure}},nr=class l extends wt{static SUCCESS_ON_ALL="success_on_all";static SUCCESS_ON_ONE="success_on_one";policy;constructor(e,t=l.SUCCESS_ON_ALL,r){super("Parallel",e,r,{policy:t}),this.policy=t}tick(e){let t=!1,r=0,s=0;for(let n of this.children){let i=n.execute(e);i===x.Running?t=!0:i===x.Failure?r+=1:i===x.Success&&(s+=1)}if(this.policy===l.SUCCESS_ON_ONE){if(s>0)return x.Success;if(r===this.children.length)return x.Failure}else{if(r>0)return x.Failure;if(s===this.children.length)return x.Success}return t?x.Running:x.Failure}},at=class extends Cr{constructor(e,t,r,s){super(t??"Decorator",r,s),this.child=e??null}},Ai=class extends at{constructor(e){super(e,"Inverter")}tick(e){if(!this.child)return x.Error;let t=this.child.execute(e);return t===x.Success?x.Failure:t===x.Failure?x.Success:t}},Ii=class extends at{maxAttempts;constructor(e,t=3){super(e,"Retry",void 0,{maxAttempts:t}),this.maxAttempts=t}open(e){e.blackboard.set("attempts",0,e.tree.id,this.id)}tick(e){if(!this.child)return x.Error;let t=e.blackboard.get("attempts",e.tree.id,this.id)??0;for(;t<this.maxAttempts;){let r=this.child.execute(e);if(r===x.Success)return x.Success;if(r===x.Running)return x.Running;t+=1,e.blackboard.set("attempts",t,e.tree.id,this.id)}return x.Failure}},Pi=class extends at{maxTime;constructor(e,t=10){super(e,"MaxTime",void 0,{maxTime:t}),this.maxTime=t}open(e){e.blackboard.set("startTime",Date.now()/1e3,e.tree.id,this.id)}tick(e){if(!this.child)return x.Error;let t=e.blackboard.get("startTime",e.tree.id,this.id)??Date.now()/1e3;return Date.now()/1e3-t>this.maxTime?x.Failure:this.child.execute(e)}},Li=class extends at{maxLoop;constructor(e,t=3){super(e,"Repeater",void 0,{maxLoop:t}),this.maxLoop=t}open(e){e.blackboard.set("loopCount",0,e.tree.id,this.id)}tick(e){if(!this.child)return x.Error;let t=e.blackboard.get("loopCount",e.tree.id,this.id)??0;return this.child.execute(e),t+=1,e.blackboard.set("loopCount",t,e.tree.id,this.id),t>=this.maxLoop?x.Success:x.Running}},Ni=class extends at{maxLoop;constructor(e,t=30){super(e,"RepeatUntilFailure",void 0,{maxLoop:t}),this.maxLoop=t}open(e){e.blackboard.set("loopCount",0,e.tree.id,this.id)}tick(e){if(!this.child)return x.Error;let t=e.blackboard.get("loopCount",e.tree.id,this.id)??0;for(;t<this.maxLoop;){let r=this.child.execute(e);if(r===x.Failure)return x.Success;if(r===x.Running)return x.Running;t+=1,e.blackboard.set("loopCount",t,e.tree.id,this.id)}return x.Failure}},Oi=class extends at{maxLoop;constructor(e,t=30){super(e,"RepeatUntilSuccess",void 0,{maxLoop:t}),this.maxLoop=t}open(e){e.blackboard.set("loopCount",0,e.tree.id,this.id)}tick(e){if(!this.child)return x.Error;let t=e.blackboard.get("loopCount",e.tree.id,this.id)??0;for(;t<this.maxLoop;){let r=this.child.execute(e);if(r===x.Success)return x.Success;if(r===x.Running)return x.Running;t+=1,e.blackboard.set("loopCount",t,e.tree.id,this.id)}return x.Failure}},Ke=class extends Cr{constructor(e,t,r){super(e??"Action",t,r)}},$i=class extends Ke{tick(e){return x.Success}},Di=class extends Ke{tick(e){return x.Failure}},Fi=class extends Ke{tick(e){return x.Running}},Cn=class extends Ke{tick(e){return x.Error}},Bi=class extends Ke{milliseconds;constructor(e=0){super("Wait",`Wait ${e}ms`,{milliseconds:e}),this.milliseconds=e}open(e){e.blackboard.set("startTime",Date.now()/1e3,e.tree.id,this.id)}tick(e){let t=e.blackboard.get("startTime",e.tree.id,this.id)??Date.now()/1e3;return(Date.now()/1e3-t)*1e3>this.milliseconds?x.Success:x.Running}},Nd=new Set(["Sequence","Priority","MemSequence","MemPriority","Parallel"]),Od=new Set(["Inverter","Retry","MaxTime","Repeater","RepeatUntilFailure","RepeatUntilSuccess"]),q=class l{id;root=null;title="BehaviorTree";description="";static _registry=new Map;constructor(){this.id=l.newNodeId()}static newNodeId(){return Math.random().toString(16).slice(2,14)}static registerNode(e,t){l._registry.set(e,t)}tick(e=null,t=null){let r=t??new Er,s=new Ei;if(s.tree=this,s.target=e,s.blackboard=r,!this.root)return x.Error;let n=this.root.execute(s),i=r.get("openNodes",this.id)??[],o=[...s.openNodes],a=0;for(let c=0;c<Math.min(i.length,o.length)&&(a=c+1,i[c]===o[c]);c++);for(let c=i.length-1;c>=a;c--)i[c].close(s);return r.set("openNodes",o,this.id),r.set("nodeCount",s.nodeCount,this.id),n}load(e){let t={},r=e.nodes??{};for(let[n,i]of Object.entries(r)){let o=i.name,a=l._registry.get(o);if(a===void 0)throw new Cn(`Unknown node type: ${o}`);let c=i.properties??{},d=i.category,u;d==="composite"||Nd.has(o)?u=new a([],i.title):d==="decorator"||Od.has(o)?(u=new a(null,i.title),"maxAttempts"in c&&"maxAttempts"in u&&(u.maxAttempts=c.maxAttempts),"maxTime"in c&&"maxTime"in u&&(u.maxTime=c.maxTime),"maxLoop"in c&&"maxLoop"in u&&(u.maxLoop=c.maxLoop)):u=new a(i.title),i.id&&(u.id=i.id),u.name=i.name??u.name,u.title=i.title??u.title,u.properties=c,t[n]=u}for(let[n,i]of Object.entries(r)){let o=t[n],a=i.children??[];for(let d of a)o.children.push(t[d]);let c=i.child;c&&(o.child=t[c])}this.title=e.title??this.title,this.description=e.description??"";let s=e.root;this.root=s?t[s]??null:null}};q.registerNode("Sequence",Ci);q.registerNode("Priority",Ri);q.registerNode("MemSequence",Rr);q.registerNode("MemPriority",Mi);q.registerNode("Parallel",nr);q.registerNode("Inverter",Ai);q.registerNode("Retry",Ii);q.registerNode("MaxTime",Pi);q.registerNode("Repeater",Li);q.registerNode("RepeatUntilFailure",Ni);q.registerNode("RepeatUntilSuccess",Oi);q.registerNode("Succeeder",$i);q.registerNode("Failer",Di);q.registerNode("Runner",Fi);q.registerNode("Error",Cn);q.registerNode("Wait",Bi);var ji=class extends Ke{static _STATUS_MAP={[E.Pending]:x.Running,[E.InProgress]:x.Running,[E.Completed]:x.Success,[E.Failed]:x.Failure,[E.Skipped]:x.Failure};_task;_taskParams;_pending=!0;constructor(e,t=null){super(`Task:${e.id}`,e.description?e.description.slice(0,40):e.id),this._task=e,this._taskParams=t}open(e){this._task.status=E.InProgress,this._pending=!0,e.blackboard.set("task_in_progress",this._task.id,e.tree.id,this.id)}tick(e){let t=e.blackboard.get(`result:${this._task.id}`,e.tree.id);if(t!=null)return this._task.result=t,this._task.status=E.Completed,e.blackboard.set(`result:${this._task.id}`,null,e.tree.id),console.log(`[BT] ${this._task.id} \u5B8C\u6210`),x.Success;let r=e.blackboard.get(`error:${this._task.id}`,e.tree.id);if(r!=null)return this._task.status=E.Failed,this._task.error=r,e.blackboard.set(`error:${this._task.id}`,null,e.tree.id),console.warn(`[BT] ${this._task.id} \u5931\u8D25: ${r}`),x.Failure;if(this._pending){if(this._pending=!1,this._taskParams!==null){e.blackboard.set(`task_node:${this._task.id}`,this._task,e.tree.id);let s=e.blackboard.get("pending_delegation",e.tree.id);s==null&&(s=[]),Array.isArray(s)||(s=[s]),s.push(this._taskParams),e.blackboard.set("pending_delegation",s,e.tree.id)}return x.Running}return x.Running}close(e){this._pending=!0}},Rn=class l{_config;_assembler;_graph=null;constructor(e=xi,t=null){this._config=e,this._assembler=t}async build(e){if(e.allNodes.size===0)return console.warn("[BTBuilder] TaskGraph \u4E3A\u7A7A"),new q;let t=e.allNodes,r=l.layerSort(t);this._graph=e;let s=[],n=new Set;for(let a of r){let c=a.filter(u=>u!==e.root.id);if(c.length===0)continue;if(c.every(u=>t.get(u).dependsOn.every(m=>n.has(m)))&&c.length>1){let u=[];for(let p of c)u.push(await this._wrap(t.get(p)));s.push(new nr(u,nr.SUCCESS_ON_ALL))}else{let u=l.sortByDeps(c,t);for(let p of u)s.push(await this._wrap(t.get(p)))}for(let u of c)n.add(u)}let i=s.length===1?s[0]:new Rr(s),o=new q;return o.root=i,o.title=this._graph?.name??"BT-TaskGraph",console.log(`[BTBuilder] TaskGraph \u2192 BT (${r.length} \u5C42, ${s.length} \u4EFB\u52A1)`),o}async _wrap(e){let t=await sr.buildDelegationTask(this._graph,e,this._config,this._assembler);return new ji(e,t)}static layerSort(e){let t={};for(let i of e.keys())t[i]=e.get(i).dependsOn.length;let r=[],s=[];for(let[i,o]of Object.entries(t))o===0&&s.push(i);let n=0;for(;s.length>0;){let i=[],o=[];for(let a of s){i.push(a),n+=1;for(let[c,d]of e)d.dependsOn.includes(a)&&(t[c]-=1,t[c]===0&&o.push(c))}i.length>0&&r.push(i),s.splice(0,s.length,...o)}if(n<e.size){let i=Object.entries(t).filter(([,o])=>o>0).map(([o])=>o).sort();throw new Error(`DAG has cycles, cannot sort. Affected nodes: ${i.join(", ")}`)}return r}static sortByDeps(e,t){let r=[],s=new Set(e);for(;s.size>0;){let n=[...s].filter(i=>!t.get(i).dependsOn.some(a=>s.has(a)));if(n.length===0)break;r.push(...n);for(let i of n)s.delete(i)}return[...r,...s]}},Mn=class{tree;_blackboard=new Er;_done=!1;_currentBatch=[];_tickCount=0;_maxRetries;_stateManager;_retryCount={};_gapHistory=[];_completedSet=new Set;constructor(e,t={}){this.tree=e,this._maxRetries=t.maxRetries??2,this._stateManager=t.stateManager??null}isDone(){return this._done}step(){if(this._done)return null;this._currentBatch=[],this._tickCount+=1;let e=this.tree.tick(null,this._blackboard);if(e===x.Error)return console.error("[BTRunner] BT tick \u8FD4\u56DE ERROR"),this._done=!0,null;if(e===x.Success||e===x.Failure)return console.log(`[BTRunner] ${e} (${this._tickCount} ticks)`),this._done=!0,null;let t=this._blackboard.get("pending_delegation",this.tree.id);return t?(this._blackboard.set("pending_delegation",null,this.tree.id),Array.isArray(t)?this._currentBatch=t:this._currentBatch=[t],this._currentBatch):null}async feed(e){let t=[];for(let o of e){let a=o._nodeId??"",c=o.output??o.result,d=o.error;if(d||o.status==="error"){this._completedSet.add(a),this._blackboard.set(`error:${a}`,d??"\u5B50 agent \u51FA\u9519",this.tree.id),t.push([a,"failure"]);continue}if(c===void 0&&o.status==="timeout"){let u=this._retryCount[a]??0;if(u<this._maxRetries){this._retryCount[a]=u+1,console.warn(`[BTRunner] ${a} \u8D85\u65F6 (\u91CD\u8BD5 ${u+1}/${this._maxRetries})`),t.push([a,"retry"]);continue}this._blackboard.set(`error:${a}`,"\u8D85\u65F6\uFF08\u5DF2\u8FBE\u6700\u5927\u91CD\u8BD5\uFF09",this.tree.id),t.push([a,"failure"]);continue}if(c!==void 0){let u=this._blackboard.get(`task_node:${a}`,this.tree.id),p=u!==null?xr.detect(u,c):null;if(p!==null){let m=this._retryCount[a]??0;if(m<this._maxRetries){this._retryCount[a]=m+1,console.warn(`[BTRunner] ${a} \u8F93\u51FA\u6F02\u79FB: ${p} (\u91CD\u8BD5 ${m+1}/${this._maxRetries})`),t.push([a,"retry"]);continue}console.warn(`[BTRunner] ${a} \u6F02\u79FB\u5DF2\u8FBE\u6700\u5927\u91CD\u8BD5: ${p}`)}}if(c!==void 0){if(this._completedSet.add(a),this._blackboard.set(`result:${a}`,c,this.tree.id),t.push([a,"success"]),this._stateManager!==null){let u={entity:a,source:"bt_executor",summary:`BT node ${a} completed`,result:typeof c=="object"&&!Array.isArray(c)?c:{_output:String(c)}};await this._stateManager.feed(u)}}else this._blackboard.set(`error:${a}`,"\u8FD4\u56DE\u7A7A\u7ED3\u679C",this.tree.id),t.push([a,"failure"])}let s=(this.tree.root?.children.length??0)-this._completedSet.size;this._gapHistory.push(s);let n=t.filter(([,o])=>o==="retry").length,i=t.filter(([,o])=>o==="failure").length;return console.log(`[METRIC] BTRunner feed | batch=${e.length} gap=${s} retries=${n} failed=${i}`),this._currentBatch=[],t}syncFromStateManager(e){for(let[t,r]of e.entities)this._blackboard.set(`entity:${t}`,{facts:r.facts,metrics:r.metrics,summary:r.summary},this.tree.id)}};var An=class l{_llm;_dispatch;_stateManager;_lastGraph=null;get lastGraph(){return this._lastGraph}constructor(e,t,r){this._llm=e,this._dispatch=t,this._stateManager=r}onProgress=null;onNodeDone=null;dispatchSignal;_assertNotAborted(e){if(e?.aborted)throw new Error("run_state_driven_goal \u5DF2\u4E2D\u65AD\uFF08\u7528\u6237\u63D2\u8BDD\uFF09")}static buildStateGoalPrompt(e){return`\u8BF7\u5206\u6790\u4EE5\u4E0B\u76EE\u6807\uFF0C\u8F93\u51FA\u9700\u8981\u6539\u53D8\u7684\u7CFB\u7EDF\u72B6\u6001\u3002
...
`)){let c=a.trim().match(/^(\d+)\s*[|｜]\s*(.+)$/);if(!c)continue;let d=Number(c[1]);if(d>=1&&d<=e.length){let u=c[2].trim().replace(/[。！？!?]+$/g,"").slice(0,20);u&&n.set(d,u)}}let i=new Set,o=0;for(let[a,c]of n){let d=e[a-1];try{await this.sessionStore.setTaskSummary(d.sessionId,d.nodeId,c),i.add(d.sessionId),o++}catch(u){console.warn(`[task-summary] \u843D\u5E93\u5931\u8D25: ${u instanceof Error?u.message:String(u)}`)}}for(let a of i)this.config?.onTaskSummaryDone?.(a);return o}get tools(){return this._tools.all()}on(e,t){this.lifecycle.on(e,t)}off(e,t){this.lifecycle.off(e,t)}addTool(e){this._tools.add(e),this.agent.setTools(this._tools.all())}setSystemPrompt(e){this.systemPrompt=e}plannerModeActive(){return this.planner!==void 0}static extractModelName(e){return e.modelName??"unknown"}setDelegateTaskDepth(e){let t=this._tools.get("delegate_task");t&&(t.currentDepth=e)}getSubagentParentRuntime(){let e=this;return{llm:{chat:async t=>e.llm.chat(t,void 0)},tools:{all:()=>e._tools.all()},createSubagent:(t,r,s,n)=>l.createSubagent(e,t,r,s,n)}}};import*as Ra from"node:path";var xt=class l{llmProvider;llmApiKey;llmModel;llmBaseUrl;platform;qqAppId;qqClientSecret;qqToken;homeDir;memoryPath;constructor(e){let t=e.llm.defaultProvider,r=e.llm.providers[t];this.llmProvider=t||"deepseek",this.llmApiKey=l.resolveApiKey(t,r),this.llmBaseUrl=r?.baseUrl?r.baseUrl.replace(/\/+$/,""):l.defaultBaseUrl(t),this.llmModel=r?.model||e.llm.defaultModel||"deepseek-v4-flash";let s=e.platform,n=s?.adapters?Object.keys(s.adapters):[];this.platform=n.length>0?n[0]:"qq";let i=e.platform,o={};i?.adapters?.qq&&Object.assign(o,i.adapters.qq),this.qqAppId=String(o.app_id||process.env.QQ_APP_ID||""),this.qqClientSecret=String(o.client_secret||process.env.QQ_CLIENT_SECRET||""),this.qqToken=String(o.token||process.env.QQ_TOKEN||""),this.homeDir=l.findKexvimHome(),this.memoryPath=Ra.join(this.homeDir,"data","memories","guardian_memory.json")}static load(){let e=I.load();return new l(e)}static defaultBaseUrl(e){return{deepseek:"https://api.deepseek.com",openai:"https://api.openai.com/v1",openrouter:"https://openrouter.ai/api/v1",xai:"https://api.x.ai/v1",groq:"https://api.groq.com/openai/v1",together:"https://api.together.xyz/v1",mistral:"https://api.mistral.ai/v1"}[e.toLowerCase()]||"https://api.deepseek.com"}static resolveApiKey(e,t){if(t?.apiKeyEnv){let i=process.env[t.apiKeyEnv]?.trim();if(i)return i}let r=`${e.toUpperCase().replace(/-/g,"_")}_API_KEY`,s=process.env[r]?.trim();if(s)return s;let n=process.env.DEEPSEEK_API_KEY?.trim();return n||""}static findKexvimHome(){let e=process.env.KEXVIM_HOME?.trim();if(e)return e;let t=I.findProjectRoot();if(!t)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF08Guardian\uFF09\uFF1A\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\uFF0C\u6216\u8BBE\u7F6E KEXVIM_HOME\u3002");return t}};var Mr=class extends Error{constructor(e){super(e),this.name="LLMError"}},Ar=class l{registry;provider;_model;constructor(e,t){this.registry=t||new Ut,this.provider=e.llmProvider||"deepseek",this._model=e.llmModel,e.llmApiKey&&this.registry.setApiKey(this.provider,e.llmApiKey),e.llmBaseUrl&&this.registry.setBaseUrl(this.provider,e.llmBaseUrl)}get model(){return this._model}async chat(e,t,r=4096,s=.7){if(!l.hasApiKeyConfigured(this.provider))throw new Mr("LLM API key not configured");try{let n=this.registry.resolve(this.provider,this._model),i=e.map(p=>({role:p.role||"user",content:String(p.content||""),tool_call_id:p.tool_call_id,tool_calls:Array.isArray(p.tool_calls)?p.tool_calls:void 0})),o=e.find(p=>p.role==="system"),a=o?String(o.content||""):"",c=t?t.map(p=>{let m=p.function;return{name:m?.name||"",description:m?.description||"",input_schema:m?.parameters||{}}}):void 0,d=await n.chat({systemPrompt:a,messages:i,tools:c,maxOutputTokens:r}),u={choices:[{message:{content:d.content||null,tool_calls:d.toolCalls?.map(p=>({id:p.id,type:"function",function:{name:p.name,arguments:p.arguments}}))},finish_reason:d.finishReason}]};return d.usage&&(u.usage={prompt_tokens:d.usage.promptTokens,completion_tokens:d.usage.completionTokens}),u}catch(n){throw n instanceof Mr?n:new Mr(`LLM API call failed: ${n.message}`)}}static hasApiKeyConfigured(e){return!!(process.env[`${e.toUpperCase().replace(/-/g,"_")}_API_KEY`]?.trim()||process.env.DEEPSEEK_API_KEY?.trim())}};import*as Oe from"node:fs";import*as Ln from"node:path";var Ir=class{dataPath;data={};constructor(e){this.dataPath=Ln.resolve(e),this.load()}load(){try{if(Oe.existsSync(this.dataPath)){let e=Oe.readFileSync(this.dataPath,"utf-8");this.data=JSON.parse(e)}else this.data={}}catch{this.data={}}}save(){let e=Ln.dirname(this.dataPath);Oe.mkdirSync(e,{recursive:!0});let t=this.dataPath+".tmp";Oe.writeFileSync(t,JSON.stringify(this.data,null,2),"utf-8"),Oe.renameSync(t,this.dataPath)}get(e,t=null){return e in this.data?this.data[e]:t}set(e,t){this.data[e]=t,this.save()}delete(e){delete this.data[e],this.save()}keys(){return Object.keys(this.data)}all(){return{...this.data}}};import*as le from"node:fs";import*as Nn from"node:os";import*as ir from"node:path";var Pr=Nn.homedir()||process.env.HOME||process.env.USERPROFILE||"~",or=class l{static GUARDIAN_TOOL_DEFINITIONS=[{type:"function",function:{name:"terminal",description:"Run a shell command on the host. IMPORTANT: the host is Windows (cmd.exe). Use Windows commands only: tasklist, wmic, findstr, powershell, dir, type. Do NOT use Linux commands like ps, head, tail, grep, ls, cat, kill - they will fail with 'not recognized'. Returns output + exit_code.",parameters:{type:"object",properties:{command:{type:"string",description:"Shell command to execute"},timeout:{type:"integer",description:"Max seconds to wait (default 60)",default:60},workdir:{type:"string",description:"Working directory (default: current)",default:""}},required:["command"]}}},{type:"function",function:{name:"read_file",description:"Read a text file with optional pagination.",parameters:{type:"object",properties:{path:{type:"string",description:"File path to read"},offset:{type:"integer",description:"Start line (1-indexed, default 1)",default:1},limit:{type:"integer",description:"Max lines to return (default 500)",default:500}},required:["path"]}}},{type:"function",function:{name:"write_file",description:"Write content to a file (overwrites entirely). Creates parent dirs.",parameters:{type:"object",properties:{path:{type:"string",description:"File path to write"},content:{type:"string",description:"Content to write"}},required:["path","content"]}}},{type:"function",function:{name:"patch",description:"Find and replace text in a file. Uses simple search-and-replace.",parameters:{type:"object",properties:{path:{type:"string",description:"File path to edit"},old_string:{type:"string",description:"Text to find (must be unique)"},new_string:{type:"string",description:"Replacement text"},replace_all:{type:"boolean",description:"Replace all occurrences (default false)",default:!1}},required:["path","old_string","new_string"]}}},{type:"function",function:{name:"search_files",description:"Search file contents (grep) or find files by name. Uses regex for content, glob for filenames.",parameters:{type:"object",properties:{pattern:{type:"string",description:"Regex pattern (content search) or glob pattern (file search)"},target:{type:"string",enum:["content","files"],description:"Search inside files or find files by name",default:"content"},path:{type:"string",description:`Directory to search in (default: ${Pr})`,default:Pr},file_glob:{type:"string",description:"Filter by file pattern in grep mode (e.g. '*.py')",default:""},limit:{type:"integer",description:"Max results (default 20)",default:20}},required:["pattern"]}}},{type:"function",function:{name:"memory",description:"Save a durable fact to persistent memory (survives restarts).",parameters:{type:"object",properties:{key:{type:"string",description:"Memory key (e.g. 'user_name', 'project_root')"},value:{type:"string",description:"Value to remember"}},required:["key","value"]}}}];static expandPath(e){return e.startsWith("~/")?Pr+e.slice(1):e}static toolTerminal(e,t=60,r=""){let s=b.runSyncResult(e,{timeoutMs:t*1e3,maxBuffer:52428800,cwd:r?l.expandPath(r):void 0}),n="";return s.code===0?n=l.decodeOutput(s.stdout):(n=l.decodeOutput(s.stdout)+l.decodeOutput(s.stderr),n||(n=`[Error: exit code ${s.code}]`)),n.length>51200&&(n=n.slice(0,51200)+`
... [truncated]`),JSON.stringify({output:n,exit_code:s.code??-1})}static decodeOutput(e){if(typeof e=="string")return e;let t=e.toString("utf-8");if(!t.includes("\uFFFD"))return t;try{return new TextDecoder("gbk").decode(e)}catch{return t}}static toolReadFile(e,t=1,r=500){let s=l.expandPath(e);try{let i=le.readFileSync(s,"utf-8").split(`
`),o=i.length,a=Math.max(0,t-1),c=Math.min(o,a+r),p={content:i.slice(a,c).map((m,g)=>`${a+g+1}|${m}`).join(`
`),total_lines:o};return c<o&&(p.next_offset=c+1),JSON.stringify(p)}catch(n){let i=n instanceof Error?n.message:String(n);return JSON.stringify({error:`Failed to read file: ${i}`,total_lines:0})}}static toolWriteFile(e,t){let r=l.expandPath(e);try{let s=ir.dirname(r);le.mkdirSync(s,{recursive:!0}),le.writeFileSync(r,t,"utf-8");let n=Buffer.byteLength(t,"utf-8");return JSON.stringify({success:!0,bytes_written:n})}catch(s){let n=s instanceof Error?s.message:String(s);return JSON.stringify({error:`Failed to write file: ${n}`})}}static toolPatch(e,t,r,s=!1){let n=l.expandPath(e);try{let i=le.readFileSync(n,"utf-8"),o;if(s){let c=i.split(t).length-1;if(c===0)return JSON.stringify({error:"old_string not found",matches:0});i=i.split(t).join(r),o={success:!0,matches:c}}else{if(!i.includes(t))return JSON.stringify({error:"old_string not found in file"});i=i.replace(t,r),o={success:!0,matches:1}}let a=ir.join(Nn.tmpdir(),`kexvim-patch-${Date.now()}-${Math.random().toString(36).slice(2)}`);return le.writeFileSync(a,i,"utf-8"),le.renameSync(a,n),JSON.stringify(o)}catch(i){let o=i instanceof Error?i.message:String(i);return JSON.stringify({error:`Patch failed: ${o}`})}}static toolSearchFiles(e,t="content",r=Pr,s="",n=20){let i=l.expandPath(r);try{if(!le.existsSync(i))return JSON.stringify({error:`Path not found: ${r}`});if(t==="files"){let o=[],a=c=>{if(o.length>=n)return;let d;try{d=le.readdirSync(c,{withFileTypes:!0})}catch{return}for(let u of d){if(o.length>=n)return;let p=ir.join(c,u.name);u.isDirectory()?!u.name.startsWith(".")&&u.name!=="node_modules"&&a(p):l.matchGlob(u.name,e)&&o.push(p)}};return a(i),JSON.stringify({matches:o,total:o.length})}else{let o=[],a=new RegExp(e),c=d=>{if(o.length>=n)return;let u;try{u=le.readdirSync(d,{withFileTypes:!0})}catch{return}for(let p of u){if(o.length>=n)return;let m=ir.join(d,p.name);if(p.isDirectory())!p.name.startsWith(".")&&p.name!=="node_modules"&&c(m);else if(p.isFile()){if(s&&!l.matchGlob(p.name,s))continue;try{let f=le.readFileSync(m,"utf-8").split(`
`);for(let h=0;h<f.length&&!(a.test(f[h])&&(o.push({path:m,line:h+1,content:f[h].slice(0,200)}),o.length>=n));h++);}catch{}}}};return c(i),JSON.stringify({matches:o,total:o.length})}}catch(o){let a=o instanceof Error?o.message:String(o);return JSON.stringify({error:`Search failed: ${a}`})}}static matchGlob(e,t){let r=t.replace(/[.+^${}()|[\]\\]/g,"\\$&").replace(/\*/g,".*").replace(/\?/g,".");return new RegExp(`^${r}$`,"i").test(e)}static TOOL_MAP={terminal:e=>l.toolTerminal(String(e.command||""),Number(e.timeout)||60,String(e.workdir||"")),read_file:e=>l.toolReadFile(String(e.path||""),Number(e.offset)||1,Number(e.limit)||500),write_file:e=>l.toolWriteFile(String(e.path||""),String(e.content||"")),patch:e=>l.toolPatch(String(e.path||""),String(e.old_string||""),String(e.new_string||""),!!e.replace_all),search_files:e=>l.toolSearchFiles(String(e.pattern||""),String(e.target||"content"),String(e.path||Pr),String(e.file_glob||""),Number(e.limit)||20)};static executeGuardianTool(e,t,r){if(e==="memory"&&r){let n=String(t.key||""),i=String(t.value||"");return r.set(n,i),JSON.stringify({success:!0,key:n})}let s=l.TOOL_MAP[e];if(!s)return JSON.stringify({error:`Unknown tool: ${e}`});try{return s(t)}catch(n){let i=n instanceof Error?n.message:String(n);return JSON.stringify({error:`Tool execution failed: ${i}`})}}};function $d(){let l="";try{l=I.findProjectRoot()||""}catch{}return`You are Guardian, the repair agent for kexvim (a terminal AI assistant).
...
7. IMPORTANT: Always respond in the language the user wrote in. If the user writes in Chinese, respond in Chinese. / \u91CD\u8981\uFF1A\u59CB\u7EC8\u7528\u7528\u6237\u4F7F\u7528\u7684\u8BED\u8A00\u56DE\u590D\u3002\u5982\u679C\u7528\u6237\u5199\u4E2D\u6587\uFF0C\u5C31\u7528\u4E2D\u6587\u56DE\u590D\u3002`}var Tt=class{llm;_memory;messages=[];maxTurns=20;systemPrompt;constructor(e,t){this.llm=new Ar(e),this._memory=new Ir(e.memoryPath),this.systemPrompt=t||$d(),this.resetConversation()}get memory(){return this._memory}get llmClient(){return this.llm}async process(e){if(!e||!e.trim())return"";let t=e.trim().toLowerCase();if(t==="/reset"||t==="/new"||t==="/clear")return this.resetConversation(),"\u5BF9\u8BDD\u5DF2\u91CD\u7F6E\u3002";this.messages.push({role:"user",content:e});let r=0;for(;r<this.maxTurns;){r++;let n;try{n=await this.llm.chat(this.messages,or.GUARDIAN_TOOL_DEFINITIONS,4096,.7)}catch(p){let m=`LLM \u8C03\u7528\u5931\u8D25: ${p.message}`;return this.messages.push({role:"assistant",content:m}),m}let a=((n.choices||[{}])[0]||{}).message||{},c=a.content||"",d=a.tool_calls;if(!d||d.length===0){let p=c||"";return this.messages.push({role:"assistant",content:p}),p}let u={role:"assistant",content:c||"",tool_calls:d};this.messages.push(u);for(let p of d){let m=p.function,g=m?.name||"",f={};try{f=JSON.parse(m?.arguments||"{}")}catch{f={}}let h=or.executeGuardianTool(g,f,this._memory);this.messages.push({role:"tool",tool_call_id:p.id,content:h})}if(this.messages.length>40){let p=[this.messages[0],...this.messages.slice(-20)];for(this.messages=p;this.messages.length>1&&this.messages[this.messages.length-1]?.role==="tool";)this.messages.pop();let m=this.messages[this.messages.length-1],g=m?m.tool_calls:void 0;for(m?.role==="assistant"&&Array.isArray(g)&&g.length>0&&this.messages.pop();this.messages.length>2&&this.messages[1]?.role==="tool";)this.messages.splice(1,1)}}let s="\u5DF2\u8FBE\u5230\u6700\u5927\u5DE5\u5177\u8C03\u7528\u8F6E\u6B21\uFF0C\u5DF2\u622A\u65AD\u3002\u8BF7\u63D0\u4E0B\u4E00\u4E2A\u95EE\u9898\u3002";return this.messages.push({role:"assistant",content:s}),s}resetConversation(){this.messages=[{role:"system",content:this.systemPrompt}]}};import{StdioClientTransport as Bd}from"@modelcontextprotocol/sdk/client/stdio.js";import{SSEClientTransport as Na}from"@modelcontextprotocol/sdk/client/sse.js";var ce=class l{static SENSITIVE_PATTERNS=[/token/i,/secret/i,/password/i,/passwd/i,/api[_-]?key/i,/apikey/i,/auth/i,/credential/i,/private[_-]?key/i,/access[_-]?key/i,/secret[_-]?key/i,/session/i,/jwt/i];static SAFE_ENV_VARS=new Set(["PATH","HOME","USER","USERNAME","SHELL","TERM","LANG","LC_ALL","TMPDIR","TEMP","TMP","DISPLAY","XDG_RUNTIME_DIR"]);static buildSafeEnv(e){let t={};for(let r of l.SAFE_ENV_VARS){let s=typeof process<"u"?process.env[r]:void 0;s&&(t[r]=s)}if(e)for(let[r,s]of Object.entries(e))t[r]=s;return t}static sanitizeErrorMessage(e){let t=e;t=t.replace(/Bearer\s+[A-Za-z0-9\-._~+/]+=*/gi,"Bearer ***"),t=t.replace(/(Authorization:\s*)(\S+)/gi,"$1***"),t=t.replace(/([?&](?:api[_-]?key|token|secret|key)=)[^&]+/gi,"$1***"),t=t.replace(/"(token|secret|api_key|apikey|password|credential)"\s*:\s*"[^"]+"/gi,'"$1": "***"');for(let r of l.SENSITIVE_PATTERNS)t=t.replace(new RegExp(`(${r.source}\\s*[:=]\\s*)\\S+`,"gi"),"$1***");return t}static isValidUrl(e){try{let t=new URL(e);return t.protocol==="http:"||t.protocol==="https:"||t.protocol==="ws:"||t.protocol==="wss:"}catch{return!1}}static delay(e){return new Promise(t=>setTimeout(t,e))}static backoffDelay(e,t=2e3,r=3e4){let s=Math.min(t*Math.pow(2,e),r);return s+Math.random()*s*.25}};import*as be from"node:fs";import*as $n from"node:path";import*as Ma from"node:http";import*as Aa from"node:crypto";var Ui=class{server=null;port=0;pending=new Map;async start(e){if(this.server)return this.callbackUrl(e);this.server=Ma.createServer((r,s)=>this._handle(r,s)),await new Promise((r,s)=>{this.server.once("error",s),this.server.listen(0,e,()=>r())});let t=this.server.address();return this.port=typeof t=="object"&&t?t.port:0,this.callbackUrl(e)}callbackUrl(e){return`http://${e}:${this.port}/callback`}waitForCode(e,t){return new Promise((r,s)=>{let n=setTimeout(()=>{this.pending.delete(e),s(new Error(`OAuth authorization timed out after ${Math.round(t/1e3)}s`))},t);this.pending.set(e,{resolve:r,reject:s,timer:n})})}_handle(e,t){let r=new URL(e.url||"/","http://localhost");if(r.pathname==="/callback"){let s=r.searchParams.get("code"),n=r.searchParams.get("state"),i=r.searchParams.get("error"),o=n?this.pending.get(n):void 0;if(t.writeHead(200,{"Content-Type":"text/html; charset=utf-8"}),i||!s||!o){t.end(`<html><body><h3>OAuth \u6388\u6743\u5931\u8D25${i?`\uFF1A${i}`:"\uFF08\u65E0\u6548\u56DE\u8C03\uFF09"}</h3><p>\u8BF7\u8FD4\u56DE\u804A\u5929\u7A97\u53E3\u67E5\u770B\u9519\u8BEF\u4FE1\u606F\u3002</p></body></html>`),o&&(clearTimeout(o.timer),this.pending.delete(n),o.reject(new Error(i||"OAuth callback missing code")));return}clearTimeout(o.timer),this.pending.delete(n),o.resolve(s),t.end("<html><body><h3>\u2705 OAuth \u6388\u6743\u6210\u529F</h3><p>\u53EF\u4EE5\u5173\u95ED\u6B64\u7A97\u53E3\uFF0C\u8FD4\u56DE\u804A\u5929\u7A97\u53E3\u7EE7\u7EED\u3002</p></body></html>");return}t.writeHead(404),t.end("Not found")}stop(){for(let{timer:e,reject:t}of this.pending.values())clearTimeout(e),t(new Error("OAuth callback server stopped"));if(this.pending.clear(),this.server){try{this.server.close()}catch{}this.server=null}}},Et=class l{static filePath(e){let t=I.findProjectRoot();if(!t)throw new Error("Cannot locate project root (package.json) \u2014 data/mcp_oauth unavailable");return $n.join(t,"data","mcp_oauth",`${e.replace(/[^a-zA-Z0-9_-]/g,"_")}.json`)}static load(e){try{let t=l.filePath(e);return be.existsSync(t)?JSON.parse(be.readFileSync(t,"utf-8")):{}}catch(t){return console.warn(`[MCP OAuth] failed to load token store for '${e}': ${t.message}`),{}}}static save(e,t){try{let r=l.filePath(e);be.mkdirSync($n.dirname(r),{recursive:!0}),be.writeFileSync(r,JSON.stringify(t,null,2),"utf-8");try{be.chmodSync(r,384)}catch{}}catch(r){console.warn(`[MCP OAuth] failed to persist token store for '${e}': ${r.message}`)}}static clear(e){try{let t=l.filePath(e);be.existsSync(t)&&be.rmSync(t)}catch(t){console.warn(`[MCP OAuth] failed to clear token store for '${e}': ${t.message}`)}}},Dd=600*1e3,Ia=new Map;function Pa(l,e){Ia.set(l,e)}function La(l){return Ia.get(l)}var On=class{serverName;config;callback;_redirectUrl;_clientInformation;_tokens;_codeVerifier;_pendingState;openBrowser;constructor(e,t={},r){this.serverName=e,this.config=t,this.callback=new Ui,this.openBrowser=r?.openBrowser||Fd;let s=Et.load(e);this._clientInformation=s.clientInformation,this._tokens=s.tokens}get redirectUrl(){return this._redirectUrl}get clientMetadata(){let e=this.config.redirectHost||"localhost",t=this._redirectUrl||`http://${e}:0/callback`,r={client_name:this.config.clientName||"Kexvim Agent",redirect_uris:[t],grant_types:["authorization_code","refresh_token"],response_types:["code"],token_endpoint_auth_method:this.config.clientSecret?"client_secret_post":"none"};return this.config.scope&&(r.scope=this.config.scope),r}state(){return this._pendingState=Aa.randomBytes(16).toString("hex"),this._pendingState}clientInformation(){return this._clientInformation}async saveClientInformation(e){this._clientInformation=e,Et.save(this.serverName,{clientInformation:this._clientInformation,tokens:this._tokens})}tokens(){return this._tokens}async saveTokens(e){this._tokens=e,Et.save(this.serverName,{clientInformation:this._clientInformation,tokens:this._tokens})}async redirectToAuthorization(e){let t=this.config.redirectHost||"localhost";this._redirectUrl=await this.callback.start(t),console.log(`[MCP OAuth] '${this.serverName}' \u9700\u8981\u6388\u6743\uFF0C\u8BF7\u5728\u6D4F\u89C8\u5668\u4E2D\u5B8C\u6210\uFF1A${e.toString()}`),this.openBrowser(e.toString())}waitForAuthorizationCode(e=Dd){return this._pendingState?this.callback.waitForCode(this._pendingState,e):Promise.reject(new Error("No pending OAuth state \u2014 call redirectToAuthorization first"))}saveCodeVerifier(e){this._codeVerifier=e}codeVerifier(){if(!this._codeVerifier)throw new Error("No code verifier saved");return this._codeVerifier}async invalidateCredentials(e){(e==="all"||e==="tokens")&&(this._tokens=void 0,Et.save(this.serverName,{clientInformation:this._clientInformation})),(e==="all"||e==="client")&&(this._clientInformation=void 0,Et.save(this.serverName,{})),(e==="all"||e==="verifier")&&(this._codeVerifier=void 0)}dispose(){this.callback.stop()}};function Fd(l){let e=process.platform,t;e==="win32"?t=`start "" "${l}"`:e==="darwin"?t=`open "${l}"`:t=`xdg-open "${l}"`,b.runAsync(t).catch(()=>{console.log(`[MCP OAuth] \u8BF7\u5728\u6D4F\u89C8\u5668\u4E2D\u6253\u5F00\u5B8C\u6210\u6388\u6743\uFF1A
${l}`)})}var ze=class l{static async createTransport(e,t){let r=e.transport||"streamable-http";switch(r){case"stdio":return l._createStdio(e,t);case"sse":case"streamable-http":return l._createSSE(e,t);default:throw new Error(`[MCP:${t}] Unsupported transport: ${r}`)}}static async _createStdio(e,t){if(!e.command)throw new Error(`[MCP:${t}] 'command' is required for stdio transport`);return new Bd({command:e.command,args:e.args??[],env:ce.buildSafeEnv(e.env),stderr:"pipe"})}static async _createSSE(e,t){if(!e.url)throw new Error(`[MCP:${t}] 'url' is required for ${e.transport||"streamable-http"} transport`);if(!ce.isValidUrl(e.url))throw new Error(`[MCP:${t}] Invalid URL: ${e.url}`);if(e.auth==="oauth"){let s=new On(t,e.oauth||{});return Pa(t,s),new Na(new URL(e.url),{authProvider:s})}let r=e.url;if(e.headers?.Authorization&&!r.includes("?")){let s=e.headers.Authorization.replace(/^Bearer\s+/i,"");r+=(r.includes("?")?"&":"?")+`token=${encodeURIComponent(s)}`}return new Na(new URL(r))}};var de={Disconnected:"disconnected",Connecting:"connecting",Connected:"connected",Error:"error"};import{Client as Ud}from"@modelcontextprotocol/sdk/client/index.js";import{UnauthorizedError as Hd}from"@modelcontextprotocol/sdk/client/auth.js";function lt(l){return typeof l=="object"&&l!==null&&!Array.isArray(l)}function jd(l){return Array.isArray(l.anyOf)&&l.anyOf.length>0?{key:"anyOf",branches:l.anyOf}:Array.isArray(l.oneOf)&&l.oneOf.length>0?{key:"oneOf",branches:l.oneOf}:null}var Lr=class l{static normalizeSchema(e){if(!e||typeof e!="object")return{type:"object"};let t={},r=jd(e);for(let[s,n]of Object.entries(e))if(!(s==="$ref"||s==="$schema"||s==="definitions"||s==="$id")&&!(s==="anyOf"||s==="oneOf")){if(s==="title"){typeof n=="string"&&n.length<=64&&!/[{}()$@]/.test(n)&&(t[s]=n);continue}if(s!=="default"){if(lt(n)){t[s]=l.normalizeSchema(n);continue}if(Array.isArray(n)&&n.length>0&&lt(n[0])){t[s]=n.map(i=>lt(i)?l.normalizeSchema(i):i);continue}if(s==="type"){if(Array.isArray(n)){let i=n.filter(o=>o!=="null");t[s]=i.length===1?i[0]:i.length>1?i:"string"}else n==="null"?t[s]="string":t[s]=n;continue}if(s==="additionalProperties"){n==="object"||n===!0?t[s]=!0:n===!1||n===null?t[s]=!1:lt(n)?t[s]=l.normalizeSchema(n):t[s]=!0;continue}if(["minimum","maximum","minLength","maxLength","minItems","maxItems"].includes(s)){typeof n=="number"&&(t[s]=n);continue}if(s==="enum"&&Array.isArray(n)){t[s]=n;continue}if(s==="properties"&&lt(n)){let i={};for(let[o,a]of Object.entries(n))i[o]=lt(a)?l.normalizeSchema(a):a;t[s]=i;continue}if(s==="items"&&lt(n)){t[s]=l.normalizeSchema(n);continue}if(s==="description"&&typeof n=="string"){t[s]=n.length>2048?n.slice(0,2048)+"\u2026":n;continue}if(s==="required"&&Array.isArray(n)){t[s]=n.filter(i=>typeof i=="string");continue}t[s]=n}}if(r){let{branches:s,key:n}=r,i=s.filter(o=>lt(o)&&o.type!=="null").map(o=>l.normalizeSchema(o));if(i.length===1)for(let[o,a]of Object.entries(i[0]))o in t||(t[o]=a);else i.length>1&&(t[n]=i)}return!t.type&&!t.properties&&!t.$ref&&!t.enum&&!t.anyOf&&!t.oneOf&&t.items&&(t.type="array"),t}static normalizeToolList(e){return e.map(t=>({...t,description:t.description?t.description.length>1024?t.description.slice(0,1024)+"\u2026":t.description:t.name,inputSchema:l.normalizeSchema(t.inputSchema)}))}};var Dn=5,qd=1e4,Nr=class{name;config;client=null;transport=null;_state=de.Disconnected;_connectedAt=0;_reconnectAttempts=0;_tools=[];_lastError;_keepaliveTimer=null;_disconnectRequested=!1;_stateHandlers=[];_reconnectHandlers=[];constructor(e){this.name=e.name,this.config={timeout:300,connectTimeout:60,keepaliveInterval:180,supportsParallelToolCalls:!1,autoConnect:!0,...e}}get state(){return this._state}get toolCount(){return this._tools.length}get connected(){return this._state===de.Connected}get supportsParallel(){return this.config.supportsParallelToolCalls??!1}getTools(){return this._tools}async connect(e){if(this._state!==de.Connected){this._setState(de.Connecting),this._lastError=void 0,this._disconnectRequested=!1;try{let t={capabilities:{sampling:this.config.sampling?.enabled!==!1?{}:void 0}};this.client=new Ud({name:"kexvim-mcp-client",version:"1.0.0"},t),this.transport=e,this.client.onclose=()=>{this._disconnectRequested||this._handleTransportClose()},await this.client.connect(e),this._setState(de.Connected),this._connectedAt=Date.now(),this._reconnectAttempts=0,await this._discoverTools(),this._startKeepalive()}catch(t){if(this.config.auth==="oauth"&&t instanceof Hd){let r=La(this.name);if(r)throw console.log(`[MCP:${this.name}] OAuth \u6388\u6743\u5DF2\u89E6\u53D1\uFF1A\u8BF7\u5728\u6D4F\u89C8\u5668\u4E2D\u5B8C\u6210\u6388\u6743\uFF0C\u5B8C\u6210\u540E\u5C06\u81EA\u52A8\u91CD\u8FDE\u3002`),this._setState(de.Error),this._lastError="OAuth authorization required \u2014 waiting for browser authorization",this._cleanup(),this._startOAuthReconnect(e,r),new Error(`[MCP:${this.name}] OAuth \u6388\u6743\u4E2D\uFF1A\u8BF7\u5728\u6D4F\u89C8\u5668\u5B8C\u6210\u6388\u6743\uFF08\u89C1\u65E5\u5FD7 URL\uFF09\u3002\u6388\u6743\u5B8C\u6210\u540E\u81EA\u52A8\u91CD\u8FDE\u3002`)}throw this._setState(de.Error),this._lastError=ce.sanitizeErrorMessage(String(t)),this._cleanup(),t}}}async _startOAuthReconnect(e,t){try{let r=await t.waitForAuthorizationCode(),s=e;s?.finishAuth&&await s.finishAuth(r);let n=await ze.createTransport(this.config,this.name);await this.connect(n),console.log(`[MCP:${this.name}] OAuth \u6388\u6743\u5B8C\u6210\uFF0C\u5DF2\u91CD\u8FDE\u3002`)}catch(r){console.warn(`[MCP:${this.name}] OAuth \u6388\u6743/\u91CD\u8FDE\u5931\u8D25\uFF1A${r.message}`)}}async _discoverTools(){if(this.client)try{let e=await this.client.listTools({},{timeout:(this.config.connectTimeout??60)*1e3});this._tools=(e.tools??[]).map(t=>({name:t.name,description:t.description??t.name,inputSchema:Lr.normalizeSchema(t.inputSchema)}))}catch(e){console.warn(`[MCP:${this.name}] listTools failed:`,String(e)),this._tools=[]}}_startKeepalive(){let e=this.config.keepaliveInterval??180;e<=0||(this._keepaliveTimer=setInterval(async()=>{if(!(this._state!==de.Connected||!this.client))try{await this.client.ping({timeout:qd})}catch{console.warn(`[MCP:${this.name}] ping failed, reconnecting...`),this._handleTransportClose()}},e*1e3))}_handleTransportClose(){this._disconnectRequested||(this._setState(de.Disconnected),this._cleanup(),this._scheduleReconnect().catch(()=>{}))}async _scheduleReconnect(){if(this._disconnectRequested||this._reconnectAttempts>=Dn){this._reconnectAttempts>=Dn&&(this._setState(de.Error),this._lastError=`\u91CD\u8FDE\u5931\u8D25\uFF0C\u5DF2\u5C1D\u8BD5 ${Dn} \u6B21`);return}this._reconnectAttempts++;let e=ce.backoffDelay(this._reconnectAttempts-1);console.log(`[MCP:${this.name}] reconnecting in ${Math.round(e/1e3)}s (attempt ${this._reconnectAttempts}/${Dn})...`),await new Promise(t=>setTimeout(t,e)),!this._disconnectRequested&&this._emitReconnectNeeded()}async callTool(e,t,r){if(!this.client)throw new Error(`[MCP:${this.name}] not connected`);let s=(this.config.timeout??300)*1e3;try{let n=await this.client.callTool({name:e,arguments:t},void 0,{timeout:s,signal:r});return this._formatResult(n)}catch(n){let i=n instanceof Error?n.message:String(n);throw new Error(`[MCP:${this.name}:${e}] ${ce.sanitizeErrorMessage(i)}`)}}_formatResult(e){if(!e.content||e.content.length===0)return e.isError?"Error: empty response":"(empty result)";let t=[];for(let r of e.content)if(r.type==="text"&&"text"in r)t.push(r.text);else if(r.type==="resource"){let s=r;t.push(s.resource?.text??`[Resource: ${s.resource?.uri??"unknown"}]`)}else t.push(`[${r.type} data]`);return t.join(`
`)||"(empty)"}getStatus(){return{name:this.name,state:this._state,toolCount:this._tools.length,uptime:this._state===de.Connected&&this._connectedAt>0?Math.floor((Date.now()-this._connectedAt)/1e3):0,lastError:this._lastError}}onStateChange(e){this._stateHandlers.push(e)}onReconnectNeeded(e){this._reconnectHandlers.push(e)}_setState(e){this._state=e;for(let t of this._stateHandlers)try{t(this.getStatus())}catch{}}_emitReconnectNeeded(){for(let e of this._reconnectHandlers)try{e()}catch{}}async disconnect(){this._disconnectRequested=!0,this._cleanup();try{await this.client?.close()}catch{}try{await this.transport?.close()}catch{}this.client=null,this.transport=null,this._tools=[],this._setState(de.Disconnected)}_cleanup(){this._keepaliveTimer&&(clearInterval(this._keepaliveTimer),this._keepaliveTimer=null)}};var Or=class{tasks=new Map;_bridgeTools=[];register(e){if(this.tasks.has(e.name))throw new Error(`[MCP] Server '${e.name}' already registered`);let t=new Nr(e);t.onStateChange(r=>{r.state===de.Error&&console.warn(`[MCP:${r.name}] state: ${r.state}${r.lastError?` \u2014 ${r.lastError}`:""}`)}),this.tasks.set(e.name,t)}registerAll(e){for(let t of e)this.register(t)}async connectAll(e=!0){console.log(`[MCP] ${this.tasks.size} servers registered, waiting for connect calls`)}async connect(e,t){let r=this.tasks.get(e);if(!r)throw new Error(`[MCP] Server '${e}' not found`);await r.connect(t)}discoverAndBridge(){let e=[];for(let[t,r]of this.tasks)if(r.connected)for(let s of r.getTools())e.push(new $r(this,t,s));return this._bridgeTools=e,e}discoverAndBridgeServer(e){let t=this.tasks.get(e);if(!t)throw new Error(`[MCP] Server '${e}' not found`);let r=t.getTools().map(s=>new $r(this,e,s));return this._bridgeTools=this._bridgeTools.filter(s=>s.serverName!==e),this._bridgeTools.push(...r),r}getBridgeTools(){return[...this._bridgeTools]}async callTool(e,t,r,s){let n=this.tasks.get(e);if(!n)throw new Error(`[MCP] Server '${e}' not found`);return n.callTool(t,r,s)}getStatus(){return Array.from(this.tasks.values()).map(e=>e.getStatus())}isAllConnected(){for(let e of this.tasks.values())if(!e.connected)return!1;return!0}get serverCount(){return this.tasks.size}async disconnectAll(){await Promise.allSettled(Array.from(this.tasks.values()).map(e=>e.disconnect())),this.tasks.clear(),this._bridgeTools=[]}async disconnect(e){let t=this.tasks.get(e);t&&(await t.disconnect(),this.tasks.delete(e),this._bridgeTools=this._bridgeTools.filter(r=>r.serverName!==e))}},$r=class{constructor(e,t,r){this.manager=e;this.serverName=t,this.toolName=r.name,this.name=`mcp__${t}__${r.name}`,this.description=r.description??`${t}: ${r.name}`,this.parameters=r.inputSchema}manager;name;description;parameters;serverName;toolName;async execute(e,t){return this.manager.callTool(this.serverName,this.toolName,e,t)}toJSON(){return{name:this.name,serverName:this.serverName,toolName:this.toolName,description:this.description,parameters:this.parameters}}};var ov=ze.createTransport.bind(ze),av=ce.sanitizeErrorMessage.bind(ce),lv=ce.buildSafeEnv.bind(ce),cv=ce.isValidUrl.bind(ce);import*as mt from"node:fs";import*as De from"node:path";import{Worker as Wd}from"worker_threads";import Hi from"path";import Gd from"fs";import{fileURLToPath as Kd}from"url";var Dr=class{worker;pending=new Map;nextId=0;ready;readyResolve;constructor(e){this.ready=new Promise(i=>{this.readyResolve=i});let t=Hi.dirname(Kd(import.meta.url)),r=Hi.join(t,"StoreWorkerEntry.js"),s=Hi.join(t,"StoreWorkerEntry.ts"),n=Gd.existsSync(r)?r:s;this.worker=new Wd(n,{workerData:e}),this.worker.on("message",this.onMessage.bind(this)),this.worker.on("error",i=>console.error(S.t("memory.storeworker_error"),i)),this.worker.on("exit",i=>{i!==0&&console.error(S.t("memory.storeworker_exit",{code:i}));for(let[o,a]of this.pending)a.reject(new Error(`StoreWorker exited with code ${i}`)),this.pending.delete(o)})}async call(e,t){return await this.ready,new Promise((r,s)=>{let n=this.nextId++;this.pending.set(n,{resolve:r,reject:s}),this.worker.postMessage({id:n,type:e,data:t})})}send(e,t){if(!this.worker)return;let r=()=>{try{this.worker.postMessage({id:-1,type:e,data:t})}catch{}};this.ready.then(()=>r())}async shutdown(){try{await this.call("shutdown")}catch{}await new Promise(e=>setImmediate(e)),this.worker.terminate().catch(()=>{}),this.worker=null,this.pending.clear()}createSessionStore(){let e=this;return{findByQuery(t){return e.call("findByQuery",t)},getById(t){return e.call("getById",t)},create(t){return e.call("createSession",{session:t})},update(t){return e.call("updateSession",{update:t})},delete(t){return e.call("deleteSession",t)},listRecent(t,r){return e.call("listRecentSessions",{profile:t,limit:r})},countSessions(){return e.call("countSessions")},appendMessage(t,r,s,n,i){e.call("appendMessage",{sessionId:t,role:r,content:s,metadata:n,parentId:i}).catch(()=>{})},getLastActiveMessageId(t){return e.call("getLastActiveMessageId",t)},getMessagesAsConversation(t,r){return e.call("getMessagesAsConversation",{sessionId:t,limit:r})},switchBranch(t,r){return e.call("switchBranch",{sessionId:t,parentId:r})},appendBranchSummary(t,r,s){e.call("appendBranchSummary",{sessionId:t,parentId:r,summaryText:s}).catch(()=>{})},getMessageTree(t){return e.call("getMessageTree",t)},startTaskNode(t,r,s){return e.call("startTaskNode",{sessionId:t,startMsgId:r,title:s})},completeTaskNode(t,r,s){return e.call("completeTaskNode",{sessionId:t,nodeId:r,status:s})},getTaskTree(t){return e.call("getTaskTree",t)},setTaskLastChild(t,r,s){return e.call("setTaskLastChild",{sessionId:t,taskId:r,childId:s})},setTaskSummary(t,r,s){return e.call("setTaskSummary",{sessionId:t,nodeId:r,summary:s})},saveTaskGraph(t,r){return e.call("saveTaskGraph",{chatId:t,graphJson:r})},getTaskGraph(t){return e.call("getTaskGraph",{chatId:t})},appendSystemNotice(t){return e.call("appendSystemNotice",{text:t})},listSystemNotices(t){return e.call("listSystemNotices",{limit:t})}}}createMemoryStore(){let e=this;return{insert(t){return e.call("insertMemory",{entry:t})},query(t){return e.call("queryMemory",t)},search(t,r=20){return e.call("searchMemory",{text:t,limit:r})},update(t){return e.call("updateMemory",{update:t})},delete(t){return e.call("deleteMemory",t)}}}createEntityStore(){let e=this;return{upsert(t){return e.call("upsertEntity",t)},get(t){return e.call("getEntity",t)},delete(t){return e.call("deleteEntity",t)},list(){return e.call("listEntities",{})}}}onMessage(e){if(e.type==="ready"){this.readyResolve();return}if(e.id<0)return;let t=this.pending.get(e.id);t&&(this.pending.delete(e.id),e.type==="error"?t.reject(new Error(e.error??"Unknown worker error")):t.resolve(e.result))}};import{DatabaseSync as zd}from"node:sqlite";var Jd=`
...
`,ar=class{db;constructor(e){this.db=new zd(e),this.db.exec("PRAGMA busy_timeout = 15000"),this.db.exec(Jd)}async upsert(e){let t=this.db.prepare(`
...
`,Fr=class{db;enableMemoryLogging=!1;constructor(e){this.db=new Yd(e),this.db.exec("PRAGMA busy_timeout = 15000"),this.db.exec(Vd)}async insert(e){let t=this.db.prepare(`
    ...
    `).all(e,t);return s.length>0?s.map(o=>this.rowToEntry(o)):this.db.prepare(`
      SELECT * FROM memories WHERE content LIKE ? ORDER BY priority DESC, created_at DESC LIMIT ?
    `).all(`%${e}%`,t).map(o=>this.rowToEntry(o))}async cleanExpired(){let e=this.db.prepare("DELETE FROM memories WHERE expires_at IS NOT NULL AND expires_at < ?").run(Date.now()/1e3);return Number(e.changes)}close(){this.db.close()}rowToEntry(e){return{id:e.id,content:e.content,tags:JSON.parse(e.tags_json??"[]"),source:e.source,category:e.category,priority:e.priority,createdAt:e.created_at,updatedAt:e.updated_at,expiresAt:e.expires_at}}};var Br=class{systemPromptBlock(){return""}prefetch(e,t){return""}queuePrefetch(e,t){}syncTurn(e,t,r){}handleToolCall(e,t){throw new Error(`Provider ${this.name} does not handle tool ${e}`)}shutdown(){}onTurnStart(e,t,...r){}onSessionEnd(e){}onSessionSwitch(e,t){}onPreCompress(e){return""}onMemoryWrite(e,t,r,s){}onDelegation(e,t,r){}};var Fn=class l{static createSessionStore(e,t){return t?t.createSessionStore():new vr(e.dbPath,e.profile??"default")}static createStateManager(e){let t=new ar(e.dbPath);return new lr(t)}static createMemoryStore(e){return new Fr(e.dbPath)}static createAll(e){if(e.workerThreads){let t=new Dr(e);return{sessions:t.createSessionStore(),state:new lr(t.createEntityStore()),memory:t.createMemoryStore(),worker:t}}return{sessions:l.createSessionStore(e),state:l.createStateManager(e),memory:l.createMemoryStore(e)}}};import*as qi from"node:crypto";var Bn=class extends Br{get name(){return"builtin"}_store;_sessionStore=null;constructor(e){super(),this._store=e}setSessionStore(e){this._sessionStore=e}isAvailable(){return!0}initialize(e,...t){}getToolSchemas(){return[]}prefetch(e,t){if(!e||!e.trim())return"";let r=[];try{let s=this._store.searchSync(e,5),n=this._formatEntries(s);n&&r.push(n)}catch{}return r.join(`

`)}async onMemoryWrite(e,t,r,s){switch(e){case"add":{await this._store.insert({id:qi.randomUUID(),content:r,tags:Array.isArray(s?.tags)?s.tags:[],source:t,category:t==="user"?"user":"memory",priority:1,createdAt:Date.now(),updatedAt:Date.now()});break}case"replace":{await this._store.insert({id:qi.randomUUID(),content:r,tags:Array.isArray(s?.tags)?s.tags:[],source:t,category:"user",priority:1,createdAt:Date.now(),updatedAt:Date.now()});break}case"remove":{try{let n=this._store.searchSync(r,1);n.length>0&&await this._store.delete(n[0].id)}catch{}break}}}syncTurn(e,t,r){}_formatEntries(e){return e.length===0?"":e.map(t=>`[memory] ${t.content}${t.tags.length?` (${t.tags.join(", ")})`:""}`).join(`
`)}};import*as N from"node:fs";import*as xe from"node:path";var cr=`
\xA7
`,Gi=2200,Xd=1375,jn=Gi*2,$a="memory-limit.json",Wi=10,Qd=50,Un=class{memDir;memoryEntries=[];userEntries=[];memoryCharLimit=Gi;snapshot={memory:"",user:""};fileSnapshots=new Map;constructor(e){this.memDir=xe.join(e,"memories")}loadFromDisk(){N.mkdirSync(this.memDir,{recursive:!0});try{let e=xe.join(this.memDir,$a);if(N.existsSync(e)){let t=JSON.parse(N.readFileSync(e,"utf-8")),r=Number(t.memoryCharLimit);Number.isFinite(r)&&r>=Gi&&r<=jn&&(this.memoryCharLimit=Math.round(r))}}catch{}this.recordFileSnapshot("MEMORY.md"),this.recordFileSnapshot("USER.md"),this.memoryEntries=this.readFile("MEMORY.md"),this.userEntries=this.readFile("USER.md"),this.memoryEntries=[...new Set(this.memoryEntries)],this.userEntries=[...new Set(this.userEntries)],this.snapshot={memory:this.renderBlock("memory",this.memoryEntries),user:this.renderBlock("user",this.userEntries)}}formatForSystemPrompt(e){return this.snapshot[e]||null}add(e,t){if(t=t.trim(),!t)return this.errorResult(e,"Content cannot be empty.");let r=ms(t,"strict");if(r)return this.errorResult(e,`Content rejected: detected threat pattern "${r}". Memory entries must not contain prompt injection or data exfiltration patterns.`);let s=this.entriesFor(e),n=this.charLimit(e);if(s.includes(t))return this.successResult(e,"Entry already exists (no duplicate added).");let i=[...s,t];if(this.joinLength(i)>n){let a=this.joinLength(s);return{...this.errorResult(e,`Memory at ${a}/${n} chars. Adding this entry (${t.length} chars) would exceed the limit. Replace or remove existing entries first.`),entries:s,usage:`${a}/${n}`}}return this.setEntries(e,i),this.saveToDisk(e),this.successResult(e,"Entry added.")}replace(e,t,r){if(t=t.trim(),r=r.trim(),!t)return this.errorResult(e,"old_text cannot be empty.");if(!r)return this.errorResult(e,"new_content cannot be empty. Use 'remove' to delete entries.");let s=ms(r,"strict");if(s)return this.errorResult(e,`Content rejected: detected threat pattern "${s}". Memory entries must not contain prompt injection or data exfiltration patterns.`);let n=this.entriesFor(e),i=n.map((u,p)=>[p,u]).filter(([,u])=>u.includes(t));if(i.length===0)return this.errorResult(e,`No entry matched '${t}'.`);if(i.length>1)return new Set(i.map(([,p])=>p)).size>1?this.errorResult(e,`Multiple entries matched '${t}'. Be more specific.`):this.errorResult(e,`Multiple identical entries matched '${t}'. Use remove + add to replace all, or be more specific.`);let o=i[0][0],a=this.charLimit(e),c=[...n];c[o]=r;let d=this.joinLength(c);return d>a?this.errorResult(e,`Replacement would put memory at ${d}/${a} chars. Shorten or remove other entries first.`):(n[o]=r,this.setEntries(e,n),this.saveToDisk(e),this.successResult(e,"Entry replaced."))}remove(e,t){if(t=t.trim(),!t)return this.errorResult(e,"old_text cannot be empty.");let r=this.entriesFor(e),s=r.map((i,o)=>[o,i]).filter(([,i])=>i.includes(t));if(s.length===0)return this.errorResult(e,`No entry matched '${t}'.`);if(s.length>1)return new Set(s.map(([,o])=>o)).size>1?this.errorResult(e,`Multiple entries matched '${t}'. Be more specific.`):this.errorResult(e,`Multiple identical entries matched '${t}'. Be more specific to disambiguate.`);let n=s[0][0];return r.splice(n,1),this.setEntries(e,r),this.saveToDisk(e),this.successResult(e,"Entry removed.")}getEntries(e){return[...this.entriesFor(e)]}usage(e){let t=this.joinLength(this.entriesFor(e)),r=this.charLimit(e),s=r>0?Math.min(100,Math.floor(t/r*100)):0;return{current:t,limit:r,pct:s}}compressWith(e,t){if(!t||t.length===0)return this.errorResult(e,"entries cannot be empty.");let r=[];for(let o of t){let a=(o??"").trim();if(a)if(a.includes(cr))for(let c of a.split(cr)){let d=c.trim();d&&r.push(d)}else r.push(a)}if(r.length===0)return this.errorResult(e,"No valid entries after flattening.");for(let o of r){let a=ms(o,"strict");if(a)return this.errorResult(e,`Content rejected: detected threat pattern "${a}".`)}let s=this.charLimit(e),n=this.joinLength(r);if(n>s)return this.errorResult(e,`Compressed entries (${n}/${s} chars) would exceed the limit. Shorten or split entries.`);let i=this.filePath(e);if(N.existsSync(i)){let o=`${i}.bak.${Math.floor(Date.now()/1e3)}`;try{N.copyFileSync(i,o)}catch{}try{let a=`${xe.basename(i)}.bak.`,c=N.readdirSync(this.memDir).filter(d=>d.startsWith(a)).map(d=>xe.join(this.memDir,d)).sort((d,u)=>N.statSync(u).mtimeMs-N.statSync(d).mtimeMs);for(let d of c.slice(5))N.unlinkSync(d)}catch{}}if(this.setEntries(e,r),this.saveToDisk(e),e==="memory"&&n>s*.8){let o=this.memoryCharLimit;return this.memoryCharLimit<jn&&(this.memoryCharLimit=Math.min(jn,Math.round(this.memoryCharLimit*1.2)),this.persistLimit()),this.successResult(e,`Compressed to ${r.length} entries (backup saved). Limit ${o} \u2192 ${this.memoryCharLimit} chars${this.memoryCharLimit>=jn?" (cap reached)":""}.`)}return this.successResult(e,`Compressed to ${r.length} entries (backup saved).`)}entriesFor(e){return e==="user"?this.userEntries:this.memoryEntries}setEntries(e,t){e==="user"?this.userEntries=t:this.memoryEntries=t}charLimit(e){return e==="user"?Xd:this.memoryCharLimit}persistLimit(){try{N.mkdirSync(this.memDir,{recursive:!0}),N.writeFileSync(xe.join(this.memDir,$a),JSON.stringify({memoryCharLimit:this.memoryCharLimit},null,2),"utf-8")}catch(e){console.warn(`[FileMemoryStore] persist limit failed: ${e.message}`)}}joinLength(e){return e.length===0?0:e.join(cr).length}filePath(e){return xe.join(this.memDir,e==="user"?"USER.md":"MEMORY.md")}fileName(e){return e==="user"?"USER.md":"MEMORY.md"}readFile(e){let t=xe.join(this.memDir,e);try{if(!N.existsSync(t))return[];let r=N.readFileSync(t,"utf-8");return r.trim()?r.split(cr).map(n=>n.trim()).filter(Boolean):[]}catch{return[]}}saveToDisk(e){N.mkdirSync(this.memDir,{recursive:!0});let t=this.filePath(e),r=this.fileName(e),s=this.entriesFor(e),n=s.length>0?s.join(cr):"";this.acquireLock(e);let i=!0;try{let o=this.checkDrift(r,t);if(o){let c=`${t}.bak.${Date.now()}`;try{N.copyFileSync(t,c)}catch{}throw new Error(`${o} Backup saved to ${c}. Reload the store (loadFromDisk) before writing to resolve the conflict.`)}let a=t+".tmp";try{N.writeFileSync(a,n,"utf-8"),N.renameSync(a,t)}catch(c){try{N.existsSync(a)&&N.unlinkSync(a)}catch{}throw c}this.recordFileSnapshot(r)}finally{i&&this.releaseLock(e)}}renderBlock(e,t){if(t.length===0)return"";let r=this.charLimit(e),s=t.join(cr),n=s.length,i=r>0?Math.min(100,Math.floor(n/r*100)):0,o=e==="user"?`USER PROFILE (who the user is) [${i}% \u2014 ${n.toLocaleString()}/${r.toLocaleString()} chars]`:`MEMORY (your personal notes) [${i}% \u2014 ${n.toLocaleString()}/${r.toLocaleString()} chars]`,a="\u2550".repeat(46);return`${a}
${o}
${a}
${s}`}successResult(e,t){let r=this.entriesFor(e),s=this.joinLength(r),n=this.charLimit(e),i=n>0?Math.min(100,Math.floor(s/n*100)):0,o={success:!0,target:e,entries:[...r],usage:`${i}% \u2014 ${s.toLocaleString()}/${n.toLocaleString()} chars`,entryCount:r.length};return t&&(o.message=t),o}errorResult(e,t){return{success:!1,target:e,entries:[],usage:"",entryCount:0,error:t}}acquireLock(e){let t=this.lockFilePath(e);for(let r=0;r<Wi;r++)try{N.mkdirSync(t);return}catch(s){if(s.code!=="EEXIST")throw s;r<Wi-1&&this.sleep(Qd)}throw new Error(`Failed to acquire lock for ${e} after ${Wi} attempts. Lock held by another process at ${t}.`)}releaseLock(e){let t=this.lockFilePath(e);try{N.rmdirSync(t)}catch{}}lockFilePath(e){return xe.join(this.memDir,`kexvim_mem_${e}.lock`)}recordFileSnapshot(e){let t=xe.join(this.memDir,e);try{let r=N.statSync(t);this.fileSnapshots.set(t,{size:r.size,mtimeMs:r.mtimeMs})}catch{this.fileSnapshots.delete(t)}}checkDrift(e,t){let r=this.fileSnapshots.get(t);if(!r)return null;if(!N.existsSync(t))return`Drift detected: ${e} was deleted since load time (expected snapshot size: ${r.size} bytes).`;let s;try{s=N.statSync(t)}catch{return`Drift detected: unable to stat ${e} for drift check.`}return s.size!==r.size?`Drift detected: ${e} size changed from ${r.size} to ${s.size} bytes since load time.`:s.mtimeMs!==r.mtimeMs?`Drift detected: ${e} modification time changed since load time (was mtime=${r.mtimeMs}, now=${s.mtimeMs}).`:null}sleep(e){let t=Date.now()+e;for(;Date.now()<t;);}};import*as yl from"node:worker_threads";import*as V from"node:fs";import*as mr from"node:path";import*as ct from"node:fs";import*as Hn from"node:path";var ke=class l{static MAX_RESTARTS=3;static WINDOW_SECONDS=60;static STATE_FILE="restart_loop.json";static statePath(e){return Hn.join(e,"data",l.STATE_FILE)}static _loadBoots(e){try{let t=ct.readFileSync(l.statePath(e),"utf-8"),r=JSON.parse(t);return(Array.isArray(r.boots)?r.boots:[]).filter(n=>typeof n=="number"&&Number.isFinite(n))}catch{return[]}}static recordBoot(e){let t=Date.now()/1e3,r=t-l.WINDOW_SECONDS,s=l._loadBoots(e).filter(n=>n>=r).concat(t);try{ct.mkdirSync(Hn.dirname(l.statePath(e)),{recursive:!0}),ct.writeFileSync(l.statePath(e),JSON.stringify({boots:s},null,1))}catch{}return s.length>l.MAX_RESTARTS}static isTripped(e){let r=Date.now()/1e3-l.WINDOW_SECONDS;return l._loadBoots(e).filter(n=>n>=r).length>l.MAX_RESTARTS}static clear(e){try{ct.rmSync(l.statePath(e),{force:!0})}catch{}}static bootCount(e){let t=Date.now()/1e3;return l._loadBoots(e).filter(r=>r>=t-l.WINDOW_SECONDS).length}};import*as At from"node:fs";import*as It from"node:path";import*as Xe from"node:fs";import*as pr from"node:path";import*as Je from"fs";import*as dr from"os";import*as jr from"path";var qn=class{platform="linux";findKexvimPids(){let e=b.runSyncResult('pgrep -af "dev\\\\.mjs|kexvim\\\\.js|Main\\\\.ts" | grep -v " web"',{timeoutMs:3e3});if(e.code!==0)return[];let t=[];for(let r of e.stdout.split(`
`)){let s=r.trim().match(/^(\d+)\s/);if(!s)continue;let n=parseInt(s[1],10);isNaN(n)||n===process.pid||t.push(n)}return t}killProcess(e,t=!1){try{let r=t?"SIGKILL":"SIGTERM";return process.kill(e,r),!0}catch{return!1}}installService(e){try{let t=jr.join(dr.homedir(),".config","systemd","user");Je.mkdirSync(t,{recursive:!0});let r=jr.join(t,`${e.name}.service`),s=`[Unit]
...
`;return Je.writeFileSync(r,s,"utf-8"),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3}).code!==0?!1:b.runSyncResult("loginctl enable-linger "+dr.userInfo().username,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=jr.join(dr.homedir(),".config","systemd","user",`${e}.service`);return Je.existsSync(t)&&(b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:5e3}),Je.unlinkSync(t),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3})),!0}catch{return!1}}isServiceInstalled(e){let t=jr.join(dr.homedir(),".config","systemd","user",`${e}.service`);return Je.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`systemctl --user is-active ${e}`,{timeoutMs:3e3});return t.code!==0?!1:t.stdout.trim()==="active"}startService(e){return b.runSyncResult(`systemctl --user start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`systemctl --user enable ${e}`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`systemctl --user disable ${e}`,{timeoutMs:5e3}).code===0}};import*as Ye from"fs";import*as Wn from"os";import*as Ct from"path";var Gn=class{platform="win32";findKexvimPids(){let e=b.runSyncResult(`powershell -Command "Get-Process node | Where-Object { $_.CommandLine -match 'Main\\.ts|dev\\.mjs|kexvim\\.js' } | Select-Object -ExpandProperty Id"`,{timeoutMs:5e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){let r=t?"/F":"";return b.runSyncResult(`taskkill ${r} /PID ${e}`,{timeoutMs:5e3}).code===0}installService(e){if(b.runSyncResult("nssm --version",{timeoutMs:3e3}).code===0&&b.runSyncResult(`nssm install ${e.name} "${e.execPath}" "${e.workDir}"`,{timeoutMs:1e4}).code===0)return!0;try{let r=Ct.join(process.env.APPDATA||Wn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");Ye.mkdirSync(r,{recursive:!0});let s=`@echo off
cd /d "${e.workDir}"
set KEXVIM_DIR=${e.workDir}
npx tsx src/Main.ts
`;return Ye.writeFileSync(Ct.join(r,`${e.name}.bat`),s,"utf-8"),!0}catch{return!1}}uninstallService(e){if(!(b.runSyncResult(`nssm remove ${e} confirm`,{timeoutMs:1e4}).code===0)){let r=Ct.join(process.env.APPDATA||Wn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup"),s=Ct.join(r,`${e}.bat`);if(Ye.existsSync(s))try{Ye.unlinkSync(s)}catch{}}return!0}isServiceInstalled(e){if(b.runSyncResult(`nssm status ${e}`,{timeoutMs:3e3}).code===0)return!0;let t=Ct.join(process.env.APPDATA||Wn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");return Ye.existsSync(Ct.join(t,`${e}.bat`))}isServiceRunning(e){let t=b.runSyncResult(`powershell -Command "Get-Service ${e} | Select-Object -ExpandProperty Status"`,{timeoutMs:5e3});return t.code!==0?!1:t.stdout.trim().toLowerCase()==="running"}startService(e){return b.runSyncResult(`net start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`net stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`sc config ${e} start=auto`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`sc config ${e} start=disabled`,{timeoutMs:5e3}).code===0}};import*as Ve from"fs";import*as ur from"os";import*as Rt from"path";var Kn=class{platform="darwin";findKexvimPids(){let e=b.runSyncResult('pgrep -f "tsx.*Main\\.ts"',{timeoutMs:3e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){try{return process.kill(e,t?"SIGKILL":"SIGTERM"),!0}catch{return!1}}installService(e){try{let t=Rt.join(ur.homedir(),"Library","LaunchAgents");Ve.mkdirSync(t,{recursive:!0});let r=Rt.join(t,`${e.name}.plist`),n=`<?xml version="1.0" encoding="UTF-8"?>
...
</plist>`;return Ve.writeFileSync(r,n,"utf-8"),b.runSyncResult(`launchctl load ${r}`,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=Rt.join(ur.homedir(),"Library","LaunchAgents",`${e}.plist`);return Ve.existsSync(t)&&(b.runSyncResult(`launchctl unload ${t}`,{timeoutMs:5e3}),Ve.unlinkSync(t)),!0}catch{return!1}}isServiceInstalled(e){let t=Rt.join(ur.homedir(),"Library","LaunchAgents",`${e}.plist`);return Ve.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`launchctl list local.${e}`,{timeoutMs:3e3});if(t.code!==0)return!1;let r=t.stdout.trim();return r.includes(`local.${e}`)&&!r.startsWith("-")}startService(e){let t=Rt.join(ur.homedir(),"Library","LaunchAgents",`${e}.plist`);return b.runSyncResult(`launchctl load ${t}`,{timeoutMs:5e3}).code===0}stopService(e){let t=Rt.join(ur.homedir(),"Library","LaunchAgents",`${e}.plist`);return b.runSyncResult(`launchctl unload ${t}`,{timeoutMs:5e3}).code===0}enableService(e){return this.startService(e)}disableService(e){return this.stopService(e)}};function Zd(){switch(typeof process<"u"?process.platform:"linux"){case"win32":return new Gn;case"darwin":return new Kn;default:return new qn}}var Ki=null;function Mt(){return Ki||(Ki=Zd()),Ki}var dt=class l{static webPort(){let e=Number(process.env.KEXVIM_WEB_PORT);return Number.isFinite(e)&&e>0?e:8787}static findPid(){let e=l.webPort();try{if(process.platform==="win32"){let t=b.runSyncResult(`netstat -ano | findstr :${e} | findstr LISTENING`,{timeoutMs:5e3});for(let r of(t.stdout||"").split(/\r?\n/)){let s=r.trim().match(/LISTENING\s+(\d+)\s*$/);if(s)return Number(s[1])}}else{let r=(b.runSyncResult(`ss -ltnp 'sport = :${e}'`,{timeoutMs:5e3}).stdout||"").match(/pid=(\d+)/);if(r)return Number(r[1])}}catch{}return null}static webEntry(e){let t=pr.join(e,"dist","dev.mjs");if(Xe.existsSync(t))return t;let r=pr.join(e,"kexvim.js");return Xe.existsSync(r)?r:pr.join(e,"src","Main.ts")}static restart(e){let t=l.findPid();if(t!==null){let o=Mt();try{process.platform==="win32"?b.runSyncResult(`taskkill /PID ${t} /F /T`,{timeoutMs:5e3}):o.killProcess(t,!0)}catch{}}let r=l.webEntry(e),s=pr.join(e,"data","log");try{Xe.mkdirSync(s,{recursive:!0})}catch{}let n=null;try{n=Xe.openSync(pr.join(s,"web.log"),"a")}catch{}let i=b.spawn(process.execPath,[r,"web"],{cwd:e,stdio:n!==null?["ignore",n,n]:"ignore",detached:!0});return n!==null&&Xe.closeSync(n),i.unref(),!0}};var ut=class l{static resolveEntry(e){let t=It.join(e,"dist","dev.mjs");if(At.existsSync(t))return t;let r=It.join(e,"kexvim.js");return At.existsSync(r)?r:It.join(e,"src","Main.ts")}static buildEnv(e){let t={};for(let[n,i]of Object.entries(process.env))i!==void 0&&(t[n]=i);let r=It.join(e,"data",".env");if(!At.existsSync(r))return t;let s=At.readFileSync(r,"utf-8");for(let n of s.split(`
`)){let i=n.trim();if(!i||i.startsWith("#"))continue;let o=i.indexOf("=");if(o===-1)continue;let a=i.slice(0,o).trim(),c=i.slice(o+1).trim();(c.startsWith('"')&&c.endsWith('"')||c.startsWith("'")&&c.endsWith("'"))&&(c=c.slice(1,-1)),a&&!(a in t)&&(t[a]=c)}return t}static spawnFreshDaemon(e,t){let r=t;if(!r){let n=process.argv[1];r=n&&At.existsSync(n)?It.resolve(n):l.resolveEntry(e)}let s=b.spawn(process.execPath,[r,"--daemon"],{cwd:e,stdio:"ignore",env:l.buildEnv(e),detached:!0});return s.unref(),s.pid??null}static selfRestart(e,t){try{dt.restart(e)}catch(r){t?.(`[kexvim] web \u91CD\u542F\u5931\u8D25: ${r instanceof Error?r.message:String(r)}
`)}return l.spawnFreshDaemon(e,process.argv[1])}};import*as $e from"node:worker_threads";var gr=class l{static workersLaunched=!1;static workerInstances=new Map;static async startWatchdog(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D .stop_watchdog\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let t=mr.join(e,"data",".stop_watchdog");setInterval(()=>{if(V.existsSync(t)){try{V.unlinkSync(t)}catch{}process.exit(0)}},3e3),process.on("SIGTERM",()=>{ut.spawnFreshDaemon(e),process.exit(0)}),await new Promise(()=>{})}static async startGuardian(){let e=I.load();S.init(e.language||"zh-CN");let t=new Tt(xt.load());console.log(`[guardian] ready \u2014 ${t.llmClient.model}`);let r=$e.parentPort;if(!r){await new Promise(()=>{});return}r.on("message",async s=>{let n=s;if(!(!n||n.type!=="repair"))try{let i=await t.process(String(n.text??""));r.postMessage({type:"repair-result",reqId:n.reqId,text:i})}catch(i){r.postMessage({type:"repair-result",reqId:n.reqId,text:`[guardian error] ${i.message}`})}})}static terminateWorkers(){for(let e of l.workerInstances.values())e.terminate();l.workerInstances.clear()}static launchConsoleWorkers(){for(let t of["watchdog","agent"]){let r=new $e.Worker(process.argv[1],{workerData:{role:t},stdout:!0,stderr:!0});r.stdout.on("data",s=>process.stdout.write(`[${t[0]}] ${s}`)),r.stderr.on("data",s=>process.stderr.write(`[${t[0]}] ${s}`)),r.on("exit",s=>console.log(`[${t}] exited (${s})`)),l.workerInstances.set(t,r)}let e=()=>{l.terminateWorkers()};process.on("SIGTERM",e),process.on("SIGINT",e),process.on("exit",e)}static launchDaemon(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u542F\u52A8 daemon\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");if(ke.recordBoot(e))try{V.appendFileSync(l.resolveLogPath(),`[kexvim] \u26A0\uFE0F restart_loop guard TRIPPED: ${ke.bootCount(e)} boots in ${ke.WINDOW_SECONDS}s. \u81EA\u52A8\u62C9\u8D77\u6682\u505C\uFF0C\u8BF7\u4EBA\u5DE5\u68C0\u67E5\uFF08kexvim status / kexvim clear-loop\uFF09
`)}catch{}try{V.writeFileSync(mr.join(e,"data","kexvim.pid"),String(process.pid))}catch{}process.on("exit",()=>{try{V.rmSync(mr.join(e,"data","kexvim.pid"),{force:!0})}catch{}});let r=()=>{l._shuttingDown=!0,l.terminateWorkers(),process.exit(0)};process.on("SIGTERM",r),process.on("SIGINT",r);let s=()=>{if(process.platform!=="linux")return!1;try{let i=V.readFileSync("/proc/self/environ","utf-8");return i.includes("INVOCATION_ID")||i.includes("SYSTEMD_EXEC_PID")}catch{return!1}};process.platform!=="win32"&&process.on("SIGHUP",()=>{try{V.appendFileSync(l.resolveLogPath(),`[kexvim] SIGHUP received \u2014 ${s()?"systemd-managed, exiting for systemd restart":"standalone, spawning fresh daemon"}
`)}catch{}s()||ut.selfRestart(e,i=>{try{V.appendFileSync(l.resolveLogPath(),i)}catch{}}),r()});let n=l.resolveLogPath();for(let i of["watchdog","agent"]){let o=new $e.Worker(process.argv[1],{workerData:{role:i},stdout:!0,stderr:!0});l.workerInstances.set(i,o);let a=u=>{try{V.appendFileSync(n,u)}catch{}};o.stdout.on("data",a),o.stderr.on("data",a);let c=()=>{let u=new $e.Worker(process.argv[1],{workerData:{role:i},stdout:!0,stderr:!0});l.workerInstances.set(i,u),u.stdout.on("data",a),u.stderr.on("data",a),u.on("error",p=>{try{V.appendFileSync(n,`[kexvim] worker '${i}' error: ${p.message}
`)}catch{}}),u.on("exit",d)},d=u=>{l.workerInstances.delete(i);let p=u===0||u===null;try{V.appendFileSync(n,`[kexvim] worker '${i}' exited (code=${u}) ${p?"":"\u2014 CRASH, restarting"} at ${new Date().toISOString()}
`)}catch{}!p&&!l._shuttingDown&&setTimeout(c,2e3)};o.on("error",u=>{try{V.appendFileSync(n,`[kexvim] worker '${i}' error: ${u.message}
`)}catch{}}),o.on("exit",d)}}static resolveLogPath(){let e=I.findProjectRoot();if(!e)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D kexvim.log\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let t=mr.join(e,"data","log");try{V.mkdirSync(t,{recursive:!0})}catch{}return mr.join(t,"kexvim.log")}static _shuttingDown=!1;static async handleWorkerDispatch(){if(!$e.isMainThread){let t=$e.workerData?.role||"agent";return t==="watchdog"?(await l.startWatchdog(),!0):t==="guardian"?(await l.startGuardian(),!0):!1}return process.argv.includes("--daemon")?(l.launchDaemon(),!0):(I.load().platform?.enabled&&!process.argv.slice(2).includes("web")&&(l.launchConsoleWorkers(),l.workersLaunched=!0),!1)}};import*as ue from"node:fs";import*as Vr from"node:path";import*as ll from"node:worker_threads";var eu=new Set(["stop","new","newchat","reset","\u65B0\u4F1A\u8BDD","\u5207\u6362\u4F1A\u8BDD","\u5207\u4F1A\u8BDD"]);function Da(l,e){let t=process.env[l];if(t===void 0||t.trim()==="")return e;let r=parseFloat(t);return Number.isFinite(r)?r:e}function tu(l,e){let t=process.env[l];return t===void 0||t.trim()===""?e:["1","true","yes","on"].includes(t.trim().toLowerCase())}function ru(l,e){let t=process.env[l]?.trim().toLowerCase();return t==="steer"||t==="queue"||t==="interrupt"?t:e}var fr=class l{adapters=new Map;config;messageHandler=null;activeSessions=new Map;pendingMessages=new Map;debounceTimers=new Map;debounceStates=new Map;_lastBusyNotifyTime=new Map;_sessionControllers=new Map;static _BUSY_COOLDOWN=30;static splitNatural(e,t=1500){if(!e||e.length<=t)return[e];let r=[],s="",n=()=>{s.trim()&&(r.push(s.trim()),s="")},i=e.split(/\n+/).filter(o=>o.length>0);for(let o of i)if(o.length>t){n();let a=o.match(/[^。！？；.!?;]+[。！？；.!?;]?/g)||[o],c="";for(let d of a){if(d.length>t){c&&(r.push(c.trim()),c="");for(let u=0;u<d.length;u+=t)r.push(d.slice(u,u+t));continue}c&&(c+d).length>t?(r.push(c.trim()),c=d):c+=d}c.trim()&&r.push(c.trim())}else s&&(s+`
`+o).length>t?(n(),s=o):s=s?s+`
`+o:o;return n(),r.filter(o=>o.length>0)}_busyStateProvider=null;_steerProvider=null;_redirectProvider=null;constructor(e={}){this.config={timeout:e.timeout??6e4,startupWait:e.startupWait??5e3,busyTextDebounceSeconds:e.busyTextDebounceSeconds??Da("HERMES_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS",.35),busyTextHardCapSeconds:e.busyTextHardCapSeconds??Da("HERMES_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS",1),busyAckEnabled:e.busyAckEnabled??tu("HERMES_GATEWAY_BUSY_ACK_ENABLED",!0),steerAckEnabled:e.steerAckEnabled??!0,busyAckDetail:e.busyAckDetail??!0,busyInputMode:e.busyInputMode??ru("HERMES_GATEWAY_BUSY_INPUT_MODE","interrupt"),busyTextMode:e.busyTextMode??(process.env.HERMES_GATEWAY_BUSY_TEXT_MODE?.trim().toLowerCase()==="queue"?"queue":"interrupt")}}getSignalForMessage(e){return this._sessionControllers.get(this.getSessionKey(e))?.signal}register(e){if(this.adapters.has(e.name))throw new Error(`Adapter '${e.name}' already registered`);this.adapters.set(e.name,e)}async unregister(e){let t=this.adapters.get(e);t&&(await t.stop(),this.adapters.delete(e))}setMessageHandler(e,t){this.messageHandler=e,this._busyStateProvider=t?.busyStateProvider??null,this._steerProvider=t?.steerProvider??null,this._redirectProvider=t?.redirectProvider??null,this.adapters.forEach(r=>{r.setMessageHandler(this.dispatch)})}getSessionKey(e){return e.groupId?`${e.groupId}:${e.userId}`:e.userId}toRuntimeSessionKey(e){return e.source?`${e.source.platform}:${e.source.chatId}`:e.groupId?`qq:group:${e.groupId}`:`qq:${e.userId.replace(/^user:/,"")}`}getSenderKey(e){return e.groupId?e.userId:""}releaseSessionGuard(e){this.activeSessions.has(e)&&this.activeSessions.delete(e)}isBypassCommand(e){if(!e.trim().startsWith("/"))return null;let t=e.trim().slice(1).split(" ")[0]?.toLowerCase();return t&&eu.has(t)?t:null}static _BUSY_QUEUE_MAX_PENDING=32;queueOrReplacePendingEvent(e,t){let r=this.pendingMessages.get(e);if(r||(r=[],this.pendingMessages.set(e,r)),(!!t.media_urls?.length||t.message_type==="photo")&&r.length>0){let n=r[0];n.text&&t.text?n.text=n.text+`
`+t.text:t.text&&(n.text=t.text);let i=n.media_urls,o=t.media_urls;o&&Array.isArray(o)&&(n.media_urls=[...i||[],...o])}else{if(r.length>=l._BUSY_QUEUE_MAX_PENDING){console.warn(`[gateway] Dropping busy follow-up for ${e} \u2014 pending queue at cap (${l._BUSY_QUEUE_MAX_PENDING})`);return}r.push({...t})}}dequeuePending(e){let t=this.pendingMessages.get(e);if(!t||t.length===0){this.pendingMessages.delete(e);return}let r=t.shift();return t.length===0&&this.pendingMessages.delete(e),r}isDebounceCandidate(e){return!!e.text?.trim()&&!e.text.trim().startsWith("/")}textDebounceDelay(e){let t=Date.now(),r=e.lastTs+this.config.busyTextDebounceSeconds*1e3,s=e.firstTs+this.config.busyTextHardCapSeconds*1e3;return Math.max(0,Math.min(r,s)-t)}canMergeDebounceEvents(e,t){return e.senderKey===t}queueOrDebounce(e,t){this.isDebounceCandidate(t)?this.queueTextDebounce(e,t):this.queueOrReplacePendingEvent(e,t)}dispatch=async e=>{if(!this.messageHandler)return;let t=this.getSessionKey(e);if(!this.activeSessions.has(t)){this.setSessionGuard(t),this.processMessage(t,e);return}let r=H.getProgress(this.toRuntimeSessionKey(e)),s=Date.now(),n=m=>{console.log(`[gateway] busy-dispatch ${t} \u2192 ${m} text="${(e.text||"").trim().slice(0,40)}"`)};if(this.isBypassCommand(e.text)){this._abortSessionController(t),this.queueOrDebounce(t,e),this._sendBusyAck(t,e,r,s,"interrupt"),n("bypass-interrupt");return}if(!r){this.queueOrDebounce(t,e),this._sendBusyAck(t,e,r,s,"queue"),n("queue-no-progress");return}if(this._busyStateProvider){let m=this._busyStateProvider(t);if(m.hasActiveSubagents){this.queueOrDebounce(t,e),this._sendBusyAck(t,e,r,s,"subagent"),n("queue-subagent");return}if(m.isCompressing){this.queueOrDebounce(t,e),this._sendBusyAck(t,e,r,s,"compression"),n("queue-compression");return}}let o=this.config.busyInputMode,a=this.config.busyTextMode;if(!!e.text?.trim()&&!e.text.trim().startsWith("/")&&!this.isBypassCommand(e.text)&&a==="queue"&&o!=="steer"){this.queueOrDebounce(t,e),this._sendBusyAck(t,e,r,s,"queue"),n("queue-text-mode");return}let d=!1;if(o==="steer"){let m=(e.text||"").trim();m?this._steerProvider&&this._steerProvider(t,m)?(d=!0,n("steer-injected")):(o="queue",n("steer-fail-queue")):o="queue"}let u=!1;o==="interrupt"&&(u=!!e.text?.trim()&&!e.media_urls?.length&&!e.media_types?.length&&!!this._redirectProvider&&this._redirectProvider(t,(e.text||"").trim()),u?n("redirect-ok"):(this._abortSessionController(t,e.text),n("abort-hard"))),!d&&!u&&(this.queueOrDebounce(t,e),n("queue-fifo"));let p=u?"redirect":o;n(`ack:${p}`),this._sendBusyAck(t,e,r,s,p)};_abortSessionController(e,t){let r=this._sessionControllers.get(e);r&&!r.signal.aborted&&(t&&t.trim()?r.abort(t.trim()):r.abort())}_sendBusyAck(e,t,r,s,n){if(!this.config.busyAckEnabled||n==="steer"&&!this.config.steerAckEnabled)return;let i=this._lastBusyNotifyTime.get(e)??0;if(s-i<l._BUSY_COOLDOWN*1e3)return;this._lastBusyNotifyTime.set(e,s);let o="";if(this.config.busyAckDetail&&r){let d=Math.max(1,Math.round((s-r.startTime)/6e4)),u=r.current??0,p=r.max??0,m=H.getCurrentTool()??"",g=[];d>0&&g.push(`\u5DF2\u8FD0\u884C ${d} \u5206\u949F`),p&&g.push(`\u8FED\u4EE3 ${u}/${p}`),m&&g.push(`\u6B63\u5728\u6267\u884C: ${m}`),g.length>0&&(o=` (${g.join(", ")})`)}let a;switch(n){case"interrupt":a="gateway.busy_interrupt";break;case"redirect":a="gateway.busy_redirect";break;case"subagent":a="gateway.busy_queue_subagent";break;case"compression":a="gateway.busy_queue_compression";break;case"queue":a="gateway.busy_queue";break;default:a="gateway.busy_steer";break}let c=S.t(a,{status:o});t.sendReply?.(c)}setSessionGuard(e){this.activeSessions.set(e,!0)}queueTextDebounce(e,t){let r=Date.now(),s=this.getSenderKey(t),n=this.debounceStates.get(e);if(n){if(!this.canMergeDebounceEvents(n,s)){this.flushTextDebounce(e);let i=this.pendingMessages.get(e),o=i&&i.length>0?i[0]:void 0;if(o&&this.canMergeDebounceEvents({senderKey:this.getSenderKey(o)},s)){o.text=o.text?o.text+`
...
===== \u5206\u652F\u5BF9\u8BDD\u5185\u5BB9\u7ED3\u675F =====`}static async compact(e,t,r){let s=e.getMessagesAsConversation(t,200);if(s.length<10)return null;let n=Math.min(40,Math.floor(s.length*.4)),i=s.length-n,o=s.slice(0,i);if(o.length===0)return null;let a=o.reduce((d,u)=>d+l.estimateTokens(u),0);if(a<500)return null;let c;return r?c=await r(o,void 0):c=`\u538B\u5B9E\u4E86 ${o.length} \u6761\u6D88\u606F\uFF0C\u8282\u7701\u7EA6 ${a} tokens\u3002`,{summary:c,firstKeptEntryId:i,tokensBefore:a,estimatedTokensAfter:a}}};var Vu={qq:l=>new Ur({appId:l.app_id,clientSecret:l.client_secret,apiBase:l.api_base,markdownSupport:l.markdown_support}),api_server:l=>new Hr({port:l.port,host:l.host,apiKey:l.api_key}),telegram:l=>new qr({token:l.token,pollTimeout:l.poll_timeout,groupMentionOnly:l.group_mention_only}),discord:l=>new Wr({token:l.token,groupMentionOnly:l.group_mention_only}),weixin:l=>new Gr({token:l.token,accountId:l.account_id,groupMentionOnly:l.group_mention_only}),dingtalk:l=>new Kr({clientId:l.client_id,clientSecret:l.client_secret,groupMentionOnly:l.group_mention_only}),feishu:l=>new zr({appId:l.app_id,appSecret:l.app_secret,groupMentionOnly:l.group_mention_only}),ws:l=>new Yr({port:l.port,host:l.host,apiKey:l.api_key})};function ol(l,e){let t=[];for(let[r,s]of Object.entries(e)){let n=Vu[r];if(!n){console.warn(S.t("gateway.unknown_adapter",{name:r}));continue}if(r==="qq"){let o=s;if(!o.app_id)throw new Error("QQ Bot app_id \u672A\u914D\u7F6E\uFF08config.yaml \u4E2D platform.adapters.qq.app_id\uFF0C\u6CE8\u518C\uFF1Ahttps://q.qq.com\uFF09");if(!o.client_secret)throw new Error("QQ Bot client_secret \u672A\u914D\u7F6E\uFF08config.yaml \u4E2D platform.adapters.qq.client_secret\uFF0C\u6CE8\u518C\uFF1Ahttps://q.qq.com\uFF09")}let i=n(s);l.register(i),t.push(i),console.error(S.t("gateway.adapter_registered",{name:r}))}return t}function Vi(){let l=I.findProjectRoot();if(!l)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D .last_user\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");return Vr.join(l,"data",".last_user")}var zn=null,Xu=0,$t=new Map;function Qu(){if(zn)return zn;let l=new ll.Worker(process.argv[1],{workerData:{role:"guardian"},stdout:!0,stderr:!0});return l.stdout.on("data",e=>{try{console.error(`[guardian] ${e}`)}catch{}}),l.stderr.on("data",e=>{try{console.error(`[guardian] ${e}`)}catch{}}),l.on("message",e=>{let t=e;if(!t||t.type!=="repair-result")return;let r=$t.get(t.reqId);r&&(clearTimeout(r.timer),$t.delete(t.reqId),r.resolve(String(t.text??"")))}),l.on("exit",e=>{zn=null;for(let[,t]of $t)clearTimeout(t.timer),t.resolve(`[guardian] \u7EBF\u7A0B\u5DF2\u9000\u51FA (code=${e})`);$t.clear()}),l.on("error",e=>{try{console.error(`[guardian] worker error: ${e.message}`)}catch{}}),zn=l,l}function Zu(l,e=18e4){let t=Qu(),r=++Xu;return new Promise(s=>{let n=setTimeout(()=>{$t.delete(r),s(`[guardian] \u8D85\u65F6\u65E0\u54CD\u5E94\uFF08${Math.round(e/1e3)}s\uFF09\uFF0C\u7EBF\u7A0B\u53EF\u80FD\u4ECD\u5728\u5904\u7406`)},e);$t.set(r,{resolve:s,timer:n});try{t.postMessage({type:"repair",reqId:r,text:l})}catch(i){clearTimeout(n),$t.delete(r),s(`[guardian] \u53D1\u9001\u5931\u8D25: ${i.message}`)}})}function al(l,e){let t=(()=>{try{return ue.readFileSync(e,"utf-8").trim()||null}catch{return null}})();if(!t)return;let r=l.find(c=>c.name!=="api_server");if(!r)return;let s=()=>r.sendText(t,"\u2705 Kexvim \u5DF2\u91CD\u65B0\u4E0A\u7EBF").catch(()=>{}),n=100,i=15e3,o=Date.now(),a=()=>{r.isConnected()||Date.now()-o>i?s():(n=Math.min(n*1.5,1e3),setTimeout(a,n))};a()}var Jn=class l{static createGatewayHandler(e){let{runtime:t,sessionStore:r,registry:s,signalProvider:n,sendTyping:i}=e;return async o=>{try{console.error(S.t("gateway.msg_in",{userId:o.userId,text:o.text.slice(0,100)}));let a=Vi();if(!(o.source?o.source.chatType==="group":o.userId.startsWith("group:")))try{ue.writeFileSync(a,o.userId,"utf-8")}catch{}let d=o.text.trim(),u=d.toLowerCase();if(u.startsWith("repair/")||u.startsWith("\u4FEE\u590D/")){let C=(await Zu(d))?.trim();return C&&console.error(S.t("gateway.msg_out",{userId:o.userId,text:C.slice(0,100)})),C}let p="qq",m="c2c",g=o.userId,f,h=o.userId;if(o.source)p=o.source.platform,m=o.source.chatType,g=o.source.chatId,f=o.source.guildId??o.groupId,h=o.source.userId??o.userId;else{let A=o.userId.split(":");A.length>=2&&A[0]==="group"?(m="group",f=A[1],g=`group:${A[1]}`):A.length>=2&&A[0]==="user"&&(g=A[1]),h=A.length>=2?A[1]:o.userId}if(new Set(["/new","/newchat","/\u65B0\u4F1A\u8BDD","/\u5207\u6362\u4F1A\u8BDD","/\u5207\u4F1A\u8BDD","\u5207\u4F1A\u8BDD"]).has(d))try{let{sessionId:A,prevSessionId:C}=await t.startNewSession({source:p,chatType:m,chatId:g,userId:h}),P=C?`\u65E7\u4F1A\u8BDD\uFF08${C.slice(0,8)}\uFF09\u5DF2\u4FDD\u7559\uFF0C\u53EF\u968F\u65F6\u56DE\u6EAF\u3002`:"";return`\u2705 \u5DF2\u5207\u6362\u5230\u65B0\u4F1A\u8BDD\uFF08${A.slice(0,8)}\uFF09\u3002${P}`}catch(A){return`\u274C \u5207\u4F1A\u8BDD\u5931\u8D25: ${A.message}`}let y=/^\/resume\s+([A-Za-z0-9-]{8,})/;if(d==="/sessions"||d==="/\u4F1A\u8BDD\u5217\u8868"||y.test(d))try{let A=r.profile??"default";if(d==="/sessions"||d==="/\u4F1A\u8BDD\u5217\u8868"){let j=await r.listRecent(A,100);if(j.length===0)return"\u6682\u65E0\u5386\u53F2\u4F1A\u8BDD\u3002\u53D1\u9001 /new \u5F00\u59CB\u65B0\u4F1A\u8BDD\u3002";let pe="";try{let me=I.findProjectRoot(),gt=me?Vr.join(me,"data",".current_session"):"";gt&&ue.existsSync(gt)&&(pe=ue.readFileSync(gt,"utf-8").trim())}catch{}return`\u6700\u8FD1\u4F1A\u8BDD\uFF1A
${j.map((me,gt)=>{let no=me.id===pe?" \u25C0 \u5F53\u524D":"";return`${gt+1}. ${me.id.slice(0,8)} - ${et.displayTitle(me.summary,me.firstUserMsg)}  [${me.source}]${no}`}).join(`
`)}

\u7528 /resume <\u524D8\u4F4DID> \u6062\u590D\u3002`}let C=d.match(y),P=await t.resumeSession(C[1],{source:p,chatType:m,chatId:g,userId:h});return P?`\u2705 \u5DF2\u6062\u590D\u4F1A\u8BDD ${P.sessionId.slice(0,8)}\uFF08${P.messageCount} \u6761\u6D88\u606F\uFF09\uFF0C\u7EE7\u7EED\u5BF9\u8BDD\u3002`:`\u274C \u627E\u4E0D\u5230\u4F1A\u8BDD ${C[1]}\u3002\u7528 /sessions \u67E5\u770B\u53EF\u6062\u590D\u7684\u4F1A\u8BDD\u3002`}catch(A){return`\u274C \u4F1A\u8BDD\u64CD\u4F5C\u5931\u8D25: ${A.message}`}try{let A=I.findProjectRoot(),C=A?Vr.join(A,"data","session-switch.json"):null;if(C&&ue.existsSync(C)){let P=JSON.parse(ue.readFileSync(C,"utf-8"));if(ue.rmSync(C,{force:!0}),P.sessionId){let j=await t.resumeSession(P.sessionId,{source:p,chatType:m,chatId:g,userId:h});return j?(console.error(`[CLI\u5207\u6362] \u5DF2\u6062\u590D\u4F1A\u8BDD ${j.sessionId.slice(0,8)}\uFF08${j.messageCount} \u6761\u6D88\u606F\uFF09`),`\u2705 \u5DF2\u5207\u6362\u5230\u5386\u53F2\u4F1A\u8BDD ${j.sessionId.slice(0,8)}\uFF08${j.messageCount} \u6761\u6D88\u606F\uFF09\uFF0C\u7EE7\u7EED\u5BF9\u8BDD\u3002`):`\u274C \u627E\u4E0D\u5230\u4F1A\u8BDD ${P.sessionId.slice(0,8)}\u3002`}}}catch(A){console.error(`[CLI\u5207\u6362] \u6807\u8BB0\u5904\u7406\u5931\u8D25: ${A.message}`);try{ue.rmSync(Vr.join(I.findProjectRoot()??".","data","session-switch.json"),{force:!0})}catch{}}let v=r.recover({chatId:g,chatType:m,source:p,userId:h}).session?.id,w=o.sendReply?A=>{o.sendReply(A).catch(()=>{})}:void 0;try{i&&i(o.userId)}catch{}let R=n?.(o),_=Number(process.env.HERMES_AGENT_NOTIFY_INTERVAL||180),M=_>0?_*1e3:0,L=null,he=()=>{L&&(clearInterval(L),L=null)};M>0&&w&&(L=setInterval(()=>{let A=t.getActivitySummary();if(!A)return;let C=Math.floor((Date.now()-A.startTime)/6e4);w(S.t("gateway.long_running",{elapsed:C,i:A.iteration,max:A.maxIterations,tool:A.currentTool?` \u2014 ${A.currentTool}`:""}))},M));let O;try{O=await t.chat(d,{source:p,chatType:m,chatId:g,userId:h,signal:R,statusCallback:w})}finally{he()}let z=O.content?.trim();if(!z)return;if(console.error(S.t("gateway.msg_out",{userId:o.userId,text:z.slice(0,100)})),O.autoResetNotice)try{await o.sendReply?.(O.autoResetNotice)}catch(A){console.warn(`[SessionReset] notice send failed: ${A?.message}`)}if(v)try{let A=r.buildContext(v);if(A.length>20&&pt.shouldCompact(A.reduce((C,P)=>C+(typeof P.content=="string"?P.content.length/4:0),0),128e3)){let C=async(P,j)=>{try{let pe=j?pt.buildUpdateCompactionPrompt(P,j):pt.buildCompactionPrompt(P);return(await s.resolve(e.compactorProvider||"deepseek",e.compactorModel||"deepseek-chat").chat([{role:"system",content:pt.SUMMARIZATION_SYSTEM_PROMPT},{role:"user",content:pe}],void 0,50,.3))?.choices?.[0]?.message?.content?.trim()||"\uFF08\u81EA\u52A8\u538B\u5B9E\u6458\u8981\uFF09"}catch{return"\uFF08\u81EA\u52A8\u538B\u5B9E\u6458\u8981\uFF09"}};pt.compact(r,v,C).catch(()=>{})}}catch{}return z}catch(a){let c=a.message,d=/401|403|unauthorized|forbidden|auth/i.test(c)&&!/429|rate/i.test(c),u=/429|rate.?limit|too many/i.test(c),p=/ENOTFOUND|ECONNREFUSED|ECONNRESET|ETIMEDOUT|fetch failed|socket/i.test(c),m=/5\d{2}|api.*error|server.*error|internal|timeout/i.test(c)&&!d&&!u&&!p,g;d?g="gateway.error_auth":u?g="gateway.error_rate_limit":p?g="gateway.error_network":m?g="gateway.error_api":g="gateway.error_unknown";let f=S.t(g,g==="gateway.error_api"||g==="gateway.error_unknown"?{msg:c}:{});return console.error(S.t("gateway.msg_error",{msg:c})),f}}}static installCronDeliverer(e,t){se.instance.setDeliverer(async(r,s)=>{try{if(s==="all"){let n=t();if(!n)return;for(let i of e)if(i.name!=="api_server"){await i.sendText(n,r);break}}else{let n=s.indexOf(":");if(n<0)return;let i=s.slice(0,n),o=s.slice(n+1),a=e.find(c=>c.name===i);a&&await a.sendText(o,r)}}catch(n){console.error(`[CronScheduler] \u6295\u9012\u5931\u8D25 (${s}):`,n?.message)}})}static async startGatewayMode(e,t,r,s){let n=new fr,i=t.platform,o=Vi(),a=null;try{ue.existsSync(o)&&(a=ue.readFileSync(o,"utf-8").trim()||null)}catch{}let c=i.adapters?ol(n,i.adapters):[],d=c.find(g=>g.name==="qq");ve.notifyHandler=g=>{if(a){for(let f of c)if(f.name!=="api_server"){f.sendText(a,g).catch(()=>{});break}}try{r.appendSystemNotice(g)}catch{}},l.installCronDeliverer(c,()=>a);let u=c.find(g=>typeof g.sendTyping=="function"),p=l.createGatewayHandler({runtime:e,sessionStore:r,registry:s,compactorProvider:t.llm?.defaultProvider,compactorModel:t.llm?.defaultModel,signalProvider:g=>n.getSignalForMessage(g),sendTyping:u?g=>{u.sendTyping(g).catch(()=>{})}:void 0});n.setMessageHandler(p,{busyStateProvider:g=>e.getBusyState(g),steerProvider:(g,f)=>e.steer(f),redirectProvider:(g,f)=>e.redirect(f)}),console.error(S.t("gateway.starting"));let m=n.start();al(c,o),await Promise.race([m,new Promise(g=>{let f=async()=>{console.error(S.t("gateway.shutting_down")),await n.stop(),g()};process.on("SIGINT",f),process.on("SIGTERM",f)})])}static async startGatewayWorker(e,t,r,s){let n=new fr,i=t.platform,o=Vi(),a=i.adapters?ol(n,i.adapters):[];ve.notifyHandler=p=>{let m=(()=>{try{return ue.readFileSync(o,"utf-8").trim()||null}catch{return null}})();if(m){for(let g of a)if(g.name!=="api_server"){g.sendText(m,p).catch(()=>{});break}}},l.installCronDeliverer(a,()=>{try{return ue.readFileSync(o,"utf-8").trim()||null}catch{return null}});let c=a.find(p=>typeof p.sendTyping=="function"),d=l.createGatewayHandler({runtime:e,sessionStore:r,registry:s,compactorProvider:t.llm?.defaultProvider,compactorModel:t.llm?.defaultModel,sendTyping:c?p=>{c.sendTyping(p).catch(()=>{})}:void 0});n.setMessageHandler(d,{busyStateProvider:p=>e.getBusyState(p),steerProvider:(p,m)=>e.steer(m),redirectProvider:(p,m)=>e.redirect(m)});let u=n.start();al(a,o),await u}};import*as ul from"node:http";import*as U from"node:fs";import*as W from"node:path";import*as pl from"node:crypto";import{WebSocketServer as ep,WebSocket as dl}from"ws";var cl=`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
<!-- \u9996\u6B21\u8FD0\u884C\u8BBE\u7F6E\u9875\uFF08\u65E0 API Key \u65F6\u663E\u793A\uFF09 -->
<div class="login-page" id="setupPage" style="display:none">
  <div class="login-card">
    <div class="login-logo"><span class="dot"></span><span class="name">Kexvim</span></div>
    <div class="login-sub">\u9996\u6B21\u4F7F\u7528\uFF0C\u8BF7\u914D\u7F6E DeepSeek API Key</div>
    <input id="setupKey" type="password" placeholder="sk-..." autocomplete="off">
    <div class="login-err" id="setupErr"></div>
    <button class="login-btn" id="setupBtn">\u4FDD\u5B58\u5E76\u542F\u52A8</button>
    <div class="login-tip">Key \u4EC5\u4FDD\u5B58\u5728\u672C\u673A data/.env</div>
  </div>
</div>
<div id="app">
  <div class="main">
...
(function () {
  ...
  function checkAuth() {
    // \u9996\u6B21\u8FD0\u884C\uFF1A\u663E\u793A API Key \u8BBE\u7F6E\u9875\uFF08\u65E0 key \u65F6\u540E\u7AEF\u4EE5\u8BBE\u7F6E\u6A21\u5F0F\u542F\u52A8\uFF09
    fetch('/api/setup-status').then(function (r) { return r.json(); }).then(function (st) {
      if (st && st.firstRun) {
        ...
        if (setupPage) {
          ...
          var k = document.getElementById('setupKey');
          if (k) setTimeout(function () { k.focus(); }, 50);
        }
        return;
      }
  ...
  var setupKey = document.getElementById('setupKey');
  var setupErr = document.getElementById('setupErr');
  if (setupBtn && setupKey) {
    function saveSetup() {
      var key = setupKey.value.trim();
      if (!key) { setupErr.textContent = 'API Key \u4E0D\u80FD\u4E3A\u7A7A'; return; }
      ...
      fetch('/api/setup', {
        ...
        if (data && data.ok) {
          setupErr.style.color = '#5ad05a';
          setupErr.textContent = '\u5DF2\u4FDD\u5B58\uFF0C\u6B63\u5728\u91CD\u542F...';
          setTimeout(function () { location.reload(); }, 2500);
        } else {
          setupErr.style.color = '';
          setupErr.textContent = (data && data.error) || '\u4FDD\u5B58\u5931\u8D25';
          setupBtn.disabled = false;
        }
      }).catch(function () { setupErr.textContent = '\u7F51\u7EDC\u9519\u8BEF\uFF0C\u8BF7\u91CD\u8BD5'; setupBtn.disabled = false; });
    }
    setupBtn.addEventListener('click', saveSetup);
    setupKey.addEventListener('keydown', function (e) { if (e.key === 'Enter' && !setupBtn.disabled) saveSetup(); });
  }
...
platform:
  ...
  adapters:
    qq:
      app_id: ""
      client_secret: ""
`;function ml(l){let e=process.platform==="win32",t=te.join(l,e?"kexvim.cmd":"kexvim");F.existsSync(t)||(e?F.writeFileSync(t,`@echo off\r
if exist "%~dp0dist\\dev.mjs" (\r
  node "%~dp0dist\\dev.mjs" %*\r
) else (\r
  node "%~dp0dist\\kexvim.mjs" %*\r
)\r
`):(F.writeFileSync(t,`#!/bin/sh
exec node "$(dirname "$0")/kexvim.js" "$@"
`),F.chmodSync(t,493)));let r=e?null:[te.join(Dt.homedir(),".bashrc"),te.join(Dt.homedir(),".zshrc"),te.join(Dt.homedir(),".profile")].find(s=>F.existsSync(s));if(r)F.readFileSync(r,"utf-8").includes(l)?console.log("[~] PATH \u5DF2\u914D\u7F6E"):(F.appendFileSync(r,`
export PATH="$PATH:${l}"
`),console.log(`[~] \u5DF2\u5C06 ${l} \u52A0\u5165 ${te.basename(r)}\uFF0C\u91CD\u542F\u7EC8\u7AEF\u540E\u751F\u6548`));else if(e)try{let n=`$h = ${"'"+l.replace(/'/g,"''")+"'"}; $p = [Environment]::GetEnvironmentVariable('PATH','User'); if ($p -and $p.Contains($h)) { Write-Output 'exists' } elseif ($p) { [Environment]::SetEnvironmentVariable('PATH', $p + ';' + $h, 'User'); Write-Output 'added' } else { [Environment]::SetEnvironmentVariable('PATH', $h, 'User'); Write-Output 'added' }`,i=b.runFileSync("powershell",["-NoProfile","-Command",n],{maxBuffer:1024*1024});console.log(i.includes("exists")?"[~] PATH \u5DF2\u914D\u7F6E":"[~] \u5DF2\u5C06 Kexvim \u52A0\u5165 PATH\uFF0C\u65B0\u7EC8\u7AEF\u751F\u6548")}catch{console.log("[~] \u672A\u80FD\u81EA\u52A8\u8BBE\u7F6E PATH\uFF0C\u8BF7\u624B\u52A8\u5C06\u542F\u52A8\u5668\u76EE\u5F55\u52A0\u5165\u73AF\u5883\u53D8\u91CF PATH")}else console.log(`[~] \u624B\u52A8\u5C06\u4EE5\u4E0B\u884C\u52A0\u5165 shell \u914D\u7F6E\u6587\u4EF6 (~/.bashrc / ~/.zshrc):
    export PATH="$PATH:${l}"`)}async function hl(l,e){let t=i=>{try{return F.accessSync(i),!0}catch{return!1}};if(t(te.join(l,"data",".env"))){console.log("Kexvim \u5DF2\u521D\u59CB\u5316(data/.env \u5DF2\u5B58\u5728)\u3002"),ml(l);return}if(t(te.join(l,"kexvim.js")))console.log("[~] kexvim.js \u5DF2\u5B58\u5728\uFF0C\u8DF3\u8FC7\u4E0B\u8F7D");else if(F.mkdirSync(l,{recursive:!0}),gl("git")){console.log("[~] \u514B\u9686\u4ED3\u5E93...");try{b.runFileSync("git",["clone","--depth","1",e,l],{timeoutMs:12e4})}catch{console.error("[\u2717] \u514B\u9686\u5931\u8D25"),process.exit(1)}}else{console.log("[~] \u4E0B\u8F7D kexvim.js...");let i=te.join(Dt.tmpdir(),"kexvim-init.js"),o=`${e}/raw/main/kexvim.js`;try{gl("wget")?b.runFileSync("wget",["-q","-O",i,o],{timeoutMs:6e4}):b.runFileSync("curl",["-fsSL","-o",i,o],{timeoutMs:6e4})}catch{console.error("[\u2717] \u4E0B\u8F7D\u5931\u8D25"),process.exit(1)}t(i)&&(F.copyFileSync(i,te.join(l,"kexvim.js")),F.rmSync(i,{force:!0}))}let r=te.join(l,"data",".env");if(F.mkdirSync(te.join(l,"data"),{recursive:!0}),!t(r)){let i=fl.createInterface({input:process.stdin,output:process.stdout}),o=await new Promise(a=>{i.question("\u8BF7\u8F93\u5165 DeepSeek API Key: ",a)});i.close(),o&&(F.writeFileSync(r,`DEEPSEEK_API_KEY=${o}
KEXVIM_HOME=${l}
`),console.log("[\u2713] API Key \u5DF2\u4FDD\u5B58"))}let s=te.join(l,"data","config.yaml");t(s)||(F.writeFileSync(s,Xi),console.log("[\u2713] config.yaml \u5DF2\u521B\u5EFA\uFF08\u8BF7\u6309\u9700\u8865\u5145\u914D\u7F6E\uFF09"));let n=te.join(l,"skills");if(F.existsSync(n))console.log("[~] skills \u5DF2\u5B58\u5728\uFF0C\u8DF3\u8FC7");else{console.log("[~] \u4E0B\u8F7D\u516C\u5171\u6280\u80FD...");let i=F.mkdtempSync(te.join(Dt.tmpdir(),"kexvim-skills-"));try{b.runSync(`git clone --depth 1 ${e}.git "${i}" --single-branch 2>/dev/null`,{timeoutMs:6e4}),F.existsSync(te.join(i,"skills"))?(F.cpSync(te.join(i,"skills"),n,{recursive:!0}),console.log("[\u2713] \u6280\u80FD\u5DF2\u4E0B\u8F7D")):console.warn("[~] \u8FDC\u7A0B\u4ED3\u5E93\u4E2D\u6CA1\u6709 skills \u76EE\u5F55\uFF0C\u8DF3\u8FC7")}catch{console.warn("[~] \u6280\u80FD\u4E0B\u8F7D\u5931\u8D25\uFF08git \u672A\u5B89\u88C5\u6216\u65E0\u7F51\u7EDC\uFF09\uFF0C\u53EF\u624B\u52A8 clone \u540E\u590D\u5236 skills/ \u76EE\u5F55")}finally{F.rmSync(i,{recursive:!0,force:!0})}}ml(l),console.log("[\u2713] \u5B89\u88C5\u5B8C\u6210\u3002\u73B0\u5728\u53EF\u4EE5\u76F4\u63A5\u5728\u7EC8\u7AEF\u8F93\u5165: kexvim restart")}function gl(l){try{return b.runFileSync(process.platform==="win32"?"where":"which",[l]),!0}catch{return!1}}import{createInterface as tp}from"node:readline";var Vn=class l{static configureRegistry(e,t){let{llm:r}=t;for(let[s,n]of Object.entries(r.providers)){n.baseUrl&&e.setBaseUrl(s,n.baseUrl);let i={};n.promptCaching&&(i.promptCaching=n.promptCaching),Object.keys(i).length>0&&e.setProviderOptions(s,i)}}static async main(){if(await gr.handleWorkerDispatch())return;let r=I.load();S.init(r.language||"zh-CN"),I.validate(r)||console.error(S.t("config.using_default"));let s=new Ut;l.configureRegistry(s,r);let n=process.argv.slice(2).filter(C=>!C.startsWith("--")),i=s.list(),o,a,c,d=r.paths.userDataDir;d||(console.error("\u2717 \u672A\u914D\u7F6E userDataDir\uFF08data \u76EE\u5F55\uFF09\u3002\u8BF7\u5728\u9879\u76EE\u6839\u8FD0\u884C\uFF0C\u6216\u8BBE\u7F6E KEXVIM_USER_DATA_DIR\u3002"),process.exit(1));let u=n[0]?.toLowerCase();if(!u){let C=De.join(d,".env");if(!mt.existsSync(C)){console.log("[~] \u9996\u6B21\u8FD0\u884C\uFF0C\u542F\u52A8 Web \u8BBE\u7F6E\u9875..."),mt.mkdirSync(d,{recursive:!0}),mt.mkdirSync(De.join(d,"skills"),{recursive:!0});let P=De.join(d,"config.yaml");mt.existsSync(P)||mt.writeFileSync(P,Xi,"utf-8"),process.env.KEXVIM_FIRST_RUN="1",u="web"}}n.length>0&&i.includes(n[0].toLowerCase())?(o=n[0],a=n[1]||r.llm.defaultModel,c=n.slice(2).join(" ")):n.length>0&&i.includes(n[1]?.toLowerCase())?(o=n[1],a=n[0],c=n.slice(2).join(" ")):(o=r.llm.defaultProvider,a=r.llm.defaultModel,c=n.join(" ")),!process.env.KEXVIM_FIRST_RUN&&!s.has(o)&&(console.error(S.t("config.provider_unknown",{provider:o,known:s.list().join(", ")})),console.error(S.t("config.provider_help")),process.exit(1));let p;if(process.env.KEXVIM_FIRST_RUN)p=new ss,console.error("[llm] \u672A\u914D\u7F6E API Key\uFF0C\u8FDB\u5165\u9996\u6B21\u8FD0\u884C\u8BBE\u7F6E\u6A21\u5F0F");else try{p=s.resolve(o,a),console.error(S.t("main.llm_using",{provider:o,model:a}))}catch(C){console.error(S.t("main.llm_resolve_failed",{msg:C instanceof Error?C.message:String(C)})),process.exit(1)}let m;r.fallback?.enabled!==!1&&r.fallback?.providers&&r.fallback.providers.length>0&&(m={providers:r.fallback.providers.map(C=>({name:C.name,createAdapter:()=>s.resolve(C.name,C.model)}))}),r.paths.skillsDir=De.join(d,"skills");let g=I.findProjectRoot();g||(console.error("\u2717 \u65E0\u6CD5\u5B9A\u4F4D\u9879\u76EE\u6839\u76EE\u5F55\uFF08package.json\uFF09\uFF0C\u65E0\u6CD5\u89E3\u6790\u516C\u5171\u6280\u80FD\u76EE\u5F55\u3002\u8BF7\u5728\u9879\u76EE\u6839\u8FD0\u884C\u3002"),process.exit(1));let f=De.join(g,"skills"),h;for(let[C,P]of Object.entries(r.llm.providers))if(P?.vision)try{h=s.resolve(C,P.model||C),console.error(`[llm] vision provider: ${C}\uFF08\u56FE\u7247\u5185\u5BB9\u81EA\u52A8\u8DEF\u7531\uFF09`);break}catch{console.error(`[llm] vision provider ${C} \u89E3\u6790\u5931\u8D25\uFF0C\u56FE\u7247\u8DEF\u7531\u4E0D\u53EF\u7528`)}let k=De.join(g,"data","skills-market"),y,v=new Pn({llm:p,visionLlm:h,skillsDir:r.paths.skillsDir,sharedSkillsDir:f,marketSkillsDir:k,systemPrompt:r.agent.systemPrompt,maxIterations:r.agent.maxIterations,contextWindow:r.agent.contextWindow,fallback:m,createReviewLLM:()=>s.resolve(o,a),skillNudgeInterval:r.agent.skillNudgeInterval,memoryNudgeInterval:r.agent.memoryNudgeInterval,backgroundReview:r.agent.backgroundReview,sessionReset:r.sessionReset,onTaskSummaryDone:C=>{y?.pushTaskTree(C)}});if(r.mcpServers&&r.mcpServers.length>0){let C=new Or;C.registerAll(r.mcpServers);for(let j of r.mcpServers)try{let pe=await ze.createTransport(j,j.name);await C.connect(j.name,pe)}catch(pe){console.error(S.t("mcp.connect_failed",{name:j.name,msg:String(pe)}));continue}let P=C.discoverAndBridge();for(let j of P)v.addTool(j);console.error(S.t("mcp.ready",{count:P.length}))}let w=De.join(d,"kexvim.db"),R=Fn.createAll({dbPath:w,profile:"default",workerThreads:!1}),_=new wr;v.setSessionStore(R.sessions,r.compression),v.setMemoryManager(_);let M=new Bn(R.memory);_.addProvider(M),R.sessions&&M.setSessionStore(R.sessions);let L=new Un(d);L.loadFromDisk(),v.setFileMemoryStore(L),R.state&&v.setStateManager(R.state),v.initSessionMemory(d),ne.getInstance().initStore(d),R.worker?(process.on("beforeExit",()=>{v.destroy(),R.worker.shutdown()}),process.on("exit",()=>{v.destroy(),R.worker.shutdown()})):(process.on("beforeExit",()=>v.destroy()),process.on("exit",()=>v.destroy()));let he=R.sessions,O=new Tt(xt.load());if(console.error(S.t("gateway.guardian_ready")),se.instance.setAgentExecutor(async C=>{try{return(await v.chat(C,{source:"cron",chatId:"cron",chatType:"dm",skipSessionReset:!0})).content||""}catch(P){return`Error: ${P instanceof Error?P.message:String(P)}`}}),se.instance.setActivityProvider(()=>v.getLastActivityAt()),!yl.isMainThread||!r.platform?.enabled?se.instance.start():se.schedulingDisabled=!0,u==="web"){let C=Number(process.env.KEXVIM_WEB_PORT)||8787,P=new Xt(De.join(g,"data","skills"),De.join(g,"skills"),k),j={listCatalog:()=>P.listCatalog(),parseFrontmatter:Q=>P.parseFrontmatter(Q),installMarket:(Q,me)=>P.installMarket(Q,me),uninstallMarket:Q=>P.uninstallMarket(Q),toggleMarket:(Q,me)=>P.toggleMarket(Q,me),getProgress:Q=>H.getProgress(Q),getCurrentTool:()=>H.getCurrentTool(),...p?{summarizeBranch:async(Q,me,gt)=>{let io=((await p.chat({systemPrompt:"\u4F60\u662F kexvim \u7684\u5206\u652F\u6458\u8981\u52A9\u624B\u3002\u7528\u6237\u4ECE\u5BF9\u8BDD\u4E2D\u95F4\u5206\u53C9\u4E86\uFF0C\u4E0B\u9762\u662F\u5206\u53C9\u70B9\u4E4B\u540E\u88AB\u4E22\u5F03\u7684\u5BF9\u8BDD\u5185\u5BB9\u3002\u7528\u4E0D\u8D85\u8FC7 80 \u5B57\u603B\u7ED3\u8FD9\u6BB5\u5BF9\u8BDD\u505A\u4E86\u4EC0\u4E48\u3001\u7ED3\u8BBA\u662F\u4EC0\u4E48\uFF08\u4E2D\u6587\uFF09\u3002\u53EA\u8F93\u51FA\u6458\u8981\u672C\u8EAB\u3002",messages:[{role:"user",content:gt}]})).content||"").trim();io&&he.appendBranchSummary(Q,me,io)}}:{}},pe=new Yn(v,he,{port:C,projectRoot:g,marketSkillsDir:k,webDelegate:j,authServer:r.web?.authServer,sessionHours:r.web?.sessionHours});y=pe,ve.notifyHandler=Q=>{try{pe.pushSystemNotice(Q)}catch{}},await pe.start(),await new Promise(()=>{})}else if(r.platform?.enabled&&!gr.workersLaunched)await Jn.startGatewayWorker(v,r,he,s);else{console.log(""),console.log("\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557"),console.log("\u2551   Kexvim \u2014 \u7EC8\u7AEF\u4EA4\u4E92\u6A21\u5F0F              \u2551"),console.log("\u2551   Terminal Interactive Mode           \u2551"),console.log("\u2551                                      \u2551"),console.log("\u2551   \u8F93\u5165\u6D88\u606F\u5BF9\u8BDD\uFF0CCtrl+C \u6216 /bye \u9000\u51FA   \u2551"),console.log("\u255A\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255D"),console.log("");let C=tp({input:process.stdin,output:process.stdout,prompt:"\u{1F9E0} \u4F60 > "});C.prompt();for await(let P of C){let j=P.trim();if(!j||j==="/bye"||j==="/exit"||j==="/quit")break;try{let Q=(await v.chat(j,{source:"terminal",chatType:"c2c",userId:"terminal-user"})).content?.trim();Q&&console.log(`
\u{1F916} Kexvim > ${Q}
`)}catch(pe){console.error(`
[\u2717] ${pe.message}
`)}C.prompt()}C.close(),gr.terminateWorkers(),console.log(`
\u{1F44B} \u518D\u89C1 / Bye`)}}};import*as Bt from"node:path";import*as yr from"node:fs";import*as Xr from"node:path";import*as vl from"node:os";function rp(l){try{return b.runFileSync(process.platform==="win32"?"where":"which",[l]),!0}catch{return!1}}async function bl(l,e){let t=r=>{try{return yr.accessSync(r),!0}catch{return!1}};if(t(Xr.join(l,"kexvim.js"))||(console.error("[\u2717] \u672A\u5B89\u88C5 Kexvim\uFF0C\u5148\u8FD0\u884C node kexvim.js init"),process.exit(1)),t(Xr.join(l,".git"))){console.log("[~] git pull...");try{b.runFileSync("git",["pull","--ff-only"],{cwd:l,timeoutMs:12e4})}catch{console.error("[\u2717] \u62C9\u53D6\u5931\u8D25"),process.exit(1)}}else{console.log("[~] \u4E0B\u8F7D\u6700\u65B0 kexvim.js...");let r=Xr.join(vl.tmpdir(),"kexvim-update.js");try{let s=`${e}/raw/main/kexvim.js`;rp("wget")?b.runFileSync("wget",["-q","-O",r,s],{timeoutMs:6e4}):b.runFileSync("curl",["-fsSL","-o",r,s],{timeoutMs:6e4})}catch{console.error("[\u2717] \u4E0B\u8F7D\u5931\u8D25"),process.exit(1)}t(r)&&(yr.copyFileSync(r,Xr.join(l,"kexvim.js")),yr.rmSync(r,{force:!0}))}console.log("[\u2713] \u66F4\u65B0\u5B8C\u6210\uFF0C\u6B63\u5728\u91CD\u542F...")}import*as G from"fs";import*as Ft from"path";var kl=Mt();function sp(l,e){if(!G.existsSync(e))return!0;let t=G.statSync(e).mtimeMs,r=t,s=n=>{let i;try{i=G.readdirSync(n,{withFileTypes:!0})}catch{return}for(let o of i){let a=Ft.join(n,o.name);if(o.isDirectory()){if(o.name==="node_modules"||o.name==="dist"||o.name==="data"||o.name===".git")continue;s(a)}else if(o.name.endsWith(".ts"))try{G.statSync(a).mtimeMs>r&&(r=G.statSync(a).mtimeMs)}catch{}}};return s(Ft.join(l,"src")),s(Ft.join(l,"packages")),r>t}function Qi(){if(process.platform!=="win32")return[];let l=b.runSyncResult(`wmic process where "name='node.exe'" get ProcessId,CommandLine /format:list`,{timeoutMs:5e3});if(l.code!==0)return[];let e=l.stdout.replace(/\u0000/g,""),t=[],r=null;for(let s of e.split(/\r?\n/)){let n=s.trim();if(n.startsWith("CommandLine="))r=n.slice(12).replace(/^"|"$/g,"");else if(n.startsWith("ProcessId=")){let i=Number(n.slice(10));if(r&&i>0&&i!==process.pid&&!/\sweb(\s|$)/.test(r)){let o=r.match(/(\S*(?:dev\.mjs|kexvim\.js|Main\.ts))/);o&&t.push({pid:i,entry:o[1]})}r=null}}return t}function np(l,e=64*1024){try{let t=G.statSync(l);if(!t.isFile()||t.size===0)return"";if(t.size<=e)return G.readFileSync(l,"utf-8");let r=G.openSync(l,"r");try{let s=Buffer.alloc(e),n=G.readSync(r,s,0,e,t.size-e);return s.toString("utf-8",0,n)}finally{G.closeSync(r)}}catch{return""}}function ip(l,e){return new Promise(t=>{let r=0,s=setInterval(()=>{r++;try{if(G.existsSync(l)){let n=np(l);if(n.includes("Guardian agent \u5DF2\u5C31\u7EEA")||n.includes("kexvim \u5DF2\u5C31\u7EEA")){clearInterval(s),console.log(`[kexvim] \u2705 kexvim \u5DF2\u5C31\u7EEA\uFF08${r}s\uFF09`),t(!0);return}}}catch{}r>=e&&(clearInterval(s),t(!1))},1e3)})}async function Sl(l){let e=l,t=Ft.join(e,"data","log");try{G.mkdirSync(t,{recursive:!0})}catch{}let r=Ft.join(t,"kexvim.log"),s=Ft.join(e,"dist","dev.mjs");if(sp(e,s)){console.log("[kexvim] \u68C0\u6D4B\u5230\u6E90\u7801\u53D8\u66F4\uFF0C\u81EA\u52A8\u6784\u5EFA...");let d=b.runSyncResult("npm run build:dev",{cwd:e,timeoutMs:12e4});d.code!==0&&(console.error(`[kexvim] \u274C \u6784\u5EFA\u5931\u8D25\uFF0C\u4E2D\u6B62\u91CD\u542F\uFF1A${d.stderr?.trim()||d.stdout?.trim()}`),process.exit(1)),console.log("[kexvim] \u2705 \u6784\u5EFA\u5B8C\u6210")}if(process.platform==="linux")try{let d=b.runSyncResult("systemctl --user is-active kexvim.service",{timeoutMs:5e3});if(d.code===0&&d.stdout.trim()==="active"){console.log("[kexvim] systemd \u6258\u7BA1\uFF08kexvim.service active\uFF09\uFF0C\u8D70 systemctl --user restart");let u=b.runSyncResult("systemctl --user restart kexvim.service",{timeoutMs:3e4});u.code!==0&&(console.error(`[kexvim] \u274C systemctl restart \u5931\u8D25: ${u.stderr?.trim()||u.stdout?.trim()}`),process.exit(1)),console.log("[kexvim] \u2705 systemctl restart \u5B8C\u6210");return}}catch{}let n=Qi(),i=n.length>0?n.map(d=>d.pid):kl.findKexvimPids(),o=n.length>0?n[0].entry:ut.resolveEntry(e);if(console.log(`[kexvim] \u65E7\u8FDB\u7A0B: ${i.length} \u4E2A; \u5165\u53E3: ${o}`),G.writeFileSync(r,"","utf-8"),i.length>0){console.log("[kexvim] \u6E05\u7406\u65E7\u8FDB\u7A0B...");for(let d of i)process.platform==="win32"?b.runSyncResult(`taskkill /PID ${d} /F`,{timeoutMs:5e3}):kl.killProcess(d,!0);console.log("[kexvim] \u2705 \u65E7\u8FDB\u7A0B\u5DF2\u6E05\u7406")}console.log("[kexvim] \u6B63\u5728\u542F\u52A8\u65B0 kexvim...");let a=ut.spawnFreshDaemon(e,o);a===null&&(console.error("[kexvim] \u274C \u65E0\u6CD5\u542F\u52A8\u65B0\u8FDB\u7A0B"),process.exit(1)),console.log(`[kexvim] \u65B0 PID: ${a}\uFF0C\u7B49\u5F85\u5C31\u7EEA...`),await ip(r,30)||console.warn("[kexvim] \u26A0\uFE0F \u7B49\u5F85\u5C31\u7EEA\u8D85\u65F6\uFF0830s\uFF09\uFF0C\u8BF7\u68C0\u67E5 data/log/kexvim.log");try{dt.restart(e),console.log("[kexvim] \u2705 web \u8FDB\u7A0B\u5DF2\u5C31\u7EEA\uFF08\u5DF2\u786E\u4FDD\u62C9\u8D77\uFF09")}catch(d){console.error(`[kexvim] \u26A0\uFE0F web \u91CD\u542F\u5931\u8D25: ${d instanceof Error?d.message:String(d)}`)}console.log("[kexvim] \u65E5\u5FD7:",r),process.exit(0)}import*as _l from"fs";import*as wl from"path";async function xl(l){let e=Qi();for(let r of e)r.pid!==process.pid&&(process.platform==="win32"?b.runSyncResult(`taskkill /PID ${r.pid} /F /T`,{timeoutMs:5e3}):Mt().killProcess(r.pid,!0),console.log(`[kexvim] \u5DF2\u505C\u6B62 PID ${r.pid} (${r.entry})`));let t=!1;try{let r=dt.findPid();r!==null&&(process.platform==="win32"?b.runSyncResult(`taskkill /PID ${r} /F /T`,{timeoutMs:5e3}):Mt().killProcess(r,!0),console.log(`[kexvim] \u5DF2\u505C\u6B62 web \u8FDB\u7A0B PID ${r}`),t=!0)}catch{}try{_l.rmSync(wl.join(l,"data","kexvim.pid"),{force:!0})}catch{}e.length===0&&!t?console.log("[kexvim] \u6CA1\u6709\u8FD0\u884C\u4E2D\u7684 kexvim \u8FDB\u7A0B"):console.log("[kexvim] \u2705 kexvim \u5DF2\u505C\u6B62")}import*as K from"node:fs";import*as Qr from"node:os";import*as X from"node:path";import{fileURLToPath as op}from"node:url";var fe=class l{static SERVICE_NAME="kexvim";static AUTOSTART_DIR="autostart";static KEEPALIVE_INTERVAL_MIN=1;static SYSTEMD_TEMPLATE=(e,t,r)=>`[Unit]
...
<dict>
  ...
  <true/>
  <key>StandardOutPath</key>
  <string>${X.join(t,"data","log","kexvim.log")}</string>
  <key>StandardErrorPath</key>
  <string>${X.join(t,"data","log","kexvim.log")}</string>
</dict>
</plist>
`;static platform=process.platform;static _findRoot(){let e=X.dirname(op(import.meta.url));for(let t=0;t<8;t++){if(K.existsSync(X.join(e,"package.json")))return e;let r=X.dirname(e);if(r===e)break;e=r}throw new Error("[kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF08\u65E0 package.json\uFF09\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002")}static _systemPath(e){if(l.platform==="win32")return"";let t=Qr.homedir();return l.platform==="darwin"?X.join(t,"Library","LaunchAgents","com.kexvim.daemon.plist"):X.join(t,".config","systemd","user","kexvim.service")}static _dataPath(e){let t=X.join(e,"data",l.AUTOSTART_DIR);return K.mkdirSync(t,{recursive:!0}),l.platform==="win32"?X.join(t,"keepalive.cmd"):l.platform==="darwin"?X.join(t,"com.kexvim.daemon.plist"):X.join(t,"kexvim.service")}static _devEntry(e){let t=X.join(e,"dist","dev.mjs");if(K.existsSync(t))return t;let r=X.join(e,"kexvim.js");if(K.existsSync(r))return r;throw new Error("[kexvim] \u672A\u627E\u5230 daemon \u5165\u53E3\uFF08dist/dev.mjs \u6216 kexvim.js\uFF09\u3002")}static _template(e){let t=process.execPath,r=l._devEntry(e);return l.platform==="win32"?`@echo off\r
${t} "${X.join(e,"dist","dev.mjs")}" status >nul 2>&1 || ${t} "${X.join(e,"dist","dev.mjs")}" --daemon\r
`:l.platform==="darwin"?l.LAUNCHD_TEMPLATE(t,e,r):l.SYSTEMD_TEMPLATE(t,e,r)}static writeTemplate(e){let t=e??l._findRoot(),r=l._dataPath(t);return K.writeFileSync(r,l._template(t)),r}static install(e){let t=e??l._findRoot(),r=l.writeTemplate(t),s=l._systemPath(t),n=[`[\u2713] \u6A21\u677F\u5DF2\u751F\u6210: ${r}`];if(l.platform==="win32")l._winInstall(t,n);else if(l.platform==="darwin"){K.mkdirSync(X.dirname(s),{recursive:!0}),K.copyFileSync(r,s),n.push(`[\u2713] \u5DF2\u5199\u5165: ${s}`);try{b.runFileSync("launchctl",["load","-w",s]),n.push("[\u2713] launchctl load \u5B8C\u6210")}catch{n.push("[\u2717] launchctl load \u5931\u8D25\uFF08\u9700\u5F53\u524D\u7528\u6237\u6709 GUI \u4F1A\u8BDD\uFF09")}}else{K.mkdirSync(X.dirname(s),{recursive:!0}),K.copyFileSync(r,s),n.push(`[\u2713] \u5DF2\u5199\u5165: ${s}`);try{b.runFileSync("systemctl",["--user","daemon-reload"]),b.runFileSync("systemctl",["--user","enable","kexvim.service"]),n.push("[\u2713] systemd user \u5355\u5143\u5DF2 enable")}catch(i){n.push(`[\u2717] systemctl \u6CE8\u518C\u5931\u8D25: ${i instanceof Error?i.message:String(i)}`)}try{b.runFileSync("loginctl",["enable-linger",Qr.userInfo().username]),n.push("[\u2713] linger \u5DF2\u542F\u7528\uFF08\u5F00\u673A\u81EA\u542F\uFF09")}catch{n.push("[~] linger \u542F\u7528\u5931\u8D25\uFF08\u90E8\u5206\u7CFB\u7EDF\u9700 root\uFF09\uFF0C\u81EA\u542F\u53EF\u80FD\u53EA\u5728\u767B\u5F55\u540E\u751F\u6548")}}return n.join(`
`)}static uninstall(e){let t=e??l._findRoot(),r=l._systemPath(t),s=[];if(l.platform==="win32")for(let n of["KexvimDaemon","KexvimKeepAlive"])try{b.runFileSync("schtasks",["/Delete","/TN",n,"/F"]),s.push(`[\u2713] \u5DF2\u5220\u9664\u8BA1\u5212\u4EFB\u52A1 ${n}`)}catch{s.push(`[~] \u8BA1\u5212\u4EFB\u52A1 ${n} \u4E0D\u5B58\u5728\u6216\u5220\u9664\u5931\u8D25`)}else if(l.platform==="darwin"){try{b.runFileSync("launchctl",["unload",r])}catch{}K.existsSync(r)?(K.rmSync(r,{force:!0}),s.push(`[\u2713] \u5DF2\u5220\u9664: ${r}`)):s.push("[~] \u672A\u5B89\u88C5")}else{try{b.runFileSync("systemctl",["--user","disable","kexvim.service"]),s.push("[\u2713] systemd \u5355\u5143\u5DF2 disable")}catch{}K.existsSync(r)?(K.rmSync(r,{force:!0}),s.push(`[\u2713] \u5DF2\u5220\u9664: ${r}`)):s.push("[~] \u672A\u5B89\u88C5")}return s.join(`
`)}static status(e){let t=e??l._findRoot(),r=l._systemPath(t),s=l._dataPath(t),n=!1,i=!1,o=!1;if(l.platform==="win32")try{b.runFileSync("schtasks",["/Query","/TN","KexvimDaemon"]),n=!0}catch{}else if(n=K.existsSync(r),n)try{let a=l._template(t);i=K.readFileSync(r,"utf-8")===a}catch{i=!1}if(l.platform==="darwin")try{o=b.runFileSync("launchctl",["print",`gui/${process.getuid?.()??Qr.userInfo().uid}/com.kexvim.daemon`]).includes("state = running")}catch{o=!1}else if(l.platform==="win32")o=l.isProcessRunning(t);else try{o=b.runFileSync("systemctl",["--user","is-active","kexvim.service"]).trim()==="active"}catch{o=!1}return{platform:l.platform,installed:n,configPath:r,dataPath:s,consistent:i,serviceActive:o}}static _winInstall(e,t){let r=process.execPath,s=l._devEntry(e),n=`cmd /c "${r}" "${s}" --daemon`,i=`cmd /c "${r}" "${s}" status >nul 2>&1 || "${r}" "${s}" --daemon`;try{b.runFileSync("schtasks",["/Create","/TN","KexvimDaemon","/SC","ONLOGON","/TR",n,"/F"]),t.push("[\u2713] \u8BA1\u5212\u4EFB\u52A1 KexvimDaemon\uFF08\u767B\u5F55\u81EA\u542F\uFF09\u5DF2\u521B\u5EFA")}catch(o){t.push(`[\u2717] \u521B\u5EFA KexvimDaemon \u5931\u8D25: ${o instanceof Error?o.message:String(o)}`)}try{b.runFileSync("schtasks",["/Create","/TN","KexvimKeepAlive","/SC","MINUTE","/MO",String(l.KEEPALIVE_INTERVAL_MIN),"/TR",i,"/F"]),t.push(`[\u2713] \u8BA1\u5212\u4EFB\u52A1 KexvimKeepAlive\uFF08\u6BCF ${l.KEEPALIVE_INTERVAL_MIN} \u5206\u949F\u4FDD\u6D3B\uFF09\u5DF2\u521B\u5EFA`)}catch(o){t.push(`[\u2717] \u521B\u5EFA KexvimKeepAlive \u5931\u8D25: ${o instanceof Error?o.message:String(o)}`)}}static isProcessRunning(e){let t=e??l._findRoot(),r=X.join(t,"data","kexvim.pid");try{let s=Number.parseInt(K.readFileSync(r,"utf-8").trim(),10);return!Number.isFinite(s)||s<=0?!1:(process.kill(s,0),!0)}catch{return!1}}static resolveRoot(){return l._findRoot()}};function Tl(){let l=fe.resolveRoot();console.log(fe.install(l));let e=fe.status(l);console.log(`[~] \u72B6\u6001: platform=${e.platform} installed=${e.installed} consistent=${e.consistent} active=${e.serviceActive}`),process.exit(0)}function El(){console.log(fe.uninstall(fe.resolveRoot())),process.exit(0)}function Cl(){let l=fe.resolveRoot(),e=fe.status(l),t=fe.isProcessRunning(l);console.log(`kexvim \u5E73\u53F0:        ${e.platform}`),console.log(`\u8FDB\u7A0B:               ${t?"\u8FD0\u884C\u4E2D":"\u672A\u8FD0\u884C"} (PID \u6587\u4EF6: data/kexvim.pid)`),console.log(`\u81EA\u542F\u914D\u7F6E:           ${e.installed?"\u5DF2\u5B89\u88C5":"\u672A\u5B89\u88C5"} @ ${e.configPath||"(Windows \u8BA1\u5212\u4EFB\u52A1)"}`),console.log(`\u914D\u7F6E\u4E00\u81F4\u6027(\u7EDF\u4E00):    ${e.consistent?"\u4E00\u81F4(\u6A21\u677F)":"\u4E0D\u4E00\u81F4(\u5C06\u88AB install \u8986\u76D6)"}`),console.log(`\u670D\u52A1\u72B6\u6001:           ${e.serviceActive?"active":"inactive"}`),console.log(`\u91CD\u542F\u5FAA\u73AF\u9632\u62A4:       ${ke.isTripped(l)?"\u26A0\uFE0F TRIPPED\uFF08\u7528 kexvim clear-loop \u89E3\u9664\uFF09":`\u6B63\u5E38 (${ke.bootCount(l)}/${ke.MAX_RESTARTS} in ${ke.WINDOW_SECONDS}s)`}`),process.exit(t?0:1)}function Rl(){ke.clear(fe.resolveRoot()),console.log("[\u2713] restart_loop \u72B6\u6001\u5DF2\u6E05\u9664"),process.exit(0)}import*as Zr from"node:fs";import*as Zi from"node:path";import{DatabaseSync as ap}from"node:sqlite";function Ml(l){let e=Zi.join(l,"kexvim.db");Zr.existsSync(e)||(console.error("[kexvim] \u274C \u627E\u4E0D\u5230\u4F1A\u8BDD\u5E93:",e),process.exit(1));let t=new ap(e,{readOnly:!0});try{let r=t.prepare(`
      ...
      WHERE s.is_test = 0
      ORDER BY s.updated_at DESC
    `).all();if(r.length===0){console.log("\u6682\u65E0\u5386\u53F2\u4F1A\u8BDD\u3002");return}let s="";try{let n=Zi.join(l,".current_session");Zr.existsSync(n)&&(s=Zr.readFileSync(n,"utf-8").trim())}catch{}console.log(`\u5171 ${r.length} \u4E2A\u4F1A\u8BDD\uFF1A
`),r.forEach((n,i)=>{let o=et.displayTitle(n.summary,n.firstUserMsg),a=n.id===s?"  \u25C0 \u5F53\u524D":"";console.log(`${String(i+1).padStart(2)}. ${n.id.slice(0,8)} - ${o}  [${n.source}] ${String(n.msgCount).padStart(4)}\u6761${a}`)}),console.log(`
\u7528\u6CD5: kexvim session <\u524D8\u4F4DID>  \u5207\u6362/\u6062\u590D\u8BE5\u4F1A\u8BDD`)}finally{t.close()}}import*as Al from"node:fs";import*as eo from"node:path";import{DatabaseSync as lp}from"node:sqlite";var cp="session-switch.json";function Il(l,e){let t=eo.join(l,"kexvim.db"),r=new lp(t,{readOnly:!0}),s;try{s=r.prepare(`
      SELECT id, COALESCE(summary, '') AS summary,
             (SELECT COUNT(*) FROM messages m WHERE m.session_id = s.id) AS msgCount
      FROM sessions s
      WHERE s.is_test = 0 AND s.id LIKE ?
    `).get(`${e}%`)}finally{r.close()}s||(console.error(`[kexvim] \u274C \u627E\u4E0D\u5230\u4F1A\u8BDD "${e}"\u3002\u7528 kexvim sessions \u67E5\u770B\u5168\u90E8\u4F1A\u8BDD\u3002`),process.exit(1)),Al.writeFileSync(eo.join(l,cp),JSON.stringify({sessionId:s.id,requestedAt:Date.now()/1e3}),"utf-8");let n=(s.summary||"(\u65E0\u6807\u9898)").slice(0,30);console.log(`\u2705 \u5DF2\u6807\u8BB0\u5207\u6362\u5230\u4F1A\u8BDD ${s.id.slice(0,8)}\uFF08${s.msgCount} \u6761\u6D88\u606F\uFF0C${n}\uFF09`),console.log("   \u53D1\u9001\u4E0B\u4E00\u6761\u6D88\u606F\u540E daemon \u5C06\u6062\u590D\u8BE5\u4F1A\u8BDD\u7EE7\u7EED\u5BF9\u8BDD\u3002")}function to(){console.log(`\u7528\u6CD5: kexvim <command>
...
\u5165\u53E3: kexvim \u547D\u4EE4\u7EDF\u4E00\u8F6C\u53D1\u4E3A node dev.js\uFF08\u5F00\u53D1\uFF09\u6216 node kexvim.js\uFF08\u5B89\u88C5\u7248\uFF09\uFF0C
      \u547D\u4EE4\u8FDB\u7A0B\u5B8C\u5168\u72EC\u7ACB\uFF0C\u4E0D\u4F9D\u8D56\u8FD0\u884C\u4E2D\u7684 kexvim \u5B9E\u4F8B\u3002`),process.exit(0)}function Pl(){console.log("kexvim (\u5F00\u53D1\u5165\u53E3 dist/dev.mjs / \u5B89\u88C5\u5165\u53E3 kexvim.js)"),process.exit(0)}var Xn=class l{static resolveUserDataDir(){let e=process.env.KEXVIM_USER_DATA_DIR;if(e)return e;let t=I.findProjectRoot()??I.findProjectRoot(Bt.dirname(process.argv[1]??""));if(!t)throw new Error("[kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\uFF0C\u6216\u8BBE\u7F6E KEXVIM_USER_DATA_DIR");return Bt.join(t,"data")}static async dispatchFromArgv(){let e=process.argv[2];if(!e)return!1;let t=e.toLowerCase(),r=t==="--help"||t==="-h"?"help":t==="--version"||t==="-v"?"version":t;return new Set(["init","update","restart","stop","install","uninstall","status","clear-loop","sessions","session","help","version"]).has(r)?(await l.handleCliCommand(r),!0):!1}static async handleCliCommand(e,t){let r=Bt.resolve(t??l.resolveUserDataDir(),".."),s=Bt.join(r,"data"),n="https://gitee.com/moscowzk/kexvim";switch(e){case"init":await hl(r,n);return;case"update":await bl(r,n);return;case"install":Tl();return;case"uninstall":El();return;case"status":Cl();return;case"clear-loop":Rl();return;case"restart":await Sl(r),process.exit(0);return;case"stop":await xl(r),process.exit(0);return;case"sessions":Ml(s);return;case"session":{let i=process.argv[3];i||(console.error("\u7528\u6CD5: kexvim session <\u524D8\u4F4DID>\uFF08\u7528 kexvim sessions \u67E5\u770B\u5217\u8868\uFF09"),process.exit(1)),Il(s,i);return}case"help":to();return;case"version":Pl();return;default:to();return}}};import*as Se from"node:fs";import*as ro from"node:path";import*as Nl from"node:worker_threads";var so=()=>{let l=I.findProjectRoot();if(!l)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D kexvim.log\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let e=ro.join(l,"data","log");try{Se.mkdirSync(e,{recursive:!0})}catch{}return ro.join(e,"kexvim.log")},dp=100*1024*1024;try{let l=so();Se.existsSync(l)&&Se.statSync(l).size>dp&&(Se.rmSync(`${l}.1`,{force:!0}),Se.renameSync(l,`${l}.1`))}catch{}process.on("uncaughtException",l=>{try{Se.appendFileSync(so(),`[kexvim] uncaughtException at ${new Date().toISOString()}:
${l.stack??l.message}
`)}catch{}});process.on("unhandledRejection",l=>{try{let e=l instanceof Error?l.stack??l.message:String(l);Se.appendFileSync(so(),`[kexvim] unhandledRejection at ${new Date().toISOString()}:
${e}
`)}catch{}try{console.error("[kexvim] unhandledRejection:",l)}catch{}});async function Ll(){try{await Vn.main()}catch(l){console.error(l instanceof Error?l.message:String(l)),process.exit(1)}}Nl.isMainThread?Xn.dispatchFromArgv().then(l=>{l?process.exit(0):Ll()}):Ll();
