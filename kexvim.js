...
`)}catch{return""}}static discoverContextFiles(e,t){let r=[],s=t||Ke;for(let n of Ao){if(n.includes("*"))continue;let i=`${e}/${n}`;try{s.existsSync(i)&&r.push({name:n,fullPath:i})}catch{}}return r}static loadProjectContext(e,t=2e4){for(let r of Ao){if(r==="SOUL.md"||r.includes("*"))continue;let s=Yt.join(e,r);try{if(!Ke.existsSync(s))continue;let n=Ke.readFileSync(s,"utf-8").trim();if(!n)continue;return n=yt.stripFrontmatter(n).trim(),n.length>t&&(n=`${n.slice(0,t)}
...
`+h),n.sessionMessages.set(i,[...n.messages]),n.session&&n.sessionInstances.set(i,n.session),ve.flushTurnNotifications();let k;if(n._autoResetAt>0){let y=n._autoResetReason??"idle",v=n.config.sessionReset,w="\u4F1A\u8BDD\u5DF2\u81EA\u52A8\u91CD\u7F6E";if(v){let R=Math.floor(v.idleMinutes/60),_=v.idleMinutes%60,A=_===0?`${R}h`:R>0?`${R}h ${_}m`:`${_}m`;y==="daily"?w=`\u4F1A\u8BDD\u5DF2\u6309\u6BCF\u65E5\u8BA1\u5212\uFF08${v.atHour}:00\uFF09\u81EA\u52A8\u91CD\u7F6E`:(v.mode==="idle"||v.mode==="both")&&(w=`\u4F1A\u8BDD\u56E0\u7A7A\u95F2\u8D85\u8FC7 ${A} \u81EA\u52A8\u91CD\u7F6E`)}k=`\u25D0 ${w}\u3002\u5386\u53F2\u8BB0\u5F55\u5DF2\u6E05\u7A7A\uFF0C\u8FD9\u662F\u5168\u65B0\u4F1A\u8BDD\u3002
...
`;return Qe.writeFileSync(r,s,"utf-8"),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3}).code!==0?!1:b.runSyncResult("loginctl enable-linger "+ur.userInfo().username,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=Ur.join(ur.homedir(),".config","systemd","user",`${e}.service`);return Qe.existsSync(t)&&(b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:5e3}),Qe.unlinkSync(t),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3})),!0}catch{return!1}}isServiceInstalled(e){let t=Ur.join(ur.homedir(),".config","systemd","user",`${e}.service`);return Qe.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`systemctl --user is-active ${e}`,{timeoutMs:3e3});return t.code!==0?!1:t.stdout.trim()==="active"}startService(e){return b.runSyncResult(`systemctl --user start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`systemctl --user enable ${e}`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`systemctl --user disable ${e}`,{timeoutMs:5e3}).code===0}};import*as Ze from"fs";import*as qn from"os";import*as Ct from"path";var Gn=class{platform="win32";findKexvimPids(){let e=b.runSyncResult(`powershell -Command "Get-Process node | Where-Object { $_.CommandLine -match 'dev\\.mjs|kexvim\\.js' } | Select-Object -ExpandProperty Id"`,{timeoutMs:5e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){let r=t?"/F":"";return b.runSyncResult(`taskkill ${r} /PID ${e}`,{timeoutMs:5e3}).code===0}installService(e){if(b.runSyncResult("nssm --version",{timeoutMs:3e3}).code===0&&b.runSyncResult(`nssm install ${e.name} "${e.execPath}" "${e.workDir}"`,{timeoutMs:1e4}).code===0)return!0;try{let r=Ct.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");Ze.mkdirSync(r,{recursive:!0});let s=`@echo off
cd /d "${e.workDir}"
...
`;return Ze.writeFileSync(Ct.join(r,`${e.name}.bat`),s,"utf-8"),!0}catch{return!1}}uninstallService(e){if(!(b.runSyncResult(`nssm remove ${e} confirm`,{timeoutMs:1e4}).code===0)){let r=Ct.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup"),s=Ct.join(r,`${e}.bat`);if(Ze.existsSync(s))try{Ze.unlinkSync(s)}catch{}}return!0}isServiceInstalled(e){if(b.runSyncResult(`nssm status ${e}`,{timeoutMs:3e3}).code===0)return!0;let t=Ct.join(process.env.APPDATA||qn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");return Ze.existsSync(Ct.join(t,`${e}.bat`))}isServiceRunning(e){let t=b.runSyncResult(`powershell -Command "Get-Service ${e} | Select-Object -ExpandProperty Status"`,{timeoutMs:5e3});return t.code!==0?!1:t.stdout.trim().toLowerCase()==="running"}startService(e){return b.runSyncResult(`net start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`net stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`sc config ${e} start=auto`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`sc config ${e} start=disabled`,{timeoutMs:5e3}).code===0}};import*as et from"fs";import*as pr from"os";import*as Rt from"path";var Kn=class{platform="darwin";findKexvimPids(){let e=b.runSyncResult('pgrep -af "dev\\.mjs|kexvim\\.js" | grep -v " web"',{timeoutMs:3e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){try{return process.kill(e,t?"SIGKILL":"SIGTERM"),!0}catch{return!1}}installService(e){try{let t=Rt.join(pr.homedir(),"Library","LaunchAgents");et.mkdirSync(t,{recursive:!0});let r=Rt.join(t,`${e.name}.plist`),n=`<?xml version="1.0" encoding="UTF-8"?>
...
\u5165\u53E3: kexvim \u547D\u4EE4\u7EDF\u4E00\u8F6C\u53D1\u4E3A node dev.js\uFF08\u5F00\u53D1\uFF09\u6216 node kexvim.js\uFF08\u5B89\u88C5\u7248\uFF09\uFF0C
      \u547D\u4EE4\u8FDB\u7A0B\u5B8C\u5168\u72EC\u7ACB\uFF0C\u4E0D\u4F9D\u8D56\u8FD0\u884C\u4E2D\u7684 kexvim \u5B9E\u4F8B\u3002`),process.exit(0)}function Nl(){console.log("kexvim (\u5F00\u53D1\u5165\u53E3 dist/dev.mjs / \u5B89\u88C5\u5165\u53E3 kexvim.js)"),process.exit(0)}var Xn=class l{static resolveUserDataDir(){let e=process.env.KEXVIM_USER_DATA_DIR;if(e)return e;let t=I.findProjectRoot()??I.findProjectRoot(Bt.dirname(process.argv[1]??""));if(!t)throw new Error("[kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\uFF0C\u6216\u8BBE\u7F6E KEXVIM_USER_DATA_DIR");return Bt.join(t,"data")}static async dispatchFromArgv(){let e=process.argv[2];if(!e)return!1;let t=e.toLowerCase(),r=t==="--help"||t==="-h"?"help":t==="--version"||t==="-v"?"version":t;return new Set(["init","update","restart","stop","install","uninstall","status","keepalive","clear-loop","sessions","session","help","version"]).has(r)?(await l.handleCliCommand(r),!0):!1}static async handleCliCommand(e,t){let r=Bt.resolve(t??l.resolveUserDataDir(),".."),s=Bt.join(r,"data"),n="https://gitee.com/moscowzk/kexvim";switch(e){case"init":await vl(r,n);return;case"update":await kl(r,n);return;case"install":El();return;case"uninstall":Cl();return;case"status":Rl();return;case"keepalive":Ml();return;case"clear-loop":Al();return;case"restart":await _l(r),process.exit(0);return;case"stop":await Tl(r),process.exit(0);return;case"sessions":Il(s);return;case"session":{let i=process.argv[3];i||(console.error("\u7528\u6CD5: kexvim session <\u524D8\u4F4DID>\uFF08\u7528 kexvim sessions \u67E5\u770B\u5217\u8868\uFF09"),process.exit(1)),Ll(s,i);return}case"help":to();return;case"version":Nl();return;default:to();return}}};import*as _e from"node:fs";import*as ro from"node:path";import*as $l from"node:worker_threads";var so=()=>{let l=I.findProjectRoot();if(!l)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D kexvim.log\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let e=ro.join(l,"data","log");try{_e.mkdirSync(e,{recursive:!0})}catch{}return ro.join(e,"kexvim.log")},gp=100*1024*1024;try{let l=so();_e.existsSync(l)&&_e.statSync(l).size>gp&&(_e.rmSync(`${l}.1`,{force:!0}),_e.renameSync(l,`${l}.1`))}catch{}process.on("uncaughtException",l=>{try{_e.appendFileSync(so(),`[kexvim] uncaughtException at ${new Date().toISOString()}:
${l.stack??l.message}
`)}catch{}});process.on("unhandledRejection",l=>{try{let e=l instanceof Error?l.stack??l.message:String(l);_e.appendFileSync(so(),`[kexvim] unhandledRejection at ${new Date().toISOString()}:
${e}
