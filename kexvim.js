...
\u8F93\u5165 goal_text \u4E3A\u81EA\u7136\u8BED\u8A00\u76EE\u6807\u63CF\u8FF0\uFF0C\u7CFB\u7EDF\u4F1A\u81EA\u52A8\u89C4\u5212\u3001\u62C6\u89E3\u3001\u5E76\u884C\u6267\u884C\u3002
\u793A\u4F8B\uFF1A\u8C03\u7528 delegate_task(goal_text='\u7ED9\u652F\u4ED8\u6A21\u5757\u52A0\u9000\u6B3E\u529F\u80FD\uFF0C\u652F\u6301\u5FAE\u4FE1\u548C\u652F\u4ED8\u5B9D')
\u7B80\u5355\u67E5\u8BE2\uFF08\u95EE\u95EE\u9898\u3001\u67E5\u72B6\u6001\u3001\u5355\u6B65\u64CD\u4F5C\uFF09\u4E0D\u9700\u8981\u7528\u6B64\u5DE5\u5177\uFF0C\u76F4\u63A5\u56DE\u590D\u5373\u53EF\u3002`;static PLATFORM_HINTS={}},Io=["AGENTS.md",".cursorrules",".cursor/rules/wildcard.mdc",".cursor/rules/*.mdc","SOUL.md",".zk.md","ZK.md",".github/copilot-instructions.md",".windsurfrules"],yt=class{static stripFrontmatter(e){if(e.startsWith("---")){let t=e.indexOf(`
...
`)}catch{return""}}static discoverContextFiles(e,t){let r=[],s=t||Ke;for(let n of Io){if(n.includes("*"))continue;let i=`${e}/${n}`;try{s.existsSync(i)&&r.push({name:n,fullPath:i})}catch{}}return r}static loadProjectContext(e,t=2e4){for(let r of Io){if(r==="SOUL.md"||r.includes("*"))continue;let s=Vt.join(e,r);try{if(!Ke.existsSync(s))continue;let n=Ke.readFileSync(s,"utf-8").trim();if(!n)continue;return n=yt.stripFrontmatter(n).trim(),n.length>t&&(n=`${n.slice(0,t)}
...
`)}static _computeSummaryBudget(e,t){let r=l.estimateMessagesTokens(e),s=Math.floor(r*Lc);return Math.max(Ic,Math.min(s,t))}static _buildFallbackSummary(e){let t=[],s=0;for(let n of e){let i=n.role||"?",o=l._getContentText(n.content).replace(/\s+/g," ").trim();o.length>700&&(o=o.slice(0,685)+"...");let a=`[${i.toUpperCase()}]: ${o}`;if(s+=a.length,s>8e3)break;t.push(a)}return t.join(`
`)}};import*as Xt from"node:fs";import*as ye from"node:path";var bs=class l{static autoValidate(e){let t=Date.now(),r=ye.extname(e).toLowerCase(),s=[];console.error(`[AutoValidate] >>> start file=${e} ext=${r}`);let n=l.validateSyntax(e,r);if(n&&s.push(n),[".ts",".tsx",".py",".js",".jsx"].includes(r)){let o=l.checkCodeQuality(e);o&&s.push(o)}let i=Date.now()-t;return console.error(`[AutoValidate] <<< done file=${ye.basename(e)} elapsed=${i}ms issues=${s.length}`),s.length>0?s.join(`
...
`+l.sanitizeContext(e)+`
</memory-context>`}},Er=class l{providers=[];toolToProvider=new Map;hasExternal=!1;bg=new Ti;addProvider(e){if(!(e.name==="builtin")){if(this.hasExternal){console.warn(S.t("memory.memmgr_rejected_provider",{name:e.name}));return}this.hasExternal=!0}this.providers.push(e);for(let r of e.getToolSchemas()){let s=l.normalizeToolSchema(r);if(!s)continue;let n=s.name;if(!(!n||typeof n!="string")){if(Ea.has(n)){console.warn(S.t("memory.memmgr_shadowed_tool",{providerName:e.name,toolName:n}));continue}this.toolToProvider.has(n)?console.warn(`Memory tool name conflict: '${n}' already registered by ${this.toolToProvider.get(n).name}`):this.toolToProvider.set(n,e)}}}get providersList(){return[...this.providers]}getProvider(e){return this.providers.find(t=>t.name===e)}buildSystemPrompt(){let e=[];for(let t of this.providers)try{let r=t.systemPromptBlock();r&&r.trim()&&e.push(r)}catch{}return e.join(`
...
`,cr=class{db;constructor(e){this.db=new tu(e),this.db.exec("PRAGMA busy_timeout = 15000"),this.db.exec(ru)}async upsert(e){let t=this.db.prepare(`
...
`,Ur=class{db;enableMemoryLogging=!1;constructor(e){this.db=new su(e),this.db.exec("PRAGMA busy_timeout = 15000"),this.db.exec(nu)}async insert(e){let t=this.db.prepare(`
...
`;return Qe.writeFileSync(r,s,"utf-8"),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3}).code!==0?!1:b.runSyncResult("loginctl enable-linger "+pr.userInfo().username,{timeoutMs:5e3}).code===0}catch{return!1}}uninstallService(e){try{let t=Wr.join(pr.homedir(),".config","systemd","user",`${e}.service`);return Qe.existsSync(t)&&(b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:5e3}),Qe.unlinkSync(t),b.runSyncResult("systemctl --user daemon-reload",{timeoutMs:5e3})),!0}catch{return!1}}isServiceInstalled(e){let t=Wr.join(pr.homedir(),".config","systemd","user",`${e}.service`);return Qe.existsSync(t)}isServiceRunning(e){let t=b.runSyncResult(`systemctl --user is-active ${e}`,{timeoutMs:3e3});return t.code!==0?!1:t.stdout.trim()==="active"}startService(e){return b.runSyncResult(`systemctl --user start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`systemctl --user stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`systemctl --user enable ${e}`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`systemctl --user disable ${e}`,{timeoutMs:5e3}).code===0}};import*as Ze from"fs";import*as Kn from"os";import*as Rt from"path";var zn=class{platform="win32";findKexvimPids(){let e=b.runSyncResult(`powershell -Command "Get-Process node | Where-Object { $_.CommandLine -match 'dev\\.mjs|kexvim\\.js' } | Select-Object -ExpandProperty Id"`,{timeoutMs:5e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
`).map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s!==r)}killProcess(e,t=!1){let r=t?"/F":"";return b.runSyncResult(`taskkill ${r} /PID ${e}`,{timeoutMs:5e3}).code===0}installService(e){if(b.runSyncResult("nssm --version",{timeoutMs:3e3}).code===0&&b.runSyncResult(`nssm install ${e.name} "${e.execPath}" "${e.workDir}"`,{timeoutMs:1e4}).code===0)return!0;try{let r=Rt.join(process.env.APPDATA||Kn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");Ze.mkdirSync(r,{recursive:!0});let s=`@echo off
cd /d "${e.workDir}"
...
`;return Ze.writeFileSync(Rt.join(r,`${e.name}.bat`),s,"utf-8"),!0}catch{return!1}}uninstallService(e){if(!(b.runSyncResult(`nssm remove ${e} confirm`,{timeoutMs:1e4}).code===0)){let r=Rt.join(process.env.APPDATA||Kn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup"),s=Rt.join(r,`${e}.bat`);if(Ze.existsSync(s))try{Ze.unlinkSync(s)}catch{}}return!0}isServiceInstalled(e){if(b.runSyncResult(`nssm status ${e}`,{timeoutMs:3e3}).code===0)return!0;let t=Rt.join(process.env.APPDATA||Kn.homedir(),"Microsoft","Windows","Start Menu","Programs","Startup");return Ze.existsSync(Rt.join(t,`${e}.bat`))}isServiceRunning(e){let t=b.runSyncResult(`powershell -Command "Get-Service ${e} | Select-Object -ExpandProperty Status"`,{timeoutMs:5e3});return t.code!==0?!1:t.stdout.trim().toLowerCase()==="running"}startService(e){return b.runSyncResult(`net start ${e}`,{timeoutMs:1e4}).code===0}stopService(e){return b.runSyncResult(`net stop ${e}`,{timeoutMs:1e4}).code===0}enableService(e){return b.runSyncResult(`sc config ${e} start=auto`,{timeoutMs:5e3}).code===0}disableService(e){return b.runSyncResult(`sc config ${e} start=disabled`,{timeoutMs:5e3}).code===0}};import*as et from"fs";import*as mr from"os";import*as Ct from"path";var Jn=class{platform="darwin";findKexvimPids(){let e=b.runSyncResult('pgrep -af "dev\\.mjs|kexvim\\.js" | grep -v " web"',{timeoutMs:3e3});if(e.code!==0)return[];let t=e.stdout.trim();if(!t)return[];let r=process.pid;return t.split(`
...
`),console.log(`[~] \u5DF2\u5C06 ${l} \u52A0\u5165 ${ee.basename(r)}\uFF0C\u91CD\u542F\u7EC8\u7AEF\u540E\u751F\u6548`));else if(e)try{let n=`$h = ${"'"+l.replace(/'/g,"''")+"'"}; $p = [Environment]::GetEnvironmentVariable('PATH','User'); if ($p -and $p.Contains($h)) { Write-Output 'exists' } elseif ($p) { [Environment]::SetEnvironmentVariable('PATH', $p + ';' + $h, 'User'); Write-Output 'added' } else { [Environment]::SetEnvironmentVariable('PATH', $h, 'User'); Write-Output 'added' }`,i=b.runFileSync("powershell",["-NoProfile","-Command",n],{maxBuffer:1024*1024});console.log(i.includes("exists")?"[~] PATH \u5DF2\u914D\u7F6E":"[~] \u5DF2\u5C06 Kexvim \u52A0\u5165 PATH\uFF0C\u65B0\u7EC8\u7AEF\u751F\u6548")}catch{console.log("[~] \u672A\u80FD\u81EA\u52A8\u8BBE\u7F6E PATH\uFF0C\u8BF7\u624B\u52A8\u5C06\u542F\u52A8\u5668\u76EE\u5F55\u52A0\u5165\u73AF\u5883\u53D8\u91CF PATH")}else console.log(`[~] \u624B\u52A8\u5C06\u4EE5\u4E0B\u884C\u52A0\u5165 shell \u914D\u7F6E\u6587\u4EF6 (~/.bashrc / ~/.zshrc):
    export PATH="$PATH:${l}"`)}async function Sl(l,e){let t=i=>{try{return j.accessSync(i),!0}catch{return!1}};if(t(ee.join(l,"data",".env"))){console.log("Kexvim \u5DF2\u521D\u59CB\u5316(data/.env \u5DF2\u5B58\u5728)\u3002"),vl(l);return}if(t(ee.join(l,"kexvim.js")))console.log("[~] kexvim.js \u5DF2\u5B58\u5728\uFF0C\u8DF3\u8FC7\u4E0B\u8F7D");else if(j.mkdirSync(l,{recursive:!0}),bl("git")){console.log("[~] \u514B\u9686\u4ED3\u5E93...");try{b.runFileSync("git",["clone","--depth","1",e,l],{timeoutMs:12e4})}catch{console.error("[\u2717] \u514B\u9686\u5931\u8D25"),process.exit(1)}}else{console.log("[~] \u4E0B\u8F7D kexvim.js...");let i=ee.join(Bt.tmpdir(),"kexvim-init.js"),o=`${e}/raw/main/kexvim.js`;try{bl("wget")?b.runFileSync("wget",["-q","-O",i,o],{timeoutMs:6e4}):b.runFileSync("curl",["-fsSL","-o",i,o],{timeoutMs:6e4})}catch{console.error("[\u2717] \u4E0B\u8F7D\u5931\u8D25"),process.exit(1)}t(i)&&(j.copyFileSync(i,ee.join(l,"kexvim.js")),j.rmSync(i,{force:!0}))}let r=ee.join(l,"data",".env");if(j.mkdirSync(ee.join(l,"data"),{recursive:!0}),!t(r)){let i=kl.createInterface({input:process.stdin,output:process.stdout}),o=await new Promise(a=>{i.question("\u8BF7\u8F93\u5165 DeepSeek API Key: ",a)});i.close(),o&&(j.writeFileSync(r,`DEEPSEEK_API_KEY=${o}
KEXVIM_HOME=${l}
`),console.log("[\u2713] API Key \u5DF2\u4FDD\u5B58"))}let s=ee.join(l,"data","config.yaml");t(s)||(j.writeFileSync(s,up),console.log("[\u2713] config.yaml \u5DF2\u521B\u5EFA\uFF08\u8BF7\u6309\u9700\u8865\u5145\u914D\u7F6E\uFF09"));let n=ee.join(l,"skills");if(j.existsSync(n))console.log("[~] skills \u5DF2\u5B58\u5728\uFF0C\u8DF3\u8FC7");else{console.log("[~] \u4E0B\u8F7D\u516C\u5171\u6280\u80FD...");let i=j.mkdtempSync(ee.join(Bt.tmpdir(),"kexvim-skills-"));try{let o=b.isWindows?"nul":"/dev/null";b.runSync(`git clone --depth 1 ${e}.git "${i}" --single-branch 2>${o}`,{timeoutMs:6e4}),j.existsSync(ee.join(i,"skills"))?(j.cpSync(ee.join(i,"skills"),n,{recursive:!0}),console.log("[\u2713] \u6280\u80FD\u5DF2\u4E0B\u8F7D")):console.warn("[~] \u8FDC\u7A0B\u4ED3\u5E93\u4E2D\u6CA1\u6709 skills \u76EE\u5F55\uFF0C\u8DF3\u8FC7")}catch{console.warn("[~] \u6280\u80FD\u4E0B\u8F7D\u5931\u8D25\uFF08git \u672A\u5B89\u88C5\u6216\u65E0\u7F51\u7EDC\uFF09\uFF0C\u53EF\u624B\u52A8 clone \u540E\u590D\u5236 skills/ \u76EE\u5F55")}finally{j.rmSync(i,{recursive:!0,force:!0})}}vl(l),console.log("[\u2713] \u5B89\u88C5\u5B8C\u6210\u3002\u73B0\u5728\u53EF\u4EE5\u76F4\u63A5\u5728\u7EC8\u7AEF\u8F93\u5165: kexvim restart")}function bl(l){try{return b.runFileSync(process.platform==="win32"?"where":"which",[l]),!0}catch{return!1}}import*as vr from"node:fs";import*as es from"node:path";import*as _l from"node:os";function pp(l){try{return b.runFileSync(process.platform==="win32"?"where":"which",[l]),!0}catch{return!1}}async function wl(l,e){let t=r=>{try{return vr.accessSync(r),!0}catch{return!1}};if(t(es.join(l,"kexvim.js"))||(console.error("[\u2717] \u672A\u5B89\u88C5 Kexvim\uFF0C\u5148\u8FD0\u884C node kexvim.js init"),process.exit(1)),t(es.join(l,".git"))){console.log("[~] git pull...");try{b.runFileSync("git",["pull","--ff-only"],{cwd:l,timeoutMs:12e4})}catch{console.error("[\u2717] \u62C9\u53D6\u5931\u8D25"),process.exit(1)}}else{console.log("[~] \u4E0B\u8F7D\u6700\u65B0 kexvim.js...");let r=es.join(_l.tmpdir(),"kexvim-update.js");try{let s=`${e}/raw/main/kexvim.js`;pp("wget")?b.runFileSync("wget",["-q","-O",r,s],{timeoutMs:6e4}):b.runFileSync("curl",["-fsSL","-o",r,s],{timeoutMs:6e4})}catch{console.error("[\u2717] \u4E0B\u8F7D\u5931\u8D25"),process.exit(1)}t(r)&&(vr.copyFileSync(r,es.join(l,"kexvim.js")),vr.rmSync(r,{force:!0}))}console.log("[\u2713] \u66F4\u65B0\u5B8C\u6210\uFF0C\u6B63\u5728\u91CD\u542F...")}import*as H from"fs";import*as Se from"path";var xl=Mt();function mp(l,e){if(!H.existsSync(Se.join(l,"src")))return!1;if(!H.existsSync(e))return!0;let t=H.statSync(e).mtimeMs,r=t,s=n=>{let i;try{i=H.readdirSync(n,{withFileTypes:!0})}catch{return}for(let o of i){let a=Se.join(n,o.name);if(o.isDirectory()){if(o.name==="node_modules"||o.name==="dist"||o.name==="data"||o.name===".git")continue;s(a)}else if(o.name.endsWith(".ts"))try{H.statSync(a).mtimeMs>r&&(r=H.statSync(a).mtimeMs)}catch{}}};return s(Se.join(l,"src")),s(Se.join(l,"packages")),r>t}function Zi(l){if(process.platform!=="win32")return[];let e=b.runSyncResult(`wmic process where "name='node.exe'" get ProcessId,CommandLine /format:list`,{timeoutMs:5e3});if(e.code!==0)return[];let t=e.stdout.replace(/\u0000/g,""),r=Se.resolve(l).toLowerCase(),s=o=>{let a=Se.resolve(o).toLowerCase();return a===r||a.startsWith(r+Se.sep)},n=[],i=null;for(let o of t.split(/\r?\n/)){let a=o.trim();if(a.startsWith("CommandLine="))i=a.slice(12).replace(/^"|"$/g,"");else if(a.startsWith("ProcessId=")){let c=Number(a.slice(10));if(i&&c>0&&c!==process.pid&&!/\sweb(\s|$)/.test(i)){let d=i.match(/(\S*(?:dev\.mjs|kexvim\.js))/);d&&s(d[1])&&n.push({pid:c,entry:d[1]})}i=null}}return n}function gp(l,e=64*1024){try{let t=H.statSync(l);if(!t.isFile()||t.size===0)return"";if(t.size<=e)return H.readFileSync(l,"utf-8");let r=H.openSync(l,"r");try{let s=Buffer.alloc(e),n=H.readSync(r,s,0,e,t.size-e);return s.toString("utf-8",0,n)}finally{H.closeSync(r)}}catch{return""}}function fp(l,e){return new Promise(t=>{let r=0,s=setInterval(()=>{r++;try{if(H.existsSync(l)){let n=gp(l);if(n.includes("Guardian agent \u5DF2\u5C31\u7EEA")||n.includes("kexvim \u5DF2\u5C31\u7EEA")){clearInterval(s),console.log(`[kexvim] \u2705 kexvim \u5DF2\u5C31\u7EEA\uFF08${r}s\uFF09`),t(!0);return}}}catch{}r>=e&&(clearInterval(s),t(!1))},1e3)})}async function Tl(l){let e=l,t=Se.join(e,"data","log");try{H.mkdirSync(t,{recursive:!0})}catch{}let r=Se.join(t,"kexvim.log"),s=Se.join(e,"dist","dev.mjs");if(mp(e,s)){console.log("[kexvim] \u68C0\u6D4B\u5230\u6E90\u7801\u53D8\u66F4\uFF0C\u81EA\u52A8\u6784\u5EFA...");let d=b.runSyncResult("npm run build:dev",{cwd:e,timeoutMs:12e4});d.code!==0&&(console.error(`[kexvim] \u274C \u6784\u5EFA\u5931\u8D25\uFF0C\u4E2D\u6B62\u91CD\u542F\uFF1A${d.stderr?.trim()||d.stdout?.trim()}`),process.exit(1)),console.log("[kexvim] \u2705 \u6784\u5EFA\u5B8C\u6210")}if(process.platform==="linux")try{let d=b.runSyncResult("systemctl --user is-active kexvim.service",{timeoutMs:5e3});if(d.code===0&&d.stdout.trim()==="active"){console.log("[kexvim] systemd \u6258\u7BA1\uFF08kexvim.service active\uFF09\uFF0C\u8D70 systemctl --user restart");let u=b.runSyncResult("systemctl --user restart kexvim.service",{timeoutMs:3e4});u.code!==0&&(console.error(`[kexvim] \u274C systemctl restart \u5931\u8D25: ${u.stderr?.trim()||u.stdout?.trim()}`),process.exit(1)),console.log("[kexvim] \u2705 systemctl restart \u5B8C\u6210");return}}catch{}let n=Zi(e),i=n.length>0?n.map(d=>d.pid):xl.findKexvimPids(),o=n.length>0?n[0].entry:Be.resolveEntry(e);if(console.log(`[kexvim] \u65E7\u8FDB\u7A0B: ${i.length} \u4E2A; \u5165\u53E3: ${o}`),H.writeFileSync(r,"","utf-8"),i.length>0){console.log("[kexvim] \u6E05\u7406\u65E7\u8FDB\u7A0B...");for(let d of i)process.platform==="win32"?b.runSyncResult(`taskkill /PID ${d} /F`,{timeoutMs:5e3}):xl.killProcess(d,!0);console.log("[kexvim] \u2705 \u65E7\u8FDB\u7A0B\u5DF2\u6E05\u7406")}console.log("[kexvim] \u6B63\u5728\u542F\u52A8\u65B0 kexvim...");let a=Be.spawnFreshDaemon(e,o);a===null&&(console.error("[kexvim] \u274C \u65E0\u6CD5\u542F\u52A8\u65B0\u8FDB\u7A0B"),process.exit(1)),console.log(`[kexvim] \u65B0 PID: ${a}\uFF0C\u7B49\u5F85\u5C31\u7EEA...`),await fp(r,30)||console.warn("[kexvim] \u26A0\uFE0F \u7B49\u5F85\u5C31\u7EEA\u8D85\u65F6\uFF0830s\uFF09\uFF0C\u8BF7\u68C0\u67E5 data/log/kexvim.log");try{xe.restart(e),console.log("[kexvim] \u2705 web \u8FDB\u7A0B\u5DF2\u5C31\u7EEA\uFF08\u5DF2\u786E\u4FDD\u62C9\u8D77\uFF09")}catch(d){console.error(`[kexvim] \u26A0\uFE0F web \u91CD\u542F\u5931\u8D25: ${d instanceof Error?d.message:String(d)}`)}console.log("[kexvim] \u65E5\u5FD7:",r),process.exit(0)}import*as El from"fs";import*as Rl from"path";async function Cl(l){let e=Zi(l);for(let r of e)r.pid!==process.pid&&(process.platform==="win32"?b.runSyncResult(`taskkill /PID ${r.pid} /F /T`,{timeoutMs:5e3}):Mt().killProcess(r.pid,!0),console.log(`[kexvim] \u5DF2\u505C\u6B62 PID ${r.pid} (${r.entry})`));let t=!1;try{let r=xe.findPid();r!==null&&(process.platform==="win32"?b.runSyncResult(`taskkill /PID ${r} /F /T`,{timeoutMs:5e3}):Mt().killProcess(r,!0),console.log(`[kexvim] \u5DF2\u505C\u6B62 web \u8FDB\u7A0B PID ${r}`),t=!0)}catch{}try{El.rmSync(Rl.join(l,"data","kexvim.pid"),{force:!0})}catch{}e.length===0&&!t?console.log("[kexvim] \u6CA1\u6709\u8FD0\u884C\u4E2D\u7684 kexvim \u8FDB\u7A0B"):console.log("[kexvim] \u2705 kexvim \u5DF2\u505C\u6B62")}import*as z from"node:fs";import*as ts from"node:os";import*as te from"node:path";import{fileURLToPath as hp}from"node:url";var ie=class l{static SERVICE_NAME="kexvim";static AUTOSTART_DIR="autostart";static KEEPALIVE_INTERVAL_MIN=1;static SYSTEMD_TEMPLATE=(e,t,r)=>`[Unit]
...
`;static platform=process.platform;static _findRoot(){let e=te.dirname(hp(import.meta.url));for(let t=0;t<8;t++){if(z.existsSync(te.join(e,"package.json")))return e;let r=te.dirname(e);if(r===e)break;e=r}throw new Error("[kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF08\u65E0 package.json\uFF09\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002")}static _systemPath(e){if(l.platform==="win32")return"";let t=ts.homedir();return l.platform==="darwin"?te.join(t,"Library","LaunchAgents","com.kexvim.daemon.plist"):te.join(t,".config","systemd","user","kexvim.service")}static _dataPath(e){let t=te.join(e,"data",l.AUTOSTART_DIR);return z.mkdirSync(t,{recursive:!0}),l.platform==="win32"?te.join(t,"keepalive.cmd"):l.platform==="darwin"?te.join(t,"com.kexvim.daemon.plist"):te.join(t,"kexvim.service")}static _devEntry(e){let t=te.join(e,"dist","dev.mjs");if(z.existsSync(t))return t;let r=te.join(e,"kexvim.js");if(z.existsSync(r))return r;throw new Error("[kexvim] \u672A\u627E\u5230 daemon \u5165\u53E3\uFF08dist/dev.mjs \u6216 kexvim.js\uFF09\u3002")}static _template(e){let t=process.execPath,r=l._devEntry(e);return l.platform==="win32"?`@echo off\r
set NODE_NO_WARNINGS=1\r
cd /d "${e}"\r
${t} "${r}" keepalive\r
`:l.platform==="darwin"?l.LAUNCHD_TEMPLATE(t,e,r):l.SYSTEMD_TEMPLATE(t,e,r)}static writeTemplate(e){let t=e??l._findRoot(),r=l._dataPath(t);return z.writeFileSync(r,l._template(t)),r}static install(e){let t=e??l._findRoot(),r=l.writeTemplate(t),s=l._systemPath(t),n=[`[\u2713] \u6A21\u677F\u5DF2\u751F\u6210: ${r}`];if(l.platform==="win32")l._winInstall(t,n);else if(l.platform==="darwin"){z.mkdirSync(te.dirname(s),{recursive:!0}),z.copyFileSync(r,s),n.push(`[\u2713] \u5DF2\u5199\u5165: ${s}`);try{b.runFileSync("launchctl",["load","-w",s]),n.push("[\u2713] launchctl load \u5B8C\u6210")}catch{n.push("[\u2717] launchctl load \u5931\u8D25\uFF08\u9700\u5F53\u524D\u7528\u6237\u6709 GUI \u4F1A\u8BDD\uFF09")}}else{z.mkdirSync(te.dirname(s),{recursive:!0}),z.copyFileSync(r,s),n.push(`[\u2713] \u5DF2\u5199\u5165: ${s}`);try{b.runFileSync("systemctl",["--user","daemon-reload"]),b.runFileSync("systemctl",["--user","enable","kexvim.service"]),n.push("[\u2713] systemd user \u5355\u5143\u5DF2 enable")}catch(i){n.push(`[\u2717] systemctl \u6CE8\u518C\u5931\u8D25: ${i instanceof Error?i.message:String(i)}`)}try{b.runFileSync("loginctl",["enable-linger",ts.userInfo().username]),n.push("[\u2713] linger \u5DF2\u542F\u7528\uFF08\u5F00\u673A\u81EA\u542F\uFF09")}catch{n.push("[~] linger \u542F\u7528\u5931\u8D25\uFF08\u90E8\u5206\u7CFB\u7EDF\u9700 root\uFF09\uFF0C\u81EA\u542F\u53EF\u80FD\u53EA\u5728\u767B\u5F55\u540E\u751F\u6548")}}return n.join(`
...
Register-ScheduledTask -TaskName 'KexvimKeepAlive' -Action (New-ScheduledTaskAction -Execute '${r}' -Argument '"--no-warnings" "${s}" keepalive' -WorkingDirectory '${e}') -Trigger (New-ScheduledTaskTrigger -Once -At (Get-Date).AddMinutes(1) -RepetitionInterval (New-TimeSpan -Minutes ${l.KEEPALIVE_INTERVAL_MIN})) -Principal $p -Force | Out-Null`;try{b.runFileSync("powershell",["-NoProfile","-Command",n]),t.push("[\u2713] \u8BA1\u5212\u4EFB\u52A1 KexvimDaemon\uFF08\u5F00\u673A\u5EF6\u8FDF 1 \u5206\u949F\u62C9\u8D77\uFF0CS4U \u65E0\u7A97\u53E3\uFF09\u5DF2\u521B\u5EFA"),t.push(`[\u2713] \u8BA1\u5212\u4EFB\u52A1 KexvimKeepAlive\uFF08\u6BCF ${l.KEEPALIVE_INTERVAL_MIN} \u5206\u949F\u4FDD\u6D3B daemon+web\uFF0CS4U \u65E0\u7A97\u53E3\uFF09\u5DF2\u521B\u5EFA`)}catch(i){t.push(`[\u2717] \u521B\u5EFA\u8BA1\u5212\u4EFB\u52A1\u5931\u8D25: ${i instanceof Error?i.message:String(i)}`)}}static isProcessRunning(e){let t=e??l._findRoot(),r=te.join(t,"data","kexvim.pid");try{let s=Number.parseInt(z.readFileSync(r,"utf-8").trim(),10);return!Number.isFinite(s)||s<=0?!1:(process.kill(s,0),!0)}catch{return!1}}static resolveRoot(){return l._findRoot()}};function Ml(){let l=ie.resolveRoot();console.log(ie.install(l));let e=ie.status(l);console.log(`[~] \u72B6\u6001: platform=${e.platform} installed=${e.installed} consistent=${e.consistent} active=${e.serviceActive}`),process.exit(0)}function Al(){console.log(ie.uninstall(ie.resolveRoot())),process.exit(0)}function Pl(){let l=ie.resolveRoot(),e=ie.status(l),t=ie.isProcessRunning(l);console.log(`kexvim \u5E73\u53F0:        ${e.platform}`),console.log(`\u8FDB\u7A0B:               ${t?"\u8FD0\u884C\u4E2D":"\u672A\u8FD0\u884C"} (PID \u6587\u4EF6: data/kexvim.pid)`),console.log(`\u81EA\u542F\u914D\u7F6E:           ${e.installed?"\u5DF2\u5B89\u88C5":"\u672A\u5B89\u88C5"} @ ${e.configPath||"(Windows \u8BA1\u5212\u4EFB\u52A1)"}`),console.log(`\u914D\u7F6E\u4E00\u81F4\u6027(\u7EDF\u4E00):    ${e.consistent?"\u4E00\u81F4(\u6A21\u677F)":"\u4E0D\u4E00\u81F4(\u5C06\u88AB install \u8986\u76D6)"}`),console.log(`\u670D\u52A1\u72B6\u6001:           ${e.serviceActive?"active":"inactive"}`),console.log(`\u91CD\u542F\u5FAA\u73AF\u9632\u62A4:       ${ke.isTripped(l)?"\u26A0\uFE0F TRIPPED\uFF08\u7528 kexvim clear-loop \u89E3\u9664\uFF09":`\u6B63\u5E38 (${ke.bootCount(l)}/${ke.MAX_RESTARTS} in ${ke.WINDOW_SECONDS}s)`}`),process.exit(t?0:1)}function Il(){ke.clear(ie.resolveRoot()),console.log("[\u2713] restart_loop \u72B6\u6001\u5DF2\u6E05\u9664"),process.exit(0)}import*as br from"node:fs";import*as eo from"node:path";var yp=9e4;function vp(l){let e=eo.join(l,"data","daemon.heartbeat");try{let t=br.statSync(e);if(Date.now()-t.mtimeMs>yp)return null;let r=Number.parseInt(br.readFileSync(e,"utf-8").trim(),10);return Number.isFinite(r)&&r>0?r:null}catch{return null}}function Ll(){let l=ie.resolveRoot(),e=vp(l);if(e===null)Be.spawnFreshDaemon(l)===null&&process.exit(1);else try{br.writeFileSync(eo.join(l,"data","kexvim.pid"),String(e))}catch{}try{xe.ensureRunning(l)}catch{}process.exit(0)}import*as rs from"node:fs";import*as to from"node:path";import{DatabaseSync as bp}from"node:sqlite";function Nl(l){let e=to.join(l,"kexvim.db");rs.existsSync(e)||(console.error("[kexvim] \u274C \u627E\u4E0D\u5230\u4F1A\u8BDD\u5E93:",e),process.exit(1));let t=new bp(e,{readOnly:!0});try{let r=t.prepare(`
      ...
      WHERE s.is_test = 0
      ORDER BY s.updated_at DESC
    `).all();if(r.length===0){console.log("\u6682\u65E0\u5386\u53F2\u4F1A\u8BDD\u3002");return}let s="";try{let n=to.join(l,".current_session");rs.existsSync(n)&&(s=rs.readFileSync(n,"utf-8").trim())}catch{}console.log(`\u5171 ${r.length} \u4E2A\u4F1A\u8BDD\uFF1A
...
\u7528\u6CD5: kexvim session <\u524D8\u4F4DID>  \u5207\u6362/\u6062\u590D\u8BE5\u4F1A\u8BDD`)}finally{t.close()}}import*as Ol from"node:fs";import*as ro from"node:path";import{DatabaseSync as kp}from"node:sqlite";var Sp="session-switch.json";function $l(l,e){let t=ro.join(l,"kexvim.db"),r=new kp(t,{readOnly:!0}),s;try{s=r.prepare(`
      SELECT id, COALESCE(summary, '') AS summary,
             (SELECT COUNT(*) FROM messages m WHERE m.session_id = s.id) AS msgCount
      FROM sessions s
      WHERE s.is_test = 0 AND s.id LIKE ?
    `).get(`${e}%`)}finally{r.close()}s||(console.error(`[kexvim] \u274C \u627E\u4E0D\u5230\u4F1A\u8BDD "${e}"\u3002\u7528 kexvim sessions \u67E5\u770B\u5168\u90E8\u4F1A\u8BDD\u3002`),process.exit(1)),Ol.writeFileSync(ro.join(l,Sp),JSON.stringify({sessionId:s.id,requestedAt:Date.now()/1e3}),"utf-8");let n=(s.summary||"(\u65E0\u6807\u9898)").slice(0,30);console.log(`\u2705 \u5DF2\u6807\u8BB0\u5207\u6362\u5230\u4F1A\u8BDD ${s.id.slice(0,8)}\uFF08${s.msgCount} \u6761\u6D88\u606F\uFF0C${n}\uFF09`),console.log("   \u53D1\u9001\u4E0B\u4E00\u6761\u6D88\u606F\u540E daemon \u5C06\u6062\u590D\u8BE5\u4F1A\u8BDD\u7EE7\u7EED\u5BF9\u8BDD\u3002")}function so(){console.log(`\u7528\u6CD5: kexvim <command>
...
\u5165\u53E3: kexvim \u547D\u4EE4\u7EDF\u4E00\u8F6C\u53D1\u4E3A node dev.js\uFF08\u5F00\u53D1\uFF09\u6216 node kexvim.js\uFF08\u5B89\u88C5\u7248\uFF09\uFF0C
      \u547D\u4EE4\u8FDB\u7A0B\u5B8C\u5168\u72EC\u7ACB\uFF0C\u4E0D\u4F9D\u8D56\u8FD0\u884C\u4E2D\u7684 kexvim \u5B9E\u4F8B\u3002`),process.exit(0)}function Dl(){console.log("kexvim (\u5F00\u53D1\u5165\u53E3 dist/dev.mjs / \u5B89\u88C5\u5165\u53E3 kexvim.js)"),process.exit(0)}import*as kr from"node:fs";import*as no from"node:path";import*as Bl from"node:readline";import{load as _p,dump as wp}from"js-yaml";function xp(l){let e=kr.readFileSync(l,"utf-8"),t=_p(e);if(!t||typeof t!="object"||Array.isArray(t))throw new Error(`config.yaml \u683C\u5F0F\u5F02\u5E38: ${l}`);return t}function Fl(l,e){return new Promise(t=>{l.question(e,t)})}function Tp(l,e){let t=!!l?.enabled,r=l?.adapters??{};console.log(`\u914D\u7F6E\u6587\u4EF6: ${no.join(e,"config.yaml")}`),console.log(`platform.enabled: ${t}`);let s=Object.keys(r);if(s.length===0)console.log("\u5DF2\u914D\u7F6E\u5E73\u53F0: \u65E0");else{console.log("\u5DF2\u914D\u7F6E\u5E73\u53F0:");for(let n of s){let i=r[n]??{},o=Object.entries(i).map(([a,c])=>`${a}=${c===void 0?"":String(c)}`).join(", ");console.log(`  ${n}: ${o||"(\u7A7A)"}`)}}console.log()}async function jl(l){let e=no.join(l,"config.yaml");kr.existsSync(e)||(console.error("[\u2717] \u672A\u627E\u5230 config.yaml\uFF0C\u8BF7\u5148\u6267\u884C: kexvim init"),process.exit(1));let t=xp(e),r=t.platform??{enabled:!0,adapters:{}};Tp(r,l);let s=Object.keys(Dt);console.log("\u652F\u6301\u7684\u5E73\u53F0:"),s.forEach((i,o)=>{let a=Dt[i];console.log(`  ${o+1}) ${a.name.padEnd(16)} ${a.desc}`)}),console.log("  0) \u9000\u51FA"),console.log();let n=Bl.createInterface({input:process.stdin,output:process.stdout});try{let i=(await Fl(n,"\u8BF7\u9009\u62E9\u5E73\u53F0\u7F16\u53F7: ")).trim(),o=parseInt(i,10);if(!i||i==="0"||isNaN(o)||o<1||o>s.length){console.log("\u5DF2\u9000\u51FA");return}let a=s[o-1],c=Dt[a];console.log(),console.log(`\u914D\u7F6E ${c.name}\uFF08${a}\uFF09:`);let d={},u=r.adapters??{},p=u[a]??{};for(let m of c.fields){let g=p[m.key]!==void 0?` (\u5F53\u524D: ${String(p[m.key])})`:"",f=(await Fl(n,`  ${m.label}${g}: `)).trim();if(f===""){if(!m.optional){console.error(`[\u2717] ${m.key} \u5FC5\u586B\uFF0C\u5DF2\u53D6\u6D88\uFF08\u672A\u4FEE\u6539\u914D\u7F6E\uFF09`);return}p[m.key]!==void 0&&(d[m.key]=p[m.key]);continue}f==="true"||f==="false"?d[m.key]=f==="true":/^\d+$/.test(f)?d[m.key]=Number(f):d[m.key]=f}u[a]=d,r.enabled=!0,r.adapters=u,t.platform=r,kr.writeFileSync(e,wp(t,{indent:2,lineWidth:-1})),console.log(),console.log(`[\u2713] \u5DF2\u5199\u5165 platform.adapters.${a}:`);for(let[m,g]of Object.entries(d))console.log(`    ${m}: ${String(g)}`);console.log("[\u2713] platform.enabled = true"),console.log(),console.log("\u91CD\u542F\u540E\u751F\u6548: kexvim restart")}finally{n.close()}}var Zn=class l{static resolveUserDataDir(){let e=process.env.KEXVIM_USER_DATA_DIR;if(e)return e;let t=P.findProjectRoot()??P.findProjectRoot(jt.dirname(process.argv[1]??""));if(!t)throw new Error("[kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\uFF0C\u6216\u8BBE\u7F6E KEXVIM_USER_DATA_DIR");return jt.join(t,"data")}static async dispatchFromArgv(){let e=process.argv[2];if(!e)return!1;let t=e.toLowerCase(),r=t==="--help"||t==="-h"?"help":t==="--version"||t==="-v"?"version":t;return new Set(["init","update","restart","stop","install","uninstall","status","keepalive","clear-loop","sessions","session","platform","help","version"]).has(r)?(await l.handleCliCommand(r),!0):!1}static async handleCliCommand(e,t){let r=jt.resolve(t??l.resolveUserDataDir(),".."),s=jt.join(r,"data"),n="https://gitee.com/moscowzk/kexvim";switch(e){case"init":await Sl(r,n);return;case"update":await wl(r,n);return;case"install":Ml();return;case"uninstall":Al();return;case"status":Pl();return;case"keepalive":Ll();return;case"clear-loop":Il();return;case"restart":await Tl(r),process.exit(0);return;case"stop":await Cl(r),process.exit(0);return;case"sessions":Nl(s);return;case"platform":await jl(s);return;case"session":{let i=process.argv[3];i||(console.error("\u7528\u6CD5: kexvim session <\u524D8\u4F4DID>\uFF08\u7528 kexvim sessions \u67E5\u770B\u5217\u8868\uFF09"),process.exit(1)),$l(s,i);return}case"help":so();return;case"version":Dl();return;default:so();return}}};import*as _e from"node:fs";import*as io from"node:path";import*as Hl from"node:worker_threads";var oo=()=>{let l=P.findProjectRoot();if(!l)throw new Error("[Kexvim] \u627E\u4E0D\u5230\u9879\u76EE\u6839\uFF1A\u65E0\u6CD5\u5B9A\u4F4D kexvim.log\u3002\u8BF7\u5728 kexvim \u9879\u76EE\u76EE\u5F55\u5185\u8FD0\u884C\u3002");let e=io.join(l,"data","log");try{_e.mkdirSync(e,{recursive:!0})}catch{}return io.join(e,"kexvim.log")},Ep=100*1024*1024;try{let l=oo();_e.existsSync(l)&&_e.statSync(l).size>Ep&&(_e.rmSync(`${l}.1`,{force:!0}),_e.renameSync(l,`${l}.1`))}catch{}process.on("uncaughtException",l=>{try{_e.appendFileSync(oo(),`[kexvim] uncaughtException at ${new Date().toISOString()}:
${l.stack??l.message}
...
${e}
`)}catch{}try{console.error("[kexvim] unhandledRejection:",l)}catch{}});async function Ul(){try{await Qn.main()}catch(l){console.error(l instanceof Error?l.message:String(l)),process.exit(1)}}Hl.isMainThread?Zn.dispatchFromArgv().then(l=>{l?process.exit(0):Ul()}):Ul();
